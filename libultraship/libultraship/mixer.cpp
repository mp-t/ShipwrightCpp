...
#define BUF_S16(a) (rspa.buf.as_s16 + ((a) - 0x3C0) / sizeof(std::int16_t))
...
static struct {
    ...
    std::uint16_t vol[2];
    std::uint16_t rate[2];
    std::uint16_t vol_wet;
    std::uint16_t rate_wet;

    ADPCM_STATE *adpcm_loop_state;

    std::int16_t adpcm_table[8][2][8];

    std::uint16_t filter_count;
    std::int16_t filter[8];

    union {
        std::int16_t as_s16[DMEM_BUF_SIZE / sizeof(std::int16_t)];
        std::uint8_t as_u8[DMEM_BUF_SIZE];
    } buf;
} rspa;

static std::int16_t resample_table[64][4] = {
...
static inline std::int16_t clamp16(std::int32_t v) {
    if (v < -0x8000) {
        return -0x8000;
    } else if (v > 0x7fff) {
        return 0x7fff;
...
static inline std::int32_t clamp32(std::int64_t v) {
    if (v < -0x7fffffff - 1) {
        return -0x7fffffff - 1;
...
void aSetBufferImpl([[maybe_unused]] std::uint8_t flags, std::uint16_t in, std::uint16_t out, std::uint16_t nbytes) {
    rspa.in = in;
    rspa.out = out;
    rspa.nbytes = nbytes;
}
...
void aInterleaveImpl(std::uint16_t dest, std::uint16_t left, std::uint16_t right, std::uint16_t c) {
    int count = ROUND_UP_8(c) / sizeof(std::int16_t) / 4;
    std::int16_t *l = BUF_S16(left);
    std::int16_t *r = BUF_S16(right);
    std::int16_t *d = BUF_S16(dest);
    while (count > 0) {
        std::int16_t l0 = *l++;
        std::int16_t l1 = *l++;
        std::int16_t l2 = *l++;
...
void aADPCMdecImpl(std::uint8_t flags, ADPCM_STATE state) {
    std::uint8_t *in = BUF_U8(rspa.in);
    std::int16_t *out = BUF_S16(rspa.out);
    int nbytes = ROUND_UP_32(rspa.nbytes);
    if (flags & A_INIT) {
        memset(out, 0, 16 * sizeof(std::int16_t));
    } else if (flags & A_LOOP) {
        memcpy(out, rspa.adpcm_loop_state, 16 * sizeof(std::int16_t));
    } else {
        memcpy(out, state, 16 * sizeof(std::int16_t));
    }
    out += 16;
    ...
    while (nbytes > 0) {
        ...
        std::int16_t (*tbl)[8] = rspa.adpcm_table[table_index];
        int i;

        for (i = 0; i < 2; i++) {
            std::int16_t ins[8];
            std::int16_t prev1 = out[-1];
            std::int16_t prev2 = out[-2];
            int j, k;
			if (flags & 4) {
				for (j = 0; j < 2; j++) {
            ...
            for (j = 0; j < 8; j++) {
                std::int32_t acc = tbl[0][j] * prev2 + tbl[1][j] * prev1 + (ins[j] << 11);
                for (k = 0; k < j; k++) {
        ...
        nbytes -= 16 * sizeof(std::int16_t);
    }
    memcpy(state, out - 16, 16 * sizeof(std::int16_t));
}

void aResampleImpl(std::uint8_t flags, std::uint16_t pitch, RESAMPLE_STATE state) {
    std::int16_t tmp[16];
    std::int16_t *in_initial = BUF_S16(rspa.in);
    std::int16_t *in = in_initial;
    std::int16_t *out = BUF_S16(rspa.out);
    ...
    if (flags & A_INIT) {
        memset(tmp, 0, 5 * sizeof(std::int16_t));
    } else {
        memcpy(tmp, state, 16 * sizeof(std::int16_t));
    }
    if (flags & 2) {
        memcpy(in - 8, tmp + 8, 8 * sizeof(std::int16_t));
        in -= tmp[5] / sizeof(std::int16_t);
    }
    in -= 4;
    pitch_accumulator = (std::uint16_t)tmp[4];
    memcpy(in, tmp, 4 * sizeof(std::int16_t));
    ...
    do {
        ...
        nbytes -= 8 * sizeof(std::int16_t);
    } while (nbytes > 0);

    state[4] = (std::int16_t)pitch_accumulator;
    memcpy(state, in, 4 * sizeof(std::int16_t));
    i = (in - in_initial + 4) & 7;
    in -= i;
    if (i != 0) {
        i = -8 - i;
...
void aEnvMixerImpl(std::uint16_t in_addr, std::uint16_t n_samples, bool swap_reverb,
                   ...
                   std::int32_t wet_dry_addr, u32)
{
    std::int16_t *in = BUF_S16(in_addr);
    std::int16_t *dry[2] = {BUF_S16(((wet_dry_addr >> 24) & 0xFF) << 4), BUF_S16(((wet_dry_addr >> 16) & 0xFF) << 4)};
    std::int16_t *wet[2] = {BUF_S16(((wet_dry_addr >> 8) & 0xFF) << 4), BUF_S16(((wet_dry_addr) & 0xFF) << 4)};
    std::int16_t negs[4] = {neg_left ? -1 : 0, neg_right ? -1 : 0, neg_3 ? -4 : 0, neg_2 ? -2 : 0};
    int swapped[2] = {swap_reverb ? 1 : 0, swap_reverb ? 0 : 1};
    int n = ROUND_UP_16(n_samples);

    std::uint16_t vols[2] = {rspa.vol[0], rspa.vol[1]};
    std::uint16_t rates[2] = {rspa.rate[0], rspa.rate[1]};
    std::uint16_t vol_wet = rspa.vol_wet;
    std::uint16_t rate_wet = rspa.rate_wet;

    do {
        for (int i = 0; i < 8; i++) {
            std::int16_t samples[2] = {*in, *in}; in++;
            for (int j = 0; j < 2; j++) {
...
void aMixImpl(std::uint16_t count, std::int16_t gain, std::uint16_t in_addr, std::uint16_t out_addr) {
    ...
    std::int16_t *in = BUF_S16(in_addr);
    std::int16_t *out = BUF_S16(out_addr);
    int i;
    std::int32_t sample;
...
void aDuplicateImpl(std::uint16_t count, std::uint16_t in_addr, std::uint16_t out_addr) {
    std::uint8_t* in = BUF_U8(in_addr);
    std::uint8_t *out = BUF_U8(out_addr);
...
void aResampleZohImpl(std::uint16_t pitch, std::uint16_t start_fract) {
    std::int16_t *in = BUF_S16(rspa.in);
    std::int16_t *out = BUF_S16(rspa.out);
    int nbytes = ROUND_UP_8(rspa.nbytes);
    std::uint32_t pos = start_fract;
    std::uint32_t pitch_add = pitch << 2;
...
void aFilterImpl(std::uint8_t flags, std::uint16_t count_or_buf, std::int16_t *state_or_filter) {
    if (flags > A_INIT) {
        ...
        std::int16_t tmp[16], tmp2[8];
        int count = rspa.filter_count;
        std::int16_t *buf = BUF_S16(count_or_buf);

        if (flags == A_INIT) {
            memset(tmp, 0, 8 * sizeof(std::int16_t));
            memset(tmp2, 0, 8 * sizeof(std::int16_t));
        } else {
            memcpy(tmp, state_or_filter, 8 * sizeof(std::int16_t));
            memcpy(tmp2, state_or_filter + 8, 8 * sizeof(std::int16_t));
        }
        ...
        do {
            memcpy(tmp + 8, buf, 8 * sizeof(std::int16_t));
            for (int i = 0; i < 8; i++) {
                std::int64_t sample = 0x4000; // round term
                for (int j = 0; j < 8; j++) {
                    sample += tmp[i + j] * rspa.filter[7 - j];
                }
                buf[i] = clamp16((std::int32_t)(sample >> 15));
            }
            memcpy(tmp, tmp + 8, 8 * sizeof(std::int16_t));

            buf += 8;
            count -= 8 * sizeof(std::int16_t);
        } while (count > 0);

        memcpy(state_or_filter, tmp, 8 * sizeof(std::int16_t));
        memcpy(state_or_filter + 8, rspa.filter, 8 * sizeof(std::int16_t));
    }
}

void aHiLoGainImpl(std::uint8_t g, std::uint16_t count, std::uint16_t addr) {
    std::int16_t *samples = BUF_S16(addr);
...
void aUnkCmd19Impl(std::uint8_t f, std::uint16_t count, std::uint16_t out_addr, std::uint16_t in_addr) {
    ...
    std::int16_t *in = BUF_S16(in_addr + f);
    std::int16_t *out = BUF_S16(out_addr);
    std::int16_t tbl[32];

    memcpy(tbl, in, 32 * sizeof(std::int16_t));
    do {
        for (int i = 0; i < 32; i++) {
            out[i] = clamp16(out[i] * tbl[i]);
        }
        out += 32;
        nbytes -= 32 * sizeof(std::int16_t);
    } while (nbytes > 0);
}
