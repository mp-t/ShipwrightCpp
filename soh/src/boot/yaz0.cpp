...
void* Yaz0_FirstDMA(void) {
    ...
    DmaMgr_DmaRomToRam(sYaz0CurRomStart, reinterpret_cast<std::uintptr_t>(sYaz0DataBuffer), dmaSize);
    ...
    do {
        if (bitIdx == 0) {
            if ((sYaz0MaxPtr < (uintptr_t)src) && (sYaz0CurSize != 0)) {
                src = static_cast<u8*>(Yaz0_NextDMA(src));
            }
...
void Yaz0_Decompress(uintptr_t romStart, void* dst, size_t size) {
    ...
    sYaz0CurSize = size;
    sYaz0CurDataEnd = reinterpret_cast<std::uintptr_t>(sYaz0DataBuffer + sizeof(sYaz0DataBuffer));
    Yaz0_DecompressImpl(static_cast<Yaz0Header*>(Yaz0_FirstDMA()), static_cast<u8*>(dst));
}
