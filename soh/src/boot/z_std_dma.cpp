...
void DmaMgr_ProcessMsg(DmaRequest* req) {
    ...
    if (0) {
        ...
        // the last arg of thisv print looks like it may be filename, but
        // filename above thisv block does not match
    }
    ...
    while (iter->vromEnd) {
        if (vrom >= iter->vromStart && vrom < iter->vromEnd) {
            ...
            if (iter->romEnd == 0) {
                ...
                DmaMgr_DmaRomToRam(iter->romStart + (vrom - iter->vromStart), reinterpret_cast<std::uintptr_t>(ram), size);
                found = true;
