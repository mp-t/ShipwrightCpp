...
typedef struct {
    ...
    /* 0x04 */ const void* drawArg; // segment address (display list or texture) passed to the draw function when called
} DebugDispObjectInfo;        // size = 0x8

typedef void (*DebugDispObject_DrawFunc)(DebugDispObject*, const void*, GlobalContext*);

void DebugDisplay_DrawSpriteI8(DebugDispObject* dispObj, const void* texture, GlobalContext* globalCtx);
void DebugDisplay_DrawPolygon(DebugDispObject* dispObj, const void* dlist, GlobalContext* globalCtx);
...
DebugDispObject* DebugDisplay_AddObject(f32 posX, f32 posY, f32 posZ, s16 rotX, s16 rotY, s16 rotZ, f32 scaleX,
    ...
    sDebugObjectListHead = static_cast<DebugDispObject*>(Graph_Alloc(gfxCtx, sizeof(DebugDispObject)));
...
void DebugDisplay_DrawPolygon(DebugDispObject* dispObj, const void* dlist, GlobalContext* globalCtx) {
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_debug_display.c", 228),
              G_MTX_MODELVIEW | G_MTX_LOAD);
    gSPDisplayList(POLY_XLU_DISP++, static_cast<const Gfx*>(dlist));
