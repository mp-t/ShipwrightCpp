...
void Skin_InitAnimatedLimb(GlobalContext* globalCtx, Skin* skin, s32 limbIndex) {
    s32 i;
    SkinLimb** skeleton = reinterpret_cast<SkinLimb**>(SEGMENTED_TO_VIRTUAL(skin->skeletonHeader->segment));
    SkinAnimatedLimbData* animatedLimbData = static_cast<SkinAnimatedLimbData*>(
...
void Skin_Init(GlobalContext* globalCtx, Skin* skin, const SkeletonHeader* skeletonHeader, const AnimationHeader* animationHeader) {
    ...
    skeleton = reinterpret_cast<decltype(skeleton)>( SEGMENTED_TO_VIRTUAL(skin->skeletonHeader->segment) );
    limbCount = skin->skeletonHeader->limbCount;

    skin->vtxTable = static_cast<decltype(skin->vtxTable)>(
        ZeldaArena_MallocDebug(limbCount * sizeof(SkinLimbVtx), "../z_skin_awb.c", 212));
    ...
    for (i = 0; i < limbCount; i++) {
        ...
        if ((limb->segmentType != SKIN_LIMB_TYPE_ANIMATED) || (limb->segment == NULL)) {
            ...
            SkinAnimatedLimbData* animatedLimbData = static_cast<SkinAnimatedLimbData*>( SEGMENTED_TO_VIRTUAL(((void)0, limb->segment)) );

            vtxEntry->index = 0;

            vtxEntry->buf[0] = static_cast<Vtx*>(
            ...
            vtxEntry->buf[1] = static_cast<Vtx*>(
...
s32 Skin_ApplyAnimTransformations(Skin* skin, MtxF* limbMatrices, Actor* actor, s32 setTranslation) {
    ...
    ret = func_800A698C(skin, reinterpret_cast<SkinLimb**>( SEGMENTED_TO_VIRTUAL(skin->skeletonHeader->segment) ), limbMatrices, LIMB_DONE, 0);
    if (!ret) {
        return ret;
    }
