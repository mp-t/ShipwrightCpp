...
s32 Object_Spawn(ObjectContext* objectCtx, s16 objectId) {
    ...
    ASSERT(((objectCtx->num < OBJECT_EXCHANGE_BANK_MAX) &&
           ...
           "thisv->num < OBJECT_EXCHANGE_BANK_MAX && (thisv->status[thisv->num].Segment + size) < thisv->endSegment",
           "../z_scene.c", 142);
...
void* func_800982FC(ObjectContext* objectCtx, s32 bankIndex, s16 objectId) {
    ...
    ASSERT(nextPtr < objectCtx->spaceEnd, "nextptr < thisv->endSegment", "../z_scene.c", 381);
...
void func_800985DC(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    globalCtx->setupActorList = static_cast<ActorEntry*>(SEGMENTED_TO_VIRTUAL(cmd->actorList.segment));
}
...
void func_80098674(GlobalContext* globalCtx, SceneCmd* cmd) {
    CollisionHeader* colHeader = static_cast<CollisionHeader*>(SEGMENTED_TO_VIRTUAL(cmd->colHeader.segment));
...
void func_800987A4(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    globalCtx->roomList = static_cast<RomFile*>(SEGMENTED_TO_VIRTUAL(cmd->roomList.segment));
}

// Scene Command 0x06: Entrance List
void func_800987F8(GlobalContext* globalCtx, SceneCmd* cmd) {
    globalCtx->setupEntranceList = static_cast<EntranceEntry*>(SEGMENTED_TO_VIRTUAL(cmd->entranceList.segment));
}
...
void func_8009883C(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    if (cmd->specialFiles.cUpElfMsgNum != 0) {
        globalCtx->cUpElfMsgs = static_cast<ElfMessage*>(Gameplay_LoadFile(globalCtx, &sNaviMsgFiles[cmd->specialFiles.cUpElfMsgNum - 1]));
    }
}
...
void func_80098958(GlobalContext* globalCtx, SceneCmd* cmd) {
    globalCtx->roomCtx.curRoom.mesh = static_cast<Mesh*>(SEGMENTED_TO_VIRTUAL(cmd->mesh.segment));
}
...
void func_8009899C(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    s16* objectEntry = static_cast<s16*>(SEGMENTED_TO_VIRTUAL(cmd->objectList.segment));
    void* nextPtr;
...
void func_80098B74(GlobalContext* globalCtx, SceneCmd* cmd) {
    s32 i;
    LightInfo* lightInfo = static_cast<LightInfo*>(SEGMENTED_TO_VIRTUAL(cmd->lightList.segment));
...
void func_80098C24(GlobalContext* globalCtx, SceneCmd* cmd) {
    globalCtx->setupPathList = static_cast<Path*>(SEGMENTED_TO_VIRTUAL(cmd->pathList.segment));
}
...
void func_80098C68(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    globalCtx->transiActorCtx.list = static_cast<TransitionActorEntry*>(SEGMENTED_TO_VIRTUAL(cmd->transiActorList.segment));
}
...
void func_80098CC8(GlobalContext* globalCtx, SceneCmd* cmd) {
    ...
    globalCtx->envCtx.lightSettingsList = static_cast<EnvLightSettings*>(SEGMENTED_TO_VIRTUAL(cmd->lightSettingList.segment));
}
...
void func_800990F0(GlobalContext* globalCtx, SceneCmd* cmd) {
    globalCtx->setupExitList = static_cast<s16*>(SEGMENTED_TO_VIRTUAL(cmd->exitList.segment));
}
