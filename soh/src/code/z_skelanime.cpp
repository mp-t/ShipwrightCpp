...
void SkelAnime_DrawFlexOpa(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, s32 dListCount,
    ...
    rootLimb = static_cast<decltype(rootLimb)>( SEGMENTED_TO_VIRTUAL(skeleton[0]) );
 ...
 */
void SkelAnime_GetFrameData(const AnimationHeader* animation, s32 frame, s32 limbCount, Vec3s* frameTable) {
    if (ResourceMgr_OTRSigCheck(animation) != 0)
        animation = reinterpret_cast<const AnimationHeader*>( ResourceMgr_LoadAnimByName(reinterpret_cast<const char*>(animation)) );
...
s16 Animation_GetLength(const void* animation) {
    if (ResourceMgr_OTRSigCheck(animation) != 0)
        animation = ResourceMgr_LoadAnimByName(static_cast<const char*>(animation));

    const auto* common = static_cast<const AnimationHeaderCommon*>( SEGMENTED_TO_VIRTUAL(animation) );

    return common->frameCount;
}

s16 Animation_GetLastFrame(const void* animation) {
    if (ResourceMgr_OTRSigCheck(animation) != 0)
        animation = ResourceMgr_LoadAnimByName(static_cast<const char*>(animation));
    const auto common = static_cast<const AnimationHeaderCommon*>( SEGMENTED_TO_VIRTUAL(animation) );
 ...
 */
void AnimationContext_SetLoadFrame(GlobalContext* globalCtx, const LinkAnimationHeader* animation, s32 frame, s32 limbCount,
                                   Vec3s* frameTable) {
    ...
    if (entry != NULL)
    {
        if (ResourceMgr_OTRSigCheck(animation) != 0)
            animation =
                reinterpret_cast<decltype(animation)>(ResourceMgr_LoadAnimByName(reinterpret_cast<const char*>(animation)));

        const LinkAnimationHeader* linkAnimHeader = SEGMENTED_TO_VIRTUAL(animation);
        ...
        char* animData = ResourceMgr_LoadPlayerAnimByName(animPath);

        memcpy(frameTable, animData + (((sizeof(Vec3s) * limbCount + 2) * frame)), sizeof(Vec3s) * limbCount + 2);
 ...
 * tables if not given.
 */
void SkelAnime_InitLink(GlobalContext* globalCtx, SkelAnime* skelAnime, const FlexSkeletonHeader* skeletonHeaderSeg,
                        const LinkAnimationHeader* animation, s32 flags, Vec3s* jointTable, Vec3s* morphTable,
                        s32 limbBufCount) {
    if (ResourceMgr_OTRSigCheck(skeletonHeaderSeg) != 0)
        skeletonHeaderSeg = reinterpret_cast<decltype(skeletonHeaderSeg)>(
            ResourceMgr_LoadSkeletonByName(reinterpret_cast<const char*>(skeletonHeaderSeg)));

    const FlexSkeletonHeader* skeletonHeader = SEGMENTED_TO_VIRTUAL(skeletonHeaderSeg);
    ...
    if (jointTable == NULL) {
        skelAnime->jointTable = static_cast<Vec3s*>( ZeldaArena_MallocDebug(allocSize, "../z_skelanime.c", 2364) );
        skelAnime->morphTable = static_cast<Vec3s*>( ZeldaArena_MallocDebug(allocSize, "../z_skelanime.c", 2365) );
    } else {
...
s32 LinkAnimation_Update(GlobalContext* globalCtx, SkelAnime* skelAnime) {
    using UpdateFunc = s32 (*)(GlobalContext*, SkelAnime*);
    return static_cast<UpdateFunc>(skelAnime->update)(globalCtx, skelAnime);
}
...
void LinkAnimation_AnimateFrame(GlobalContext* globalCtx, SkelAnime* skelAnime) {
    AnimationContext_SetLoadFrame(globalCtx, static_cast<const LinkAnimationHeader*>(skelAnime->animation), skelAnime->curFrame, skelAnime->limbCount,
 ...
 */
void LinkAnimation_Change(GlobalContext* globalCtx, SkelAnime* skelAnime, const LinkAnimationHeader* animation, f32 playSpeed,
                          f32 startFrame, f32 endFrame, u8 mode, f32 morphFrames) {
    const LinkAnimationHeader* ogAnim = animation;
...
s32 SkelAnime_InitFlex(GlobalContext* globalCtx, SkelAnime* skelAnime, const FlexSkeletonHeader* skeletonHeaderSeg,
    ...
    if (jointTable == NULL) {
        ...
        ASSERT(limbCount == skelAnime->limbCount, "joint_buff_num == thisv->joint_num", "../z_skelanime.c", 3052);
        skelAnime->jointTable = jointTable;
        skelAnime->morphTable = morphTable;
    }
    if ((skelAnime->jointTable == NULL) || (skelAnime->morphTable == NULL)) {
...
s32 SkelAnime_Update(SkelAnime* skelAnime) {
    using UpdateFunc = s32 (*)(SkelAnime*);
    return static_cast<UpdateFunc>(skelAnime->update)(skelAnime);
}
...
void SkelAnime_AnimateFrame(SkelAnime* skelAnime) {
    ...
    SkelAnime_GetFrameData(static_cast<const AnimationHeader*>(skelAnime->animation), skelAnime->curFrame, skelAnime->limbCount, skelAnime->jointTable);
    if (skelAnime->mode & ANIM_INTERP) {
        ...
        SkelAnime_GetFrameData(static_cast<const AnimationHeader*>(skelAnime->animation), frame, skelAnime->limbCount, nextjointTable);
...
s32 SkelAnime_Once(SkelAnime* skelAnime) {
    ...
    if (skelAnime->curFrame == skelAnime->endFrame) {
        SkelAnime_GetFrameData(static_cast<const AnimationHeader*>(skelAnime->animation), (s32)skelAnime->curFrame, skelAnime->limbCount,
                               skelAnime->jointTable);
        SkelAnime_AnimateFrame(skelAnime);
        return 1;
    }
 ...
 */
void Animation_ChangeImpl(SkelAnime* skelAnime, const AnimationHeader* animation, f32 playSpeed, f32 startFrame, f32 endFrame,
 ...
 */
void Animation_Change(SkelAnime* skelAnime, const AnimationHeader* animation, f32 playSpeed, f32 startFrame, f32 endFrame,
                      u8 mode, f32 morphFrames) {
    const AnimationHeader* ogAnim = animation;

    if (ResourceMgr_OTRSigCheck(animation) != 0)
        animation = reinterpret_cast<decltype(animation)>( ResourceMgr_LoadAnimByName(reinterpret_cast<const char*>(animation)) );
 ...
 */
void Animation_PlayOnce(SkelAnime* skelAnime, const AnimationHeader* animation) {
 ...
 * animation change.
 */
void Animation_MorphToPlayOnce(SkelAnime* skelAnime, const AnimationHeader* animation, f32 morphFrames) {
 ...
 */
void Animation_PlayLoop(SkelAnime* skelAnime, const AnimationHeader* animation) {
 ...
 * animation change.
 */
void Animation_MorphToLoop(SkelAnime* skelAnime, const AnimationHeader* animation, f32 morphFrames) {
