#include "global.h"

u16 sGfxPrintFontTLUT[64] = {
    0x0000, 0xFFFF, 0x0000, 0xFFFF, 0x0000, 0xFFFF, 0x0000, 0xFFFF, 0x0000, 0xFFFF, 0x0000, 0xFFFF, 0x0000,
    0xFFFF, 0x0000, 0xFFFF, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0x0000, 0x0000,
    0xFFFF, 0xFFFF, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0x0000, 0x0000, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0xFFFF,
    0xFFFF, 0x0000, 0x0000, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF,
};

u16 sGfxPrintRainbowTLUT[16] = {
    0xF801, 0xFBC1, 0xFFC1, 0x07C1, 0x0421, 0x003F, 0x803F, 0xF83F,
    0xF801, 0xFBC1, 0xFFC1, 0x07C1, 0x0421, 0x003F, 0x803F, 0xF83F,
};

u8 sGfxPrintRainbowData[8] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77 };

u8 sGfxPrintFontData[(16 * 256) / 2] = {
    ,
};

// Can be used to set GFXP_FLAG_ENLARGE by default
static u8 sDefaultSpecialFlags;

void GfxPrint_Setup(GfxPrint* thisx) {
    s32 width = 16;
    s32 height = 256;
    s32 i;

    gDPPipeSync(thisx->dList++);
    gDPSetOtherMode(thisx->dList++,
                    G_AD_DISABLE | G_CD_DISABLE | G_CK_NONE | G_TC_FILT | G_TF_BILERP | G_TT_IA16 | G_TL_TILE |
                        G_TD_CLAMP | G_TP_NONE | G_CYC_1CYCLE | G_PM_NPRIMITIVE,
                    G_AC_NONE | G_ZS_PRIM | G_RM_XLU_SURF | G_RM_XLU_SURF2);
    gDPSetCombineMode(thisx->dList++, G_CC_DECALRGBA, G_CC_DECALRGBA);
    gDPLoadTextureBlock_4b(thisx->dList++, sGfxPrintFontData, G_IM_FMT_CI, width, height, 0, G_TX_NOMIRROR | G_TX_WRAP,
                           G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
    gDPLoadTLUT(thisx->dList++, 64, 256, sGfxPrintFontTLUT);

    for (i = 1; i < 4; i++) {
        gDPSetTile(thisx->dList++, G_IM_FMT_CI, G_IM_SIZ_4b, 1, 0, i * 2, i, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK,
                   G_TX_NOLOD, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOLOD);
        gDPSetTileSize(thisx->dList++, i * 2, 0, 0, 60, 1020);
    }

    gDPSetColor(thisx->dList++, G_SETPRIMCOLOR, thisx->color.rgba);

    /*gDPLoadMultiTile_4b(thisx->dList++, sGfxPrintRainbowData, 0, 1, G_IM_FMT_CI, 2, 8, 0, 0, 1, 7, 4,
                        G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 1, 3, G_TX_NOLOD, G_TX_NOLOD);

    gDPLoadTLUT(thisx->dList++, 16, 320, sGfxPrintRainbowTLUT);

    for (i = 1; i < 4; i++) {
        gDPSetTile(thisx->dList++, G_IM_FMT_CI, G_IM_SIZ_4b, 1, 0, i * 2 + 1, 4, G_TX_NOMIRROR | G_TX_WRAP, 3,
                   G_TX_NOLOD, G_TX_NOMIRROR | G_TX_WRAP, 1, G_TX_NOLOD);
        gDPSetTileSize(thisx->dList++, i * 2 + 1, 0, 0, 4, 28);
    }*/
}

void GfxPrint_SetColor(GfxPrint* thisx, u32 r, u32 g, u32 b, u32 a) {
    thisx->color.r = r;
    thisx->color.g = g;
    thisx->color.b = b;
    thisx->color.a = a;
    gDPPipeSync(thisx->dList++);
    gDPSetColor(thisx->dList++, G_SETPRIMCOLOR, thisx->color.rgba);
}

void GfxPrint_SetPosPx(GfxPrint* thisx, s32 x, s32 y) {
    thisx->posX = thisx->baseX + (x * 4);
    thisx->posY = thisx->baseY + (y * 4);
}

void GfxPrint_SetPos(GfxPrint* thisx, s32 x, s32 y) {
    GfxPrint_SetPosPx(thisx, x * 8, y * 8);
}

void GfxPrint_SetBasePosPx(GfxPrint* thisx, s32 x, s32 y) {
    thisx->baseX = x * 4;
    thisx->baseY = y * 4;
}

void GfxPrint_PrintCharImpl(GfxPrint* thisx, u8 c) {
    u32 tile = (c & 0xFF) * 2;

    if (thisx->flags & GFXP_FLAG_UPDATE) {
        thisx->flags &= ~GFXP_FLAG_UPDATE;

        gDPPipeSync(thisx->dList++);
        if (thisx->flags & GFXP_FLAG_RAINBOW) {
            gDPSetTextureLUT(thisx->dList++, G_TT_RGBA16);
            gDPSetCycleType(thisx->dList++, G_CYC_2CYCLE);
            gDPSetRenderMode(thisx->dList++, G_RM_OPA_CI, G_RM_XLU_SURF2);
            gDPSetCombineMode(thisx->dList++, G_CC_INTERFERENCE, G_CC_PASS2);
        } else {
            gDPSetTextureLUT(thisx->dList++, G_TT_IA16);
            gDPSetCycleType(thisx->dList++, G_CYC_1CYCLE);
            gDPSetRenderMode(thisx->dList++, G_RM_XLU_SURF, G_RM_XLU_SURF2);
            gDPSetCombineMode(thisx->dList++, G_CC_MODULATEIDECALA_PRIM, G_CC_MODULATEIDECALA_PRIM);
        }
    }

    if (thisx->flags & GFXP_FLAG_SHADOW) {
        gDPSetColor(thisx->dList++, G_SETPRIMCOLOR, 0);

        if (thisx->flags & GFXP_FLAG_ENLARGE) {
            gSPTextureRectangle(thisx->dList++, (thisx->posX + 4) << 1, (thisx->posY + 4) << 1, (thisx->posX + 4 + 32) << 1,
                                (thisx->posY + 4 + 32) << 1, tile, (u16)(c & 4) * 64, (u16)(c >> 3) * 256, 1 << 9,
                                1 << 9);
        } else {
            gSPTextureRectangle(thisx->dList++, thisx->posX + 4, thisx->posY + 4, thisx->posX + 4 + 32, thisx->posY + 4 + 32,
                                tile, (u16)(c & 4) * 64, (u16)(c >> 3) * 256, 1 << 10, 1 << 10);
        }

        gDPSetColor(thisx->dList++, G_SETPRIMCOLOR, thisx->color.rgba);
    }

    if (thisx->flags & GFXP_FLAG_ENLARGE) {
        gSPTextureRectangle(thisx->dList++, (thisx->posX) << 1, (thisx->posY) << 1, (thisx->posX + 32) << 1,
                            (thisx->posY + 32) << 1, tile, (u16)(c & 4) * 64, (u16)(c >> 3) * 256, 1 << 9, 1 << 9);
    } else {
        gSPTextureRectangle(thisx->dList++, thisx->posX, thisx->posY, thisx->posX + 32, thisx->posY + 32, tile,
                            (u16)(c & 4) * 64, (u16)(c >> 3) * 256, 1 << 10, 1 << 10);
    }

    thisx->posX += 32;
}

void GfxPrint_PrintStringWithSize(GfxPrint* thisx, const void* buffer, u32 charSize, u32 charCount) {
    OTRGfxPrint((const char*)buffer, thisx, GfxPrint_PrintCharImpl);
}

void GfxPrint_PrintString(GfxPrint* thisx, const char* str) {
    OTRGfxPrint(str, thisx, GfxPrint_PrintCharImpl);
}

void GfxPrint_Callback(void* arg, const char* str, size_t size) {
    GfxPrint* thisx = static_cast<GfxPrint*>(arg);

    GfxPrint_PrintStringWithSize(thisx, str, sizeof(char), size);
}

void GfxPrint_Init(GfxPrint* thisx) {
    thisx->flags &= ~GFXP_FLAG_OPEN;

    thisx->callback = GfxPrint_Callback;
    thisx->dList = NULL;
    thisx->posX = 0;
    thisx->posY = 0;
    thisx->baseX = 0;
    thisx->baseY = 0;
    thisx->color.rgba = 0;

    thisx->flags &= ~GFXP_FLAG_HIRAGANA;
    thisx->flags &= ~GFXP_FLAG_RAINBOW;
    thisx->flags |= GFXP_FLAG_SHADOW;
    thisx->flags |= GFXP_FLAG_UPDATE;

    if (sDefaultSpecialFlags & GFXP_FLAG_ENLARGE) {
        thisx->flags |= GFXP_FLAG_ENLARGE;
    } else {
        thisx->flags &= ~GFXP_FLAG_ENLARGE;
    }
}

void GfxPrint_Destroy(GfxPrint* thisx) {
}

void GfxPrint_Open(GfxPrint* thisx, Gfx* dList) {
    if (!(thisx->flags & GFXP_FLAG_OPEN)) {
        thisx->flags |= GFXP_FLAG_OPEN;
        thisx->dList = dList;
        GfxPrint_Setup(thisx);
    } else {
        osSyncPrintf("gfxprint_open:２重オープンです\n");
    }
}

Gfx* GfxPrint_Close(GfxPrint* thisx) {
    Gfx* ret;

    thisx->flags &= ~GFXP_FLAG_OPEN;
    gDPPipeSync(thisx->dList++);
    ret = thisx->dList;
    thisx->dList = NULL;

    return ret;
}

s32 GfxPrint_VPrintf(GfxPrint* thisx, const char* fmt, va_list args) {
    return PrintUtils_VPrintf(&thisx->callback, fmt, args);
}

s32 GfxPrint_Printf(GfxPrint* thisx, const char* fmt, ...) {
    s32 ret;
    va_list args;
    va_start(args, fmt);

    ret = GfxPrint_VPrintf(thisx, fmt, args);

    va_end(args);

    return ret;
}
