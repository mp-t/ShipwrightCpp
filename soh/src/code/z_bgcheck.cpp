 ...
 */
s32 BgCheck_PosErrorCheck(Vec3f* pos, const char* file, s32 line) {
...
void DynaSSNodeList_Alloc(GlobalContext* globalCtx, DynaSSNodeList* nodeList, s32 max) {
    nodeList->tbl = static_cast<SSNode*>( THA_AllocEndAlign(&globalCtx->state.tha, max * sizeof(SSNode), -2) );
...
s16 CollisionPoly_GetMinY(CollisionPoly* poly, Vec3s* vtxList) {
    ...
    //! @bug Due to rounding errors, some polys with a slight slope have a y normal of 1.0f/-1.0f. As such, thisv
...
void func_80038A28(CollisionPoly* poly, f32 tx, f32 ty, f32 tz, MtxF* dest) {
    ...
    dest->mf_raw.xx = phi_f2;
    dest->mf_raw.yx = (-nx) * phi_f14;
    dest->mf_raw.zx = nx * phi_f12;
    dest->mf_raw.xy = nx;
    dest->mf_raw.yy = ny;
    dest->mf_raw.zy = nz;
    dest->mf_raw.yz = phi_f12;
    dest->mf_raw.zz = phi_f14;
    dest->mf_raw.wx = 0.0f;
    dest->mf_raw.wy = 0.0f;
    dest->mf_raw.xz = 0.0f;
    dest->mf_raw.wz = 0.0f;
...
void BgCheck_Allocate(CollisionContext* colCtx, GlobalContext* globalCtx, CollisionHeader* colHeader) {
    ...
    colCtx->lookupTbl = static_cast<StaticLookup*>( THA_AllocEndAlign(
        &globalCtx->state.tha,
    ...
    if (customNodeListMax > 0) {
        ...
        // thisv is a non-issue as long as sceneSubdivisionList.nodeListMax is -1
 ...
 */
const CollisionHeader* BgCheck_GetCollisionHeader(CollisionContext* colCtx, s32 bgId) {
    if (bgId == BGCHECK_SCENE) {
 ...
 * Init SSNodeList
 */
void SSNodeList_Initialize(SSNodeList* thisx) {
    thisx->max = 0;
    thisx->count = 0;
    thisx->tbl = NULL;
    thisx->polyCheckTbl = NULL;
}
 ...
 */
void SSNodeList_Alloc(GlobalContext* globalCtx, SSNodeList* thisx, s32 tblMax, s32 numPolys) {
    thisx->max = tblMax;
    thisx->count = 0;
    thisx->tbl = static_cast<SSNode*>( THA_AllocEndAlign(&globalCtx->state.tha, tblMax * sizeof(SSNode), -2) );

    ASSERT(thisx->tbl != NULL, "thisx->short_slist_node_tbl != NULL", "../z_bgcheck.c", 5975);

    thisx->polyCheckTbl = static_cast<u8*>( GameState_Alloc(&globalCtx->state, numPolys, "../z_bgcheck.c", 5979) );

    ASSERT(thisx->polyCheckTbl != NULL, "thisx->polygon_check != NULL", "../z_bgcheck.c", 5981);
}

/**
 * Get next SSNodeList SSNode
 */
SSNode* SSNodeList_GetNextNode(SSNodeList* thisx) {
    SSNode* result = &thisx->tbl[thisx->count];

    thisx->count++;
    ASSERT(thisx->count < thisx->max, "thisx->short_slist_node_last_index < thisx->short_slist_node_size", "../z_bgcheck.c",
           5998);
    if (!(thisx->count < thisx->max)) {
        return NULL;
    }
    return result;
}
...
u16 SSNodeList_GetNextNodeIdx(SSNodeList* thisx) {
    u16 new_index = thisx->count++;

    ASSERT(new_index < thisx->max, "new_index < thisx->short_slist_node_size", "../z_bgcheck.c", 6021);
    return new_index;
}
 ...
 * setActor internal
 */
void BgActor_SetActor(BgActor* bgActor, Actor* actor, const CollisionHeader* colHeader) {
    bgActor->actor = actor;
...
void DynaPoly_AllocPolyList(GlobalContext* globalCtx, CollisionPoly** polyList, s32 numPolys) {
    *polyList = static_cast<CollisionPoly*>( THA_AllocEndAlign(&globalCtx->state.tha, numPolys * sizeof(CollisionPoly), -2) );
...
void DynaPoly_AllocVtxList(GlobalContext* globalCtx, Vec3s** vtxList, s32 numVtx) {
    *vtxList = static_cast<Vec3s*>( THA_AllocEndAlign(&globalCtx->state.tha, numVtx * sizeof(Vec3s), -2) );
        ...
        *vtxStartIndex += pbgdata->numVertices;
        ...
        for (i = 0; i < pbgdata->numPolygons; i++) {
            ...
            *newPoly = pbgdata->polyList[i];
            ...
            // Yeah, thisv is all kinds of fake, but my God, it matches.
            newPoly->flags_vIA =
 ...
 */
void CollisionHeader_SegmentedToVirtual(const CollisionHeader* colHeader) {
    if (ResourceMgr_OTRSigCheck(colHeader))
        colHeader = ResourceMgr_LoadColByName(reinterpret_cast<const char*>(colHeader));
}

/**
 * Convert CollisionHeader Segmented to Virtual addressing
 */
void CollisionHeader_GetVirtual(const CollisionHeader* colHeader, const CollisionHeader** dest) 
{
    if (ResourceMgr_OTRSigCheck(colHeader))
        colHeader = ResourceMgr_LoadColByName(reinterpret_cast<const char*>(colHeader));
...
u16 SurfaceType_GetCameraSType(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    const CollisionHeader* colHeader = BgCheck_GetCollisionHeader(colCtx, bgId);
    CamData* camData;
    SurfaceType* surfaceTypes;
