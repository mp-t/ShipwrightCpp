...
OSMesgQueue* PadMgr_LockSerialMesgQueue(PadMgr* padMgr) {
    ...
    osRecvMesg(&padMgr->serialMsgQ, reinterpret_cast<OSMesg*>(&ctrlrQ), OS_MESG_BLOCK);
    ...
    for (i = 0; i < padMgr->nControllers; i++, input++, padnow1++) {
        ...
        switch (padnow1->err_no) {
            ...
            case 4:
                ...
                LOG_NUM("thisv->Key_switch[i]", padMgr->ctrlrIsConnected[i], "../padmgr.c", 380);
                osSyncPrintf(VT_FGCOL(YELLOW));
    ...
    for (i = 0; i < 4; i++) {
        if (padMgr->padStatus[i].err_no == 0) {
            if (padMgr->padStatus[i].type == CONT_TYPE_NORMAL) {
                ...
                //LOG_HEX("thisv->pad_status[i].type", padMgr->padStatus[i].type, "../padmgr.c", 458);
...
void PadMgr_ThreadEntry(PadMgr* padMgr) {
    ...
    while (!exit) {
        ...
        osRecvMesg(&padMgr->interruptMsgQ, reinterpret_cast<OSMesg*>(&mesg), OS_MESG_BLOCK);
