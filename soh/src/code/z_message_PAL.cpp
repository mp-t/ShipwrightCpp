...
void Message_DrawTextChar(GlobalContext* globalCtx, void* textureImage, Gfx** p) {
    ...
    gSPInvalidateTexCache(gfx++, reinterpret_cast<std::uintptr_t>(textureImage));
    ...
    if (gSaveContext.language == LANGUAGE_ENG) {
        ...
        while (messageTableEntry->textId != 0xFFFF) {
            ...
            if (messageTableEntry->textId == textId) {
                ...
                font->msgOffset = reinterpret_cast<std::uintptr_t>(messageTableEntry->segment);
...
u16 Message_DrawItemIcon(GlobalContext* globalCtx, u16 itemId, Gfx** p, u16 i) {
    ...
    if (itemId >= ITEM_MEDALLION_FOREST) {
        gDPLoadTextureBlock(gfx++, static_cast<u8*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE, G_IM_FMT_RGBA,
                            G_IM_SIZ_32b,
                            24, 24, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK,
                            G_TX_NOLOD, G_TX_NOLOD);
    } else {
        gDPLoadTextureBlock(gfx++, static_cast<u8*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE, G_IM_FMT_RGBA, G_IM_SIZ_32b,
    ...
    for (i = 0; i < msgCtx->textDrawPos; i++) {
        ...
        switch (character) {
            ...
            case MESSAGE_BACKGROUND:
                ...
                gDPLoadTextureBlock_4b(gfx++, static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE, G_IM_FMT_I, 96, 48,
                ...
                gDPLoadTextureBlock_4b(gfx++, static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE + 0x900, G_IM_FMT_I,
                ...
                gDPLoadTextureBlock_4b(gfx++, (static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE), G_IM_FMT_I, 96,
                ...
                gDPLoadTextureBlock_4b(gfx++, (static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE + 0x900),
...
void Message_LoadItemIcon(GlobalContext* globalCtx, u16 itemId, s16 y) {
    ...
    if (itemId < ITEM_MEDALLION_FOREST) {
        ...
        memcpy(static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE, gItemIcons[itemId], 0x1000);
        // "Item 32-0"
        ...
        R_TEXTBOX_ICON_SIZE = 24;
         memcpy(static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE, gItemIcons[itemId], 0x900);
        // "Item 24"
...
void Message_Decode(GlobalContext* globalCtx) {
    ...
    while (true) {
        ...
        if (temp_s2 == MESSAGE_BOX_BREAK || temp_s2 == MESSAGE_TEXTID || temp_s2 == MESSAGE_BOX_BREAK_DELAYED ||
            ...
            memcpy(static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE,
                   ResourceMgr_LoadTexByName(gRedMessageXLeftTex), 0x900);
            memcpy(static_cast<char*>(msgCtx->textboxSegment) + MESSAGE_STATIC_TEX_SIZE + 0x900,
...
void Message_OpenText(GlobalContext* globalCtx, u16 textId) {
    ...
    if (sTextIsCredits) {
        ...
        char* src = reinterpret_cast<char*>(font->msgOffset);
...
void Message_DrawMain(GlobalContext* globalCtx, Gfx** p) {
    ...
    static const void* sOcarinaNoteTextures[] = {
        ...
        case MSGMODE_TEXT_CLOSING:
            ...
            if (msgCtx->ocarinaAction != OCARINA_ACTION_CHECK_NOWARP_DONE) {
                if (sLastPlayedSong == OCARINA_SONG_SARIAS) {
                    ...
                    //       before it reaches thisv point after playing Saria's song, the song will be "stored".
                    //       Later, if the ocarina has not been played and another textbox is closed, thisv handling
...
// late_rodata section of thisv file is in the way
