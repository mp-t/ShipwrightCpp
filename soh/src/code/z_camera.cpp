...
void Camera_DebugPrintSplineArray(const char* name, s16 length, CutsceneCameraPoint cameraPoints[]) {
    s32 i;
...
s32 Camera_Demo5(Camera* camera) {
    ...
    if (camera->target->category == ACTORCAT_PLAYER) {
        ...
        if (!func_800C0D34(camera->globalCtx, camera->target, &sp4A)) {
            osSyncPrintf(VT_COL(YELLOW, BLACK) "camera: attention demo: thisv door is dummy door!\n" VT_RST);
    ...
    if ((pad >= 0x33) || (pad < -0x32)) {
        func_80078884(reinterpret_cast<u16>(camera->data1)); //WAT
    }
...
Camera* Camera_Create(View* view, CollisionContext* colCtx, GlobalContext* globalCtx) {
    Camera* newCamera = static_cast<Camera*>( ZeldaArena_MallocDebug(sizeof(*newCamera), "../z_camera.c", 9370) );
...
void Camera_PrintSettings(Camera* camera) {
    ...
    if ((OREG(0) & 1) && (camera->globalCtx->activeCamera == camera->thisIdx) && !gDbgCamEnabled) {
        ...
        //! @bug: thisv code was clearly meaning to print `abs(camera->camDataIdx)` as a
