...
void Fault_AddClient(FaultClient* client, void* callback, void* param0, const void* param1) 
{
#if 0
    OSIntMask mask;
...
void Fault_DrawStackTrace(OSThread* thread, s32 x, s32 y, s32 height) {
    ...
    for (line = 1; line < height && (ra != 0 || sp != 0) && pc != (u32)__osCleanupThread; line++) {
        ...
        addr = Fault_ConvertAddress(reinterpret_cast<FaultAddrConvClient*>(pc));
        if (addr != 0) {
...
void Fault_ResumeThread(OSThread* t) {
    ...
    *(u32*)t->context.pc = 0xD;
    osWritebackDCache(reinterpret_cast<void*>(t->context.pc), 4);
    osInvalICache(reinterpret_cast<void*>(t->context.pc), 4);
    osStartThread(t);
}
