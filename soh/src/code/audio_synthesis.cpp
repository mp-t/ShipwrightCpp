...
Acmd* func_800DB4E4(Acmd* cmd, s32 arg1, SynthesisReverb* reverb, s16 arg3) {
    ...
    aResample(cmd++, reverb->resampleFlags, reverb->unk_0E, static_cast<s16*>(reverb->unk_30));
    aSetBuffer(cmd++, 0, DMEM_WET_TEMP + DEFAULT_LEN_1CH + offsetA, DMEM_WET_RIGHT_CH, arg1 * 2);
    aResample(cmd++, reverb->resampleFlags, reverb->unk_0E, static_cast<s16*>(reverb->unk_34));
    return cmd;
}
...
Acmd* func_800DB680(Acmd* cmd, SynthesisReverb* reverb, s16 bufIndex) {
    ...
    aResample(cmd++, reverb->resampleFlags, bufItem->unk_16, static_cast<s16*>(reverb->unk_38));
    ...
    aResample(cmd++, reverb->resampleFlags, bufItem->unk_16, static_cast<s16*>(reverb->unk_3C));
...
Acmd* func_800DB828(Acmd* cmd, s32 arg1, SynthesisReverb* reverb, s16 arg3) {
    ...
    aSetBuffer(cmd++, 0, DMEM_WET_TEMP + offsetA, DMEM_WET_LEFT_CH, arg1 * 2);
    aResample(cmd++, reverb->resampleFlags, item->unk_14, static_cast<s16*>(reverb->unk_30));
    aSetBuffer(cmd++, 0, DMEM_WET_TEMP + DEFAULT_LEN_1CH + offsetA, DMEM_WET_RIGHT_CH, arg1 * 2);
    aResample(cmd++, reverb->resampleFlags, item->unk_14, static_cast<s16*>(reverb->unk_34));
    return cmd;
}
...
void AudioSynth_LoadBuffer(Acmd* cmd, u16 dest_addr, u16 nbytes, const void* source_addr) {
    aLoadBuffer(cmd, source_addr, dest_addr, nbytes);
}

void AudioSynth_SaveBuffer(Acmd* cmd, u16 source_addr, u16 nbytes, s16* dest_addr) {
    aSaveBuffer(cmd, source_addr, dest_addr, nbytes);
}
...
void AudioSynth_LoadFilter(Acmd* cmd, u8 flags, u16 countOrBuf, s16* addr) {
    aFilter(cmd, flags, countOrBuf, addr);
}

void AudioSynth_LoadFilterCount(Acmd* cmd, u16 count, s16* addr) {
...
Acmd* AudioSynth_ProcessNote(s32 noteIndex, NoteSubEu* noteSubEu, NoteSynthesisState* synthState, s16* aiBuf,
    ...
    // makes it clear that thisv is how it should look.
    ...
    if (noteSubEu->bitField1.isSyntheticWave) {
        ...
        sampleAddr = reinterpret_cast<u32>(audioFontSample->sampleAddr);
        resampledTempLen = 0;
        ...
        for (curPart = 0; curPart < nParts; curPart++) {
            ...
            if (audioFontSample->bits.codec == CODEC_ADPCM || audioFontSample->bits.codec == CODEC_SMALL_ADPCM) {
            ...
            while (nSamplesProcessed != samplesLenAdjusted) {
                ...
                switch (audioFontSample->bits.codec) {
                    case CODEC_ADPCM:
                        frameSize = 9;
                ...
                if (nFramesToDecode != 0) {
                    ...
                    if (audioFontSample->bits.medium == MEDIUM_RAM) {
                        sampleData = (u8*)(sampleDataStart + sampleDataOffset + sampleAddr);
                    } else if (audioFontSample->bits.medium == MEDIUM_UNK) {
                        return cmd;
                    } else {
                        sampleData = static_cast<u8*>(AudioLoad_DmaSampleData(sampleDataStart + sampleDataOffset + sampleAddr,
                                                             ALIGN16((nFramesToDecode * frameSize) + 0x10), flags,
                                                             &synthState->sampleDmaIndex, audioFontSample->bits.medium));
                    }
                ...
                if (synthState->restart) {
                    aSetLoop(cmd++, reinterpret_cast<ADPCM_STATE*>(audioFontSample->loop->state));
                    flags = A_LOOP;
