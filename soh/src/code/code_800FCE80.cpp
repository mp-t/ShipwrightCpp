...
f32 Math_FAtanTaylorF(f32 x) {
    ...
    if (t <= std::numbers::sqrt2_v<float> - 1.0f) {
        return Math_FAtanTaylorQF(x);
    }

    if (t >= std::numbers::sqrt2_v<float> + 1.0f) {
        q = std::numbers::pi_v<float> / 2 - Math_FAtanTaylorQF(1.0f / t);
    } else {
        q = std::numbers::pi_v<float> / 4 - Math_FAtanTaylorQF((1.0f - t) / (1.0f + t));
    }
...
f32 Math_FAtanContFracF(f32 x) {
    ...
    if (sector == 0) {
        ...
        return std::numbers::pi_v<float> / 2 - conv;
    } else {
        return -std::numbers::pi_v<float> / 2 - conv;
    }
}
...
f32 Math_FAtan2F(f32 y, f32 x) {
    if (x == 0.0f) {
        ...
        return Math_FAtanF(y / x) - std::numbers::pi_v<float>;
    } else {
        return std::numbers::pi_v<float> - Math_FAtanF(-(y / x));
    }
}
...
f32 Math_FAcosF(f32 x) {
    return std::numbers::pi_v<float> / 2 - Math_FAsinF(x);
}
