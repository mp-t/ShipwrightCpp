...
s32 AudioSeq_SeqChannelSetLayer(SequenceChannel* channel, s32 layerIdx) {
    ...
    if (channel->layers[layerIdx] == NULL) {
        SequenceLayer* layer;
        layer = static_cast<SequenceLayer*>(AudioSeq_AudioListPopBack(&gAudioContext.layerFreeList));
...
void AudioSeq_SequenceChannelEnable(SequencePlayer* seqPlayer, u8 channelIdx, void* script) {
    ...
    channel->finished = false;
    channel->scriptState.depth = 0;
    channel->scriptState.pc = static_cast<u8*>(script);
    channel->delay = 0;
    for (i = 0; i < 4; i++) {
...
s32 AudioSeq_SeqLayerProcessScriptStep5(SequenceLayer* layer, s32 sameSound) {
    if (!layer->stopSomething && layer->sound != NULL && layer->sound->sample->bits.codec == CODEC_S16_INMEMORY &&
        layer->sound->sample->bits.medium != MEDIUM_RAM) {
        layer->stopSomething = true;
        return -1;
    }
            ...
            if (command >= 0xF2) {
                ...
                switch (command) {
                    ...
                    case 0xC2:
                        ...
                        channel->dynTable = reinterpret_cast<decltype(channel->dynTable)>(&seqPlayer->seqData[offset]);
                        break;
                    case 0xC5:
                    ...
                    case 0xB2:
                        ...
                        break;
                    case 0xB4:
                        channel->dynTable = reinterpret_cast<decltype(channel->dynTable)>(&seqPlayer->seqData[channel->unk_22]);
                        break;
                    case 0xB5:
                        // OTRTODO: Byteswap added for quick audio
...
void AudioSeq_InitSequencePlayerChannels(s32 playerIdx) {
    ...
    for (i = 0; i < 0x10; i++) {
        seqPlayer->channels[i] = static_cast<SequenceChannel*>(AudioHeap_AllocZeroed(&gAudioContext.notesAndBuffersPool, sizeof(SequenceChannel)));
