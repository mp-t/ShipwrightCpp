...
void Main(void* arg) {
    ...
    Fault_Init();
    SysCfb_Init(0);
    sysHeap = reinterpret_cast<std::uintptr_t>(gSystemHeap);
    fb = reinterpret_cast<std::uintptr_t>(SysCfb_GetFbPtr(0));
    gSystemHeapSize = 1024 * 1024 * 4;
    // "System heap initalization"
    osSyncPrintf("システムヒープ初期化 %08x-%08x %08x\n", sysHeap, fb, gSystemHeapSize);
    SystemHeap_Init(reinterpret_cast<void*>(sysHeap), gSystemHeapSize); // initializes the system heap
    if (osMemSize >= 0x800000) {
        debugHeap = reinterpret_cast<void*>(SysCfb_GetFbEnd());
