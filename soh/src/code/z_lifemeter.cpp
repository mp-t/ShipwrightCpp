...
static const void* sHeartTextures[] = {
...
static const void* sHeartDDTextures[] = {
...
void HealthMeter_Draw(GlobalContext* globalCtx) {
    ...
    for (i = 0; i < totalHeartCount; i++) {
        ...
        if (curBgImgLoaded != heartBgImg) {
            curBgImgLoaded = static_cast<const u8*>(heartBgImg);
        ...
        if (i != fullHeartCount) {
            ...
            {
                Mtx* matrix = static_cast<Mtx*>(Graph_Alloc(gfxCtx, sizeof(Mtx)));
                ...
                gSPVertex(OVERLAY_DISP++, reinterpret_cast<std::uintptr_t>(sp154), 4, 0);
