...
u32 UCodeDisas_TranslateAddr(UCodeDisas* thisv, u32 addr) {
    u32 physical = thisv->segments[SEGMENT_NUMBER(addr)] + SEGMENT_OFFSET(addr);
...
void UCodeDisas_Init(UCodeDisas* thisv) {
    u32 i;

    memset(thisv,0, sizeof(UCodeDisas));
    for (i = 0; i < NUM_SEGMENTS; i++) {
        thisv->segments[i] = gSegments[i];
    }
}

void UCodeDisas_Destroy(UCodeDisas* thisv) {
}

void UCodeDisas_SetCurUCodeImpl(UCodeDisas* thisv, void* ptr) {
    s32 i;

    for (i = 0; i < thisv->ucodeInfoCount; i++) {
        if (ptr == thisv->ucodeInfo[i].ptr) {
            thisv->ucodeType = thisv->ucodeInfo[i].type;
            break;
        }
    }
    if (i >= thisv->ucodeInfoCount) {
        DISAS_LOG("マイクロコードが一致しなかった\n"); // "Microcode did not match"
        thisv->ucodeType = UCODE_NULL;
    }
}

void UCodeDisas_ParseGeometryMode(UCodeDisas* thisv, u32 mode) {
    u32 first = true;
    s32 i;
...
void UCodeDisas_ParseRenderMode(UCodeDisas* thisv, u32 mode) {
    ...
    // clang-format off
    if (thisv->enableLog == 0) {} else { osSyncPrintf("\nGBL_c1(%s, %s, %s, %s)|",
...
void UCodeDisas_PrintVertices(UCodeDisas* thisv, Vtx* vtx, s32 count, s32 start) {
    s32 i;
    for (i = 0; i < count; i++) {
        if (thisv->geometryMode & G_LIGHTING) {
...
void UCodeDisas_Disassemble(UCodeDisas* thisv, GfxMod* ptr) {
    u32 pad;
    u32 addr;
    u32 rdpHalf;
    u16 linkDlLow;
    ...
    while (!exit) {
        ...
        ptr = UCodeDisas_TranslateAddr(thisv, ptr);
        DISAS_LOG("%08x:", ptr);

        *curGfx = *ptr;
        cmd = curGfx->dma.cmd;
        addr = UCodeDisas_TranslateAddr(thisv, curGfx->dma.addr);

        DISAS_LOG("%08x-%08x:", curGfx->words.w0, curGfx->words.w1);

        for (i0 = 0; i0 < thisv->dlDepth; i0++) {
            DISAS_LOG(" ");
        }
        ...
        switch (cmd) {
            ...
            case G_DL: {
                ...
                switch (dma.par) {
                    case 0: {
                        ...
                        thisv->dlStack[thisv->dlDepth++] = (Gfx*)(ptr + 1);
                        ptr = (GfxMod*)addr - 1;
                    } break;
            ...
            case G_LOAD_UCODE: {
                ...
                UCodeDisas_SetCurUCodeImpl(thisv, (void*)UCodeDisas_TranslateAddr(thisv, curGfx->dma.addr));
                thisv->loaducodeCnt++;
            } break;
            ...
            case G_ENDDL: {
                ...
                if (thisv->dlDepth <= 0) {
                    ...
                    ptr = (GfxMod*)thisv->dlStack[--thisv->dlDepth] - 1;
                }
            } break;
            ...
            case G_SETOTHERMODE_L: {
                ...
                if (sft == G_MDSFT_RENDERMODE) {
                    ...
                    UCodeDisas_ParseRenderMode(thisv, s2);
                    DISAS_LOG("\n),");
                } else {
            ...
            case G_RDPSETOTHERMODE: {
                ...
                thisv->modeH = curGfx->words.w0 & 0xFFF;
                thisv->modeL = curGfx->words.w1;
            ...
            case G_RDPTILESYNC: {
                ...
                thisv->tileSyncRequired = false;
            } break;

            case G_RDPPIPESYNC: {
                DISAS_LOG("gsDPPipeSync(),");
                thisv->pipeSyncRequired = false;
            } break;

            case G_RDPLOADSYNC: {
                DISAS_LOG("gsDPLoadSync(),");
                thisv->loadSyncRequired = false;
            } break;
            ...
            case G_NOOP: {
                switch (curGfx->noop.type) {
                    ...
                    case 5: {
                        ...
                        thisv->enableLog = curGfx->noop.len;
                    } break;

                    case 6: {
                        /*! @bug arguments are not printed */
                        DISAS_LOG("count_gsDPNoOpCallBack(%08x,%d),");
                        ((void (*)(UCodeDisas*, u32))curGfx->noop.value.u32)(thisv, curGfx->noop.len);
                    } break;
                ...
                switch (thisv->ucodeType) {
                    ...
                    case UCODE_UNK: {
                        switch (cmd) {
                            ...
                            case G_VTX: {
                                ...
                                if (thisv->enableLog >= 2) {
                                    UCodeDisas_PrintVertices(thisv, addr, numv, vbidx);
                                }
                            } break;
                            ...
                            case G_BRANCH_Z: {
                                addr = UCodeDisas_TranslateAddr(thisv, rdpHalf);
                            ...
                            case G_GEOMETRYMODE: {
                                ...
                                if (clearbits == 0) {
                                    ...
                                    UCodeDisas_ParseGeometryMode(thisv, setbits);
                                    DISAS_LOG("),");
                                } else if (setbits == 0) {
                                    DISAS_LOG("gsSPClearGeometryMode(");
                                    UCodeDisas_ParseGeometryMode(thisv, ~clearbits);
                                    DISAS_LOG("),");
                                ...
                                thisv->geometryMode &= clearbits;
                                thisv->geometryMode |= setbits;
                                break; // thisv break needs to be inside, but the other cases need their breaks to be
                                       // outside...
                            }
                            ...
                            case G_MOVEWORD: {
                                ...
                                switch (movewd.index) {
                                    case G_MW_SEGMENT: {
                                        ...
                                        thisv->segments[movewd.offset / 4] = movewd.data & 0xFFFFFF;
                                    } break;
                    ...
                    case UCODE_S2DEX: {
                        switch (cmd) {
                            ...
                            case G_SELECT_DL: {
                                Gdma dma = ptr->dma;
                                u32 dlAddr = UCodeDisas_TranslateAddr(thisv, (dma.len << 16) | (linkDlLow));
                            ...
                            case G_MOVEWORD: {
                                ...
                                switch (movewd.index) {
                                    case 6: {
                                        ...
                                        thisv->segments[segId] = movewd.data & 0xFFFFFF;
                                    } break;
...
void UCodeDisas_RegisterUCode(UCodeDisas* thisv, s32 count, UCodeInfo* ucodeArray) {
    thisv->ucodeInfoCount = count;
    thisv->ucodeInfo = ucodeArray;
}

void UCodeDisas_SetCurUCode(UCodeDisas* thisv, void* ptr) {
    UCodeDisas_SetCurUCodeImpl(thisv, ptr);
}
#endif
