...
void func_80095AB4(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    polygonDlist = static_cast<PolygonDlist*>(SEGMENTED_TO_VIRTUAL(polygon0->start));
...
void func_80095D04(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    polygonDlist = static_cast<PolygonDlist2*>(SEGMENTED_TO_VIRTUAL(polygon2->start));
    spA4 = spB8;
    ...
    iREG(87) = polygon2->num & 0xFFFF & 0xFFFF & 0xFFFF; // if thisv is real then I might not be
...
void func_8009638C(Gfx** displayList, void* source, const void* tlut, u16 width, u16 height, u8 fmt, u8 siz, u16 mode0,
    ...
    bg->b.imagePtr = static_cast<u64*>(source);
    ...
    displayListHead = reinterpret_cast<Gfx*>(bg + 1);
    if (fmt == G_IM_FMT_CI) {
...
void func_80096680(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    polygonDlist = reinterpret_cast<PolygonDlist*>( SEGMENTED_TO_VIRTUAL(polygon1->dlist) );
    ...
    if (sp90 || sp94) {
        ...
        if (sp94) {
...

            {
                ...
                func_8009638C(&spA8, bgImage->source, reinterpret_cast<void*>(bgImage->tlut), bgImage->width, bgImage->height, bgImage->fmt,
...
s32 func_8009728C(GlobalContext* globalCtx, RoomContext* roomCtx, s32 roomNum) {
    ...
    if (roomCtx->status == 0) {
        ...
        DmaMgr_SendRequest2(&roomCtx->dmaRequest, reinterpret_cast<std::uintptr_t>(roomCtx->unk_34), globalCtx->roomList[roomNum].vromStart, size, 0,
...
s32 func_800973FC(GlobalContext* globalCtx, RoomContext* roomCtx) {
    ...
    if (roomCtx->status == 1) {
        if (!osRecvMesg(&roomCtx->loadQueue, NULL, OS_MESG_NOBLOCK)) {
            ...
            Scene_ExecuteCommands(globalCtx, static_cast<SceneCmd*>(roomCtx->curRoom.segment));
...
void Room_Draw(GlobalContext* globalCtx, Room* room, u32 flags) {
    if (room->segment != NULL)
    {
        ...
        ASSERT(room->mesh->polygon.type < ARRAY_COUNTU(sRoomDrawHandlers),
               "thisv->ground_shape->polygon.type < number(Room_Draw_Proc)", "../z_room.c", 1125);
