...
// effects that use thisx draw function are responsible for making sure their regs line up with the usage here

void EffectSs_DrawGEffect(GlobalContext* globalCtx, EffectSs* thisx, const void* texture) {
    ...
    s32 pad1;
    Mtx* mtx;
    void* object = globalCtx->objectCtx.status[thisx->rgObjBankIdx].segment;

    OPEN_DISPS(gfxCtx, "../z_effect_soft_sprite_old_init.c", 196);

    scale = thisx->rgScale * 0.0025f;
    SkinMatrix_SetTranslate(&mfTrans, thisx->pos.x, thisx->pos.y, thisx->pos.z);
    ...
    if (mtx != NULL) {
        ...
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, thisx->rgPrimColorR, thisx->rgPrimColorG, thisx->rgPrimColorB,
                        thisx->rgPrimColorA);
        gDPSetEnvColor(POLY_XLU_DISP++, thisx->rgEnvColorR, thisx->rgEnvColorG, thisx->rgEnvColorB, thisx->rgEnvColorA);
        gSPDisplayList(POLY_XLU_DISP++, thisx->gfx);
    }
...
void EffectSsHahen_Spawn(GlobalContext* globalCtx, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 unused, s16 scale,
                         s16 objId, s16 life, const Gfx* dList) {
...
void EffectSsHahen_SpawnBurst(GlobalContext* globalCtx, Vec3f* pos, f32 burstScale, s16 unused, s16 scale,
                              s16 randScaleRange, s16 count, s16 objId, s16 life, const Gfx* dList) {
    s32 i;
    Vec3f velocity;
    Vec3f accel;
 ...
 * Note: thisx type requires OBJECT_FHG to be loaded
 */
 ...
 *
 * Note: thisx effect requires OBJECT_FZ to be loaded
 */
