...
s32 Audio_SetFontInstrument(s32 instrumentType, s32 fontId, s32 index, void* value) {
    ...
    switch (instrumentType) {
        case 0:
            ...
            gAudioContext.soundFonts[fontId].drums[index] = static_cast<Drum*>(value);
            break;
        ...
        default:
            ...
            gAudioContext.soundFonts[fontId].instruments[index] = static_cast<Instrument*>(value);
            break;
    }
...
void Audio_NotePoolFill(NotePool* pool, s32 count) {
    ...
    for (i = 0, j = 0; j < count; i++) {
        ...
        while (j < count) {
            note = static_cast<Note*>(AudioSeq_AudioListPopBack(source));
            if (note == NULL) {
                break;
            }
...
Note* Audio_FindNodeWithPrioLessThan(AudioListItem* list, s32 limit) {
    ...
    return static_cast<Note*>(best->u.value);
}
...
Note* Audio_AllocNoteFromDisabled(NotePool* pool, SequenceLayer* layer) {
    Note* note = static_cast<Note*>(AudioSeq_AudioListPopBack(&pool->disabled));
    if (note != NULL) {
...
Note* Audio_AllocNoteFromDecaying(NotePool* pool, SequenceLayer* layer) {
    Note* note = static_cast<Note*>(AudioSeq_AudioListPopBack(&pool->decaying));
    if (note != NULL) {
...
void Audio_NoteInitAll(void) {
    ...
    for (i = 0; i < gAudioContext.numNotes; i++) {
        ...
        note->unk_BC = 0;
        note->synthesisState.synthesisBuffers = static_cast<NoteSynthesisBuffers*>(AudioHeap_AllocDmaMemory(&gAudioContext.notesAndBuffersPool, 0x1E0));
    }
}
