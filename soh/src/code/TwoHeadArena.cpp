...
Gfx* THGA_GetHead(TwoHeadGfxArena* thga) {
    return static_cast<Gfx*>(THA_GetHead((TwoHeadArena*)thga));
}
...
Gfx* THGA_GetTail(TwoHeadGfxArena* thga) {
    return static_cast<Gfx*>(THA_GetTail((TwoHeadArena*)thga));
}

Gfx* THGA_AllocStartArray8(TwoHeadGfxArena* thga, u32 count) {
    return static_cast<Gfx*>(THA_AllocStart((TwoHeadArena*)thga, count * 8));
}
...
Gfx* THGA_AllocEnd(TwoHeadGfxArena* thga, size_t size) {
    return static_cast<Gfx*>(THA_AllocEnd((TwoHeadArena*)thga, size));
}
...
void* THA_AllocStart(TwoHeadArena* tha, size_t size) {
    ...
    tha->head = static_cast<std::byte*>(tha->head) + size;
    return start;
}
...
void* THA_AllocEnd(TwoHeadArena* tha, size_t size) {
    ...
    tha->tail = reinterpret_cast<void*>( (((uintptr_t)tha->tail & mask) - size) & mask );
    return tha->tail;
}
...
void THA_Init(TwoHeadArena* tha) {
    tha->head = tha->bufp;
    tha->tail = static_cast<std::byte*>(tha->bufp) + tha->size;
}
