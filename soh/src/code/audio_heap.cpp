...
void* AudioHeap_AllocZeroed(AudioAllocPool* pool, size_t size) {
    u8* ret = static_cast<u8*>(AudioHeap_Alloc(pool, size));
    u8* ptr;
...
void AudioHeap_PopCache(s32 tableType) {
    ...
    persistentPool->cur = static_cast<u8*>(entryPtr);
...
void* AudioHeap_AllocCached(s32 tableType, ptrdiff_t size, s32 cache, s32 id) {
    ...
    loadedPool->persistent.entries[loadedPool->persistent.numEntries].ptr = static_cast<u8*>(mem);
...
void func_800DF1D8(f32 p, f32 q, u16* out) {
    // With the bug below fixed, thisv mysterious unused function computes two recurrences
...
void AudioHeap_Init(void) {
    ...
    gAudioContext.notes = static_cast<Note*>(AudioHeap_AllocZeroed(&gAudioContext.notesAndBuffersPool, gAudioContext.numNotes * sizeof(Note)));
    Audio_NoteInitAll();
    Audio_InitNoteFreeList();
    gAudioContext.noteSubsEu = static_cast<NoteSubEu*>(AudioHeap_AllocZeroed(&gAudioContext.notesAndBuffersPool, gAudioContext.audioBufferParameters.updatesPerFrame *
                                                                      gAudioContext.numNotes * sizeof(NoteSubEu)));

    for (i = 0; i != 2; i++) {
        gAudioContext.abiCmdBufs[i] = static_cast<Acmd*>(AudioHeap_AllocDmaMemoryZeroed(&gAudioContext.notesAndBuffersPool,
                                                                     gAudioContext.maxAudioCmds * sizeof(u64)));
    }

    gAudioContext.unk_3520 = static_cast<f32*>(AudioHeap_Alloc(&gAudioContext.notesAndBuffersPool, 0x100 * sizeof(f32)));
    func_800DDE3C();
    for (i = 0; i < 4; i++) {
    ...
    for (i = 0; i < gAudioContext.numSynthesisReverbs; i++) {
        ...
        reverb->sample.bits.codec = CODEC_REVERB;
        reverb->sample.bits.medium = MEDIUM_RAM;
        reverb->sample.bits.size = reverb->windowSize * 2;
        ...
        if (reverb->downsampleRate != 1) {
            ...
            for (j = 0; j < gAudioContext.audioBufferParameters.updatesPerFrame; j++) {
                mem = static_cast<s16*>(AudioHeap_AllocZeroedAttemptExternal(&gAudioContext.notesAndBuffersPool, 0x340));
        ...
        if (settings->lowPassFilterCutoffLeft != 0) {
            reverb->filterLeftState = static_cast<s16*>(AudioHeap_AllocDmaMemoryZeroed(&gAudioContext.notesAndBuffersPool, 0x40));
            reverb->filterLeft = static_cast<s16*>(AudioHeap_AllocDmaMemory(&gAudioContext.notesAndBuffersPool, 8 * sizeof(s16)));
        ...
        if (settings->lowPassFilterCutoffRight != 0) {
            reverb->filterRightState = static_cast<s16*>(AudioHeap_AllocDmaMemoryZeroed(&gAudioContext.notesAndBuffersPool, 0x40));
            reverb->filterRight = static_cast<s16*>(AudioHeap_AllocDmaMemory(&gAudioContext.notesAndBuffersPool, 8 * sizeof(s16)));
...
void* AudioHeap_AllocPermanent(s32 tableType, s32 id, size_t size) {
    ...
    gAudioContext.permanentCache[index].ptr = static_cast<u8*>(ret);
    if (ret == NULL) {
        return NULL;
    }
    ...
    //! @bug UB: missing return. "ret" is in v0 at thisv point, but doing an
...
SampleCacheEntry* AudioHeap_AllocTemporarySampleCacheEntry(size_t size) {
    ...
    for (i = 0; i < gAudioContext.preloadSampleStackTop; i++) {
        ...
        if (preload->isFree == false) {
            ...
            end = preload->ramAddr + preload->sample->bits.size - 1;
    ...
    ret->inUse = true;
    ret->allocatedAddr = static_cast<u8*>(mem);
    ret->size = size;
    return ret;
}
...
void AudioHeap_UnapplySampleCache(SampleCacheEntry* entry, SoundFontSample* sample) {
    if (sample != NULL) {
        if (sample->sampleAddr == entry->allocatedAddr) {
            sample->sampleAddr = static_cast<u8*>(entry->sampleAddr);
            sample->bits.medium = entry->origMedium;
        }
    }
}
...
SampleCacheEntry* AudioHeap_AllocPersistentSampleCacheEntry(size_t size) {
    ...
    entry->inUse = true;
    entry->allocatedAddr = static_cast<u8*>(mem);
    entry->size = size;
    pool->size++;
    return entry;
}
...
void AudioHeap_ChangeStorage(StorageChange* change, SoundFontSample* sample) {
    if (sample != NULL) {
        ...
        if (start <= (u32)sample->sampleAddr && (u32)sample->sampleAddr < end) {
            ...
            sample->bits.medium = change->newMedium;
        }
    }
}
...
void AudioHeap_ApplySampleBankCacheInternal(s32 apply, s32 sampleBankId) {
    ...
    numFonts = gAudioContext.soundFontTable->numEntries;
    change.oldAddr = reinterpret_cast<u32>(AudioHeap_SearchCaches(SAMPLE_TABLE, CACHE_EITHER, sampleBankId));
    if (change.oldAddr == 0) {
        return;
    }
