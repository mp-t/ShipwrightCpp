...
void ActorShadow_Draw(Actor* actor, Lights* lights, GlobalContext* globalCtx, const Gfx* dlist, Color_RGBA8* color) {
    f32 temp1;
    f32 temp2;
    MtxF sp60;
    ...
    if (actor->floorPoly != NULL) {
        ...
        if (temp1 >= -50.0f && temp1 < 500.0f) {
            ...
            if (dlist != gCircleShadowDL) {
                Matrix_RotateY(actor->shape.rot.y * (std::numbers::pi_v<float> / 0x8000), MTXMODE_APPLY);
            }
...
void func_8002C124(TargetContext* targetCtx, GlobalContext* globalCtx) {
        ...
        if ((!(player->stateFlags1 & 0x40)) || (actor != player->unk_664)) {
            ...
            for (spB0 = 0, spAC = targetCtx->unk_4C; spB0 < spB8; spB0++, spAC = (spAC + 1) % 3) {
                ...
                if (entry->unk_0C < 500.0f) {
                    ...
                    Matrix_RotateZ((targetCtx->unk_4B & 0x7F) * (std::numbers::pi_v<float> / 64), MTXMODE_APPLY);
    ...
    if ((actor != NULL) && !(actor->flags & ACTOR_FLAG_27)) {
        ...
        Matrix_RotateY((f32)((u16)(globalCtx->gameplayFrames * 3000)) * (std::numbers::pi_v<float> / 0x8000), MTXMODE_APPLY);
...
void TitleCard_InitBossName(GlobalContext* globalCtx, TitleCardContext* titleCtx, const char* texture, s16 x, s16 y, u8 width,
                            u8 height) {
...
void TitleCard_InitPlaceName(GlobalContext* globalCtx, TitleCardContext* titleCtx, const char* texture, s32 x, s32 y,
...
void TitleCard_Draw(GlobalContext* globalCtx, TitleCardContext* titleCtx) {
    ...
    if (titleCtx->alpha != 0) {
        ...
        gDPLoadTextureBlock(OVERLAY_DISP++, titleCtx->texture + textureLanguageOffset, G_IM_FMT_IA,
                            G_IM_SIZ_8b,
        ...
        if (height > 0) {
            gDPLoadTextureBlock(OVERLAY_DISP++, titleCtx->texture + textureLanguageOffset + 0x1000,
                                G_IM_FMT_IA,
...
Gfx* func_8002E830(Vec3f* object, Vec3f* eye, Vec3f* lightDir, GraphicsContext* gfxCtx, Gfx* gfx, Hilite** hilite) {
    ...
    lookAt = static_cast<LookAt*>(Graph_Alloc(gfxCtx, sizeof(LookAt)));

    correctedEyeX = (eye->x == object->x) && (eye->z == object->z) ? eye->x + 0.001f : eye->x;

    *hilite = static_cast<Hilite*>(Graph_Alloc(gfxCtx, sizeof(Hilite)));
...
void func_8002EBCC(Actor* actor, GlobalContext* globalCtx, s32 flag) {
    ...
    if (flag != 0) {
        displayList = static_cast<Gfx*>(Graph_Alloc(globalCtx->state.gfxCtx, 2 * sizeof(Gfx)));
...
void Actor_DrawFaroresWindPointer(GlobalContext* globalCtx) {
    ...
    if (params) {
        ...
        if ((globalCtx->csCtx.state == CS_STATE_IDLE) &&
            ...
            Matrix_RotateZ(((globalCtx->gameplayFrames * 1500) & 0xFFFF) * std::numbers::pi_v<float> / 32768.0f, MTXMODE_APPLY);
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_actor.c", 5458),
                      G_MTX_MODELVIEW | G_MTX_LOAD | G_MTX_NOPUSH);
            gSPDisplayList(POLY_XLU_DISP++, gEffFlash1DL);

            Matrix_Pop();
            Matrix_RotateZ(~((globalCtx->gameplayFrames * 1200) & 0xFFFF) * std::numbers::pi_v<float> / 32768.0f, MTXMODE_APPLY);
...
void Actor_FaultPrint(Actor* actor, char* command) {
    ...
    overlayEntry = actor->overlayEntry;
    const char* name = overlayEntry->name != NULL ? overlayEntry->name : "";
...
Actor* Actor_Spawn(ActorContext* actorCtx, GlobalContext* globalCtx, s16 actorId, f32 posX, f32 posY, f32 posZ,
    ...
    if (overlayEntry->vramStart == 0) {
        ...
        actorInit = ((overlayEntry->initInfo != NULL)
                                     ? reinterpret_cast<ActorInit*>((uintptr_t)overlayEntry->initInfo -
    ...
    actor = static_cast<Actor*>(ZeldaArena_MallocDebug(actorInit->instanceSize, name, 1));
...
void BodyBreak_Alloc(BodyBreak* bodyBreak, s32 count, GlobalContext* globalCtx) {
    ...
    bodyBreak->matrices = static_cast<MtxF*>(ZeldaArena_MallocDebug(matricesSize, "../z_actor.c", 7540));
    ...
    if (bodyBreak->matrices != NULL) {
        ...
        if (bodyBreak->dLists != NULL) {
            ...
            bodyBreak->objectIds = static_cast<s16*>(ZeldaArena_MallocDebug(objectIdsSize, "../z_actor.c", 7546));
...
s32 BodyBreak_SpawnParts(Actor* actor, BodyBreak* bodyBreak, GlobalContext* globalCtx, s16 type) {
    ...
    while (bodyBreak->count > 0) {
        ...
        spawnedEnPart =
            (EnPart*)Actor_SpawnAsChild(&globalCtx->actorCtx, actor, globalCtx, ACTOR_EN_PART, mtx->mf_raw.xw,
                                        mtx->mf_raw.yw, mtx->mf_raw.zw, 0, 0, objBankIndex, type);
...
Actor* Actor_GetProjectileActor(GlobalContext* globalCtx, Actor* refActor, f32 radius) {
    ...
    while (actor != NULL) {
        if (((actor->id != ACTOR_ARMS_HOOK) && (actor->id != ACTOR_EN_ARROW)) || (actor == refActor)) {
            ...
            //  which will rarely ever be 0. So it's very unlikely for thisv bug to cause an issue.
...
typedef struct {
    ...
    /* 0x14 */ const Gfx* chainDL;
    /* 0x18 */ const Gfx* lockDL;
...
void Actor_DrawDoorLock(GlobalContext* globalCtx, s32 frame, s32 type) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (i % 2) {
            ...
            rotZStep = std::numbers::pi_v<float> - (2.0f * entry->chainAngle);
        }
...
Gfx* func_80034B28(GraphicsContext* gfxCtx) {
    ...
    displayList = static_cast<Gfx*>(Graph_Alloc(gfxCtx, sizeof(Gfx)));
...
Gfx* func_80034B54(GraphicsContext* gfxCtx) {
    ...
    displayList = displayListHead = static_cast<Gfx*>(Graph_Alloc(gfxCtx, 2 * sizeof(Gfx)));
 ...
 * an actor if there is one. If the ID provided is -1, thisv will look for any actor of the
...
Actor* func_800358DC(Actor* actor, Vec3f* spawnPos, Vec3s* spawnRot, f32* arg3, s32 timer, s16* unused,
                     GlobalContext* globalCtx, s16 params, Gfx* arg8) {
    EnPart* spawnedEnPart;
...
void func_800359B8(Actor* actor, s16 arg1, Vec3s* arg2) {
    ...
    if (actor->floorPoly != NULL) {
        ...
        arg2->x = -(s16)(Math_FAtan2F(sp28 * sp40, 1.0f) * (32768 / std::numbers::pi_v<float>));

        sp2C = Math_SinS(arg1 - 16375);
        sp30 = Math_CosS(arg1 - 16375);
        sp24 = (-(sp44 * sp2C) - (sp3C * sp30));
        arg2->z = -(s16)(Math_FAtan2F(sp24 * sp40, 1.0f) * (32768 / std::numbers::pi_v<float>));
    }
}
