...
OSMesg MK_ASYNC_MSG(const s32 retData, const s32 tableType, const s32 id, const s32 status)
{
    return reinterpret_cast<OSMesg>(((retData) << 24) | ((tableType) << 16) | ((id) << 8) | (status));
}
u8 ASYNC_TBLTYPE(const OSMesg v) { return static_cast<u8>(reinterpret_cast<u32>(v) >> 16); }
u8 ASYNC_ID(const OSMesg v) { return static_cast<u8>(reinterpret_cast<u32>(v) >> 8); }
u8 ASYNC_STATUS(const OSMesg v) { return static_cast<u8>(reinterpret_cast<u32>(v) >> 0); }
...
// opaque type for unpatched sound font data (should maybe get rid of thisv?)
...
AudioAsyncLoad* AudioLoad_StartAsyncLoadUnkMedium(s32 unkMediumParam, u32 devAddr, void* ramAddr, ptrdiff_t size, s32 medium, s32 nChunks, OSMesgQueue* retQueue, OSMesg retMsg);
AudioAsyncLoad* AudioLoad_StartAsyncLoad(u32 devAddr, void* ramAddr, size_t size, s32 medium, s32 nChunks,
                                         OSMesgQueue* retQueue, OSMesg retMsg);
...
void* AudioLoad_DmaSampleData(u32 devAddr, size_t size, s32 arg2, u8* dmaIndexRef, s32 medium) {
    ...
    if (arg2 != 0 || *dmaIndexRef >= gAudioContext.sampleDmaListSize1) {
        for (i = gAudioContext.sampleDmaListSize1; i < gAudioContext.sampleDmaCount; i++) {
            ...
            if (0 <= bufferPos && (u32)bufferPos <= dma->size - size) {
                // We already have a DMA request for thisv memory range.
                if (dma->ttl == 0 &&
    ...
    again:
        bufferPos = devAddr - dma->devAddr;
        if (0 <= bufferPos && (u32)bufferPos <= dma->size - size) {
            // We already have DMA for thisv memory range.
            if (dma->ttl == 0) {
                // Move the DMA out of the reuse queue, by swapping it with the
                // read pos, and then incrementing the read pos.
...
void AudioLoad_InitSampleDmaBuffers(s32 arg0) {
    ...
    gAudioContext.sampleDmas = static_cast<SampleDma*>(
...
s32 AudioLoad_SyncLoadSample(SoundFontSample* sample, s32 fontId) {
    ...
    if (sample->bits.unk_bit25 == 1) {
        if (sample->bits.medium != MEDIUM_RAM) {
            sampleAddr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, fontId,
                                                                                 (void*)sample->sampleAddr,
                                                                                 sample->bits.medium,
                                                    CACHE_PERSISTENT));
            if (sampleAddr == NULL) {
                return -1;
            ...
            if (sample->bits.medium == MEDIUM_UNK) {
                AudioLoad_SyncDmaUnkMedium(reinterpret_cast<u32>(sample->sampleAddr), sampleAddr, sample->bits.size,
                                           gAudioContext.sampleBankTable->unkMediumParam);
            } else {
                AudioLoad_SyncDma(reinterpret_cast<u32>(sample->sampleAddr), sampleAddr, sample->bits.size, sample->bits.medium);
            }
            sample->bits.medium = MEDIUM_RAM;
...
void AudioLoad_AsyncLoad(s32 tableType, s32 id, s32 nChunks, s32 retData, OSMesgQueue* retQueue) {
    if (AudioLoad_AsyncLoadInner(tableType, id, nChunks, retData, retQueue) == NULL) {
        osSendMesg(retQueue, reinterpret_cast<OSMesg>(0xFFFFFFFF), OS_MESG_NOBLOCK);
    }
}
...
s32 AudioLoad_SyncInitSeqPlayer(s32 playerIdx, s32 seqId, s32 arg2) {
    ...
    // The callers of thisv function do not use the return value, so it's fine.
}
...
u8* AudioLoad_SyncLoadSeq(s32 seqId) {
    ...
    return static_cast<u8*>(AudioLoad_SyncLoad(SEQUENCE_TABLE, seqId, &didAllocate));
}
...
u32 AudioLoad_TrySyncLoadSampleBank(u32 sampleBankId, u32* outMedium, s32 noLoad) {
    ...
    if (ret != NULL) {
        ...
        *outMedium = MEDIUM_RAM;
        return reinterpret_cast<u32>(ret);
    }
    ...
    if (ret != NULL) {
        ...
        *didAllocate = true;
        if (medium == MEDIUM_UNK) {
            AudioLoad_SyncDmaUnkMedium(romAddr, static_cast<u8*>(ret), size, (s16)table->unkMediumParam);
        } else {
            AudioLoad_SyncDma(romAddr, static_cast<u8*>(ret), size, medium);
        }
...
void AudioLoad_RelocateFont(s32 fontId, SoundFontData* mem, RelocInfo* relocInfo) {
    ...
    void** ptrs = (void**)mem; //wat, thisv whole function is wat

#define BASE_OFFSET(x) (void*)((u32)(x) + (u32)(mem))

    reloc2 = reinterpret_cast<u32>(ptrs[0]);
    if (1) {}
    if ((reloc2 != 0) && (numDrums != 0)) {
        ptrs[0] = BASE_OFFSET(reloc2);
        for (i = 0; i < numDrums; i++) {
            reloc = reinterpret_cast<u32>( ((Drum**)ptrs[0])[i] ); //wat

            if (reloc != 0) {
                reloc = reinterpret_cast<u32>( BASE_OFFSET(reloc) );

                ((Drum**)ptrs[0])[i] = drum = reinterpret_cast<Drum*>(reloc);
                if (!drum->loaded) {
                    AudioLoad_RelocateSample(&drum->sound, mem, relocInfo);
                    reloc = reinterpret_cast<u32>(drum->envelope);
                    drum->envelope = static_cast<AdsrEnvelope*>(BASE_OFFSET(reloc));
                    drum->loaded = 1;
                }
            }
        }
    ...
    reloc2 = reinterpret_cast<u32>(ptrs[1]);
    
    if ((reloc2 != 0) && (numSfx != 0)) {
        ptrs[1] = BASE_OFFSET(reloc2);
        for (i = 0; i < numSfx; i++) {
            reloc = reinterpret_cast<u32>((SoundFontSound*)ptrs[1] + i);
            if (reloc != 0) {
                sfx = reinterpret_cast<SoundFontSound*>(reloc);
                if (sfx->sample != NULL) {
    ...
    for (i = 2; i <= 2 + numInstruments - 1; i++) {
        if (ptrs[i] != NULL) {
            ...
            inst = static_cast<Instrument*>(ptrs[i]);
            if (!inst->loaded) {
                ...
                reloc = reinterpret_cast<u32>(inst->envelope);
                inst->envelope = static_cast<AdsrEnvelope*>(BASE_OFFSET(reloc));
                inst->loaded = 1;
            }
        }
    }
...
#undef BASE_OFFSET
    ...
    gAudioContext.soundFonts[fontId].drums = static_cast<Drum**>(ptrs[0]);
    gAudioContext.soundFonts[fontId].soundEffects = static_cast<SoundFontSound*>(ptrs[1]);
...
s32 AudioLoad_Dma(OSIoMesg* mesg, u32 priority, s32 direction, const u32 devAddr, void* ramAddr, size_t size,
                  OSMesgQueue* reqQueue, s32 medium, const char* dmaFuncType) {
    if (gAudioContext.resetTimer > 0x10) {
        return -1;
    }

    memcpy(ramAddr, reinterpret_cast<const void*>(devAddr), size);
...
void AudioLoad_InitSwapFont(void) {
    ...
    for (fontId = 0; fontId < (numFonts - 1); fontId++) {
        ...
        for (i = 0; i < numInstruments; i++) {
            ...
            if (instList[i] != NULL) {
                inst = static_cast<Instrument*>(BASE_ROM_OFFSET(instList[i]));
...
void AudioLoad_Init(void* heap, u32 heapSize) {
    ...
    if (heap == NULL) {
        ...
        void** hp = &heap;
        gAudioContext.audioHeap = static_cast<u8*>(*hp);
    ...
    uintptr_t seqStart = reinterpret_cast<std::uintptr_t>(ResourceMgr_LoadFileRaw(_AudioseqSegmentRomStart));
    uintptr_t bankStart = reinterpret_cast<std::uintptr_t>(ResourceMgr_LoadFileRaw(_AudiobankSegmentRomStart));
    uintptr_t tableStart = reinterpret_cast<std::uintptr_t>(ResourceMgr_LoadFileRaw(_AudiotableSegmentRomStart));
    ...
    gAudioContext.soundFonts = static_cast<SoundFont*>(AudioHeap_Alloc(&gAudioContext.audioInitPool, numFonts * sizeof(SoundFont)));
...
s32 AudioLoad_SlowLoadSample(s32 fontId, s32 instId, s8* isDone) {
    ...
    slowLoad->curRamAddr = static_cast<u8*>(
        AudioHeap_AllocSampleCache(sample->bits.size, fontId, sample->sampleAddr, sample->bits.medium, CACHE_TEMPORARY));

    if (slowLoad->curRamAddr == NULL) {
        if (sample->bits.medium == MEDIUM_UNK || sample->bits.codec == CODEC_S16_INMEMORY) {
            *isDone = 0;
            return -1;
        } else {
            *isDone = 3;
    ...
    slowLoad->bytesRemaining = ALIGN16(sample->bits.size);
    slowLoad->ramAddr = slowLoad->curRamAddr;
    slowLoad->curDevAddr = reinterpret_cast<u32>(sample->sampleAddr);
    slowLoad->medium = sample->bits.medium;
...
AudioAsyncLoad* AudioLoad_StartAsyncLoadUnkMedium(s32 unkMediumParam, u32 devAddr, void* ramAddr, ptrdiff_t size, s32 medium,
                                                  s32 nChunks, OSMesgQueue* retQueue, OSMesg retMsg) {
...
AudioAsyncLoad* AudioLoad_StartAsyncLoad(u32 devAddr, void* ramAddr, size_t size, s32 medium, s32 nChunks,
    ...
    asyncLoad->ramAddr = static_cast<u8*>(ramAddr);
    asyncLoad->curRamAddr = static_cast<u8*>(ramAddr);
...
void AudioLoad_RelocateSample(SoundFontSound* sound, SoundFontData* mem, RelocInfo* relocInfo) {
    ...
    if ((uintptr_t)sound->sample < maxSoundBankSize) {
        sample = sound->sample = static_cast<SoundFontSample*>(RELOC(sound->sample, mem));
        if (sample->bits.size != 0 && sample->bits.unk_bit25 != 1) {
            sample->loop = static_cast<AdpcmLoop*>(RELOC(sample->loop, mem));
            sample->book = static_cast<AdpcmBook*>(RELOC(sample->book, mem));

            // Resolve the sample medium 2-bit bitfield into a real value based on relocInfo.
            switch (sample->bits.medium) {
                case 0:
                    sample->sampleAddr = static_cast<u8*>(RELOC(sample->sampleAddr, relocInfo->baseAddr1));
                    sample->bits.medium = relocInfo->medium1;
                    break;
                case 1:
                    sample->sampleAddr = static_cast<u8*>(RELOC(sample->sampleAddr, relocInfo->baseAddr2));
                    sample->bits.medium = relocInfo->medium2;
                    break;
                case 2:
                case 3:
                    // Invalid? This leaves sample->bits.medium as MEDIUM_CART and MEDIUM_DISK_DRIVE
                    // respectively, and the sampleAddr unrelocated.
                    break;
            }

            sample->bits.unk_bit25 = 1;
            if (sample->bits.unk_bit26 && (sample->bits.medium != MEDIUM_RAM)) {
...
void AudioLoad_RelocateFontAndPreloadSamples(s32 fontId, SoundFontData* mem, RelocInfo* relocInfo, s32 async) {
    ...
    for (i = 0; i < gAudioContext.numUsedSamples; i++) {
        size += ALIGN16(gAudioContext.usedSamples[i]->bits.size);
    }
    if (size && size) {}
    ...
    for (i = 0; i < gAudioContext.numUsedSamples; i++) {
        ...
        switch (async) {
            case false:
                if (sample->bits.medium == relocInfo->medium1) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, relocInfo->sampleBankId1,
                                                                       sample->sampleAddr,
                                                      sample->bits.medium, CACHE_PERSISTENT));
                } else if (sample->bits.medium == relocInfo->medium2) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, relocInfo->sampleBankId2, sample->sampleAddr,
                                                      sample->bits.medium, CACHE_PERSISTENT));
                } else if (sample->bits.medium == MEDIUM_DISK_DRIVE) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, 0xFE, sample->sampleAddr, sample->bits.medium,
                                                      CACHE_PERSISTENT));
                }
                break;
            ...
            case true:
                if (sample->bits.medium == relocInfo->medium1) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, relocInfo->sampleBankId1,
                                                                       sample->sampleAddr,
                                                      sample->bits.medium, CACHE_TEMPORARY));
                } else if (sample->bits.medium == relocInfo->medium2) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, relocInfo->sampleBankId2, sample->sampleAddr,
                                                      sample->bits.medium, CACHE_TEMPORARY));
                } else if (sample->bits.medium == MEDIUM_DISK_DRIVE) {
        ...
        switch (async) {
            case false:
                if (sample->bits.medium == MEDIUM_UNK) {
                    AudioLoad_SyncDmaUnkMedium((u32)sample->sampleAddr, addr, sample->bits.size,
                                               gAudioContext.sampleBankTable->unkMediumParam);
                    sample->sampleAddr = addr;
                    sample->bits.medium = MEDIUM_RAM;
                } else {
                    AudioLoad_SyncDma((u32)sample->sampleAddr, addr, sample->bits.size, sample->bits.medium);
                    sample->sampleAddr = addr;
                    sample->bits.medium = MEDIUM_RAM;
                }
                if (sample->bits.medium == MEDIUM_DISK_DRIVE) {}
                break;
            ...
            case true:
                ...
                preload->endAndMediumKey = (u32)sample->sampleAddr + sample->bits.size + sample->bits.medium;
    ...
    if (gAudioContext.preloadSampleStackTop != 0 && !preloadInProgress) {
        ...
        nChunks = (sample->bits.size >> 12) + 1;
        AudioLoad_StartAsyncLoad((u32)sample->sampleAddr, topPreload->ramAddr, sample->bits.size, sample->bits.medium, nChunks,
                                 &gAudioContext.preloadSampleQueue, reinterpret_cast<OSMesg>(topPreload->encodedInfo));
    }
}
...
s32 AudioLoad_ProcessSamplePreloads(s32 resetStatus) {
    ...
    if (gAudioContext.preloadSampleStackTop > 0) {
        ...
        if (preload->isFree == false) {
            ...
            key = (u32)sample->sampleAddr + sample->bits.size + sample->bits.medium;
        ...
        for (;;) {
            ...
            if (key != preload->endAndMediumKey) {
                ...
                AudioLoad_StartAsyncLoad((u32)sample->sampleAddr, preload->ramAddr, sample->bits.size, sample->bits.medium,
                                         nChunks, &gAudioContext.preloadSampleQueue, reinterpret_cast<OSMesg>(preload->encodedInfo));
                break;
            }
        }
    }
...
s32 AudioLoad_GetSamplesForFont(s32 fontId, SoundFontSample** sampleSet) {
    ...
    // Should really also process sfx, but thisv method is never called, so whatever.
    return numSamples;
}
...
void AudioLoad_AddUsedSample(SoundFontSound* sound) {
    ...
    if ((sample->bits.size != 0) && (sample->bits.unk_bit26) && (sample->bits.medium != MEDIUM_RAM)) {
        gAudioContext.usedSamples[gAudioContext.numUsedSamples++] = sample;
    }
}
...
void AudioLoad_PreloadSamplesForFont(s32 fontId, s32 async, RelocInfo* relocInfo) {
    ...
    for (i = 0; i < gAudioContext.numUsedSamples; i++) {
        ...
        switch (async) {
            case false:
                if (sample->bits.medium == relocInfo->medium1) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, relocInfo->sampleBankId1, sample->sampleAddr,
                                                      sample->bits.medium, CACHE_PERSISTENT));
                } else if (sample->bits.medium == relocInfo->medium2) {
                    addr = static_cast<u8*>(AudioHeap_AllocSampleCache(sample->bits.size, relocInfo->sampleBankId2, sample->sampleAddr,
                                                      sample->bits.medium, CACHE_PERSISTENT));
