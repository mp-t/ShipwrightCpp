...
s16 OnePointCutscene_Init(GlobalContext* globalCtx, s16 csId, s16 timer, Actor* actor, s16 parentCamIdx) {
    ...
    // Removes all lower priority cutscenes in front of thisv cutscene from the queue.
    vCurCamIdx = csCamIdx;
...
s32 OnePointCutscene_Attention(GlobalContext* globalCtx, Actor* actor) {
    ...
    // Actorcat is only undefined if the actor is in a higher category than all other attention cutscenes. In thisv case,
    ...
    // If the previous attention cutscene has an actor in the same category, skip thisv actor.
