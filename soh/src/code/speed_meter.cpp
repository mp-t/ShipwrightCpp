...
void SpeedMeter_InitImpl(SpeedMeter* thisx, u32 arg1, u32 y) {
    LogUtils_CheckNullPointer("thisx", thisx, "../speed_meter.c", 181);
    thisx->unk_18 = arg1;
    thisx->y = y;
}

void SpeedMeter_Init(SpeedMeter* thisx) {
    SpeedMeter_InitImpl(thisx, 32, 22);
}
...
void SpeedMeter_InitAllocEntry(SpeedMeterAllocEntry* thisx, u32 maxval, u32 val, u16 backColor, u16 foreColor, u32 ulx,
                               u32 lrx, u32 uly, u32 lry) {
    thisx->maxval = maxval;
    thisx->val = val;
    thisx->backColor = backColor;
    thisx->foreColor = foreColor;
    thisx->ulx = ulx;
    thisx->lrx = lrx;
    thisx->uly = uly;
    thisx->lry = lry;
...
void SpeedMeter_DrawAllocEntry(SpeedMeterAllocEntry* thisx, GraphicsContext* gfxCtx) {
    ...
    if (thisx->maxval == 0) {
        ...
        LOG_NUM("thisx->maxval", thisx->maxval, "../speed_meter.c", 313);
        osSyncPrintf(VT_RST);
    } else {
        ...
        usedOff = ((thisx->lrx - thisx->ulx) * thisx->val) / thisx->maxval + thisx->ulx;
        DrawRec(gfx++, thisx->backColor, usedOff, thisx->uly, thisx->lrx, thisx->lry);
        DrawRec(gfx++, thisx->foreColor, thisx->ulx, thisx->uly, usedOff, thisx->lry);
