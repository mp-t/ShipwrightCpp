...
void Gameplay_Init(GameState* thisx) {
    ...
    zAlloc = reinterpret_cast<std::uintptr_t>(GameState_Alloc(&globalCtx->state, zAllocSize, "../z_play.c", 2918));
    zAllocAligned = (zAlloc + 8) & ~0xF;
    ZeldaArena_Init(reinterpret_cast<void*>(zAllocAligned), zAllocSize - zAllocAligned + zAlloc);
    // "Zelda Heap"
...
void Gameplay_Draw(GlobalContext* globalCtx) {
    ...
    if ((HREG(80) != 10) || (HREG(82) != 0)) {
        ...
        // The billboard is still a viewing matrix at thisv stage
        ...
        globalCtx->billboardMtx = Matrix_MtxFToMtx(Matrix_CheckFloats(&globalCtx->billboardMtxF, "../z_play.c", 4005),
                                                   static_cast<Mtx*>(Graph_Alloc(gfxCtx, sizeof(Mtx))));
...
f32 func_800BFCB8(GlobalContext* globalCtx, MtxF* mf, Vec3f* vec) {
    ...
    if (floorY > BGCHECK_Y_MIN) {
        ...
        mf->mf_raw.xx = temp1;
        mf->mf_raw.yx = -nx * temp2;
        mf->mf_raw.zx = nx * temp3;
        mf->mf_raw.xy = nx;
        mf->mf_raw.yy = ny;
        mf->mf_raw.zy = nz;
        mf->mf_raw.yz = temp3;
...
void func_800C016C(GlobalContext* globalCtx, Vec3f* src, Vec3f* dest) {
    ...
    temp = globalCtx->viewProjectionMtxF.mf_raw.ww +
           (globalCtx->viewProjectionMtxF.mf_raw.wx * src->x + globalCtx->viewProjectionMtxF.mf_raw.wy * src->y +
            globalCtx->viewProjectionMtxF.mf_raw.wz * src->z);
