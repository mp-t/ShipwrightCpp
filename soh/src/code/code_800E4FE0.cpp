...
void AudioMgr_CreateNextAudioBuffer(s16* samples, u32 num_samples) {
    ...
    if (gAudioContext.resetStatus != 0) {
        if (AudioHeap_ResetStep() == 0) {
            if (gAudioContext.resetStatus == 0) {
                osSendMesg(gAudioContext.audioResetQueueP, reinterpret_cast<OSMesg>(gAudioContext.audioResetSpecIdToLoad), OS_MESG_NOBLOCK);
            }
        }
    }
...
AudioTask* func_800E5000(void) {
    ...
    osSendMesg(gAudioContext.taskStartQueueP, reinterpret_cast<OSMesg>(gAudioContext.totalTaskCnt), OS_MESG_NOBLOCK);
...
void func_800E5584(AudioCmd* cmd) {
    ...
    switch (cmd->args.op) {
        case 0x81:
            AudioLoad_SyncLoadSeqParts(cmd->args.arg1, cmd->args.arg2);
            return;
        case 0x82:
            AudioLoad_SyncInitSeqPlayer(cmd->args.arg0, cmd->args.arg1, cmd->args.arg2);
            func_800E59AC(cmd->args.arg0, reinterpret_cast<s32>(cmd->data));
            return;
        case 0x85:
            AudioLoad_SyncInitSeqPlayerSkipTicks(cmd->args.arg0, cmd->args.arg1, reinterpret_cast<s32>(cmd->data));
            return;
        case 0x83:
            if (gAudioContext.seqPlayers[cmd->args.arg0].enabled) {
                if (cmd->asInt == 0) {
                    AudioSeq_SequencePlayerDisableAsFinished(&gAudioContext.seqPlayers[cmd->args.arg0]);
                } else {
                    func_800E5958(cmd->args.arg0, cmd->asInt);
                }
            }
            return;
        case 0xF0:
        ...
        case 0xF2:
            ...
            return;
        case 0xF3:
            AudioLoad_SyncLoadInstrument(cmd->args.arg0, cmd->args.arg1, cmd->args.arg2);
            return;
        case 0xF4:
            AudioLoad_AsyncLoadSampleBank(cmd->args.arg0, cmd->args.arg1, cmd->args.arg2, &gAudioContext.externalLoadQueue);
            return;
        case 0xF5:
            AudioLoad_AsyncLoadFont(cmd->args.arg0, cmd->args.arg1, cmd->args.arg2, &gAudioContext.externalLoadQueue);
            return;
        case 0xFC:
            AudioLoad_AsyncLoadSeq(cmd->args.arg0, cmd->args.arg1, cmd->args.arg2, &gAudioContext.externalLoadQueue);
            return;
        case 0xF6:
            AudioLoad_DiscardSeqFonts(cmd->args.arg1);
            return;
        case 0x90:
            gAudioContext.unk_5BDC[cmd->args.arg0] = cmd->u16_.asUShort;
            return;
        case 0xF9:
        ...
        case 0xE1:
        case 0xE2:
            Audio_SetFontInstrument(cmd->args.op - 0xE0, cmd->args.arg0, cmd->args.arg1, cmd->data);
            return;
        case 0xFE:
            temp_t7 = cmd->asUInt;
            if (temp_t7 == 1) {
...
void Audio_ProcessCmd(AudioCmd* cmd) {
    ...
    if ((cmd->args.op & 0xF0) == 0xF0) {
        func_800E5584(cmd);
        return;
    }

    if (cmd->args.arg0 < gAudioContext.audioBufferParameters.numSequencePlayers) {
        seqPlayer = &gAudioContext.seqPlayers[cmd->args.arg0];
        if (cmd->args.op & 0x80) {
            func_800E5584(cmd);
            return;
        }
        ...
        if (cmd->args.arg1 < 0x10) {
            func_800E6300(seqPlayer->channels[cmd->args.arg1], cmd);
            return;
        }
        if (cmd->args.arg1 == 0xFF) {
            phi_v0 = gAudioContext.unk_5BDC[cmd->args.arg0];
            for (i = 0; i < 0x10; i++) {
                if (phi_v0 & 1) {
...
void func_800E6128(SequencePlayer* seqPlayer, AudioCmd* cmd) {
    ...
    switch (cmd->args.op) {
        ...
        case 0x4E:
            ...
            return;
        case 0x48:
            seqPlayer->transposition = cmd->s8_.asSbyte;
            return;
        case 0x46:
            seqPlayer->soundScriptIO[cmd->args.arg2] = cmd->s8_.asSbyte;
            return;
        case 0x4A:
            fadeVolume = (s32)cmd->args.arg1 / 127.0f;
            goto block_11;
        case 0x4B:
            fadeVolume = ((s32)cmd->args.arg1 / 100.0f) * seqPlayer->fadeVolume;
        block_11:
...
void func_800E6300(SequenceChannel* channel, AudioCmd* cmd) {
    switch (cmd->args.op) {
        ...
        case CHAN_UPD_PAN_SIGNED:
            if (channel->newPan != cmd->s8_.asSbyte) {
                channel->newPan = cmd->s8_.asSbyte;
                channel->changes.s.pan = 1;
            }
            return;
        case CHAN_UPD_PAN_UNSIGNED:
            if (channel->newPan != cmd->s8_.asSbyte) {
                channel->panChannelWeight = cmd->s8_.asSbyte;
                channel->changes.s.pan = 1;
            }
            return;
        ...
        case CHAN_UPD_REVERB:
            if (channel->reverb != cmd->s8_.asSbyte) {
                channel->reverb = cmd->s8_.asSbyte;
            }
            return;
        case CHAN_UPD_SCRIPT_IO:
            if (cmd->args.arg2 < 8) {
                channel->soundScriptIO[cmd->args.arg2] = cmd->s8_.asSbyte;
            }
            return;
        case CHAN_UPD_STOP_SOMETHING2:
            channel->stopSomething2 = cmd->s8_.asSbyte;
            return;
        case CHAN_UPD_MUTE_BEHAVE:
            channel->muteBehavior = cmd->s8_.asSbyte;
            return;
        case CHAN_UPD_VIBE_X8:
            channel->vibratoExtentTarget = cmd->u8_.asUbyte * 8;
            channel->vibratoExtentChangeDelay = 1;
            return;
        case CHAN_UPD_VIBE_X32:
            channel->vibratoRateTarget = cmd->u8_.asUbyte * 32;
            channel->vibratoRateChangeDelay = 1;
            return;
        case CHAN_UPD_UNK_0F:
            channel->unk_0F = cmd->u8_.asUbyte;
            return;
        case CHAN_UPD_UNK_20:
            channel->unk_20 = cmd->u16_.asUShort;
            return;
        case CHAN_UPD_STEREO:
            channel->stereo.asByte = cmd->u8_.asUbyte;
            return;
    }
}
        ...
        if (note->noteSubEu.bitField0.enabled) {
            ...
            if (temp_a2->adsr.action.s.state != 0) {
                if (arg0 >= 2) {
                    ...
                    if (sound->sample->bits.medium == MEDIUM_RAM) {
                        continue;
                    }
                }
