...
// TODO extract thisv information from the texture definitions themselves
...
Gfx* Gfx_TextureIA8(Gfx* displayListHead, const void* texture, s16 textureWidth, s16 textureHeight, s16 rectLeft, s16 rectTop,
...
Gfx* Gfx_TextureI8(Gfx* displayListHead, const void* texture, s16 textureWidth, s16 textureHeight, s16 rectLeft, s16 rectTop,
                   s16 rectWidth, s16 rectHeight, u16 dsdx, u16 dtdy) {
...
void Interface_LoadItemIcon1(GlobalContext* globalCtx, u16 button) {
    ...
    DmaMgr_SendRequest2(&interfaceCtx->dmaRequest_160, reinterpret_cast<std::uintptr_t>(interfaceCtx->iconItemSegment + button * 0x1000),
...
void Interface_LoadItemIcon2(GlobalContext* globalCtx, u16 button) {
    ...
    osCreateMesgQueue(&interfaceCtx->loadQueue, &interfaceCtx->loadMsg, OS_MESG_BLOCK);
    DmaMgr_SendRequest2(&interfaceCtx->dmaRequest_180, reinterpret_cast<std::uintptr_t>(interfaceCtx->iconItemSegment + button * 0x1000),
                        (uintptr_t)_icon_item_staticSegmentRomStart + (gSaveContext.equips.buttonItems[button] * 0x1000),
...
void Interface_LoadActionLabel(InterfaceContext* interfaceCtx, u16 action, s16 loadOffset) {
    static const void* sDoActionTextures[] = { gAttackDoActionENGTex, gCheckDoActionENGTex };
    if (action >= DO_ACTION_MAX) {
        action = DO_ACTION_NONE;
    }

    const char* doAction = actionsTbl[action];
    ...
    if (action != DO_ACTION_NONE) {
        ...
        func_80086D5C(reinterpret_cast<s32*>( interfaceCtx->doActionSegment + (loadOffset * DO_ACTION_TEX_SIZE) ), DO_ACTION_TEX_SIZE / 4);
    }
}
...
void Interface_DrawItemButtons(GlobalContext* globalCtx) {
    static const void* cUpLabelTextures[] = { gNaviCUpENGTex, gNaviCUpENGTex, gNaviCUpENGTex };
...
void Interface_DrawItemIconTexture(GlobalContext* globalCtx, const void* texture, s16 button) {
...
void Interface_InitVertices(GlobalContext* globalCtx) {
    ...
    interfaceCtx->actionVtx = static_cast<Vtx*>( Graph_Alloc(globalCtx->state.gfxCtx, 8 * sizeof(Vtx)) );
    ...
    interfaceCtx->beatingHeartVtx = static_cast<Vtx*>( Graph_Alloc(globalCtx->state.gfxCtx, 4 * sizeof(Vtx)) );
...
void Interface_Draw(GlobalContext* globalCtx) {
    ...
    gSPInvalidateTexCache(OVERLAY_DISP++, reinterpret_cast<std::uintptr_t>(interfaceCtx->doActionSegment));
    gSPInvalidateTexCache(OVERLAY_DISP++, reinterpret_cast<std::uintptr_t>(interfaceCtx->doActionSegment + DO_ACTION_TEX_SIZE));
