...
void EffectSs_InitInfo(GlobalContext* globalCtx, s32 tableSize) {
    ...
    sEffectSsInfo.table = static_cast<EffectSs*>(
...
void EffectSs_Spawn(GlobalContext* globalCtx, s32 type, s32 priority, void* initParams) {
    ...
    if (EffectSs_FindSlot(priority, &index) != 0) {
        // Abort because we couldn't find a suitable slot to add thisv effect in
        return;
    }
    ...
    if (overlayEntry->vramStart == NULL) {
        ...
        initInfo = ((overlayEntry->initInfo != NULL)
                                    ? reinterpret_cast<EffectSsInit*>((uintptr_t)overlayEntry->initInfo -
    ...
    if (initInfo->init == NULL) {
        ...
        // Please fix thisv. (Waste of memory) %08x %d"
