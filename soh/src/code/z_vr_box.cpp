...
s32 func_800ADBB0(SkyboxContext* skyboxCtx, Vtx* roomVtx, s32 arg2, s32 arg3, s32 arg4, s32 arg5, s32 arg6, s32 arg7,
    ...
    for (phi_a2_4 = 0, sp424 = 0; sp424 < 2; sp424++) {
        ...
        gSPVertex(skyboxCtx->unk_138++, reinterpret_cast<std::uintptr_t>(&roomVtx[arg2]), 32, 0);
        arg2 += i;
        ...
        for (phi_t2_4 = 0, phi_ra = 0; phi_ra < 4; phi_ra++, phi_a2_4 += 0x1F) {
            for (phi_a0_4 = 0, phi_t1 = 0; phi_t1 < 4; phi_t1++, phi_a0_4 += 0x3F, phi_t2_4 += 4) {
                gDPLoadTextureTile(skyboxCtx->unk_138++, static_cast<u8*>(skyboxCtx->staticSegments[0]) + D_8012AC90[arg8],
...
s32 func_800AE2C0(SkyboxContext* skyboxCtx, Vtx* roomVtx, s32 arg2, s32 arg3, s32 arg4, s32 arg5, s32 arg6, s32 arg7,
    ...
    //WAT
    skyboxCtx->unk_138 = reinterpret_cast<decltype(skyboxCtx->unk_138)>(&skyboxCtx->dListBuf[2 * arg8]);
    ...
    if ((arg8 == 4) || (arg8 == 5)) {
        ...
        for (phi_t2_4 = 0, phi_ra = 0; phi_ra < 4; phi_ra++, phi_a2_4 += 0x1F) {
            for (phi_a0_4 = 0, phi_t1 = 0; phi_t1 < 4; phi_t1++, phi_a0_4 += 0x1F, phi_t2_4 += 4) {
                gDPLoadMultiTile(skyboxCtx->unk_138++, static_cast<u8*>(skyboxCtx->staticSegments[0]) + D_8012ADC0[arg8], 0,
                                 G_TX_RENDERTILE, G_IM_FMT_CI, G_IM_SIZ_8b, 128, 0, phi_a0_4, phi_a2_4, phi_a0_4 + 0x1F,
                                 phi_a2_4 + 0x1F, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOLOD,
                                 G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOLOD);
                gDPLoadMultiTile(skyboxCtx->unk_138++, static_cast<u8*>(skyboxCtx->staticSegments[1]) + D_8012ADC0[arg8], 0x80, 1,
...
void LoadSkyboxTex(GlobalContext* globalCtx, SkyboxContext* skyboxCtx, int segmentIndex, int imageIndex, const char* tex, int width, int height, int offsetW, int offsetH)
{
    if (globalCtx != NULL && globalCtx->state.gfxCtx != NULL && globalCtx->state.gfxCtx != reinterpret_cast<GraphicsContext*>(0xABABABAB))
    ...
    memcpy(static_cast<u8*>(skyboxCtx->staticSegments[segmentIndex]) + (imageIndex * (offsetW * offsetH)), ResourceMgr_LoadTexByName(tex), width * height);
}

void LoadSkyboxTexAtOffset(SkyboxContext* skyboxCtx, int segmentIndex, const char* tex, int width, int height, int offset)
{
    memcpy(reinterpret_cast<u8*>(skyboxCtx->staticSegments[segmentIndex]) + offset, ResourceMgr_LoadTexByName(tex), width * height);
}

void LoadSkyboxPalette(GlobalContext* globalCtx, SkyboxContext* skyboxCtx, int paletteIndex, const char* palTex, int width,
                       int height) {
    if (globalCtx != NULL && globalCtx->state.gfxCtx != NULL && globalCtx->state.gfxCtx != reinterpret_cast<GraphicsContext*>(0xABABABAB))
        gSPInvalidateTexCache(globalCtx->state.gfxCtx->polyOpa.p++, (uintptr_t)skyboxCtx->palettes + (paletteIndex * (width * height * 2)));

    memcpy(reinterpret_cast<u8*>(skyboxCtx->palettes) + (paletteIndex * (width * height * 2)), ResourceMgr_LoadTexByName(palTex), width * height * 2);
}
...
void Skybox_Setup(GlobalContext* globalCtx, SkyboxContext* skyboxCtx, s16 skyboxId) {
    ...
    case SKYBOX_NORMAL_SKY:
      ...
      for (int i = 0; i < 5; i++)
        ...
        if ((sp41 & 1) ^ ((sp41 & 4) >> 2)) 
        {
            skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(GameState_Alloc(&globalCtx->state, 256 * 2, "../z_vr_box.c", 1072));
        ...
        {
            skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(
                GameState_Alloc(&globalCtx->state, 256 * 2, "../z_vr_box.c", 1085));
    ...
    case SKYBOX_BAZAAR:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(
            GameState_Alloc(&globalCtx->state, 0x200 * 2, "../z_vr_box.c", 1231));
    ...
    case SKYBOX_HOUSE_LINK:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(
            GameState_Alloc(&globalCtx->state, 0x200 * 4, "../z_vr_box.c", 1231));
    ...
    case SKYBOX_OVERCAST_SUNSET:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(
            GameState_Alloc(&globalCtx->state, 0x100, "../z_vr_box.c", 1231));
    ...
    case SKYBOX_CUTSCENE_MAP:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(
            GameState_Alloc(&globalCtx->state, 0x100 * 2, "../z_vr_box.c", 1231));
    ...
    case SKYBOX_HAPPY_MASK_SHOP:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(GameState_Alloc(&globalCtx->state, 0x200 * 2, "../z_vr_box.c", 1231));
    ...
    case SKYBOX_HOUSE_KNOW_IT_ALL_BROTHERS:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(GameState_Alloc(&globalCtx->state, 0x200 * 4, "../z_vr_box.c", 1231));
    ...
    case SKYBOX_HOUSE_OF_TWINS:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(GameState_Alloc(&globalCtx->state, 0x200 * 3, "../z_vr_box.c", 1231));
    ...
    default:
        ...
        skyboxCtx->palettes = static_cast<decltype(skyboxCtx->palettes)>(GameState_Alloc(&globalCtx->state, 0x1000, "../z_vr_box.c", 1226));
        break;
    }
}
...
void Skybox_Init(GameState* state, SkyboxContext* skyboxCtx, s16 skyboxId) {
    ...
    if (skyboxId != SKYBOX_NONE) {
        ...
        if (skyboxCtx->unk_140 != 0) {
            skyboxCtx->dListBuf = static_cast<decltype(skyboxCtx->dListBuf)>(
                GameState_Alloc(state, 8 * 150 * sizeof(Gfx), "../z_vr_box.c", 1636));
            ASSERT(skyboxCtx->dListBuf != NULL, "vr_box->dpList != NULL", "../z_vr_box.c", 1637);

            skyboxCtx->roomVtx = static_cast<Vtx*>( GameState_Alloc(state, 256 * sizeof(Vtx), "../z_vr_box.c", 1639) );
            ASSERT(skyboxCtx->roomVtx != NULL, "vr_box->roomVtx != NULL", "../z_vr_box.c", 1640);

            func_800AEFC8(skyboxCtx, skyboxId);
        } else {
            skyboxCtx->dListBuf = static_cast<decltype(skyboxCtx->dListBuf)>(
                GameState_Alloc(state, 12 * 150 * sizeof(Gfx), "../z_vr_box.c", 1643));
            ASSERT(skyboxCtx->dListBuf != NULL, "vr_box->dpList != NULL", "../z_vr_box.c", 1644);

            if (skyboxId == SKYBOX_CUTSCENE_MAP) {
                skyboxCtx->roomVtx = static_cast<Vtx*>( GameState_Alloc(state, 192 * sizeof(Vtx), "../z_vr_box.c", 1648) );
                ASSERT(skyboxCtx->roomVtx != NULL, "vr_box->roomVtx != NULL", "../z_vr_box.c", 1649);

                func_800AF178(skyboxCtx, 6);
            } else {
                skyboxCtx->roomVtx = static_cast<Vtx*>( GameState_Alloc(state, 160 * sizeof(Vtx), "../z_vr_box.c", 1653) );
