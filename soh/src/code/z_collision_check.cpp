...
void Collider_DrawPoly(GraphicsContext* gfxCtx, Vec3f* vA, Vec3f* vB, Vec3f* vC, u8 r, u8 g, u8 b) {
    ...
    vtxTbl = static_cast<Vtx*>(Graph_Alloc(gfxCtx, 3 * sizeof(Vtx)));
    ...
    gSPVertex(POLY_OPA_DISP++, reinterpret_cast<std::uintptr_t>(vtxTbl), 3, 0);
...
s32 Collider_SetJntSphToActor(GlobalContext* globalCtx, ColliderJntSph* dest, ColliderJntSphInitToActor* src) {
    ...
    dest->elements = static_cast<ColliderJntSphElement*>( ZeldaArena_MallocDebug(src->count * sizeof(ColliderJntSphElement), "../z_collision_check.c", 1443) );
...
s32 Collider_SetJntSphAllocType1(GlobalContext* globalCtx, ColliderJntSph* dest, Actor* actor,
    ...
    dest->count = src->count;
    dest->elements = static_cast<ColliderJntSphElement*>( ZeldaArena_MallocDebug(src->count * sizeof(ColliderJntSphElement), "../z_collision_check.c", 1490) );
...
s32 Collider_SetJntSphAlloc(GlobalContext* globalCtx, ColliderJntSph* dest, Actor* actor, ColliderJntSphInit* src) {
    ...
    dest->count = src->count;
    dest->elements = static_cast<ColliderJntSphElement*>( ZeldaArena_MallocDebug(src->count * sizeof(ColliderJntSphElement), "../z_collision_check.c", 1551) );
...
s32 Collider_SetTrisAllocType1(GlobalContext* globalCtx, ColliderTris* dest, Actor* actor, ColliderTrisInitType1* src) {
    ...
    Collider_SetBaseType1(globalCtx, &dest->base, actor, &src->base);
    dest->count = src->count;
    dest->elements = static_cast<ColliderTrisElement*>( ZeldaArena_MallocDebug(dest->count * sizeof(ColliderTrisElement), "../z_collision_check.c", 2156) );
    if (dest->elements == NULL) {
        dest->count = 0;
        osSyncPrintf(VT_FGCOL(RED));
...
s32 Collider_SetTrisAlloc(GlobalContext* globalCtx, ColliderTris* dest, Actor* actor, ColliderTrisInit* src) {
    ...
    Collider_SetBase(globalCtx, &dest->base, actor, &src->base);
    dest->count = src->count;
    dest->elements = static_cast<ColliderTrisElement*>( ZeldaArena_MallocDebug(dest->count * sizeof(ColliderTrisElement), "../z_collision_check.c", 2207) );
...
/**
 * Sets a line as an OC collider for thisv frame.
...
// In order to reproduce thisv behavior, we need a specific number of bss variables in the file before that point.
// For thisv, we introduce a certain amount of dummy variables throughout the file, which we fit inside padding added
