...
void PlayerCall_InitFuncPtrs(void) {
    sPlayerCallInitFunc = static_cast<decltype(sPlayerCallInitFunc)>(KaleidoManager_GetRamAddr(Player_Init));
    sPlayerCallDestroyFunc = static_cast<decltype(sPlayerCallDestroyFunc)>(KaleidoManager_GetRamAddr(Player_Destroy));
    sPlayerCallUpdateFunc = static_cast<decltype(sPlayerCallUpdateFunc)>(KaleidoManager_GetRamAddr(Player_Update));
    sPlayerCallDrawFunc = static_cast<decltype(sPlayerCallDrawFunc)>(KaleidoManager_GetRamAddr(Player_Draw));
}
