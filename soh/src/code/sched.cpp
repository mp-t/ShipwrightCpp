...
constexpr u32 RSP_DONE_MSG_ID = 667;
constexpr u32 RDP_DONE_MSG_ID = 668;
constexpr u32 ENTRY_MSG_ID = 670;

#define RSP_DONE_MSG (reinterpret_cast<OSMesg>(RSP_DONE_MSG_ID))
#define RDP_DONE_MSG (reinterpret_cast<OSMesg>(RDP_DONE_MSG_ID))
#define ENTRY_MSG (reinterpret_cast<OSMesg>(ENTRY_MSG_ID))
...
void Sched_HandleEntry(SchedContext* sc) {
    ...
    while (osRecvMesg(&sc->cmdQ, &msg, OS_MESG_NOBLOCK) != -1) {
        Sched_QueueTask(sc, static_cast<OSScTask*>(msg));
    }
...
void Sched_ThreadEntry(void* arg) {
    ...
    while (true) {
        ...
        switch (reinterpret_cast<u32>(msg)) {
            case ENTRY_MSG_ID:
                if (sLogScheduler) {
