...
s32 SkelCurve_Init(GlobalContext* globalCtx, SkelAnimeCurve* skelCurve, const SkelCurveLimbList* limbListSeg,
                   const TransformUpdateIndex* transUpdIdx) {

    if (ResourceMgr_OTRSigCheck(limbListSeg))
        limbListSeg = reinterpret_cast<SkelCurveLimbList*>(ResourceMgr_LoadSkeletonByName(reinterpret_cast<const char*>(limbListSeg)));

    SkelCurveLimb** limbs;
    const SkelCurveLimbList* limbList = SEGMENTED_TO_VIRTUAL(limbListSeg);

    skelCurve->limbCount = limbList->limbCount;
    skelCurve->limbList = SEGMENTED_TO_VIRTUAL(limbList->limbs);

    skelCurve->transforms = static_cast<LimbTransform*>( ZeldaArena_MallocDebug(sizeof(*skelCurve->transforms) * skelCurve->limbCount,
                                                   "../z_fcurve_data_skelanime.c", 125) );
    ASSERT(skelCurve->transforms != NULL, "thisv->now_joint != NULL", "../z_fcurve_data_skelanime.c", 127);
    skelCurve->animCurFrame = 0.0f;
    return 1;
}
...
void SkelCurve_SetAnim(SkelAnimeCurve* skelCurve, const TransformUpdateIndex* transUpdIdx, f32 arg2, f32 animFinalFrame,
...
s32 SkelCurve_Update(GlobalContext* globalCtx, SkelAnimeCurve* skelCurve) {
    s16* transforms;
    u8* transformRefIdx;
    const TransformUpdateIndex* transformIndex;
    ...
    if (ResourceMgr_OTRSigCheck(transformIndex))
        transformIndex = reinterpret_cast<TransformUpdateIndex*>(ResourceMgr_LoadAnimByName(reinterpret_cast<const char*>(transformIndex)));
    
    transformRefIdx = SEGMENTED_TO_VIRTUAL(transformIndex->refIndex);
    transData = SEGMENTED_TO_VIRTUAL(transformIndex->transformData);
    transformCopyValues = reinterpret_cast<u16*>( SEGMENTED_TO_VIRTUAL(transformIndex->copyValues) );
