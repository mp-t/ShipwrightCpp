...
void func_80064824(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdBase* cmd) {
...
void Cutscene_Command_SetLighting(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdEnvLighting* cmd) {
    if (csCtx->frames == cmd->startFrame) {
...
void Cutscene_Command_PlayBGM(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdMusicChange* cmd) {
    if (csCtx->frames == cmd->startFrame) {
...
void Cutscene_Command_StopBGM(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdMusicChange* cmd) {
    if (csCtx->frames == cmd->startFrame) {
...
void Cutscene_Command_FadeBGM(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdMusicFade* cmd) {
    u8 var1;
...
// Command 9: ?
void Cutscene_Command_09(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdUnknown9* cmd) {
    if (csCtx->frames == cmd->startFrame) {
...
void func_80065134(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdDayTime* cmd) {
    s16 temp1;
    s16 temp2;
...
void Cutscene_Command_Terminator(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdBase* cmd) {
    Player* player = GET_PLAYER(globalCtx);
    s32 temp = 0;
...
void Cutscene_Command_TransitionFX(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdBase* cmd) {
    f32 temp;
...
size_t Cutscene_Command_CameraPositions(GlobalContext* globalCtx, CutsceneContext* csCtx, const u8* cmd, u8 relativeToLink) {
...
size_t Cutscene_Command_CameraFocus(GlobalContext* globalCtx, CutsceneContext* csCtx, const u8* cmd, u8 relativeToLink) {
    s32 shouldContinue = true;
    CsCmdBase* cmdBase = (CsCmdBase*)cmd;
    size_t size;
...
size_t Cutscene_Command_07(GlobalContext* globalCtx, CutsceneContext* csCtx, const u8* cmd, u8 unused) {
    CsCmdBase* cmdBase = (CsCmdBase*)cmd;
    size_t size;
    Vec3f sp3C;
    Vec3f sp30;
...
size_t Cutscene_Command_08(GlobalContext* globalCtx, CutsceneContext* csCtx, const u8* cmd, u8 unused) {
    CsCmdBase* cmdBase = (CsCmdBase*)cmd;
    size_t size;
    Vec3f sp3C;
    Vec3f sp30;
...
void Cutscene_Command_Textbox(GlobalContext* globalCtx, CutsceneContext* csCtx, const CsCmdTextbox* cmd) {
    u8 dialogState;
...
void Cutscene_ProcessCommands(GlobalContext* globalCtx, CutsceneContext* csCtx, const u8* cutscenePtr) {
    s16 i;
    s32 totalEntries;
    s32 cmdType;
    s32 cmdEntries;
    ...
    if (ResourceMgr_OTRSigCheck(cutscenePtr))
        cutscenePtr = ResourceMgr_LoadCSByName(reinterpret_cast<const char*>(cutscenePtr));
        ...
        switch (cmdType) {
            case CS_CMD_MISC:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    func_80064824(globalCtx, csCtx, reinterpret_cast<const CsCmdBase*>(cutscenePtr));
            ...
            case CS_CMD_SET_LIGHTING:
                ...
                cutscenePtr += sizeof(CutsceneData);
                for (j = 0; j < cmdEntries; j++) {
                    Cutscene_Command_SetLighting(globalCtx, csCtx, reinterpret_cast<const CsCmdEnvLighting*>(cutscenePtr));
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_PLAYBGM:
                ...
                cutscenePtr += sizeof(CutsceneData);
                for (j = 0; j < cmdEntries; j++) {
                    Cutscene_Command_PlayBGM(globalCtx, csCtx, reinterpret_cast<const CsCmdMusicChange*>(cutscenePtr));
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_STOPBGM:
                ...
                cutscenePtr += sizeof(CutsceneData);
                for (j = 0; j < cmdEntries; j++) {
                    Cutscene_Command_StopBGM(globalCtx, csCtx, reinterpret_cast<const CsCmdMusicChange*>(cutscenePtr));
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_FADEBGM:
                ...
                cutscenePtr += sizeof(CutsceneData);
                for (j = 0; j < cmdEntries; j++) {
                    Cutscene_Command_FadeBGM(globalCtx, csCtx, reinterpret_cast<const CsCmdMusicFade*>(cutscenePtr));
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_09:
                ...
                cutscenePtr += sizeof(CutsceneData);
                for (j = 0; j < cmdEntries; j++) {
                    Cutscene_Command_09(globalCtx, csCtx, reinterpret_cast<const CsCmdUnknown9*>(cutscenePtr));
            ...
            case CS_CMD_SETTIME:
                ...
                cutscenePtr += sizeof(CutsceneData);
                for (j = 0; j < cmdEntries; j++) {
                    func_80065134(globalCtx, csCtx, reinterpret_cast<const CsCmdDayTime*>(cutscenePtr));
                    cutscenePtr += sizeof(CutsceneData) * 3;
                }
                break;
            case CS_CMD_SET_PLAYER_ACTION:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    ...
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->linkAction = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 144:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[0] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 141:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[1] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 132:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[2] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 133:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[3] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 135:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[4] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 136:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[5] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 137:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[6] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            ...
            case 142:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[7] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_SET_ACTOR_ACTION_9:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[8] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_SET_ACTOR_ACTION_10:
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if ((cmd->startFrame < csCtx->frames) && (csCtx->frames <= cmd->endFrame)) {
                        csCtx->npcActions[9] = reinterpret_cast<const CsCmdActorAction*>(cutscenePtr);
                    }
                    cutscenePtr += sizeof(CutsceneData) * 12;
                }
                break;
            case CS_CMD_CAM_EYE:
                cutscenePtr += Cutscene_Command_CameraPositions(globalCtx, csCtx, cutscenePtr, 0);
                break;
            case CS_CMD_CAM_EYE_REL_TO_PLAYER:
                cutscenePtr += Cutscene_Command_CameraPositions(globalCtx, csCtx, cutscenePtr, 1);
                break;
            case CS_CMD_CAM_AT:
                cutscenePtr += Cutscene_Command_CameraFocus(globalCtx, csCtx, cutscenePtr, 0);
                break;
            case CS_CMD_CAM_AT_REL_TO_PLAYER:
                cutscenePtr += Cutscene_Command_CameraFocus(globalCtx, csCtx, cutscenePtr, 1);
                break;
            case CS_CMD_07:
                cutscenePtr += Cutscene_Command_07(globalCtx, csCtx, cutscenePtr, 0);
                break;
            case CS_CMD_08:
                cutscenePtr += Cutscene_Command_08(globalCtx, csCtx, cutscenePtr, 0);
                break;
            case CS_CMD_TERMINATOR:
                cutscenePtr += 4;
                Cutscene_Command_Terminator(globalCtx, csCtx, reinterpret_cast<const CsCmdBase*>(cutscenePtr));
                cutscenePtr += 8;
                break;
            case CS_CMD_TEXTBOX:
                memcpy(&cmdEntries, cutscenePtr, sizeof(CutsceneData));
                ...
                for (j = 0; j < cmdEntries; j++) {
                    cmd = (CsCmdBase*)cutscenePtr;
                    if (cmd->base != 0xFFFF) {
                        Cutscene_Command_Textbox(globalCtx, csCtx, reinterpret_cast<const CsCmdTextbox*>(cutscenePtr));
                    }
                    cutscenePtr += sizeof(CutsceneData) * 3;
                }
                break;
            case CS_CMD_SCENE_TRANS_FX:
                cutscenePtr += sizeof(CutsceneData);
                Cutscene_Command_TransitionFX(globalCtx, csCtx, reinterpret_cast<const CsCmdBase*>(cutscenePtr));
...
void func_80068C3C(GlobalContext* globalCtx, CutsceneContext* csCtx) {
    ...
    if (gSaveContext.cutsceneIndex >= 0xFFF0) {
        ...
        csCtx->frames++;
        if (dREG(95) != 0) {
            Cutscene_ProcessCommands(globalCtx, csCtx, static_cast<u8*>(D_8012D1F0));
        } else {
            Cutscene_ProcessCommands(globalCtx, csCtx, static_cast<const u8*>(globalCtx->csCtx.segment));
        }
    }
}
...
void Cutscene_SetSegment(GlobalContext* globalCtx, const void* segment) {
