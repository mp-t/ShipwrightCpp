...
void Skin_UpdateVertices(MtxF* mtx, SkinVertex* skinVertices, SkinLimbModif* modifEntry, Vtx* vtxBuf, Vec3f* pos) {
    ...
    for (vertexEntry = skinVertices; vertexEntry < &skinVertices[modifEntry->vtxCount]; vertexEntry++) {
        ...
        xwTemp = mtx->mf_raw.xw;
        ywTemp = mtx->mf_raw.yw;
        zwTemp = mtx->mf_raw.zw;

        mtx->mf_raw.xw = mtx->mf_raw.yw = mtx->mf_raw.zw = 0.0f;
        ...
        mtx->mf_raw.xw = xwTemp;
        mtx->mf_raw.yw = ywTemp;
        mtx->mf_raw.zw = zwTemp;
    }
}
...
void Skin_ApplyLimbModifications(GraphicsContext* gfxCtx, Skin* skin, s32 limbIndex, s32 arg3) {
    ...
    data = static_cast<SkinAnimatedLimbData*>( SEGMENTED_TO_VIRTUAL(((SkinLimb*)SEGMENTED_TO_VIRTUAL(skeleton[limbIndex]))->segment) );
 ...
 * The vertices of thisv limb are modified dynamically
 */
void Skin_DrawAnimatedLimb(GraphicsContext* gfxCtx, Skin* skin, s32 limbIndex, s32 arg3, s32 drawFlags) {
    ...
    skeleton = reinterpret_cast<SkinLimb**>(SEGMENTED_TO_VIRTUAL(skin->skeletonHeader->segment));
    data = static_cast<SkinAnimatedLimbData*>( SEGMENTED_TO_VIRTUAL(((SkinLimb*)SEGMENTED_TO_VIRTUAL(skeleton[limbIndex]))->segment) );
...
void Skin_DrawLimb(GraphicsContext* gfxCtx, Skin* skin, s32 limbIndex, const Gfx* dlistOverride, s32 drawFlags) {
    ...
    if (dlistOverride == NULL) {
        gfx = static_cast<Gfx*>( ((SkinLimb*)SEGMENTED_TO_VIRTUAL(skeleton[limbIndex]))->segment );
    }
