...
u16 func_80AA19A0(GlobalContext* globalCtx, Actor* thisv);
s16 func_80AA1A38(GlobalContext* globalCtx, Actor* thisv);

void func_80AA1AE4(EnMa2* thisv, GlobalContext* globalCtx);
s32 func_80AA1C68(EnMa2* thisv);
void EnMa2_UpdateEyes(EnMa2* thisv);
void func_80AA1DB4(EnMa2* thisv, GlobalContext* globalCtx);
void func_80AA2018(EnMa2* thisv, GlobalContext* globalCtx);
void func_80AA204C(EnMa2* thisv, GlobalContext* globalCtx);
void func_80AA20E4(EnMa2* thisv, GlobalContext* globalCtx);
void func_80AA21C8(EnMa2* thisv, GlobalContext* globalCtx);
...
void func_80AA1AE4(EnMa2* thisv, GlobalContext* globalCtx) {
    ...
    if ((thisv->unk_1E0.unk_00 == 0) && (thisv->skelAnime.animation == &gMalonAdultSingAnim)) {
        phi_a3 = 1;
    } else {
        phi_a3 = 0;
    }
    ...
    thisv->unk_1E0.unk_14 = 0.0f;

    func_80034A14(&thisv->actor, &thisv->unk_1E0, 0, phi_a3);
}

u16 func_80AA1B58(EnMa2* thisv, GlobalContext* globalCtx) {
    if (LINK_IS_CHILD) {
        return 0;
    }
    if (!(gSaveContext.eventChkInf[1] & 0x100) && (globalCtx->sceneNum == SCENE_MALON_STABLE) && IS_DAY &&
        (thisv->actor.shape.rot.z == 5)) {
        return 1;
    }
    if (!(gSaveContext.eventChkInf[1] & 0x100) && (globalCtx->sceneNum == SCENE_SPOT20) && IS_NIGHT &&
        (thisv->actor.shape.rot.z == 6)) {
        return 2;
    }
    if (!(gSaveContext.eventChkInf[1] & 0x100) || (globalCtx->sceneNum != SCENE_SPOT20)) {
        return 0;
    }
    if ((thisv->actor.shape.rot.z == 7) && IS_DAY) {
        return 3;
    }
    if ((thisv->actor.shape.rot.z == 8) && IS_NIGHT) {
        return 3;
    }
    return 0;
}
...
s32 func_80AA1C68(EnMa2* thisv) {
    if (thisv->skelAnime.animation != &gMalonAdultSingAnim) {
        return 0;
    }
...
void EnMa2_UpdateEyes(EnMa2* thisv) {
    if ((!func_80AA1C68(thisv)) && (DECR(thisv->blinkTimer) == 0)) {
        thisv->eyeIndex += 1;
        if (thisv->eyeIndex >= 3) {
...
void EnMa2_ChangeAnim(EnMa2* thisv, s32 index) {
...
void EnMa2_Init(Actor* thisx, GlobalContext* globalCtx) {
    EnMa2* thisv = (EnMa2*)thisx;
    s32 pad;

    ActorShape_Init(&thisv->actor.shape, 0.0f, ActorShadow_DrawCircle, 18.0f);
    SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gMalonAdultSkel, NULL, NULL, NULL, 0);
    Collider_InitCylinder(globalCtx, &thisv->collider);
    Collider_SetCylinder(globalCtx, &thisv->collider, &thisv->actor, &sCylinderInit);
    CollisionCheck_SetInfo2(&thisv->actor.colChkInfo, DamageTable_Get(22), &sColChkInfoInit);

    switch (func_80AA1B58(thisv, globalCtx)) {
        case 1:
            EnMa2_ChangeAnim(thisv, ENMA2_ANIM_2);
            thisv->actionFunc = func_80AA2018;
            break;
        case 2:
            EnMa2_ChangeAnim(thisv, ENMA2_ANIM_3);
            thisv->actionFunc = func_80AA204C;
            break;
        case 3:
            if (gSaveContext.infTable[8] & 0x2000) {
                EnMa2_ChangeAnim(thisv, ENMA2_ANIM_0);
            } else {
                EnMa2_ChangeAnim(thisv, ENMA2_ANIM_3);
            }
            thisv->actionFunc = func_80AA2018;
...
void func_80AA204C(EnMa2* thisv, GlobalContext* globalCtx) {
    ...
    if (player->stateFlags2 & 0x1000000) {
        ...
        thisv->actionFunc = func_80AA20E4;
...
void func_80AA20E4(EnMa2* thisv, GlobalContext* globalCtx) {
    ...
    if (globalCtx->msgCtx.ocarinaMode >= OCARINA_MODE_04) {
        ...
        thisv->actionFunc = func_80AA21C8;
        globalCtx->msgCtx.ocarinaMode = OCARINA_MODE_04;
    } else {
        player->stateFlags2 |= 0x800000;
    }
...
void EnMa2_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    EnMa2_UpdateEyes(thisv);
    thisv->actionFunc(thisv, globalCtx);
    func_80AA1DB4(thisv, globalCtx);
    func_80AA1AE4(thisv, globalCtx);
    if (thisv->actionFunc != func_80AA20E4) {
        func_800343CC(globalCtx, &thisv->actor, &thisv->unk_1E0.unk_00, (f32)thisv->collider.dim.radius + 30.0f,
                      func_80AA19A0, func_80AA1A38);
    }
}

s32 EnMa2_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnMa2* thisv = (EnMa2*)thisx;
    Vec3s vec;
    ...
    if ((limbIndex == MALON_ADULT_CHEST_AND_NECK_LIMB) || (limbIndex == MALON_ADULT_LEFT_SHOULDER_LIMB) ||
        ...
        rot->y += Math_SinS(thisv->unk_212[limbIndex].y) * 200.0f;
        rot->z += Math_CosS(thisv->unk_212[limbIndex].z) * 200.0f;
    }
    return false;
}

void EnMa2_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3s* rot, void* thisx) {
    EnMa2* thisv = (EnMa2*)thisx;
    ...
    if ((limbIndex == MALON_ADULT_LEFT_HAND_LIMB) && (thisv->skelAnime.animation == &gMalonAdultStandStillAnim)) {
...
void EnMa2_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    someFloat = Math_Vec3f_DistXZ(&thisv->actor.world.pos, &camera->eye);
    ...
    SkelAnime_DrawFlexOpa(globalCtx, thisv->skelAnime.skeleton, thisv->skelAnime.jointTable, thisv->skelAnime.dListCount,
                          EnMa2_OverrideLimbDraw, EnMa2_PostLimbDraw, thisv);
