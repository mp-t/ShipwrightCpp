...
void BgIceTurara_Stalagmite(BgIceTurara* thisv, GlobalContext* globalCtx);
void BgIceTurara_Wait(BgIceTurara* thisv, GlobalContext* globalCtx);
void BgIceTurara_Shiver(BgIceTurara* thisv, GlobalContext* globalCtx);
void BgIceTurara_Fall(BgIceTurara* thisv, GlobalContext* globalCtx);
void BgIceTurara_Regrow(BgIceTurara* thisv, GlobalContext* globalCtx);
...
void BgIceTurara_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgIceTurara* thisv = (BgIceTurara*)thisx;
    s32 pad;
    ...
    if (thisv->dyna.actor.params == TURARA_STALAGMITE) {
        thisv->actionFunc = BgIceTurara_Stalagmite;
    } else {
        thisv->dyna.actor.shape.rot.x = -0x8000;
        thisv->dyna.actor.shape.yOffset = 1200.0f;
        thisv->actionFunc = BgIceTurara_Wait;
    }
}
...
void BgIceTurara_Break(BgIceTurara* thisv, GlobalContext* globalCtx, f32 arg2) {
    ...
    SoundSource_PlaySfxAtFixedWorldPos(globalCtx, &thisv->dyna.actor.world.pos, 30, NA_SE_EV_ICE_BROKEN);
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 10; j++) {
            pos.x = thisv->dyna.actor.world.pos.x + Rand_CenteredFloat(8.0f);
            pos.y = thisv->dyna.actor.world.pos.y + (Rand_ZeroOne() * arg2) + (i * arg2);
            pos.z = thisv->dyna.actor.world.pos.z + Rand_CenteredFloat(8.0f);
...
void BgIceTurara_Stalagmite(BgIceTurara* thisv, GlobalContext* globalCtx) {
    if (thisv->collider.base.acFlags & AC_HIT) {
        BgIceTurara_Break(thisv, globalCtx, 50.0f);
...
void BgIceTurara_Wait(BgIceTurara* thisv, GlobalContext* globalCtx) {
    if (thisv->dyna.actor.xzDistToPlayer < 60.0f) {
        thisv->shiverTimer = 10;
        thisv->actionFunc = BgIceTurara_Shiver;
    }
}
...
void BgIceTurara_Shiver(BgIceTurara* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->shiverTimer != 0) {
        thisv->shiverTimer--;
    }
    if (!(thisv->shiverTimer % 4)) {
        Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EV_ICE_SWING);
    }
    if (thisv->shiverTimer == 0) {
        ...
        thisv->actionFunc = BgIceTurara_Fall;
    } else {
        sp28 = Rand_ZeroOne();
        phi_v0_2 = (Rand_ZeroOne() < 0.5f ? -1 : 1);
        thisv->dyna.actor.world.pos.x = (phi_v0_2 * ((0.5f * sp28) + 0.5f)) + thisv->dyna.actor.home.pos.x;
        sp28 = Rand_ZeroOne();
        phi_v0_3 = (Rand_ZeroOne() < 0.5f ? -1 : 1);
        thisv->dyna.actor.world.pos.z = (phi_v0_3 * ((0.5f * sp28) + 0.5f)) + thisv->dyna.actor.home.pos.z;
    }
}

void BgIceTurara_Fall(BgIceTurara* thisv, GlobalContext* globalCtx) {
    if ((thisv->collider.base.atFlags & AT_HIT) || (thisv->dyna.actor.bgCheckFlags & 1)) {
        thisv->collider.base.atFlags &= ~AT_HIT;
        thisv->dyna.actor.bgCheckFlags &= ~1;
        if (thisv->dyna.actor.world.pos.y < thisv->dyna.actor.floorHeight) {
            thisv->dyna.actor.world.pos.y = thisv->dyna.actor.floorHeight;
        }
        BgIceTurara_Break(thisv, globalCtx, 40.0f);
        if (thisv->dyna.actor.params == TURARA_STALACTITE_REGROW) {
            thisv->dyna.actor.world.pos.y = thisv->dyna.actor.home.pos.y + 120.0f;
            func_8003EC50(globalCtx, &globalCtx->colCtx.dyna, thisv->dyna.bgId);
            thisv->actionFunc = BgIceTurara_Regrow;
        } else {
            Actor_Kill(&thisv->dyna.actor);
            return;
        }
        ...
        thisv->dyna.actor.world.pos.y += 40.0f;
