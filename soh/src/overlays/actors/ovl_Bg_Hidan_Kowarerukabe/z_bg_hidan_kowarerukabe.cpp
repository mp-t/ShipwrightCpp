...
static const Gfx* sBreakableWallDLists[] = {
...
void BgHidanKowarerukabe_InitDynaPoly(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    static const CollisionHeader* collisionHeaders[] = {
    ...
    if (collisionHeaders[thisv->dyna.actor.params & 0xFF] != NULL) {
        DynaPolyActor_Init(&thisv->dyna, DPM_UNK);
        CollisionHeader_GetVirtual(collisionHeaders[thisv->dyna.actor.params & 0xFF], &colHeader);
...
void BgHidanKowarerukabe_InitColliderSphere(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    ...
    thisv->collider.elements[0].dim.modelSphere.radius = sphereRadii[thisv->dyna.actor.params & 0xFF];
    thisv->collider.elements[0].dim.modelSphere.center.y = sphereYPositions[thisv->dyna.actor.params & 0xFF];
}

void BgHidanKowarerukabe_OffsetActorYPos(BgHidanKowarerukabe* thisv) {
    static f32 actorYPosOffsets[] = { 0.7f, 0.0f, 0.0f };

    thisv->dyna.actor.world.pos.y = actorYPosOffsets[thisv->dyna.actor.params & 0xFF] + thisv->dyna.actor.home.pos.y;
}
...
void BgHidanKowarerukabe_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgHidanKowarerukabe* thisv = (BgHidanKowarerukabe*)thisx;

    BgHidanKowarerukabe_InitDynaPoly(thisv, globalCtx);

    if (((thisv->dyna.actor.params & 0xFF) < CRACKED_STONE_FLOOR) ||
        ((thisv->dyna.actor.params & 0xFF) > LARGE_BOMBABLE_WALL)) {
        ...
        osSyncPrintf("Error : 炎の神殿 壊れる壁 の arg_data が判別出来ない(%s %d)(arg_data 0x%04x)\n",
                     "../z_bg_hidan_kowarerukabe.c", 254, thisv->dyna.actor.params);
    ...
    BgHidanKowarerukabe_InitColliderSphere(thisv, globalCtx);
    BgHidanKowarerukabe_OffsetActorYPos(thisv);
...
void BgHidanKowarerukabe_SpawnDust(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    s32 pad;
    Vec3f pos;
    ...
    pos.x = ((Rand_ZeroOne() - 0.5f) * 80.0f) + thisv->dyna.actor.world.pos.x;
    pos.y = (Rand_ZeroOne() * 100.0f) + thisv->dyna.actor.world.pos.y;
    pos.z = ((Rand_ZeroOne() - 0.5f) * 80.0f) + thisv->dyna.actor.world.pos.z;

    func_80033480(globalCtx, &pos, 100.0f, 4, 200, 250, 1);
}

void BgHidanKowarerukabe_FloorBreak(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    s32 i;
    s32 j;
    Vec3f velocity;
    Vec3f pos;
...
void func_8088A67C(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    s32 i;
    s32 j;
    Vec3f velocity;
    Vec3f pos;
...
void BgHidanKowarerukabe_LargeWallBreak(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    s32 i;
    s32 j;
    Vec3f velocity;
    Vec3f pos;
...
void BgHidanKowarerukabe_Break(BgHidanKowarerukabe* thisv, GlobalContext* globalCtx) {
    switch (thisv->dyna.actor.params & 0xFF) {
        case CRACKED_STONE_FLOOR:
            BgHidanKowarerukabe_FloorBreak(thisv, globalCtx);
            break;
        case BOMBABLE_WALL:
            func_8088A67C(thisv, globalCtx);
            break;
        case LARGE_BOMBABLE_WALL:
            BgHidanKowarerukabe_LargeWallBreak(thisv, globalCtx);
            break;
    }

    BgHidanKowarerukabe_SpawnDust(thisv, globalCtx);
}
...
void BgHidanKowarerukabe_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Actor_GetCollidedExplosive(globalCtx, &thisv->collider.base) != NULL) {
        BgHidanKowarerukabe_Break(thisv, globalCtx);
        Flags_SetSwitch(globalCtx, (thisv->dyna.actor.params >> 8) & 0x3F);

        if ((thisv->dyna.actor.params & 0xFF) == 0) {
            SoundSource_PlaySfxAtFixedWorldPos(globalCtx, &thisv->dyna.actor.world.pos, 40, NA_SE_EV_EXPLOSION);
        } else {
            SoundSource_PlaySfxAtFixedWorldPos(globalCtx, &thisv->dyna.actor.world.pos, 40, NA_SE_EV_WALL_BROKEN);
        }
...
void BgHidanKowarerukabe_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPDisplayList(POLY_OPA_DISP++, sBreakableWallDLists[thisv->dyna.actor.params & 0xFF]);
