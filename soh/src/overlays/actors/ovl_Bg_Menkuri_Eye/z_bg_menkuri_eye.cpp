...
void BgMenkuriEye_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgMenkuriEye* thisv = (BgMenkuriEye*)thisx;
...
void BgMenkuriEye_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (!Flags_GetSwitch(globalCtx, thisv->actor.params)) {
        if (thisv->framesUntilDisable != -1) {
            if (thisv->framesUntilDisable != 0) {
                thisv->framesUntilDisable -= 1;
            }
            if (thisv->framesUntilDisable == 0) {
                thisv->framesUntilDisable = -1;
    ...
    if ((thisv->collider.base.acFlags & AC_HIT) &&
        (ABS((s16)(thisv->collider.base.ac->world.rot.y - thisv->actor.shape.rot.y)) > 0x5000)) {
        thisv->collider.base.acFlags &= ~AC_HIT;
        if (thisv->framesUntilDisable == -1) {
            Audio_PlayActorSound2(&thisv->actor, NA_SE_EN_AMOS_DAMAGE);
            D_8089C1A0 += 1;
            D_8089C1A0 = CLAMP_MAX(D_8089C1A0, 4);
        }
        thisv->framesUntilDisable = 416;
        if (D_8089C1A0 == 4) {
...
void BgMenkuriEye_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    } else if (thisv->framesUntilDisable == -1) {
