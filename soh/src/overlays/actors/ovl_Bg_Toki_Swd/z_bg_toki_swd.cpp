...
void func_808BAF40(BgTokiSwd* thisv, GlobalContext* globalCtx);
void func_808BB0AC(BgTokiSwd* thisv, GlobalContext* globalCtx);
void func_808BB128(BgTokiSwd* thisv, GlobalContext* globalCtx);
...
void BgTokiSwd_SetupAction(BgTokiSwd* thisv, BgTokiSwdActionFunc actionFunc) {
    thisv->actionFunc = actionFunc;
}

void BgTokiSwd_Init(Actor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    BgTokiSwd* thisv = (BgTokiSwd*)thisx;

    Actor_ProcessInitChain(&thisv->actor, sInitChain);
    thisv->actor.shape.yOffset = 800.0f;
    BgTokiSwd_SetupAction(thisv, func_808BAF40);
...
void func_808BAF40(BgTokiSwd* thisv, GlobalContext* globalCtx) {
    if (((gSaveContext.eventChkInf[4] & 0x8000) == 0) && (gSaveContext.sceneSetupIndex < 4) &&
        Actor_IsFacingAndNearPlayer(&thisv->actor, 800.0f, 0x7530) && !Gameplay_InCsMode(globalCtx)) {
    ...
    if (!LINK_IS_ADULT || ((gSaveContext.eventChkInf[5] & 0x20))) {
        if (Actor_HasParent(&thisv->actor, globalCtx)) {
            ...
            BgTokiSwd_SetupAction(thisv, func_808BB0AC);
        } else {
            if (Actor_IsFacingPlayer(&thisv->actor, 0x2000)) {
...
void func_808BB0AC(BgTokiSwd* thisv, GlobalContext* globalCtx) {
    ...
    if (Actor_HasParent(&thisv->actor, globalCtx)) {
        if (!LINK_IS_ADULT) {
            Audio_PlayActorSound2(&thisv->actor, NA_SE_IT_SWORD_PUTAWAY_STN);
            thisv->actor.draw = NULL; // sword has been pulled, dont draw sword
        } else {
            thisv->actor.draw = BgTokiSwd_Draw; // sword has been placed, draw the master sword
        }
        BgTokiSwd_SetupAction(thisv, func_808BB128);
    } else {
        player = GET_PLAYER(globalCtx);
        player->interactRangeActor = &thisv->actor;
    }
}
