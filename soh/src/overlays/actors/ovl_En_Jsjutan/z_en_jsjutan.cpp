...
void EnJsjutan_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisv->dyna.bgId = DynaPoly_SetBgActor(globalCtx, &globalCtx->colCtx.dyna, thisx, header);
    Actor_SetScale(thisx, 0.02f);
    thisv->unk_164 = true;
    thisv->shadowAlpha = 100.0f;
}
...
void func_80A89860(EnJsjutan* thisv, GlobalContext* globalCtx) {
    s16 i;
    Vtx* oddVtx;
    Vtx* evenVtx;
    ...
    for (i = 0; i < ARRAY_COUNT(D_80A8EE10); i++, oddVtx++, evenVtx++) {
        ...
        if (thisv->dyna.actor.params == ENJSJUTAN_TYPE_01) {
            oddVtx->v.ob[1] = evenVtx->v.ob[1] = 0x585;
        } else {
            thisv->dyna.actor.world.pos.x = oddVtx->v.ob[0] * 0.02f + actorPos.x;
            thisv->dyna.actor.world.pos.z = oddVtx->v.ob[2] * 0.02f + actorPos.z;
            Actor_UpdateBgCheckInfo(globalCtx, &thisv->dyna.actor, 10.0f, 10.0f, 10.0f, 4);
            oddVtx->v.ob[1] = evenVtx->v.ob[1] = thisv->dyna.actor.floorHeight;
            thisv->dyna.actor.world.pos = actorPos;
        }
    }
}

void func_80A89A6C(EnJsjutan* thisv, GlobalContext* globalCtx) {
    ...
    spB4 = (player->actor.world.pos.y - thisv->unk_168) * 50.0f;
    spB0 = (player->actor.world.pos.z - thisv->dyna.actor.world.pos.z) * 50.0f;
    phi_s0_2 = carpetVtx;
    ...
    spD4[0] = (parent->world.pos.x - thisv->dyna.actor.world.pos.x) * 50.0f;
    spC8[0] = ((parent->world.pos.y - 8.0f) - thisv->unk_168) * 50.0f;
    spBC[0] = (parent->world.pos.z - thisv->dyna.actor.world.pos.z) * 50.0f;
    spE0[0] = 1;
    ...
    if ((gSaveContext.entranceIndex == 0x157) && (gSaveContext.sceneSetupIndex == 8)) {
        ...
        spD4[1] = 50.0f * (actorProfessor->world.pos.x - thisv->dyna.actor.world.pos.x);
        spC8[1] = 50.0f * (actorProfessor->world.pos.y - thisv->unk_168);
        spBC[1] = 50.0f * (actorProfessor->world.pos.z - thisv->dyna.actor.world.pos.z);
        spE0[1] = 1;

        spD4[2] = 50.0f * (actorBeanGuy->world.pos.x - thisv->dyna.actor.world.pos.x);
        spC8[2] = 50.0f * (actorBeanGuy->world.pos.y - thisv->unk_168);
        spBC[2] = 50.0f * (actorBeanGuy->world.pos.z - thisv->dyna.actor.world.pos.z);
        spE0[2] = 1;
    } else {
        // Player can place bombs in carpet and it will react to it.
        while (actorExplosive != NULL) {
            if (i < 3) {
                spD4[i] = (actorExplosive->world.pos.x - thisv->dyna.actor.world.pos.x) * 50.0f;
                spC8[i] = (actorExplosive->world.pos.y - thisv->unk_168) * 50.0f;
                spBC[i] = (actorExplosive->world.pos.z - thisv->dyna.actor.world.pos.z) * 50.0f;

                if ((fabsf(spD4[i]) < 5500.0f) && (fabsf(spC8[i]) < 3000.0f) && (fabsf(spBC[i]) < 5500.0f)) {
                    if (actorExplosive->params == BOMB_EXPLOSION) {
                        spE0[i] = 35; // Code never checks thisv, so it goes unused. Maybe it was planned to damage the
    ...
    for (i = 0; i < ARRAY_COUNT(D_80A8EE10); i++, carpetVtx++, shadowVtx++) {
        if (isPlayerOnTop) {
            ...
            // Distance percentage. 0.0f to 1.0f. 2500.0f is the max distance to an actor that thisv wave will consider.
            weight = (2500.0f - distVtx) / 2500.0f;
            if (weight < 0.0f) {
                weight = 0.0f;
            }
            offset = (spB4 * weight) + ((thisv->unk_170 - (thisv->unk_170 * weight)) - 200.0f);
        ...
        for (j = 0; j < 3; j++) {
            if (spE0[j] != 0) {
                ...
                // maxoffset = (spC8[i] * weight) + ((thisv->unk_170 - (thisv->unk_170 * weight)) - 200.0f);
                maxOffset = (spC8[j] * weight);
                maxOffset += ((thisv->unk_170 - (thisv->unk_170 * weight)) - 200.0f);
        ...
        if (thisv->unk_174) {
            ...
            s16 temp_a0_3 = (shadowVtx->n.ob[1] - thisv->unk_168) * 50.0f;
    ...
    if (!thisv->unk_174) {
        ...
        thisv->dyna.actor.world.pos.y = thisv->unk_168;
        ...
        thisv->shadowAlpha = (dayTime * 0.00275f) + 10.0f; // (1.0f / 364.0f) ?
        thisv->unk_170 = 1000.0f;
    } else {
        Math_ApproachF(&thisv->dyna.actor.world.pos.y, thisv->unk_168 - 1000.0f, 1.0f, thisv->dyna.actor.velocity.y);
        Math_ApproachF(&thisv->dyna.actor.velocity.y, 5.0f, 1.0f, 0.5f);
        Math_ApproachF(&thisv->shadowAlpha, 0.0f, 1.0f, 3.0f);
        Math_ApproachF(&thisv->unk_170, -5000.0f, 1.0f, 100.0f);
    }
...
void EnJsjutan_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (thisx->params == ENJSJUTAN_TYPE_01) {
        ...
        thisv->unk_168 = thisx->world.pos.y;
        if (!thisv->unk_175) {
            thisv->unk_175 = true;
            func_80A89860(thisv, globalCtx);
        }
    } else if (!thisv->unk_175) {
        thisv->unk_175 = true;
    ...
    gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, 0, 0, 0, (s16)thisv->shadowAlpha);
    ...
    func_80093D18(globalCtx->state.gfxCtx);
    Matrix_Translate(thisx->world.pos.x, thisv->unk_168 + 3.0f, thisx->world.pos.z, MTXMODE_NEW);
