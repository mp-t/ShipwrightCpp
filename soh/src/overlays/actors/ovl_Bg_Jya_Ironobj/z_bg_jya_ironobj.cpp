...
void func_808992D8(BgJyaIronobj* thisv);
void func_808992E8(BgJyaIronobj* thisv, GlobalContext* globalCtx);

void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* thisv, GlobalContext* globalCtx, EnIk* enIk);
void BgJyaIronobj_SpawnThoneParticles(BgJyaIronobj* thisv, GlobalContext* arg1, EnIk* enIk);
...
static const Gfx* sOpaDL[] = { gPillarDL, gThroneDL };
...
static const CollisionHeader* sCollisionHeaders[] = { &gPillarCol, &gThroneCol };

void BgJyaIronobj_InitCylinder(BgJyaIronobj* thisv, GlobalContext* globalCtx) {
    ColliderCylinder* colCylinder = &thisv->colCylinder;

    Collider_InitCylinder(globalCtx, colCylinder);
    Collider_SetCylinder(globalCtx, colCylinder, &thisv->dyna.actor, &sCylinderInit);
    if ((thisv->dyna.actor.params & 1) == 1) {
        thisv->colCylinder.dim.radius = 40;
        thisv->colCylinder.dim.height = 100;
    }
    Collider_UpdateCylinder(&thisv->dyna.actor, colCylinder);
}
...
void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* thisv, GlobalContext* globalCtx, EnIk* enIk) {
    ...
    rotY = Actor_WorldYawTowardActor(&thisv->dyna.actor, &enIk->actor) + D_808994D8[enIk->unk_2FF - 1];

    for (i = 0; i < 8; i++) {
        Actor* actor =
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_JYA_HAHENIRON, thisv->dyna.actor.world.pos.x,
                        Rand_ZeroOne() * 80.0f + thisv->dyna.actor.world.pos.y + 20.0f, thisv->dyna.actor.world.pos.z, 0,
    ...
    Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_JYA_HAHENIRON, thisv->dyna.actor.world.pos.x,
                thisv->dyna.actor.world.pos.y + 150.0f, thisv->dyna.actor.world.pos.z, 0, 0, 0, 1);
    Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_JYA_HAHENIRON, thisv->dyna.actor.world.pos.x,
                thisv->dyna.actor.world.pos.y, thisv->dyna.actor.world.pos.z, 0, 0, 0, 2);
    sins = Math_SinS(rotY);
    coss = Math_CosS(rotY);
    for (j = 0; j < 32; j++) {
        ...
        pos.y = thisv->dyna.actor.world.pos.y + ((4.375f * j) + 10.0f);
...
void BgJyaIronobj_SpawnThoneParticles(BgJyaIronobj* thisv, GlobalContext* arg1, EnIk* enIk) {
    ...
    for (i = 0; i < 8; i++) {
        Actor* actor =
            Actor_Spawn(&arg1->actorCtx, arg1, ACTOR_BG_JYA_HAHENIRON, thisv->dyna.actor.world.pos.x,
                        (Rand_ZeroOne() * 80.0f) + thisv->dyna.actor.world.pos.y + 10.0f, thisv->dyna.actor.world.pos.z,
    ...
    for (j = 0; j < 32; j++) {
        ...
        pos.x = thisv->dyna.actor.world.pos.x + (Rand_ZeroOne() * 40 - 20);
        pos.y = thisv->dyna.actor.world.pos.y + (3.75f * j);
        pos.z = thisv->dyna.actor.world.pos.z + (Rand_ZeroOne() * 40 - 20);
        temp_f22 = fabsf(j - 15.5f) * (1.0f / 31) + 0.5f;
        vel.x = 2.0f * (Rand_ZeroOne() * 6.0f - 3.0f) + (Rand_ZeroOne() * sins * 8.0f * temp_f22);
        vel.y = Rand_ZeroOne() * 8.0f - 3.0f;
        vel.z = 2.0f * (Rand_ZeroOne() * 6.0f - 3.0f) + (Rand_ZeroOne() * coss * 8.0f * temp_f22);
...
void BgJyaIronobj_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgJyaIronobj* thisv = (BgJyaIronobj*)thisx;
    s32 pad;
    const CollisionHeader* colHeader = NULL;

    DynaPolyActor_Init(&thisv->dyna, 0);
    Actor_ProcessInitChain(&thisv->dyna.actor, sInitChain);
    BgJyaIronobj_InitCylinder(thisv, globalCtx);
...
void func_808992D8(BgJyaIronobj* thisv) {
    thisv->actionFunc = func_808992E8;
}
...
void func_808992E8(BgJyaIronobj* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->colCylinder.base.acFlags & AC_HIT) {
        actor = thisv->colCylinder.base.ac;
        thisv->colCylinder.base.acFlags &= ~AC_HIT;
        if (actor != NULL && actor->id == ACTOR_EN_IK) {
            particleFunc[thisv->dyna.actor.params & 1](thisv, globalCtx, (EnIk*)actor);
            SoundSource_PlaySfxAtFixedWorldPos(globalCtx, &thisv->dyna.actor.world.pos, 80,
                                               NA_SE_EN_IRONNACK_BREAK_PILLAR);
            dropPos.x = thisv->dyna.actor.world.pos.x;
            dropPos.y = thisv->dyna.actor.world.pos.y + 20.0f;
            dropPos.z = thisv->dyna.actor.world.pos.z;
            for (i = 0; i < 3; i++) {
