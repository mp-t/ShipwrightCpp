...
void MagicDark_Init(Actor* thisvx, GlobalContext* globalCtx);
void MagicDark_Destroy(Actor* thisvx, GlobalContext* globalCtx);
void MagicDark_OrbUpdate(Actor* thisvx, GlobalContext* globalCtx);
void MagicDark_OrbDraw(Actor* thisvx, GlobalContext* globalCtx);
void MagicDark_DiamondUpdate(Actor* thisvx, GlobalContext* globalCtx);
void MagicDark_DiamondDraw(Actor* thisvx, GlobalContext* globalCtx);
...
void MagicDark_Init(Actor* thisvx, GlobalContext* globalCtx) {
    MagicDark* thisv = (MagicDark*)thisvx;
    ...
    thisvx->world.pos = player->actor.world.pos;
    Actor_SetScale(&thisv->actor, 0.0f);
    thisvx->room = -1;

    if (gSaveContext.nayrusLoveTimer != 0) {
        thisvx->update = MagicDark_DiamondUpdate;
        thisvx->draw = MagicDark_DiamondDraw;
        thisvx->scale.x = thisvx->scale.z = thisv->scale * 1.6f;
        thisvx->scale.y = thisv->scale * 0.8f;
        thisv->timer = 0;
        thisv->primAlpha = 0;
    } else {
        thisv->timer = 0;
...
void MagicDark_DiamondUpdate(Actor* thisvx, GlobalContext* globalCtx) {
    ...
    thisvx->scale.x = thisvx->scale.z = thisv->scale;

    if (thisv->timer < 20) {
        thisvx->scale.x = thisvx->scale.z = (1.6f - (thisv->timer * 0.03f)) * thisv->scale;
        thisvx->scale.y = ((thisv->timer * 0.01f) + 0.8f) * thisv->scale;
    } else {
        thisvx->scale.x = thisvx->scale.z = thisv->scale;
        thisvx->scale.y = thisv->scale;
    }

    thisvx->scale.x *= 1.3f;
    thisvx->scale.z *= 1.3f;

    phi_a0 = (thisv->timer < 20) ? (thisv->timer * 12) : 255;

    if (nayrusLoveTimer >= 1180) {
        thisv->primAlpha = 15595 - (nayrusLoveTimer * 13);
        if (nayrusLoveTimer & 1) {
            thisv->primAlpha = thisv->primAlpha >> 1;
        }
    } else if (nayrusLoveTimer >= 1100) {
        thisv->primAlpha = (u8)(nayrusLoveTimer << 7) + 127;
    } else {
        thisv->primAlpha = 255;
    }

    if (thisv->primAlpha > phi_a0) {
        thisv->primAlpha = phi_a0;
    }

    thisvx->world.rot.y += 0x3E8;
    thisvx->shape.rot.y = thisvx->world.rot.y + Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx));
    thisv->timer++;
    gSaveContext.nayrusLoveTimer = nayrusLoveTimer + 1;

    if (nayrusLoveTimer < 1100) {
        func_8002F974(thisvx, NA_SE_PL_MAGIC_SOUL_NORMAL - SFX_FLAG);
    } else {
        func_8002F974(thisvx, NA_SE_PL_MAGIC_SOUL_FLASH - SFX_FLAG);
    }
}
...
void MagicDark_OrbUpdate(Actor* thisvx, GlobalContext* globalCtx) {
    ...
    func_8002F974(&thisv->actor, NA_SE_PL_MAGIC_SOUL_BALL - SFX_FLAG);
    if (thisv->timer < 35) {
        MagicDark_DimLighting(globalCtx, thisv->timer * (1 / 45.0f));
        Math_SmoothStepToF(&thisvx->scale.x, thisv->scale * (1 / 12.000001f), 0.05f, 0.01f, 0.0001f);
        Actor_SetScale(&thisv->actor, thisvx->scale.x);
    } else if (thisv->timer < 55) {
        Actor_SetScale(&thisv->actor, thisvx->scale.x * 0.9f);
        Math_SmoothStepToF(&thisv->orbOffset.y, player->bodyPartsPos[0].y, 0.5f, 3.0f, 1.0f);
        if (thisv->timer > 48) {
            MagicDark_DimLighting(globalCtx, (54 - thisv->timer) * 0.2f);
        }
    } else {
        thisvx->update = MagicDark_DiamondUpdate;
        thisvx->draw = MagicDark_DiamondDraw;
...
void MagicDark_DiamondDraw(Actor* thisvx, GlobalContext* globalCtx) {
...

    {
        ...
        heightDiff = player->bodyPartsPos[0].y - thisv->actor.world.pos.y;
        if (heightDiff < -2.0f) {
            thisv->actor.world.pos.y = player->bodyPartsPos[0].y + 2.0f;
        ...
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 170, 255, 255, (s32)(thisv->primAlpha * 0.6f) & 0xFF);
...
void MagicDark_OrbDraw(Actor* thisvx, GlobalContext* globalCtx) {
    ...
    if (thisv->timer < 32) {
        ...
        if (thisv->timer > 20) {
            pos.y += (thisv->timer - 20) * 1.4f;
        }
        thisv->orbOffset = pos;
    } else if (thisv->timer < 130) {
        pos = thisv->orbOffset;
    } else {
        return;
    }

    pos.x -= (thisv->actor.scale.x * 300.0f * Math_SinS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx))) *
              Math_CosS(Camera_GetCamDirPitch(GET_ACTIVE_CAM(globalCtx))));
    pos.y -= (thisv->actor.scale.x * 300.0f * Math_SinS(Camera_GetCamDirPitch(GET_ACTIVE_CAM(globalCtx))));
    pos.z -= (thisv->actor.scale.x * 300.0f * Math_CosS(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx))) *
              Math_CosS(Camera_GetCamDirPitch(GET_ACTIVE_CAM(globalCtx))));
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_magic_dark.c", 632),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    Matrix_RotateZ(sp6C * (std::numbers::pi_v<float> / 32), MTXMODE_APPLY);
