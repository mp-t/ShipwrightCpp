...
void EnRiverSound_Init(Actor* thisx, GlobalContext* globalCtx) {
    EnRiverSound* thisv = (EnRiverSound*)thisx;

    thisv->playSound = 0;
    thisv->pathIndex = (thisv->actor.params >> 8) & 0xFF;
    thisv->actor.params = thisv->actor.params & 0xFF;

    if (thisv->actor.params >= RS_MAX) { // used for ganon and ganon_boss scenes
        func_800F4870(thisv->actor.params - RS_MAX);
        Actor_Kill(&thisv->actor);
    } else if (thisv->actor.params == RS_UNK_F7) {
        Audio_PlayNatureAmbienceSequence(NATURE_ID_KOKIRI_REGION);
        Actor_Kill(&thisv->actor);
    } else if (thisv->actor.params == RS_SARIAS_SONG) {
...
void EnRiverSound_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->actor.params == RS_SARIAS_SONG) {
        Audio_ClearSariaBgmAtPos(&thisv->actor.projectedPos);
    } else if (thisv->actor.params == RS_UNK_13) {
...
void EnRiverSound_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisx->params == RS_UNK_0) || (thisx->params == RS_UNK_4) || (thisx->params == RS_UNK_5)) {
        path = &globalCtx->setupPathList[thisv->pathIndex];
        pos = &thisx->world.pos;
        ...
        if (EnRiverSound_GetSoundPos(SEGMENTED_TO_VIRTUAL(path->points), path->count, &player->actor.world.pos, pos)) {
            if (BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &thisx->floorPoly, &sp34, thisx, pos) !=
                ...
                thisv->soundPitchIndex = SurfaceType_GetConveyorSpeed(&globalCtx->colCtx, thisx->floorPoly, sp34);
            } else {
                thisv->soundPitchIndex = 0;
            }

            if (thisv->soundPitchIndex == 0) {
                if (thisx->params == RS_UNK_4) {
                    thisv->soundPitchIndex = 0;
                } else if (thisx->params == RS_UNK_0) {
                    thisv->soundPitchIndex = 1;
                } else {
                    thisv->soundPitchIndex = 2;
                }
            } else {
                thisv->soundPitchIndex--;
                thisv->soundPitchIndex = CLAMP_MAX(thisv->soundPitchIndex, 2);
            }
        }
...
void EnRiverSound_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (!(thisv->playSound)) {
        thisv->playSound = true;
    } else if ((thisv->actor.params == RS_UNK_0) || (thisv->actor.params == RS_UNK_4) ||
               (thisv->actor.params == RS_UNK_5)) {
        Audio_PlaySoundRiver(&thisv->actor.projectedPos, soundPitch[thisv->soundPitchIndex]);
    } else if (thisv->actor.params == RS_UNK_11) {
        func_800F4A54(90);
    } else if (thisv->actor.params == RS_SARIAS_SONG) {
        func_800F4E30(&thisv->actor.projectedPos, thisv->actor.xzDistToPlayer);
    } else if (thisv->actor.params == RS_UNK_13) {
        Audio_PlaySariaBgm(&thisv->actor.home.pos, NA_BGM_SARIA_THEME, 1000);
    } else if (thisv->actor.params == RS_UNK_19) {
        Audio_PlaySariaBgm(&thisv->actor.home.pos, NA_BGM_GREAT_FAIRY, 800);
    } else if ((thisv->actor.params == RS_SANDSTORM) || (thisv->actor.params == RS_CHAMBER_OF_SAGES_1) ||
               (thisv->actor.params == RS_CHAMBER_OF_SAGES_2) || (thisv->actor.params == RS_RUMBLING)) {
        func_800788CC(soundEffects[thisv->actor.params]);
    } else {
        Audio_PlayActorSound2(&thisv->actor, soundEffects[thisv->actor.params]);
    }
}
