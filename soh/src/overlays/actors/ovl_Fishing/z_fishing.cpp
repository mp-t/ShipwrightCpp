...
void Fishing_InitPondProps(Fishing* thisv, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < POND_PROP_COUNT; i++) {
        ...
        if (prop->type == FS_PROP_REED) {
            ...
            prop->reedAngle = Rand_ZeroFloat(2 * std::numbers::pi_v<float>);
            if (sLinkAge == 1) {
                prop->scale *= 0.6f;
            }
            ...
            prop->rotY = Rand_ZeroFloat(2 * std::numbers::pi_v<float>);
            if (sLinkAge == 1) {
                if ((i % 4) != 0) {
                    prop->scale *= 0.6f;
                } else {
...
void Fishing_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (thisx->params < 100) {
        ...
        SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gFishingOwnerSkel, &gFishingOwnerAnim, NULL, NULL, 0);
        Animation_MorphToLoop(&thisv->skelAnime, &gFishingOwnerAnim, 0.0f);
        ...
        Fishing_InitPondProps(thisv, globalCtx);
        ...
        if ((thisx->params < 115) || (thisx->params == 200)) {
            SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gFishingFishSkel, &gFishingFishAnim, NULL, NULL, 0);
            Animation_MorphToLoop(&thisv->skelAnime, &gFishingFishAnim, 0.0f);
        } else {
            SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gFishingLoachSkel, &gFishingLoachAnim, NULL, NULL, 0);
            Animation_MorphToLoop(&thisv->skelAnime, &gFishingLoachAnim, 0.0f);
        }
        ...
        if (thisx->params == 200) {
            ...
            thisv->unk_150 = sFishInits[thisx->params - 100].unk_00;
            thisv->unk_1A8 = sFishInits[thisx->params - 100].unk_0C;
            thisv->unk_1AC = sFishInits[thisx->params - 100].unk_08;

            thisv->unk_1AC += Rand_ZeroFloat(4.99999f);

            if ((thisv->unk_1AC >= 65.0f) && (Rand_ZeroOne() < 0.05f)) {
                thisv->unk_1AC += Rand_ZeroFloat(7.99999f);
            }

            if (KREG(6) != 0) {
                thisv->unk_1AC = KREG(6) + 80.0f;
            }
            ...
            if (effect->type == FS_EFF_RIPPLE) {
                ...
                if (effect->pos.y >= rippleY) {
                    ...
                    if (Rand_ZeroOne() < 0.3f) {
                        ...
                        pos.y = rippleY;
                        Fishing_SpawnRipple(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, 20.0f, 60.0f, 150, 90);
                    }
                }
                ...
                if (effect->pos.y <= WATER_SURFACE_Y(globalCtx)) {
                    ...
                    if (Rand_ZeroOne() < 0.5f) {
                        ...
                        Fishing_SpawnRipple(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, 40.0f, 110.0f, 150, 90);
                    }
                }
                ...
                if ((effect->timer % 16) == 0) {
                    Vec3f pos = effect->pos;
                    pos.y = WATER_SURFACE_Y(globalCtx);
                    Fishing_SpawnRipple(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, 30.0f, 300.0f, 150, 90);
                }
...
void Fishing_DrawEffects(FishingEffect* effect, GlobalContext* globalCtx) {
    ...
    if (effect->type == FS_EFF_OWNER_HAT) {
        Matrix_Translate(effect->pos.x, effect->pos.y, effect->pos.z, MTXMODE_NEW);
        Matrix_RotateY((sEffOwnerHatRot.y * std::numbers::pi_v<float>) / 32768, MTXMODE_APPLY);
        Matrix_RotateX((sEffOwnerHatRot.x * std::numbers::pi_v<float>) / 32768, MTXMODE_APPLY);
        Matrix_RotateZ((sEffOwnerHatRot.z * std::numbers::pi_v<float>) / 32768, MTXMODE_APPLY);
...
void Fishing_DrawLureHook(GlobalContext* globalCtx, Vec3f* pos, Vec3f* refPos, u8 hookIndex) {
    ...
    if ((hookIndex == 1) && (D_80B7A68C != 0)) {
        ...
        if (D_80B7A690 != 0) {
            FishingEffect* effect = static_cast<FishingEffect*>(globalCtx->specialEffects);
            MtxF mf;
...
void Fishing_UpdateSinkingLure(GlobalContext* globalCtx) {
    ...
    if (D_80B7A694 == 5) {
        Matrix_RotateY(player->actor.shape.rot.y * (std::numbers::pi_v<float> / 32768), MTXMODE_NEW);
        sp94.x = 5.0f;
        sp94.y = 0.0f;
        sp94.z = 3.0f;
...
void Fishing_DrawLureAndLine(GlobalContext* globalCtx, Vec3f* linePos, Vec3f* lineRot) {
    ...
    if ((D_80B7A694 == 4) || (D_80B7A694 == 5)) {
        ...
        sLureRot.x = sReelLineRot[LINE_SEG_COUNT - 2].x + std::numbers::pi_v<float>;
...
void Fishing_DrawRod(GlobalContext* globalCtx) {
    ...
    if (D_80B7A694 == 5) {
        Matrix_RotateY(0.56f * std::numbers::pi_v<float>, MTXMODE_APPLY);
    } else {
        Matrix_RotateY(0.41f * std::numbers::pi_v<float>, MTXMODE_APPLY);
    }

    Matrix_RotateX(-std::numbers::pi_v<float> / 5.0000003f, MTXMODE_APPLY);
    Matrix_RotateZ((player->unk_858 * 0.5f) + 3.0f * std::numbers::pi_v<float> / 20.0f, MTXMODE_APPLY);
    Matrix_RotateX((D_80B7A6C0 + 20.0f) * 0.01f * std::numbers::pi_v<float>, MTXMODE_APPLY);
...
void Fishing_UpdateLure(Fishing* thisv, GlobalContext* globalCtx) {
    f32 spE4;
    f32 spE0;
    s16 phi_v0;
    s16 spDC;
    ...
    switch (D_80B7A694) {
        ...
        case 1:
            ...
            if (D_80B7E0B2 != 0) {
                ...
                sLureRot.x = 0.0f;
                sLureRot.y = Math_Atan2F(spD0, spD8) + std::numbers::pi_v<float>;
            }
            ...
            if (spC8 > SQ(920.0f)) {
                if ((KREG(56) != 0) || (sLurePos.y > 160.0f) || (sLurePos.x < 80.0f) || (sLurePos.x > 180.0f) ||
                    ...
                    sp80 = thisv->actor.world.pos;
                    thisv->actor.prevPos = thisv->actor.world.pos = sLurePos;
                    Actor_UpdateBgCheckInfo(globalCtx, &thisv->actor, 15.0f, 30.0f, 30.0f, 0x43);
                    thisv->actor.world.pos = sp80;
                ...
                if (sLurePos.y <= spE4) {
                    ...
                    if ((sLurePos.y <= spE4) && (spE4 < spE0) && (spE4 == WATER_SURFACE_Y(globalCtx))) {
                        ...
                        for (i = 0; i < 50; i++) {
                            ...
                            spA8.z += (sp9C.z * 3.0f);
                            Fishing_SpawnDustSplash(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &spA8, &sp9C,
                                                    Rand_ZeroFloat(0.02f) + 0.025f);
                        }

                        spA8 = sLurePos;
                        spA8.y = WATER_SURFACE_Y(globalCtx);
                        Fishing_SpawnRipple(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &spA8, 100.0f, 800.0f, 150, 90);
                    }
                } else {
        ...
        case 3:
            ...
            if (D_80B7E138 < 3.0f) {
                ...
                Math_ApproachF(&sLureRot.x, -std::numbers::pi_v<float> / 6.0f + spD0, 0.3f, D_80B7E110);
            ...
            if (spC8 < SQ(920.0f)) {
                if (sLurePos.y <= (spE4 + 4.0f)) {
                    ...
                    if ((sp70 > 0.2f) && (D_80B7E138 < 4.0f)) {
                        ...
                        spDC = 0x500;
                        D_80B7E134 = sReelLineRot[LINE_SEG_COUNT - 2].y + std::numbers::pi_v<float>;
                        sLureRot.x = 0.0f;
                        D_80B7E10C = 0.5f;
                        if (D_80B7E0B6 == 2) {
                            D_80B7E128.y = 0.2f;
            ...
            Math_ApproachS(&D_80B7E13C, (D_80B7E134 * 32768.0f) / std::numbers::pi_v<float>, 3, spDC);

            sLureRot.y = (D_80B7E13C / 32768.0f) * std::numbers::pi_v<float>;
            ...
            if ((D_80B7E0B6 == 1) && CHECK_BTN_ALL(input->cur.button, BTN_A)) {
                ...
                if (D_80B7E0B6 == 2) {
                    ...
                    thisv->actor.prevPos = thisv->actor.world.pos = sLurePos;
                    Actor_UpdateBgCheckInfo(globalCtx, &thisv->actor, 15.0f, 30.0f, 30.0f, 0x44);
                    thisv->actor.world.pos = sp58;
                    ...
                    if (sLurePos.y < (thisv->actor.floorHeight + 5.0f)) {
                        sReelLinePos[LINE_SEG_COUNT - 1].y = sLurePos.y = thisv->actor.floorHeight + 5.0f;
                        D_80B7E128.y = 0.0f;
                    } else {
                        D_80B7E120 = 1;
                    }
            ...
            if ((sLurePos.y <= (WATER_SURFACE_Y(globalCtx) + 4.0f)) &&
                ...
                if ((D_80B7E0AE & phi_v0) == 0) {
                    spA8 = sLurePos;
                    spA8.y = WATER_SURFACE_Y(globalCtx);
                    Fishing_SpawnRipple(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &spA8, 30.0f, 300.0f, 150, 90);
                }
            }
            break;
...
s32 func_80B70A2C(Fishing* thisv, GlobalContext* globalCtx, u8 ignorePosCheck) {
    s16 i;
    s16 count;
    f32 scale;
    Vec3f pos;
    ...
    if ((thisv->actor.world.pos.y < (WATER_SURFACE_Y(globalCtx) - 10.0f)) && !ignorePosCheck) {
        return false;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        Fishing_SpawnDustSplash(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, &vel,
    ...
    Fishing_SpawnRipple(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, 100.0f, 800.0f, 150, 90);
...
void func_80B70CF0(Fishing* thisv, GlobalContext* globalCtx) {
    s16 count;
    s16 i;
    f32 scale;
    Vec3f pos;
...
void func_80B70ED4(Fishing* thisv, Input* input) {
    Vec3f sp34;
    Vec3f sp28;
    f32 sp24;

    sp34.x = sLurePos.x - thisv->actor.world.pos.x;
    sp34.y = sLurePos.y - thisv->actor.world.pos.y;
    sp34.z = sLurePos.z - thisv->actor.world.pos.z;

    sp24 = SQ(sp34.x) + SQ(sp34.y) + SQ(sp34.z);

    if ((D_80B7A694 == 3) && (thisv->unk_1A2 == 0) && (D_80B7A68C == 0)) {
        Matrix_RotateY((-thisv->actor.shape.rot.y / 32768.0f) * std::numbers::pi_v<float>, MTXMODE_NEW);
        Matrix_MultVec3f(&sp34, &sp28);

        if ((sp28.z > 0.0f) || (thisv->unk_1AC < 40.0f)) {
            if ((thisv->unk_158 == 7) && (sp24 < SQ(200.0f))) {
                thisv->unk_158 = 4;
                thisv->unk_1B4 = sLurePos;
                thisv->unk_1B0 = 28672.0f;
                thisv->unk_188 = 5.0f;
            } else {
                if ((CHECK_BTN_ALL(input->cur.button, BTN_A) || (D_80B7E138 > 1.0f)) && (sp24 < SQ(120.0f))) {
                    ...
                    thisv->unk_15E = 0;
                    thisv->unk_17A[0] = 0;
                    thisv->unk_17A[2] = (s16)Rand_ZeroFloat(100.0f) + 100;
                    thisv->unk_1A8 = sFishInits[thisv->actor.params - 100].unk_0C;
                    thisv->unk_1B0 = 0.0f;
                }

                if ((thisv->unk_17A[1] == 0) && (sp24 < SQ(70.0f))) {
                    thisv->unk_158 = 2;
                    thisv->unk_15E = 0;
                    thisv->unk_17A[0] = 0;
                    thisv->unk_17A[2] = (s16)Rand_ZeroFloat(100.0f) + 100;
    ...
    } else if ((D_80B7A694 == 4) && (D_80B7E124 != 0) && (sp24 < SQ(100.0f)) && (thisv->unk_158 >= 10)) {
        thisv->unk_15A = 0;
        thisv->unk_158 = 1;
        thisv->unk_1A4 = 1000;
        thisv->unk_1A2 = 100;
    ...
    if ((D_80B7E0B6 != 2) && (D_80B7E114 != 0) && (thisv->unk_1AC > 60.0f) && (sp24 < SQ(30.0f)) &&
        (thisv->unk_158 >= 10)) {
        thisv->unk_15A = 0;
        thisv->unk_158 = 1;
        thisv->unk_1A4 = 1000;
...
void func_80B71278(Fishing* thisv, u8 arg1) {
    s16 sfxId;
    u8 temp;
    ...
    Audio_PlayActorSound2(&thisv->actor, sfxId);
}

void Fishing_HandleAquariumDialog(Fishing* thisv, GlobalContext* globalCtx) {
    if (sLinkAge == 1) {
        if ((HIGH_SCORE(HS_FISHING) & 0x7F) != 0) {
            if (HIGH_SCORE(HS_FISHING) & 0x80) {
                thisv->actor.textId = 0x40B1;
            } else {
                thisv->actor.textId = 0x4089;
            }
        } else {
            thisv->actor.textId = 0x40AE;
        }
    } else {
        if ((HIGH_SCORE(HS_FISHING) & 0x7F000000) != 0) {
...
void Fishing_UpdateFish(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    thisv->actor.uncullZoneForward = 700.0f;
    thisv->actor.uncullZoneScale = 50.0f;
    ...
    if ((D_80B7E0B0 != 0) || (sCameraId != 0) || ((player->actor.world.pos.z > 1150.0f) && (thisv->unk_158 != 100))) {
        thisv->actor.flags &= ~ACTOR_FLAG_0;
    } else {
        thisv->actor.flags |= ACTOR_FLAG_0;
        if (D_80B7A694 != 0) {
            if (D_80B7E0B2 == 0) {
                thisv->actor.focus.pos = sLurePos;
    ...
    Math_ApproachF(&thisv->unk_198, thisv->unk_190, 1.0f, 0.2f);

    if (thisv->unk_158 == 6) {
        Math_ApproachF(&thisv->unk_19C, thisv->unk_194, 0.2f, 200.0f);
    } else {
        phi_f0 = 1.0f;
        phi_f2 = 1.0f;
        if (thisv->actor.world.pos.y > WATER_SURFACE_Y(globalCtx)) {
            phi_f0 = (KREG(64) * 0.1f) + 1.5f;
            phi_f2 = 3.0f;
        }
        Math_ApproachF(&thisv->unk_19C, thisv->unk_194 * phi_f0, 1.0f, 500.0f * phi_f2);
    }

    Math_ApproachS(&thisv->unk_170, 0, 5, 0x1F4);

    if (thisv->unk_150 == 0) {
        Actor_SetScale(&thisv->actor, thisv->unk_1AC * 15.0f * 0.00001f);

        thisv->unk_18C += thisv->unk_198;

        temp = cosf(thisv->unk_18C);
        thisv->unk_16C = thisv->unk_16E + (s16)(temp * thisv->unk_19C);

        temp = cosf(thisv->unk_18C + -1.2f);
        thisv->unk_176 = thisv->unk_16E + (s16)(temp * thisv->unk_19C * 1.6f);
    } else {
        Actor_SetScale(&thisv->actor, thisv->unk_1AC * 65.0f * 0.000001f);

        thisv->actor.scale.x = thisv->actor.scale.z * 1.1f;
        thisv->actor.scale.y = thisv->actor.scale.z * 1.1f;

        thisv->unk_18C += thisv->unk_198 * 0.8f;

        for (i = 0; i < 3; i++) {
            temp = cosf(thisv->unk_18C + (i * 2.1f));
            thisv->unk_1CC[i] = thisv->unk_16E + (s16)(temp * thisv->unk_19C * 2.0f);
        }

        temp = cosf(thisv->unk_18C + 0.4f);
        thisv->unk_16C = (thisv->unk_19C * temp * 2.0f) * 0.6f;
    }

    sp130 = thisv->unk_1B4.x - thisv->actor.world.pos.x;
    sp12C = thisv->unk_1B4.y - thisv->actor.world.pos.y;
    sp128 = thisv->unk_1B4.z - thisv->actor.world.pos.z;
    ...
    if ((thisv->unk_1A0 != 0) && (thisv->unk_158 != 2) && (thisv->unk_158 != 3) && (thisv->unk_158 != 4)) {
        if ((thisv->unk_15C & 0x40) != 0) {
            spFC += 0x4000;
        } else {
            spFC -= 0x4000;
        }
        if (((thisv->unk_15C + 0x20) & 0x40) != 0) {
            spFE += 0x2000;
        } else {
            spFE -= 0x2000;
        }
    ...
    switch (thisv->unk_158) {
        case 100:
            Fishing_HandleAquariumDialog(thisv, globalCtx);

            thisv->actor.uncullZoneForward = 500.0f;
            thisv->actor.uncullZoneScale = 300.0f;

            Lights_PointNoGlowSetInfo(&thisv->lightInfo, (s16)thisv->actor.world.pos.x,
                                      (s16)thisv->actor.world.pos.y + 20.0f, (s16)thisv->actor.world.pos.z - 50.0f, 255,
                                      255, 255, 255);

            thisv->unk_1AC = D_80B7E078;
            sp100.y = (f32)Math_SinS(globalCtx->gameplayFrames * 300) * 1;
            sp100.z = (f32)Math_SinS(globalCtx->gameplayFrames * 230) * 2;
            thisv->actor.world.pos.x = 130.0f;
            thisv->actor.world.pos.y = 55.0f + sp100.y;
            thisv->actor.world.pos.z = 1300.0f + sp100.z;
            thisv->actor.shape.rot.y = -0x8000;

            if ((thisv->actor.projectedPos.z < 200.0f) && (thisv->actor.projectedPos.z > 0.0f)) {
                spC4.x = Rand_CenteredFloat(5.0f) + 130.0f;
                spC4.y = 40.0f;
                spC4.z = Rand_CenteredFloat(5.0f) + 1280.0f;
                Fishing_SpawnBubble(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &spC4, Rand_ZeroFloat(0.02f) + 0.03f, 1);
            }

            Math_ApproachS(&thisv->unk_172, (Math_SinS(thisv->unk_15C * 0x800) * 2500.0f) + 2500.0f, 2, 0x7D0);
            Math_ApproachS(&thisv->unk_174, Math_SinS(thisv->unk_15C * 0xA00) * 1500.0f, 2, 0x7D0);

            thisv->unk_190 = 0.3f;
            thisv->unk_194 = 1000.0f / 3.0f;
            return;

        case 10:
            thisv->unk_1B4 = thisv->actor.home.pos;

            Math_ApproachF(&thisv->actor.speedXZ, 2.0f, 1.0f, 0.5f);
            Math_ApproachF(&thisv->unk_1B0, 4096.0f, 1.0f, 256.0f);

            if (sp124 < 40.0f) {
                thisv->unk_158 = 11;
                thisv->unk_190 = 0.4f;
                thisv->unk_194 = 500.0f;
            }

            func_80B70ED4(thisv, input);

            if (thisv->actor.xzDistToPlayer < (250.0f * sp118)) {
                thisv->unk_15A = thisv->unk_158 = 0;
                thisv->unk_1A4 = 1000;
                thisv->unk_1A2 = 200;
                thisv->unk_17A[1] = 50;
            }
        ...
        case 11:
            ...
            Math_ApproachF(&thisv->actor.speedXZ, 0.0f, 1.0f, 0.05f);
            Math_ApproachF(&thisv->unk_1B0, 0.0f, 1.0f, 256.0f);

            if (sp124 >= 40.0f) {
                thisv->unk_158 = 10;
                thisv->unk_190 = 1.0f;
                thisv->unk_194 = 2000.0f;
            }
            func_80B70ED4(thisv, input);
            ...
            if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_NONE) {
                if ((gSaveContext.dayTime >= 0xC000) && (gSaveContext.dayTime <= 0xC01B)) {
                    thisv->unk_158 = 7;
                    thisv->unk_17A[3] = (s16)Rand_ZeroFloat(150.0f) + 200;
                }
            ...
            if (KREG(15) != 0) {
                KREG(15) = 0;
                thisv->unk_158 = 7;
                thisv->unk_17A[3] = (s16)Rand_ZeroFloat(150.0f) + 2000;
            }
            break;
        ...
        case 0:
            ...
            if (thisv->unk_17A[0] == 0) {
                if (thisv->unk_1A4 == 0) {
                    thisv->unk_158 = thisv->unk_15A = 10;
                } else {
                    thisv->unk_158 = 1;
                    thisv->unk_17A[0] = (s16)Rand_ZeroFloat(30.0f) + 10;
                    thisv->unk_1B4.x = Rand_CenteredFloat(300.0f);
                    thisv->unk_1B4.y = (WATER_SURFACE_Y(globalCtx) - 50.0f) - Rand_ZeroFloat(50.0f);
                    thisv->unk_1B4.z = Rand_CenteredFloat(300.0f);
                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 2000.0f;
                }
            }
        ...
        case 1:
            if (thisv->unk_150 == 1) {
                ...
                thisv->unk_1A4 = 20000;
                thisv->unk_1A2 = 20000;
                thisv->unk_1B4.x = 0.0f;
                thisv->unk_1B4.y = -140.0f;
                thisv->unk_1B4.z = 0.0f;
            } else {
                Math_ApproachF(&thisv->unk_1B0, 4096.0f, 1.0f, 256.0f);

                if ((thisv->actor.xzDistToPlayer < (250.0f * sp118)) || (thisv->unk_17A[1] != 0)) {
                    Math_ApproachF(&thisv->unk_1B0, 8192.0f, 1.0f, 768.0f);
                    Math_ApproachF(&thisv->actor.speedXZ, 4.2f, 1.0f, 0.75);
                    thisv->unk_190 = 1.2f;
                    thisv->unk_194 = 4000.0f;
                    thisv->unk_17A[0] = 20;
                } else {
                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 2000.0f;
                    Math_ApproachF(&thisv->actor.speedXZ, 1.5f, 1.0f, 0.1f);
                }

                if ((thisv->unk_17A[0] == 0) || (sp124 < 50.0f)) {
                    thisv->unk_158 = 0;
                    thisv->unk_17A[0] = (s16)Rand_ZeroFloat(30.0f) + 3;
                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 500.0f;
                }
        ...
        case -1:
            Math_ApproachS(&thisv->unk_166, 0, 0x14, 0x20);

            if ((thisv->actor.xzDistToPlayer < (250.0f * sp118)) || (thisv->unk_17A[1] != 0)) {
                Math_ApproachF(&thisv->actor.speedXZ, 3.0f, 1.0f, 0.75);
                thisv->unk_190 = 1.0f;
                thisv->unk_17A[0] = 20;
                thisv->unk_194 = 4000.0f;
                Math_ApproachF(&thisv->unk_1B0, 4096.0f, 1.0f, 256.0f);
                ...
                if ((globalCtx->gameplayFrames % 32) == 0) {
                    thisv->unk_1B4.x = Rand_CenteredFloat(600.0f);
                    thisv->unk_1B4.z = Rand_CenteredFloat(600.0f);
                    thisv->unk_1B4.y = -120.0f;
                }
            } else if (sp124 > 50.0f) {
                thisv->unk_190 = 0.8f;
                thisv->unk_194 = 1500.0f;
                Math_ApproachF(&thisv->actor.speedXZ, 1.0f, 1.0f, 0.1f);
                Math_ApproachF(&thisv->unk_1B0, 2048.0f, 1.0f, 128.0f);
            } else {
                thisv->unk_190 = 0.4f;
                thisv->unk_194 = 500.0f;
                Math_ApproachZeroF(&thisv->actor.speedXZ, 1.0f, 0.02f);
                Math_ApproachF(&thisv->unk_1B0, 0.0f, 1.0f, 256.0f);
            }
            ...
            if (thisv->unk_1A4 == 0) {
                thisv->unk_158 = 10;
                thisv->unk_15A = 10;
            } else if ((KREG(2) != 0) || (((thisv->unk_1A4 & 0x7FF) == 0) && (thisv->unk_1A4 < 15000))) {
                KREG(2) = 0;
                thisv->unk_158 = -2;
                thisv->actor.world.rot.x = thisv->actor.shape.rot.x = 0;
                thisv->unk_1B4.y = WATER_SURFACE_Y(globalCtx) + 10.0f;
                thisv->unk_1B4.x = Rand_ZeroFloat(50.0f);
                thisv->unk_1B4.z = Rand_ZeroFloat(50.0f);
            }
        ...
        case -2:
            if ((thisv->actor.xzDistToPlayer < (250.0f * sp118)) || (thisv->unk_17A[1] != 0)) {
                ...
                Math_ApproachS(&thisv->unk_166, -0x1000, 0x14, 0x100);

                if (thisv->actor.world.pos.y < (WATER_SURFACE_Y(globalCtx) - 20.0f)) {
                    Math_ApproachF(&thisv->actor.speedXZ, 0.5f, 1.0f, 0.1f);
                } else {
                    Math_ApproachZeroF(&thisv->actor.speedXZ, 1.0f, 0.01f);

                    if ((thisv->actor.speedXZ == 0.0f) ||
                        (thisv->actor.world.pos.y > (WATER_SURFACE_Y(globalCtx) - 5.0f))) {
                        thisv->unk_1B4.x = Rand_ZeroFloat(300.0f);
                        thisv->unk_1B4.z = Rand_ZeroFloat(300.0f);
                        thisv->unk_1B4.y = thisv->actor.floorHeight + 10.0f;
                        thisv->unk_158 = -25;
                        thisv->unk_1B0 = 0.0f;

                        spB8 = thisv->fishMouthPos;
                        spB8.y = WATER_SURFACE_Y(globalCtx);
                        Fishing_SpawnRipple(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &spB8, 10.0f, 300.0f,
                                            150, 90);
                        Fishing_SpawnRipple(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &spB8, 30.0f, 400.0f,
                                            150, 90);

                        Audio_PlayActorSound2(&thisv->actor, NA_SE_PL_CATCH_BOOMERANG);
                        break;
                    }
                }
        ...
        case -25:
            if ((thisv->actor.xzDistToPlayer < (250.0f * sp118)) || (thisv->unk_17A[1] != 0)) {
                ...
                Math_ApproachS(&thisv->unk_166, 0x1000, 0x14, 0x6A);

                if (sp124 > 40.0f) {
                    thisv->unk_190 = 0.7f;
                    thisv->unk_194 = 1200.0f;
                    Math_ApproachF(&thisv->actor.speedXZ, 0.5f, 1.0f, 0.01f);
                    Math_ApproachF(&thisv->unk_1B0, 2048.0f, 1.0f, 128.0f);
                } else {
                    thisv->unk_158 = -1;
                }
        ...
        case 2:
            if (((thisv->actor.params + D_80B7E118) & 1) != 0) {
                sp10C.x = 10.0f;
            } else {
                sp10C.x = -10.0f;
            }
            ...
            thisv->unk_1B4.x = sLurePos.x + sp100.x;
            thisv->unk_1B4.z = sLurePos.z + sp100.z;

            if (D_80B7E0B6 == 2) {
                thisv->unk_1B4.y = sLurePos.y;
            } else if (thisv->unk_150 == 0) {
                thisv->unk_1B4.y = sLurePos.y - 15.0f;
            } else {
                thisv->unk_1B4.y = sLurePos.y - 5.0f;
            }

            if (thisv->unk_1B4.y <= thisv->actor.floorHeight) {
                thisv->unk_1B4.y = thisv->actor.floorHeight + 3.0f;
            }

            if ((D_80B7E0B6 != 2) && (thisv->unk_1B4.y < thisv->actor.world.pos.y)) {
                Math_ApproachF(&thisv->actor.world.pos.y, thisv->unk_1B4.y, 0.1f,
                               (thisv->actor.world.pos.y - thisv->unk_1B4.y) * 0.1f);
            }

            Math_ApproachF(&thisv->unk_1B0, 8192.0f, 1.0f, (KREG(16) * 128) + 384.0f);
            if (CHECK_BTN_ALL(input->press.button, BTN_A)) {
                thisv->unk_1A8 += 0.005f;
            }
            ...
            if (CHECK_BTN_ALL(input->press.button, BTN_B)) {
                thisv->unk_1A8 += 0.008f;
            }

            if (sp124 < ((thisv->unk_1AC * 0.5f) + 20.0f)) {
                if (thisv->unk_15E == 0) {
                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 500.0f;
                    thisv->unk_17A[0] = (s16)Rand_ZeroFloat(10.0f) + 2;
                }
                Math_ApproachF(&thisv->actor.speedXZ, -0.2f, 1.0f, 0.1f);
                thisv->unk_15E = 1;
            } else {
                if (thisv->unk_15E != 0) {
                    thisv->unk_190 = 1.0f;
                    thisv->unk_1B0 = 0.0f;
                    thisv->unk_194 = 3000.0f;
                }
                Math_ApproachF(&thisv->actor.speedXZ, 3.0f, 1.0f, 0.15f);
                thisv->unk_15E = 0;
            }

            if (thisv->unk_1AC >= 60.0f) {
                multiplier = 0.3f;
            } else if (thisv->unk_1AC >= 45.0f) {
                multiplier = 0.6f;
            } else {
                multiplier = 1.0f;
            }
            ...
            if (((thisv->unk_17A[0] == 1) || (Rand_ZeroOne() < sp11C)) &&
                ((Rand_ZeroOne() < (thisv->unk_1A8 * multiplier)) || ((thisv->unk_150 + 1) == KREG(69)))) {
                if (thisv->unk_150 == 0) {
                    thisv->unk_158 = 3;
                    thisv->unk_190 = 1.2f;
                    thisv->unk_194 = 5000.0f;
                    thisv->unk_17A[0] = Rand_ZeroFloat(10.0f);
                } else {
                    thisv->unk_158 = -3;
                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 3000.0f;
                ...
                if (D_80B7E0B6 == 2) {
                    thisv->unk_188 = Rand_ZeroFloat(1.5f) + 3.0f;
                } else {
                    thisv->unk_188 = Rand_ZeroFloat(1.5f) + 4.5f;
                }
            }

            if ((D_80B7A694 != 3) || (thisv->unk_17A[2] == 0) ||
                (sqrtf(SQ(thisv->actor.world.pos.x) + SQ(thisv->actor.world.pos.z)) > 800.0f)) {
                thisv->unk_158 = thisv->unk_15A;
                thisv->unk_17A[1] = (s16)Rand_ZeroFloat(30.0f) + 50;
                thisv->unk_17A[0] = (s16)Rand_ZeroFloat(10.0f) + 5;
                thisv->unk_190 = 1.0f;
                thisv->unk_1B0 = 0.0f;
                thisv->unk_194 = 2000.0f;
            }

            if (thisv->actor.xzDistToPlayer < (100.0f * sp118)) {
                thisv->unk_15A = thisv->unk_158 = 0;
                thisv->unk_1A4 = 1000;
                thisv->unk_1A2 = 200;
                thisv->unk_17A[1] = 50;
        ...
        case 3:
            ...
            thisv->unk_1B4.x = sLurePos.x + sp100.x;
            thisv->unk_1B4.z = sLurePos.z + sp100.z;
            thisv->unk_1B4.y = sLurePos.y - 10.0f;
            thisv->unk_1B0 = 4096.0f;
            Math_ApproachF(&thisv->actor.speedXZ, thisv->unk_188 * 0.8f, 1.0f, 1.0f);
            ...
            if ((D_80B7A694 != 3) || (sLurePos.y > (WATER_SURFACE_Y(globalCtx) + 5.0f)) ||
                ...
                thisv->unk_190 = 1.0f;
                thisv->unk_194 = 2000.0f;
            } else if ((thisv->unk_17A[0] == 0) || (sp124 < 30.0f)) {
                thisv->unk_158 = 4;
                thisv->unk_1B4 = sLurePos;
                thisv->unk_1B0 = 16384.0f;
                thisv->unk_190 = 1.2f;
                thisv->unk_194 = 5000.0f;
                thisv->unk_17A[0] = 20;
            }
        ...
        case 4:
            Math_ApproachF(&thisv->unk_1B0, 16384.0f, 1.0f, 4096.0f);
            Math_ApproachS(&thisv->unk_170, 0x4E20, 4, 0x1388);

            thisv->unk_151 = 50;
            sp134 = 2;
            thisv->unk_1B4 = sLurePos;
            Math_ApproachF(&thisv->actor.speedXZ, thisv->unk_188, 1.0f, 1.0f);

            if ((D_80B7A694 != 3) || (thisv->unk_17A[0] == 0) || (sLurePos.y > (WATER_SURFACE_Y(globalCtx) + 5.0f)) ||
                (sqrtf(SQ(sLurePos.x) + SQ(sLurePos.z)) > 800.0f)) {
                ...
                if (func_80B70A2C(thisv, globalCtx, false)) {
                    func_80B71278(thisv, 0);
                }
        ...
        case -3:
            thisv->unk_151 = 50;
            thisv->unk_1B4 = sLurePos;
            Math_ApproachF(&thisv->actor.speedXZ, 2.0f, 1.0f, 1.0f);
            ...
            if ((D_80B7A694 != 3) || (thisv->unk_17A[0] == 0) || (sLurePos.y > (WATER_SURFACE_Y(globalCtx) + 5.0f)) ||
                ...
                if (sLurePos.y > (WATER_SURFACE_Y(globalCtx) - 10.0f)) {
                    Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_JUMP_OUT_WATER);
                    func_80078884(NA_SE_PL_CATCH_BOOMERANG);
                }

                func_80B70A2C(thisv, globalCtx, false);
                thisv->unk_158 = 5;
                thisv->unk_190 = 1.2f;
                thisv->unk_194 = 5000.0f;
                thisv->unk_17A[1] = 150;
        ...
        case 5:
            ...
            Math_ApproachS(&thisv->unk_170, 0x2AF8, 4, 0xBB8);
            sFishingHookedFish = thisv;
            Math_ApproachS(&player->actor.shape.rot.y, thisv->actor.yawTowardsPlayer + 0x8000, 5, 0x500);
            ...
            if ((D_80B7E122 != 0) && (D_80B7E124 == 0)) {
                if (((input->rel.stick_y < -50) && (D_80B7A6C8 > -40)) || CHECK_BTN_ALL(input->press.button, BTN_A)) {
                    if (input->rel.stick_y < -50) {
                        temp_f0 = 40.0f - ((thisv->unk_1AC - 30.0f) * 1.333333f);
                        if (temp_f0 > 0.0f) {
                            thisv->unk_152 = temp_f0;
                            thisv->unk_154 = thisv->actor.yawTowardsPlayer - thisv->actor.shape.rot.y;
                            thisv->unk_156 = 1;
                        }
                    }

                    thisv->unk_198 = 1.7f;
                    thisv->unk_19C = 7000.0f;
                    D_80B7E124 = 1;
                    Audio_QueueSeqCmd(SEQ_PLAYER_BGM_MAIN << 24 | NA_BGM_ENEMY | 0x800);
                    D_80B7E0A6 = 0;

                    if (thisv->unk_150 == 1) {
                        spA4 = (thisv->unk_1AC * 3.0f) + 120.0f;
                    } else {
                        spA4 = (2.0f * thisv->unk_1AC) + 120.0f;
                    }
                    if (spA4 > 255.0f) {
                        spA4 = 255.0f;
                    }
            ...
            if (thisv->actor.world.pos.y < WATER_SURFACE_Y(globalCtx)) {
                if (thisv->unk_17A[1] > 30) {
                    phi_v0_2 = 7;
                } else {
                    phi_v0_2 = 0xF;
                }

                if (((thisv->unk_15C & phi_v0_2) == 0) && (Rand_ZeroOne() < 0.75f) && (D_80B7E0A4 == 0)) {
                    if (thisv->unk_1AC >= 70.0f) {
                        spA4 = 255.0f;
                    } else if (thisv->unk_1AC >= 60.0f) {
                        spA4 = 230.0f;
                ...
                if (thisv->unk_17A[1] > 30) {
                    if (thisv->unk_17A[0] == 0) {
                        ...
                        for (spA2 = 0; spA2 < 100; spA2++) {
                            Matrix_RotateY(Rand_CenteredFloat(3.0f * std::numbers::pi_v<float> / 4.0f) +
                                               (((thisv->actor.yawTowardsPlayer + 0x8000) / 32768.0f) * std::numbers::pi_v<float>),
                                           MTXMODE_NEW);
                            Matrix_MultVec3f(&sp10C, &sp100);

                            thisv->unk_1B4.x = thisv->actor.world.pos.x + sp100.x;
                            thisv->unk_1B4.z = thisv->actor.world.pos.z + sp100.z;

                            if ((SQ(thisv->unk_1B4.x) + SQ(thisv->unk_1B4.z)) < SQ(750.0f)) {
                                break;
                            }
                        }

                        if ((Rand_ZeroOne() < 0.1f) && (thisv->unk_17A[3] == 0)) {
                            if (thisv->unk_1AC >= 60.0f) {
                                phi_a1 = 255;
                            } else if (thisv->unk_1AC >= 50.0f) {
                                phi_a1 = 200;
                            ...
                            thisv->unk_17A[2] = 20;
                            thisv->unk_17A[3] = 100;
                            thisv->unk_1B4.y = 300.0f;
                            D_80B7E0A4 = 0x28;
                            D_80B7E116 = (s16)Rand_ZeroFloat(30.0f) + 20;
                        } else {
                            thisv->unk_17A[0] = (s16)Rand_ZeroFloat(10.0f) + 3;
                            thisv->unk_17A[2] = 0;
                            thisv->unk_1B4.y = -70.0f - Rand_ZeroFloat(150.0f);
                        }
                    }

                    if (thisv->unk_17A[2] != 0) {
                        D_80B7E11C = 0.0f;
                        thisv->unk_190 = 1.6f;
                        thisv->unk_194 = 6000.0f;
                        Math_ApproachF(&thisv->actor.speedXZ, 7.5f, 1.0f, 1.0f);
                        Math_ApproachS(&thisv->unk_170, 0x4E20, 2, 0xFA0);
                    } else {
                        if ((D_80B7E124 == 0) && (D_80B7E0B6 == 2)) {
                            thisv->unk_190 = 1.0f;
                            thisv->unk_194 = 2000.0f;
                            Math_ApproachF(&thisv->actor.speedXZ, 3.0f, 1.0f, 0.2f);
                        } else {
                            thisv->unk_190 = 1.4f;
                            thisv->unk_194 = 5000.0f;
                            Math_ApproachF(&thisv->actor.speedXZ, 5.0f, 1.0f, 0.5f);
                        }

                        if (thisv->unk_150 == 0) {
                            D_80B7E11C = 1.0f - (thisv->unk_1AC * 0.00899f);
                        } else {
                            D_80B7E11C = 1.0f - (thisv->unk_1AC * 0.00899f * 1.4f);
                        }
                    }
                } else {
                    if (((thisv->unk_17A[1] & 0xF) == 0) && CHECK_BTN_ALL(input->cur.button, BTN_A) &&
                        (!(thisv->unk_1AC >= 60.0f) || (D_80B7E080 >= 2000))) {
                        thisv->unk_152 = (s16)Rand_ZeroFloat(30.0f) + 15;
                        thisv->unk_154 = thisv->actor.yawTowardsPlayer - thisv->actor.shape.rot.y;
                    }

                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 4500.0f;

                    if (thisv->unk_150 == 0) {
                        D_80B7E11C = 1.3f - (thisv->unk_1AC * 0.00899f);
                    } else {
                        D_80B7E11C = 1.3f - (thisv->unk_1AC * 0.00899f * 1.4f);
                    }

                    Math_ApproachF(&thisv->actor.speedXZ, 2.0f, 1.0f, 0.5f);

                    if (thisv->unk_17A[1] == 0) {
                        thisv->unk_152 = 0;

                        if (D_80B7E080 < 2000) {
                            thisv->unk_17A[1] = (s16)Rand_ZeroFloat(50.0f) + 50;
                        } else if (D_80B7E080 < 3000) {
                            thisv->unk_17A[1] = (s16)Rand_ZeroFloat(20.0f) + 30;
                        } else {
                            thisv->unk_17A[1] = (s16)Rand_ZeroFloat(10.0f) + 25;
                        }
                    }
                }
            }
            ...
            sReelLinePos[LINE_SEG_COUNT - 1] = thisv->fishMouthPos;
            ...
            if ((SQ(sp10C.x) + SQ(sp10C.y) + SQ(sp10C.z)) > SQ(20.0f)) {
                Math_ApproachF(&thisv->actor.world.pos.x, sReelLinePos[LINE_SEG_COUNT - 2].x, 0.2f,
                               2.0f * (thisv->actor.speedXZ * 1.5f));
                Math_ApproachF(&thisv->actor.world.pos.y, sReelLinePos[LINE_SEG_COUNT - 2].y, 0.2f,
                               2.0f * (thisv->actor.speedXZ * 1.5f) * 5.0f * 0.1f);
                Math_ApproachF(&thisv->actor.world.pos.z, sReelLinePos[LINE_SEG_COUNT - 2].z, 0.2f,
                               2.0f * (thisv->actor.speedXZ * 1.5f));
            }
            ...
            if ((D_80B7A694 < 3) || ((D_80B7E074 != 0) && (D_80B7E080 > 50)) || (D_80B7E080 >= 6000) ||
                ...
                D_80B7E11C = 0.5f;
                thisv->unk_152 = 0;
            } else if (thisv->actor.xzDistToPlayer < (KREG(59) + 50.0f)) {
                thisv->unk_158 = 6;
                thisv->unk_17A[0] = 100;
                player->unk_860 = 3;
                func_800A9F6C(0.0f, 1, 3, 1);
                ...
                thisv->unk_190 = 1.0f;
                thisv->unk_194 = 500.0f;
                thisv->unk_19C = 5000.0f;

                if (thisv->actor.world.pos.y <= WATER_SURFACE_Y(globalCtx)) {
                    func_80B71278(thisv, 1);
                    func_80B70A2C(thisv, globalCtx, true);
                }
                goto case_6;
            }
            break;
        ...
        case_6:
        case 6:
            Math_ApproachS(&thisv->unk_170, 0x2AF8, 2, 0xFA0);
            ...
            thisv->unk_160 = -0x4000;
            thisv->actor.shape.rot.y = player->actor.shape.rot.y + 0x5000;
            thisv->actor.shape.rot.x = thisv->actor.shape.rot.z = thisv->unk_162 = thisv->unk_164 = thisv->unk_16E = 0;

            sp10C.x = 4.0f;
            sp10C.y = -10.0f;
            sp10C.z = 5.0f;
            Matrix_MultVec3f(&sp10C, &sp100);
            Math_ApproachF(&thisv->actor.world.pos.x, player->bodyPartsPos[15].x + sp100.x, 1.0f, 6.0f);
            Math_ApproachF(&thisv->actor.world.pos.y, player->bodyPartsPos[15].y + sp100.y, 1.0f, 6.0f);
            Math_ApproachF(&thisv->actor.world.pos.z, player->bodyPartsPos[15].z + sp100.z, 1.0f, 6.0f);

            D_80B7E144 = 188.0f;

            if (thisv->unk_17A[0] <= 50) {
                switch (thisv->unk_1D5) {
                    case 0:
                        if ((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CHOICE) ||
                            ...
                            if (Message_ShouldAdvance(globalCtx)) {
                                ...
                                if (globalCtx->msgCtx.choiceIndex == 0) {
                                    if (D_80B7A670 == 0.0f) {
                                        D_80B7A670 = thisv->unk_1AC;
                                        D_80B7E07C = thisv->unk_150;
                                        D_80B7E07E = D_80B7E0B6;
                                        Actor_Kill(&thisv->actor);
                                    } else if ((thisv->unk_150 == 0) && (D_80B7E07C == 0) &&
                                               ((s16)thisv->unk_1AC < (s16)D_80B7A670)) {
                                        thisv->unk_1D5 = 1;
                                        thisv->unk_17A[0] = 0x3C;
            ...
            if (D_80B7A694 == 0) {
                if (thisv->actor.update != NULL) {
                    ...
                    if (thisv->unk_150 == 0) {
                        SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gFishingFishSkel, &gFishingFishAnim, 0, 0, 0);
                        Animation_MorphToLoop(&thisv->skelAnime, &gFishingFishAnim, 0.0f);
                    } else {
                        SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gFishingLoachSkel, &gFishingLoachAnim, 0, 0,
                                           0);
                        Animation_MorphToLoop(&thisv->skelAnime, &gFishingLoachAnim, 0.0f);
                    }
                }
        ...
        case 7:
            thisv->unk_151 = 50;
            sp134 = 5;
            thisv->unk_1B0 = 12288.0f;

            if (thisv->actor.params < 104) {
                thisv->unk_1B4 = sGroupFishes[thisv->actor.params - 100].pos;
                D_80B7A898 = 1;
            } else if (thisv->actor.params < 108) {
                thisv->unk_1B4 = sGroupFishes[thisv->actor.params - 100 + 16].pos;
                D_80B7A898 = 2;
            } else {
                thisv->unk_1B4 = sGroupFishes[thisv->actor.params - 100 + 32].pos;
                D_80B7A898 = 3;
            }
            ...
            if (sp124 < 20.0f) {
                ...
                if ((thisv->unk_17A[2] == 0) && func_80B70A2C(thisv, globalCtx, false)) {
                    func_80B71278(thisv, Rand_ZeroFloat(1.99f));
                    thisv->unk_17A[2] = (s16)Rand_ZeroFloat(20.0f) + 20;
                }
            }

            if (thisv->unk_17A[3] == 0) {
                thisv->unk_158 = 10;
                thisv->unk_15A = 10;
            } else {
                func_80B70ED4(thisv, input);
    ...
    Math_ApproachS(&thisv->unk_172, (Math_SinS(thisv->unk_15C * 0x1000) * 5000.0f) + 5000.0f, 2, 0x7D0);
    ...
    if (thisv->unk_158 != 6) {
        if (thisv->actor.world.pos.y > WATER_SURFACE_Y(globalCtx)) {
            ...
            Math_ApproachS(&thisv->unk_16E, 0, 5, 0x7D0);
            ...
            Math_ApproachZeroF(&thisv->unk_184, 1.0f, 2.0f);
            if ((thisv->unk_158 != -1) && (thisv->unk_158 != -2) && (thisv->unk_158 != -25)) {
                thisv->unk_166 = 0;
            }

            thisv->unk_168 = thisv->unk_16A = 0;
            spF4 = spF0 = spFA = 4;
            spF2 = spEE = 0x2000;

            spF6 = Fishing_SmoothStepToS(&thisv->actor.world.rot.y, spFC, sp134, thisv->unk_1B0) * 3.0f;
            Math_ApproachS(&thisv->actor.world.rot.x, spFE, sp134, thisv->unk_1B0 * 0.5f);
            ...
            if (thisv->actor.speedXZ >= 3.2f) {
                Math_ApproachS(&thisv->unk_16E, spF6, 2, 0x4E20);
            } else {
                Math_ApproachS(&thisv->unk_16E, spF6, 3, 0xBB8);
            }
        ...
        thisv->actor.world.pos.y += (thisv->unk_184 * 1.5f);

        if (1) {}

        if (thisv->unk_152 != 0) {
            thisv->unk_168 = thisv->unk_154;
            thisv->unk_152--;
            if (thisv->unk_156 != 0) {
                spF0 = 5;
                spEE = 0x4000;
            ...
            thisv->unk_166 = -0x500 - thisv->actor.shape.rot.x;
            spF4 = 5;
            spF2 = 0x4000;
        } else {
            thisv->unk_156 = 0;
        ...
        Math_ApproachS(&thisv->unk_160, thisv->unk_166, spF4, spF2);
        Math_ApproachS(&thisv->unk_162, thisv->unk_168, spF0, spEE);
        Math_ApproachS(&thisv->unk_164, thisv->unk_16A, spFA, 0x2000);

        if (thisv->actor.speedXZ <= 0.5f) {
            Math_ApproachS(&thisv->actor.shape.rot.x, 0, 10, thisv->unk_178);
            Math_ApproachS(&thisv->unk_178, 0x500, 1, 0x20);
        } else {
            Math_ApproachS(&thisv->actor.shape.rot.x, -thisv->actor.world.rot.x, 10, 0x1000);
            thisv->unk_178 = 0;
        }
        ...
        if ((thisv->unk_158 != -1) && (thisv->unk_158 != -2) && (thisv->unk_158 != -25)) {
            if ((thisv->actor.world.pos.y > WATER_SURFACE_Y(globalCtx)) &&
                (thisv->actor.prevPos.y <= WATER_SURFACE_Y(globalCtx))) {
                func_80B70A2C(thisv, globalCtx, true);
                func_80B71278(thisv, 1);
                thisv->unk_184 = thisv->actor.velocity.y;
                thisv->actor.velocity.y = 0.0f;
                thisv->unk_16A = Rand_CenteredFloat(32768.0f);
            } else if ((thisv->actor.world.pos.y < WATER_SURFACE_Y(globalCtx)) &&
                       (thisv->actor.prevPos.y >= WATER_SURFACE_Y(globalCtx))) {
                if (thisv->unk_184 < -5.0f) {
                    thisv->unk_184 = -5.0f;
                }
                thisv->actor.world.rot.x = -0xFA0;
                func_80B70A2C(thisv, globalCtx, true);
                thisv->unk_1D2 = 20;
                func_80B71278(thisv, 0);
            }
        ...
        if ((thisv->actor.world.pos.y < WATER_SURFACE_Y(globalCtx)) &&
            (thisv->actor.world.pos.y > (WATER_SURFACE_Y(globalCtx) - 10.0f)) && ((thisv->unk_15C & 1) == 0) &&
            (thisv->actor.speedXZ > 0.0f)) {
            Vec3f pos = thisv->actor.world.pos;
            pos.y = WATER_SURFACE_Y(globalCtx);
            Fishing_SpawnRipple(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, 80.0f, 500.0f, 150, 90);
        }

        if ((thisv->actor.speedXZ > 0.0f) || (thisv->unk_158 == 5)) {
            f32 velocityY = thisv->actor.velocity.y;

            spD8 = thisv->unk_1AC * 0.1f;

            thisv->actor.world.pos.y -= spD8;
            thisv->actor.prevPos.y -= spD8;
            thisv->actor.velocity.y = -1.0f;
            Actor_UpdateBgCheckInfo(globalCtx, &thisv->actor, 30.0f, 30.0f, 100.0f, 0x45);
            thisv->actor.world.pos.y += spD8;
            thisv->actor.prevPos.y += spD8;

            thisv->actor.velocity.y = velocityY;
            ...
            if (thisv->actor.bgCheckFlags & 1) {
                if (thisv->actor.world.pos.y > WATER_SURFACE_Y(globalCtx)) {
                    thisv->unk_184 = Rand_ZeroFloat(3.0f) + 3.0f;
                    thisv->actor.velocity.x = thisv->actor.world.pos.x * -0.003f;
                    thisv->actor.velocity.z = thisv->actor.world.pos.z * -0.003f;

                    Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_FISH_LEAP);
                    func_80B70CF0(thisv, globalCtx);

                    if (Rand_ZeroOne() < 0.5f) {
                        thisv->unk_16A = 0x4000;
                    } else {
                        thisv->unk_16A = -0x4000;
                    }
                    ...
                    if (Rand_ZeroOne() < 0.5f) {
                        ...
                        thisv->unk_166 = (s16)Rand_CenteredFloat(32.0f) + 0x8000;
                    }

                    thisv->unk_168 = (s16)Rand_CenteredFloat(16384.0f);
                    thisv->unk_190 = 1.0f;
                    thisv->unk_194 = 5000.0f;
                    thisv->unk_19C = 5000.0f;
                } else {
                    ...
                    if ((thisv->unk_158 == 5) && ((thisv->unk_15C & 1) == 0)) {
                        Vec3f pos;

                        pos.x = Rand_CenteredFloat(10.0f) + thisv->actor.world.pos.x;
                        pos.z = Rand_CenteredFloat(10.0f) + thisv->actor.world.pos.z;
                        pos.y = thisv->actor.floorHeight + 5.0f;
                        Fishing_SpawnWaterDust(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos,
                                               (thisv->unk_1AC * 0.005f) + 0.15f);
                    }
                }
            }
        }
    ...
    if (thisv->unk_1D2 != 0) {
        s16 i;
        Vec3f pos;
        f32 range = (thisv->unk_1AC * 0.075f) + 10.0f;

        thisv->unk_1D2--;

        for (i = 0; i < 2; i++) {
            pos.x = Rand_CenteredFloat(range) + thisv->actor.world.pos.x;
            pos.y = Rand_CenteredFloat(range) + thisv->actor.world.pos.y;
            pos.z = Rand_CenteredFloat(range) + thisv->actor.world.pos.z;
            Fishing_SpawnBubble(&thisv->actor.projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos,
                                Rand_ZeroFloat(0.035f) + 0.04f, 0);
        }
    }
}

s32 Fishing_FishOverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3f* pos, Vec3s* rot,
                                 void* thisx) {
    Fishing* thisv = (Fishing*)thisx;

    if (limbIndex == 0xD) {
        rot->z -= thisv->unk_170 - 11000;
...
void Fishing_FishPostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3s* rot, void* thisx) {
    Fishing* thisv = (Fishing*)thisx;

    if (limbIndex == 0xD) {
        Matrix_MultVec3f(&sFishMouthOffset, &thisv->fishMouthPos);
    }
}

s32 Fishing_LoachOverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3f* pos, Vec3s* rot,
                                  void* thisx) {
    Fishing* thisv = (Fishing*)thisx;

    if (limbIndex == 3) {
        rot->y += thisv->unk_1CC[0];
    } else if (limbIndex == 4) {
        rot->y += thisv->unk_1CC[1];
    } else if (limbIndex == 5) {
        rot->y += thisv->unk_1CC[2];
    }

    return 0;
}

void Fishing_LoachPostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3s* rot, void* thisx) {
    static Vec3f sLoachMouthOffset = { 500.0f, 500.0f, 0.0f };
    Fishing* thisv = (Fishing*)thisx;

    if (limbIndex == 0xB) {
        Matrix_MultVec3f(&sLoachMouthOffset, &thisv->fishMouthPos);
    }
}
...
void Fishing_DrawFish(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_RotateY(((thisv->unk_162 + thisv->actor.shape.rot.y) / 32768.0f) * std::numbers::pi_v<float>, MTXMODE_APPLY);
    Matrix_RotateX(((thisv->unk_160 + thisv->actor.shape.rot.x) / 32768.0f) * std::numbers::pi_v<float>, MTXMODE_APPLY);
    Matrix_RotateZ(((thisv->unk_164 + thisv->actor.shape.rot.z) / 32768.0f) * std::numbers::pi_v<float>, MTXMODE_APPLY);
    Matrix_Scale(thisv->actor.scale.x, thisv->actor.scale.y, thisv->actor.scale.z, MTXMODE_APPLY);

    if (thisv->unk_150 == 0) {
        Matrix_RotateY((thisv->unk_16C * (std::numbers::pi_v<float> / 32768)) - (std::numbers::pi_v<float> / 2), MTXMODE_APPLY);
        Matrix_Translate(0.0f, 0.0f, thisv->unk_16C * 10.0f * 0.01f, MTXMODE_APPLY);

        SkelAnime_DrawFlexOpa(globalCtx, thisv->skelAnime.skeleton, thisv->skelAnime.jointTable,
                              thisv->skelAnime.dListCount, Fishing_FishOverrideLimbDraw, Fishing_FishPostLimbDraw, thisv);
    } else {
        Matrix_Translate(0.0f, 0.0f, 3000.0f, MTXMODE_APPLY);
        Matrix_RotateY(thisv->unk_16C * (std::numbers::pi_v<float> / 32768), MTXMODE_APPLY);
        Matrix_Translate(0.0f, 0.0f, -3000.0f, MTXMODE_APPLY);
        Matrix_RotateY(-(std::numbers::pi_v<float> / 2), MTXMODE_APPLY);

        SkelAnime_DrawFlexOpa(globalCtx, thisv->skelAnime.skeleton, thisv->skelAnime.jointTable,
                              thisv->skelAnime.dListCount, Fishing_LoachOverrideLimbDraw, Fishing_LoachPostLimbDraw,
                              thisv);
    }
}
    ...
    for (i = 0; i < POND_PROP_COUNT; i++) {
        if (prop->type == FS_PROP_LILY_PAD) {
            ...
            if (prop->shouldDraw) {
                ...
                Matrix_RotateY(prop->lilyPadAngle * (std::numbers::pi_v<float> / 32768), MTXMODE_APPLY);
    ...
    for (i = 0; i < GROUP_FISH_COUNT; i++) {
        if (fish->type != FS_GROUP_FISH_NONE) {
            ...
            if ((dist < 10.0f) || (((fish->timer % 32) == 0) && (Rand_ZeroOne() > 0.5f))) {
                ...
                Fishing_SpawnRipple(&fish->projectedPos, static_cast<FishingEffect*>(globalCtx->specialEffects), &ripplePos, 20.0f,
    ...
    for (i = 0; i < GROUP_FISH_COUNT; i++) {
        if (fish->type != FS_GROUP_FISH_NONE) {
            ...
            if (fish->shouldDraw) {
                ...
                Matrix_RotateY(((f32)fish->unk_3E * std::numbers::pi_v<float>) / 32768.0f, MTXMODE_APPLY);
                Matrix_RotateX((-(f32)fish->unk_3C * std::numbers::pi_v<float>) / 32768.0f, MTXMODE_APPLY);
...
void Fishing_HandleOwnerDialog(Fishing* thisv, GlobalContext* globalCtx) {
    switch (thisv->unk_15C) {
        case 0:
            if (D_80B7E0AC == 0) {
                if (sLinkAge != 1) {
                    if ((HIGH_SCORE(HS_FISHING) & 0x100) && !(HIGH_SCORE(HS_FISHING) & 0x200)) {
                        thisv->actor.textId = 0x4093;
                    } else {
                        thisv->actor.textId = 0x407B;
                    }
                } else {
                    thisv->actor.textId = 0x407B;
                }
            } else if (D_80B7A68C == 0) {
                thisv->actor.textId = 0x4084;
            } else {
                thisv->actor.textId = 0x4097;
            }
                ...
                switch (globalCtx->msgCtx.choiceIndex) {
                    case 0:
                        if (gSaveContext.rupees >= 20) {
                            Rupees_ChangeBy(-20);
                            if (func_800AA148() == 0) {
                                thisv->actor.textId = 0x407C;
                            } else {
                                thisv->actor.textId = 0x407D;
                            }
            ...
            if (D_80B7A68C != 0) {
                ...
                if ((Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CHOICE) && Message_ShouldAdvance(globalCtx)) {
                    ...
                    switch (globalCtx->msgCtx.choiceIndex) {
                        case 0:
                            if (D_80B7A670 == 0.0f) {
                                thisv->actor.textId = 0x408C;
                                thisv->unk_15C = 20;
                            } else if (D_80B7E07C == 0) {
                                D_80B7A678 = D_80B7A670;
                                if ((s16)D_80B7E078 < (s16)D_80B7A670) {
                                    if (D_80B7E07E == 2) {
                                        thisv->actor.textId = 0x40B0;
                                    } else {
                                        thisv->actor.textId = 0x4086;
                                    }
                                    thisv->unk_15C = 11;
                                } else {
                                    thisv->actor.textId = 0x408B;
                                    thisv->unk_15C = 20;
                                }
                            } else {
                                thisv->actor.textId = 0x409B;
                                thisv->unk_15C = 11;
                            }
                            Message_ContinueTextbox(globalCtx, thisv->actor.textId);
                            break;
        ...
        case 23:
            ...
            if (Actor_HasParent(&thisv->actor, globalCtx)) {
                ...
                func_8002F434(&thisv->actor, globalCtx, GI_SCALE_GOLD, 2000.0f, 1000.0f);
            }
            break;
...
void Fishing_UpdateOwner(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((D_80B7A684 != 0) || (Message_GetState(&globalCtx->msgCtx) != TEXT_STATE_NONE)) {
        ...
        thisv->actor.flags |= ACTOR_FLAG_0 | ACTOR_FLAG_5;
    }

    if ((thisv->actor.xzDistToPlayer < 120.0f) || (Message_GetState(&globalCtx->msgCtx) != TEXT_STATE_NONE)) {
        headRotTarget = thisv->actor.shape.rot.y - thisv->actor.yawTowardsPlayer;
    } else {
        headRotTarget = 0;
    }
    ...
    Math_ApproachS(&thisv->unk_164, headRotTarget, 3, 0x1388);

    if (((globalCtx->gameplayFrames % 32) == 0) && (Rand_ZeroOne() < 0.3f)) {
        thisv->unk_162 = 4;
    }

    thisv->unk_160 = D_80B7AFC4[thisv->unk_162];
    ...
    Fishing_HandleOwnerDialog(thisv, globalCtx);

    D_80B7E14C = 0.0015f;
    D_80B7A680++;

    if ((D_80B7E0AC != 0) && D_80B7A674) {
        Fishing_UpdateLure(thisv, globalCtx);
    }

    Fishing_UpdateEffects(static_cast<FishingEffect*>(globalCtx->specialEffects), globalCtx);
    ...
    switch (D_80B7A6CC) {
        ...
        case 21:
            if ((D_80B7A6D0 == 0) && Message_ShouldAdvance(globalCtx)) {
                D_80B7A6CC = 22;
                D_80B7A6D0 = 40;
                func_8002DF54(globalCtx, &thisv->actor, 0x1C);
                D_80B7FEE4 = 0.0f;
            }
            break;
    ...
    if ((player->actor.floorHeight < (WATER_SURFACE_Y(globalCtx) - 3.0f)) &&
        ...
        Fishing_SpawnWaterDust(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, 0.5f);
    }
    ...
    if ((player->actor.floorHeight < WATER_SURFACE_Y(globalCtx)) &&
        ...
        for (i = 0; i < 10; i++) {
            ...
            pos.y = WATER_SURFACE_Y(globalCtx);
            pos.z += 2.0f * vel.z;
            Fishing_SpawnDustSplash(NULL, static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, &vel, Rand_ZeroFloat(0.01f) + 0.020000001f);
        }
    }
    ...
    if ((u8)D_80B7A650 > 0) {
        ...
        rot.x = std::numbers::pi_v<float> / 2.0f + 0.1f;
        rot.y = 1.0f;
        rot.z = (Camera_GetInputDirYaw(camera) * -(std::numbers::pi_v<float> / 32768)) + rot.y;
        ...
        for (i = 0; i < (u8)D_80B7A650; i++) {
            ...
            if (pos.z < 1160.0f) {
                ...
                if (projectedPos.z < 0.0f) {
                    ...
                    Fishing_SpawnRainDrop(static_cast<FishingEffect*>(globalCtx->specialEffects), &pos, &rot);
                }
            }
        }
    }
...
s32 Fishing_OwnerOverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3f* pos, Vec3s* rot,
                                  void* thisx) {
    Fishing* thisv = (Fishing*)thisx;
...
void Fishing_OwnerPostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, const Gfx** dList, Vec3s* rot, void* thisx) {
    if (limbIndex == 8) { // Head
...
static const void* sFishingOwnerEyeTexs[] = {
...
void Fishing_DrawOwner(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisv->actor.projectedPos.z < 1500.0f) &&
        (fabsf(thisv->actor.projectedPos.x) < (100.0f + thisv->actor.projectedPos.z))) {
        gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(sFishingOwnerEyeTexs[thisv->unk_160]));

        SkelAnime_DrawFlexOpa(globalCtx, thisv->skelAnime.skeleton, thisv->skelAnime.jointTable,
                              thisv->skelAnime.dListCount, Fishing_OwnerOverrideLimbDraw, Fishing_OwnerPostLimbDraw,
                              thisv);
    }

    Fishing_DrawPondProps(globalCtx);
    Fishing_DrawEffects(static_cast<FishingEffect*>(globalCtx->specialEffects), globalCtx);
