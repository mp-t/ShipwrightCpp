...
void EndTitle_Init(Actor* thisx, GlobalContext* globalCtx) {
    EndTitle* thisv = (EndTitle*)thisx;

    thisv->endAlpha = 0;
    thisv->tlozAlpha = 0;
    thisv->ootAlpha = 0;
    if (thisv->actor.params == 1) {
        thisv->actor.draw = EndTitle_DrawNintendoLogo;
    }
}
...
void EndTitle_DrawFull(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_RotateX(0xBB8 * std::numbers::pi_v<float> / 0x8000, MTXMODE_APPLY);
    ...
    if ((frameCount > 890) && (thisv->endAlpha < 200)) {
        thisv->endAlpha += 7;
    }
    if ((frameCount > 810) && (thisv->tlozAlpha < 200)) {
        thisv->tlozAlpha += 15;
    }
    if ((frameCount > 850) && (thisv->ootAlpha < 200)) {
        thisv->ootAlpha += 15;
    }
    ...
    gDPSetPrimColor(OVERLAY_DISP++, 0x00, 0x80, 0, 0, 0, thisv->endAlpha);
    ...
    gDPSetPrimColor(OVERLAY_DISP++, 0x00, 0x80, 0, 0, 0, thisv->tlozAlpha);
    ...
    gSPTextureRectangle(OVERLAY_DISP++, 100 << 2, 160 << 2, 220 << 2, 183 << 2, G_TX_RENDERTILE, 0, 0, 1 << 10,
                        1 << 10);
    gDPPipeSync(OVERLAY_DISP++);
    gDPSetPrimColor(OVERLAY_DISP++, 0x00, 0x80, 0, 0, 0, thisv->ootAlpha);
...
void EndTitle_DrawNintendoLogo(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((frames >= 1101) && (thisv->endAlpha < 255)) {
        thisv->endAlpha += 3;
    }
    ...
    OVERLAY_DISP = func_80093F34(OVERLAY_DISP);
    gDPSetPrimColor(OVERLAY_DISP++, 0, 0x80, 0, 0, 0, thisv->endAlpha);
