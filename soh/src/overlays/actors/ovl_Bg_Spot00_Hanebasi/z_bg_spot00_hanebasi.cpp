...
void BgSpot00Hanebasi_DrawbridgeWait(BgSpot00Hanebasi* thisv, GlobalContext* globalCtx);
...
void BgSpot00Hanebasi_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgSpot00Hanebasi* thisv = (BgSpot00Hanebasi*)thisx;
    s32 pad;
    Vec3f chainPos;
    ...
    if (thisv->dyna.actor.params == DT_DRAWBRIDGE) {
    ...
    if (thisv->dyna.actor.params == DT_DRAWBRIDGE) {
        ...
        if ((gSaveContext.sceneSetupIndex != 6) &&
            ...
            thisv->dyna.actor.shape.rot.x = 0;
        }
        ...
        chainPos.y =
            (10.0f * Math_CosS(thisv->dyna.actor.shape.rot.x)) - (Math_SinS(thisv->dyna.actor.shape.rot.x) * 400.0f);
        chainPos.z =
            (10.0f * Math_SinS(thisv->dyna.actor.shape.rot.x)) - (Math_CosS(thisv->dyna.actor.shape.rot.x) * 400.0f);
        chainPos.x =
            (158.0f * Math_CosS(thisv->dyna.actor.shape.rot.y)) + (Math_SinS(thisv->dyna.actor.shape.rot.y) * chainPos.z);
        chainPos.z = (-158.0f * Math_SinS(thisv->dyna.actor.shape.rot.y)) +
                     (Math_CosS(thisv->dyna.actor.shape.rot.y) * chainPos.z);

        if (Actor_SpawnAsChild(&globalCtx->actorCtx, &thisv->dyna.actor, globalCtx, ACTOR_BG_SPOT00_HANEBASI,
                               thisv->dyna.actor.world.pos.x + chainPos.x, thisv->dyna.actor.world.pos.y + chainPos.y,
                               thisv->dyna.actor.world.pos.z + chainPos.z,
                               ((thisv->dyna.actor.shape.rot.x == 0) ? 0 : 0xF020), thisv->dyna.actor.shape.rot.y, 0,
                               DT_CHAIN_1) == NULL) {
            Actor_Kill(&thisv->dyna.actor);
        }

        thisv->actionFunc = BgSpot00Hanebasi_DrawbridgeWait;
        thisv->destAngle = 40;
    } else if (thisv->dyna.actor.params == DT_CHAIN_1) {
        if (Actor_SpawnAsChild(&globalCtx->actorCtx, &thisv->dyna.actor, globalCtx, ACTOR_BG_SPOT00_HANEBASI,
                               thisv->dyna.actor.world.pos.x - (Math_CosS(thisv->dyna.actor.shape.rot.y) * 316.0f),
                               thisv->dyna.actor.world.pos.y,
                               thisv->dyna.actor.world.pos.z + (Math_SinS(thisv->dyna.actor.shape.rot.y) * 316.0f),
                               thisv->dyna.actor.shape.rot.x, thisv->dyna.actor.shape.rot.y, 0, DT_CHAIN_2) == NULL) {
            Actor_Kill(&thisv->dyna.actor);
            Actor_Kill(thisv->dyna.actor.parent);
        }

        thisv->actionFunc = BgSpot00Hanebasi_SetTorchLightInfo;
    } else {
        thisv->actionFunc = BgSpot00Hanebasi_SetTorchLightInfo;
    }

    if (thisv->dyna.actor.params >= DT_CHAIN_1) {
        thisv->lightNode = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &thisv->lightInfo);
        Lights_PointGlowSetInfo(&thisv->lightInfo, ((thisv->dyna.actor.params == DT_CHAIN_1) ? 260.0f : -260.0f), 168,
                                690, 255, 255, 0, 0);
    }
}
...
void BgSpot00Hanebasi_DrawbridgeWait(BgSpot00Hanebasi* thisv, GlobalContext* globalCtx) {
    BgSpot00Hanebasi* child = (BgSpot00Hanebasi*)thisv->dyna.actor.child;
    ...
    if ((gSaveContext.sceneSetupIndex >= 4) || !CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) ||
        ...
        if (thisv->dyna.actor.shape.rot.x != 0) {
            if (Flags_GetEnv(globalCtx, 0) || ((gSaveContext.sceneSetupIndex < 4) && IS_DAY)) {
                thisv->actionFunc = BgSpot00Hanebasi_DrawbridgeRiseAndFall;
                thisv->destAngle = 0;
                child->destAngle = 0;
                return;
            }

            if (thisv) {} // required to match
        }
        if ((thisv->dyna.actor.shape.rot.x == 0) && (gSaveContext.sceneSetupIndex < 4) && !LINK_IS_ADULT && !IS_DAY) {
            thisv->actionFunc = BgSpot00Hanebasi_DrawbridgeRiseAndFall;
            thisv->destAngle = -0x4000;
            child->destAngle = -0xFE0;
        }
    }
}

void BgSpot00Hanebasi_DoNothing(BgSpot00Hanebasi* thisv, GlobalContext* globalCtx) {
}
...
void BgSpot00Hanebasi_DrawbridgeRiseAndFall(BgSpot00Hanebasi* thisv, GlobalContext* globalCtx) {
    ...
    if (Math_ScaledStepToS(&thisv->dyna.actor.shape.rot.x, thisv->destAngle, 80)) {
        thisv->actionFunc = BgSpot00Hanebasi_DrawbridgeWait;
    }

    if (thisv->dyna.actor.shape.rot.x >= -0x27D8) {
        child = (BgSpot00Hanebasi*)thisv->dyna.actor.child;
        angle *= 0.4f;
    ...
    if (thisv->destAngle < 0) {
        if (thisv->actionFunc == BgSpot00Hanebasi_DrawbridgeWait) {
            Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EV_BRIDGE_CLOSE_STOP);
        } else {
...
void BgSpot00Hanebasi_SetTorchLightInfo(BgSpot00Hanebasi* thisv, GlobalContext* globalCtx) {
    ...
    Lights_PointGlowSetInfo(&thisv->lightInfo, (thisv->dyna.actor.params == DT_CHAIN_1) ? 260.0f : -260.0f,
        ...
        if (globalCtx->sceneNum == SCENE_SPOT00) {
            if (CHECK_QUEST_ITEM(QUEST_KOKIRI_EMERALD) && CHECK_QUEST_ITEM(QUEST_GORON_RUBY) &&
                ...
                if ((player->actor.world.pos.x > -450.0f) && (player->actor.world.pos.x < 450.0f) &&
                    ...
                    thisv->actionFunc = BgSpot00Hanebasi_DoNothing;
                    func_8002DF54(globalCtx, &player->actor, 8);
                    globalCtx->nextEntranceIndex = 0x00CD;
                    gSaveContext.nextCutsceneIndex = 0xFFF1;
                    globalCtx->sceneLoadFlag = 0x14;
                    globalCtx->fadeTransition = 4;
                } else if (Actor_IsFacingAndNearPlayer(&thisv->dyna.actor, 3000.0f, 0x7530)) {
...
void BgSpot00Hanebasi_DrawTorches(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    angle = (s16)(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) + 0x8000) * (std::numbers::pi_v<float> / 32768.0f);
