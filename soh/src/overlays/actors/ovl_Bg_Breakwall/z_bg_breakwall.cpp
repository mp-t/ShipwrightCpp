...
typedef struct {
    /* 0x00 */ const CollisionHeader* colHeader;
    /* 0x04 */ const Gfx* dList;
    /* 0x08 */ s8 colType;
} BombableWallInfo;
...
void BgBreakwall_WaitForObject(BgBreakwall* thisv, GlobalContext* globalCtx);
void BgBreakwall_Wait(BgBreakwall* thisv, GlobalContext* globalCtx);
void BgBreakwall_LavaCoverMove(BgBreakwall* thisv, GlobalContext* globalCtx);
...
void BgBreakwall_SetupAction(BgBreakwall* thisv, BgBreakwallActionFunc actionFunc) {
    thisv->actionFunc = actionFunc;
}

void BgBreakwall_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgBreakwall* thisv = (BgBreakwall*)thisx;
    s32 pad;
    s32 wallType = ((thisv->dyna.actor.params >> 13) & 3) & 0xFF;

    Actor_ProcessInitChain(&thisv->dyna.actor, sInitChain);
    DynaPolyActor_Init(&thisv->dyna, DPM_UNK);
    thisv->bombableWallDList = sBombableWallInfo[wallType].dList;
    thisv->colType = sBombableWallInfo[wallType].colType;

    if (thisv->colType == 1) {
        thisv->dyna.actor.world.rot.x = 0x4000;
    }

    if (thisv->bombableWallDList != NULL) {
        ...
        Collider_SetQuad(globalCtx, &thisv->collider, &thisv->dyna.actor, &sQuadInit);
    } else {
        thisv->dyna.actor.world.pos.y -= 40.0f;
    }

    thisv->bankIndex = (wallType >= BWALL_KD_FLOOR) ? Object_GetIndex(&globalCtx->objectCtx, OBJECT_KINGDODONGO)
                                                   : Object_GetIndex(&globalCtx->objectCtx, OBJECT_BWALL);

    if (thisv->bankIndex < 0) {
        Actor_Kill(&thisv->dyna.actor);
    } else {
        BgBreakwall_SetupAction(thisv, BgBreakwall_WaitForObject);
    }
}
 ...
 */
Actor* BgBreakwall_SpawnFragments(GlobalContext* globalCtx, BgBreakwall* thisv, Vec3f* pos, f32 velocity, f32 scaleY,
    ...
    for (i = 0; i < count; angle2 += 0x4000, i++) {
        angle1 = ABS(thisv->dyna.actor.world.rot.y) + angle2;
 ...
 */
void BgBreakwall_WaitForObject(BgBreakwall* thisv, GlobalContext* globalCtx) {
    if (Object_IsLoaded(&globalCtx->objectCtx, thisv->bankIndex)) {
        const CollisionHeader* colHeader = NULL;
        s32 wallType = ((thisv->dyna.actor.params >> 13) & 3) & 0xFF;

        thisv->dyna.actor.objBankIndex = thisv->bankIndex;
        Actor_SetObjectDependency(globalCtx, &thisv->dyna.actor);
        thisv->dyna.actor.flags &= ~ACTOR_FLAG_4;
        thisv->dyna.actor.draw = BgBreakwall_Draw;
        ...
        if (wallType == BWALL_KD_LAVA_COVER) {
            BgBreakwall_SetupAction(thisv, BgBreakwall_LavaCoverMove);
        } else {
            BgBreakwall_SetupAction(thisv, BgBreakwall_Wait);
        }
    }
}
...
void BgBreakwall_Wait(BgBreakwall* thisv, GlobalContext* globalCtx) {
    if (thisv->collider.base.acFlags & 2) {
        ...
        if (thisv->dyna.actor.world.rot.x == 0) {
            effectPos.y = 55.0f;
        } else {
            effectPos.z = 25.0f;
            effectPos.y = -10.0f;
        ...
        BgBreakwall_SpawnFragments(globalCtx, thisv, &effectPos, 0.0f, 6.4f, 5.0f, 1, 2.0f);
 ...
 */
void BgBreakwall_LavaCoverMove(BgBreakwall* thisv, GlobalContext* globalCtx) {
    Math_StepToF(&thisv->dyna.actor.world.pos.y, KREG(80) + thisv->dyna.actor.home.pos.y, 1.0f);
}
...
void BgBreakwall_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->bombableWallDList != NULL) {
        ...
        gSPDisplayList(POLY_OPA_DISP++, thisv->bombableWallDList);

        if (thisv->colType >= 0) {
            Vec3f colQuad[4];
            Vec3f* src = &sColQuadList[thisv->colType][0];
            Vec3f* dst = &colQuad[0];
            s32 i;
            ...
            Collider_SetQuadVertices(&thisv->collider, &colQuad[0], &colQuad[1], &colQuad[2], &colQuad[3]);
