/*
 * File: z_bg_hidan_hamstep.c
 * Overlay: ovl_Bg_Hidan_Hamstep
 * Description: Stone Steps and Platforms (Fire Temple)
 */

#include "z_bg_hidan_hamstep.h"
#include "objects/object_hidan_objects/object_hidan_objects.h"

#define FLAGS 0

void BgHidanHamstep_Init(Actor* thisx, GlobalContext* globalCtx);
void BgHidanHamstep_Destroy(Actor* thisx, GlobalContext* globalCtx);
void BgHidanHamstep_Update(Actor* thisx, GlobalContext* globalCtx);
void BgHidanHamstep_Draw(Actor* thisx, GlobalContext* globalCtx);

void func_808887C4(BgHidanHamstep* thisv, GlobalContext* globalCtx);
void func_80888860(BgHidanHamstep* thisv, GlobalContext* globalCtx);
void func_808889B8(BgHidanHamstep* thisv, GlobalContext* globalCtx);
void func_80888A58(BgHidanHamstep* thisv, GlobalContext* globalCtx);
void BgHidanHamstep_DoNothing(BgHidanHamstep* thisv, GlobalContext* globalCtx);

static f32 sYPosOffsets[] = {
    -20.0f, -120.0f, -220.0f, -320.0f, -420.0f,
};

static ColliderTrisElementInit sTrisElementsInit[2] = {
    {
        {
            ELEMTYPE_UNK0,
            { 0x00000000, 0x00, 0x00 },
            { 0x40000040, 0x00, 0x00 },
            TOUCH_NONE,
            BUMP_ON,
            OCELEM_NONE,
        },
        { { { -20.0f, 3.0f, -20.0f }, { -20.0f, 3.0f, 20.0f }, { 20.0f, 3.0f, 20.0f } } },
    },
    {
        {
            ELEMTYPE_UNK0,
            { 0x00000000, 0x00, 0x00 },
            { 0x40000040, 0x00, 0x00 },
            TOUCH_NONE,
            BUMP_ON,
            OCELEM_NONE,
        },
        { { { 20.0f, 3.0f, 20.0f }, { 20.0f, 3.0f, -20.0f }, { -20.0f, 3.0f, -20.0f } } },
    },
};

static ColliderTrisInit sTrisInit = {
    {
        COLTYPE_NONE,
        AT_NONE,
        AC_ON | AC_TYPE_PLAYER,
        OC1_NONE,
        OC2_NONE,
        COLSHAPE_TRIS,
    },
    2,
    sTrisElementsInit,
};

const ActorInit Bg_Hidan_Hamstep_InitVars = {
    ACTOR_BG_HIDAN_HAMSTEP,
    ACTORCAT_BG,
    FLAGS,
    OBJECT_HIDAN_OBJECTS,
    sizeof(BgHidanHamstep),
    (ActorFunc)BgHidanHamstep_Init,
    (ActorFunc)BgHidanHamstep_Destroy,
    (ActorFunc)BgHidanHamstep_Update,
    (ActorFunc)BgHidanHamstep_Draw,
    NULL,
};

static BgHidanHamstepActionFunc sActionFuncs[] = {
    func_808887C4, func_80888860, func_808889B8, func_80888A58, BgHidanHamstep_DoNothing,
};

static InitChainEntry sInitChain[] = {
    ICHAIN_VEC3F_DIV1000(scale, 100, ICHAIN_STOP),
};

static f32 sEffectPositions[][2] = {
    { -100.0f, 40.0f },  { 100.0f, 40.0f }, { -100.0f, 0.0f },   { 100.0f, 0.0f },
    { -100.0f, -40.0f }, { 100.0f, 40.0f }, { -100.0f, -80.0f }, { -50.0f, -80.0f },
    { 0.0f, -80.0f },    { 50.0f, -80.0f }, { 100.0f, -80.0f },
};

void BgHidanHamstep_SetupAction(BgHidanHamstep* thisv, s32 action) {
    thisv->action = action;
    thisv->actionFunc = sActionFuncs[action];
}

s32 BgHidanHamstep_SpawnChildren(BgHidanHamstep* thisv, GlobalContext* globalCtx2) {
    BgHidanHamstep* step = thisv;
    s32 i;
    Vec3f pos;
    f32 sin;
    f32 cos;
    s16 params;
    GlobalContext* globalCtx = globalCtx2;

    pos = pos; // Required to match
    pos.y = thisv->dyna.actor.home.pos.y - 100.0f;
    sin = Math_SinS(thisv->dyna.actor.shape.rot.y + 0x8000);
    cos = Math_CosS(thisv->dyna.actor.shape.rot.y + 0x8000);

    for (i = 0; i < 5; i++) {
        pos.x = (((i * 160.0f) + 60.0f) * sin) + thisv->dyna.actor.home.pos.x;
        pos.z = (((i * 160.0f) + 60.0f) * cos) + thisv->dyna.actor.home.pos.z;

        params = (i + 1) & 0xFF;
        params |= (thisv->dyna.actor.params & 0xFF00);

        step = (BgHidanHamstep*)Actor_SpawnAsChild(
            &globalCtx->actorCtx, &step->dyna.actor, globalCtx, ACTOR_BG_HIDAN_HAMSTEP, pos.x, pos.y, pos.z,
            thisv->dyna.actor.world.rot.x, thisv->dyna.actor.world.rot.y, thisv->dyna.actor.world.rot.z, params);

        if (step == NULL) {
            return 0;
        }
    }
    return 1;
}

void BgHidanHamstep_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgHidanHamstep* thisv = (BgHidanHamstep*)thisx;
    s32 pad;
    const CollisionHeader* colHeader = NULL;
    Vec3f sp48[3];
    s32 i;
    s32 i2;
    BgHidanHamstep* step;

    DynaPolyActor_Init(&thisv->dyna, DPM_PLAYER);
    Actor_ProcessInitChain(&thisv->dyna.actor, sInitChain);

    if ((thisv->dyna.actor.params & 0xFF) == 0) {
        Collider_InitTris(globalCtx, &thisv->collider);
        Collider_SetTris(globalCtx, &thisv->collider, &thisv->dyna.actor, &sTrisInit, thisv->colliderItems);

        for (i = 0; i < 2; i++) {
            for (i2 = 0; i2 < 3; i2++) {
                sp48[i2].x = sTrisInit.elements[i].dim.vtx[i2].x + thisv->dyna.actor.home.pos.x;
                sp48[i2].y = sTrisInit.elements[i].dim.vtx[i2].y + thisv->dyna.actor.home.pos.y;
                sp48[i2].z = sTrisInit.elements[i].dim.vtx[i2].z + thisv->dyna.actor.home.pos.z;
            }
            Collider_SetTrisVertices(&thisv->collider, i, &sp48[0], &sp48[1], &sp48[2]);
        }
    }

    if ((thisv->dyna.actor.params & 0xFF) == 0) {
        CollisionHeader_GetVirtual(&gFireTempleStoneStep1Col, &colHeader);
    } else {
        CollisionHeader_GetVirtual(&gFireTempleStoneStep2Col, &colHeader);
    }

    thisv->dyna.bgId = DynaPoly_SetBgActor(globalCtx, &globalCtx->colCtx.dyna, &thisv->dyna.actor, colHeader);

    if (Flags_GetSwitch(globalCtx, (thisv->dyna.actor.params >> 8) & 0xFF)) {
        if ((thisv->dyna.actor.params & 0xFF) == 0) {
            thisv->dyna.actor.world.pos.y = thisv->dyna.actor.home.pos.y + (-20.0f);
            BgHidanHamstep_SetupAction(thisv, 4);
        } else {
            thisv->dyna.actor.world.pos.y =
                sYPosOffsets[(thisv->dyna.actor.params & 0xFF) - 1] + thisv->dyna.actor.home.pos.y;
            BgHidanHamstep_SetupAction(thisv, 4);
        }
    } else if ((thisv->dyna.actor.params & 0xFF) == 0) {
        BgHidanHamstep_SetupAction(thisv, 0);
    } else {
        BgHidanHamstep_SetupAction(thisv, 2);
    }

    thisv->dyna.actor.gravity = -1.2f;
    thisv->dyna.actor.minVelocityY = -12.0f;

    if ((thisv->dyna.actor.params & 0xFF) == 0) {
        // "Fire Temple Object [Hammer Step] appears"
        osSyncPrintf("◯◯◯炎の神殿オブジェクト【ハンマーステップ】出現\n");
        if (BgHidanHamstep_SpawnChildren(thisv, globalCtx) == 0) {
            step = thisv;

            // "[Hammer Step] I can't create a step!"
            osSyncPrintf("【ハンマーステップ】 足場産れない！！\n");
            osSyncPrintf("%s %d\n", "../z_bg_hidan_hamstep.c", 425);

            while (step != NULL) {
                Actor_Kill(&step->dyna.actor);
                step = (BgHidanHamstep*)step->dyna.actor.child;
            }
        }
    }
}

void BgHidanHamstep_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    BgHidanHamstep* thisv = (BgHidanHamstep*)thisx;

    DynaPoly_DeleteBgActor(globalCtx, &globalCtx->colCtx.dyna, thisv->dyna.bgId);

    if ((thisv->dyna.actor.params & 0xFF) == 0) {
        Collider_DestroyTris(globalCtx, &thisv->collider);
    }
}

void func_808884C8(BgHidanHamstep* step, GlobalContext* globalCtx) {
    Vec3f pos = step->dyna.actor.world.pos;
    s32 i;
    f32 sin;
    f32 cos;

    pos.y -= 20.0f;

    func_80033480(globalCtx, &pos, 0.0f, 0, 600, 300, 0);

    sin = Math_SinS(step->dyna.actor.shape.rot.y + 0x8000);
    cos = Math_CosS(step->dyna.actor.shape.rot.y + 0x8000);

    pos.y = step->dyna.actor.world.pos.y;

    for (i = 0; i < ARRAY_COUNT(sEffectPositions); i++) {
        pos.x = (sEffectPositions[i][1] * sin) + (sEffectPositions[i][0] * cos) + step->dyna.actor.world.pos.x;
        pos.z = ((sEffectPositions[i][1] * cos) - (sEffectPositions[i][0] * sin)) + step->dyna.actor.world.pos.z;
        func_80033480(globalCtx, &pos, 0.0f, 0, 150, 150, 0);
    }
}

void func_80888638(BgHidanHamstep* thisv, GlobalContext* globalCtx) {
    BgHidanHamstep* child = (BgHidanHamstep*)thisv->dyna.actor.child;

    while (child != NULL) {
        if ((child->dyna.actor.params & 0xFF) != 0) {
            func_808884C8(child, globalCtx);
        }
        child = (BgHidanHamstep*)child->dyna.actor.child;
    }
}

void func_80888694(BgHidanHamstep* thisv, BgHidanHamstep* parent) {
    BgHidanHamstep* child;

    if ((thisv->dyna.actor.params & 0xFF) >= 2) {
        if (parent->dyna.actor.world.pos.y < thisv->dyna.actor.world.pos.y) {
            thisv->dyna.actor.world.pos.y = parent->dyna.actor.world.pos.y;
        } else if ((thisv->dyna.actor.world.pos.y - parent->dyna.actor.world.pos.y) < -100.0f) {
            thisv->dyna.actor.world.pos.y = parent->dyna.actor.world.pos.y - 100.0f;
        }
    }

    child = (BgHidanHamstep*)thisv->dyna.actor.child;

    while (child != NULL) {
        if (thisv->dyna.actor.world.pos.y < child->dyna.actor.world.pos.y) {
            child->dyna.actor.world.pos.y = thisv->dyna.actor.world.pos.y;
        }
        child = (BgHidanHamstep*)child->dyna.actor.child;
    }
}

void func_80888734(BgHidanHamstep* thisv) {
    BgHidanHamstep* parent = (BgHidanHamstep*)thisv->dyna.actor.parent;
    f32 frameDivisor = R_UPDATE_RATE * 0.5f;

    if (parent != NULL) {
        thisv->dyna.actor.velocity.y = parent->dyna.actor.velocity.y;

        if ((thisv->dyna.actor.params & 0xFF) == 1) {
            thisv->dyna.actor.world.pos.y = parent->dyna.actor.world.pos.y - 100.0f;
        } else {
            thisv->dyna.actor.world.pos.y += (thisv->dyna.actor.velocity.y * frameDivisor);
        }

        func_80888694(thisv, parent);
    }
}

void func_808887C4(BgHidanHamstep* thisv, GlobalContext* globalCtx) {
    if (thisv->collider.base.acFlags & AC_HIT) {
        OnePointCutscene_Init(globalCtx, 3310, 100, &thisv->dyna.actor, MAIN_CAM);
        Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EV_HAMMER_SWITCH);
        thisv->collider.base.acFlags = AC_NONE;
        BgHidanHamstep_SetupAction(thisv, 1);
        Flags_SetSwitch(globalCtx, (thisv->dyna.actor.params >> 8) & 0xFF);
    } else {
        CollisionCheck_SetAC(globalCtx, &globalCtx->colChkCtx, &thisv->collider.base);
    }
}

void func_80888860(BgHidanHamstep* thisv, GlobalContext* globalCtx) {
    s32 pad;
    s32 pad2;
    s32 quakeIndex;

    Actor_MoveForward(&thisv->dyna.actor);

    if (((thisv->dyna.actor.world.pos.y - thisv->dyna.actor.home.pos.y) < (-20.0f - thisv->dyna.actor.minVelocityY)) &&
        (thisv->dyna.actor.velocity.y <= 0.0f)) {
        thisv->unk_244++;

        if (thisv->unk_244 >= 7) {
            thisv->dyna.actor.world.pos.y = thisv->dyna.actor.home.pos.y + -20.0f;
            BgHidanHamstep_SetupAction(thisv, 4);
        } else {
            thisv->dyna.actor.velocity.y *= -0.24f;

            if (1) {}

            if (thisv->unk_244 == 1) {
                quakeIndex = Quake_Add(GET_ACTIVE_CAM(globalCtx), 3);
                Quake_SetSpeed(quakeIndex, -15536);
                Quake_SetQuakeValues(quakeIndex, 0, 0, 500, 0);
                Quake_SetCountdown(quakeIndex, 20);
                Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EV_BLOCK_BOUND);
                func_800AA000(thisv->dyna.actor.xyzDistToPlayerSq, 255, 20, 150);
                func_80888638(thisv, globalCtx);
                osSyncPrintf("A(%d)\n", thisv->dyna.actor.params);
            }
        }
    }
}

void func_808889B8(BgHidanHamstep* thisv, GlobalContext* globalCtx) {
    s32 pad;
    BgHidanHamstep* parent = (BgHidanHamstep*)thisv->dyna.actor.parent;

    func_80888734(thisv);

    if ((parent->action == 4) || ((parent->action == 3) && (parent->unk_244 >= 5))) {
        if ((thisv->dyna.actor.params & 0xFF) == 1) {
            thisv->dyna.actor.world.pos.y =
                sYPosOffsets[(thisv->dyna.actor.params & 0xFF) - 1] + thisv->dyna.actor.home.pos.y;
            BgHidanHamstep_SetupAction(thisv, 4);
        } else {
            BgHidanHamstep_SetupAction(thisv, 3);
        }
    }
}

void func_80888A58(BgHidanHamstep* thisv, GlobalContext* globalCtx) {
    s32 pad;
    s32 pad2;
    s32 quakeIndex;

    Actor_MoveForward(&thisv->dyna.actor);
    func_80888694(thisv, (BgHidanHamstep*)thisv->dyna.actor.parent);

    if (((thisv->dyna.actor.params & 0xFF) <= 0) || ((thisv->dyna.actor.params & 0xFF) >= 6)) {
        // "[Hammer Step] arg_data strange (arg_data = %d)"
        osSyncPrintf("【ハンマーステップ】 arg_data おかしい (arg_data = %d)", thisv->dyna.actor.params);
        osSyncPrintf("%s %d\n", "../z_bg_hidan_hamstep.c", 696);
    }

    if (((thisv->dyna.actor.world.pos.y - thisv->dyna.actor.home.pos.y) <=
         sYPosOffsets[(thisv->dyna.actor.params & 0xFF) - 1]) &&
        (thisv->dyna.actor.velocity.y <= 0.0f)) {
        thisv->unk_244++;

        if (thisv->unk_244 >= 7) {
            thisv->dyna.actor.world.pos.y =
                sYPosOffsets[(thisv->dyna.actor.params & 0xFF) - 1] + thisv->dyna.actor.home.pos.y;
            BgHidanHamstep_SetupAction(thisv, 3);
        } else {
            thisv->dyna.actor.velocity.y *= -0.24f;

            if (1) {}

            if (thisv->unk_244 == 1) {
                quakeIndex = Quake_Add(GET_ACTIVE_CAM(globalCtx), 3);
                Quake_SetSpeed(quakeIndex, -15536);
                Quake_SetQuakeValues(quakeIndex, 20, 1, 0, 0);
                Quake_SetCountdown(quakeIndex, 7);

                Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EV_BLOCK_BOUND);
                func_800AA000(10000.0f, 255, 20, 150);
                func_808884C8(thisv, globalCtx);

                if ((thisv->dyna.actor.params & 0xFF) == 5) {
                    func_80078884(NA_SE_SY_CORRECT_CHIME);
                }

                osSyncPrintf("B(%d)\n", thisv->dyna.actor.params);
            }
        }
    }
}

void BgHidanHamstep_DoNothing(BgHidanHamstep* thisv, GlobalContext* globalCtx) {
}

void BgHidanHamstep_Update(Actor* thisx, GlobalContext* globalCtx) {
    BgHidanHamstep* thisv = (BgHidanHamstep*)thisx;

    thisv->actionFunc(thisv, globalCtx);
}

void BgHidanHamstep_Draw(Actor* thisx, GlobalContext* globalCtx) {
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_hidan_hamstep.c", 782);

    func_80093D18(globalCtx->state.gfxCtx);

    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_hidan_hamstep.c", 787),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

    if ((thisx->params & 0xFF) == 0) {
        gSPDisplayList(POLY_OPA_DISP++, gFireTempleStoneStep1DL);
    } else {
        gSPDisplayList(POLY_OPA_DISP++, gFireTempleStoneStep2DL);
    }

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_hidan_hamstep.c", 796);
}
