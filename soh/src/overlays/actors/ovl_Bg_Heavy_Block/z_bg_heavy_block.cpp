...
void BgHeavyBlock_MovePiece(BgHeavyBlock* thisv, GlobalContext* globalCtx);
void BgHeavyBlock_Wait(BgHeavyBlock* thisv, GlobalContext* globalCtx);
void BgHeavyBlock_LiftedUp(BgHeavyBlock* thisv, GlobalContext* globalCtx);
void BgHeavyBlock_Fly(BgHeavyBlock* thisv, GlobalContext* globalCtx);
...
void BgHeavyBlock_SetPieceRandRot(BgHeavyBlock* thisv, f32 scale) {
    thisv->dyna.actor.world.rot.x = Rand_CenteredFloat(1024.0f) * scale;
    thisv->dyna.actor.world.rot.y = Rand_CenteredFloat(1024.0f) * scale;
    thisv->dyna.actor.world.rot.z = Rand_CenteredFloat(1024.0f) * scale;
}

void BgHeavyBlock_InitPiece(BgHeavyBlock* thisv, f32 scale) {
    f32 rand;
    f32 yawSinCos;
    f32 randChoice;

    thisv->dyna.actor.gravity = -0.6f;
    thisv->dyna.actor.minVelocityY = -12.0f;
    randChoice = Rand_CenteredFloat(12.0f * scale);
    rand = (randChoice < 0.0f) ? randChoice - 2.0f : randChoice + 2.0f;
    thisv->dyna.actor.velocity.y = (Rand_ZeroFloat(8.0f) + 4.0f) * scale;
    thisv->dyna.actor.velocity.z = Rand_ZeroFloat(-8.0f * scale);
    yawSinCos = Math_CosS(thisv->dyna.actor.world.rot.y);
    thisv->dyna.actor.velocity.x =
        (Math_SinS(thisv->dyna.actor.world.rot.y) * thisv->dyna.actor.velocity.z + (yawSinCos * rand));
    yawSinCos = Math_SinS(thisv->dyna.actor.world.rot.y);
    thisv->dyna.actor.velocity.z =
        (Math_CosS(thisv->dyna.actor.world.rot.y) * thisv->dyna.actor.velocity.z) + (-yawSinCos * rand);
    BgHeavyBlock_SetPieceRandRot(thisv, scale);
    Actor_SetScale(&thisv->dyna.actor, Rand_CenteredFloat(0.2f) + 1.0f);
}

void BgHeavyBlock_SetupDynapoly(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    s32 pad[2];
    const CollisionHeader* colHeader = NULL;
    thisv->dyna.actor.flags |= ACTOR_FLAG_4 | ACTOR_FLAG_5 | ACTOR_FLAG_17;
...
void BgHeavyBlock_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgHeavyBlock* thisv = (BgHeavyBlock*)thisx;
    ...
    switch (thisx->params & 0xFF) {
        case HEAVYBLOCK_BIG_PIECE:
            ...
            thisv->actionFunc = BgHeavyBlock_MovePiece;
            BgHeavyBlock_InitPiece(thisv, 1.0f);
            thisv->timer = 120;
            thisx->flags |= ACTOR_FLAG_4;
            thisv->unk_164.y = -50.0f;
            break;
        case HEAVYBLOCK_SMALL_PIECE:
            thisx->draw = BgHeavyBlock_DrawPiece;
            thisv->actionFunc = BgHeavyBlock_MovePiece;
            BgHeavyBlock_InitPiece(thisv, 2.0f);
            thisv->timer = 120;
            thisx->flags |= ACTOR_FLAG_4;
            thisv->unk_164.y = -20.0f;
            break;
        case HEAVYBLOCK_BREAKABLE:
            BgHeavyBlock_SetupDynapoly(thisv, globalCtx);
            ...
            thisv->actionFunc = BgHeavyBlock_Wait;
            break;
        case HEAVYBLOCK_UNBREAKABLE_OUTSIDE_CASTLE:
            BgHeavyBlock_SetupDynapoly(thisv, globalCtx);

            if (Flags_GetSwitch(globalCtx, (thisx->params >> 8) & 0x3F)) {
                thisv->actionFunc = BgHeavyBlock_DoNothing;
...
void BgHeavyBlock_MovePiece(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    ...
    if (!(thisv->pieceFlags & PIECE_FLAG_HIT_FLOOR)) {
        thisx->world.pos.y += thisv->unk_164.y;
        thisx->prevPos.y += thisv->unk_164.y;
        Actor_UpdateBgCheckInfo(globalCtx, thisx, 50.0f, 50.0f, 0.0f, 5);
        thisx->world.pos.y -= thisv->unk_164.y;
        thisx->prevPos.y -= thisv->unk_164.y;
        if (thisx->bgCheckFlags & 1) {
            thisv->pieceFlags |= PIECE_FLAG_HIT_FLOOR;
            ...
            BgHeavyBlock_SetPieceRandRot(thisv, 1.0f);
...
void BgHeavyBlock_SpawnPieces(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    s32 i;
    Vec3f spA4[] = {
    ...
    sinPitch = Math_SinS(thisv->dyna.actor.world.rot.x);
    cosPitch = Math_CosS(thisv->dyna.actor.world.rot.x);
    sinYaw = Math_SinS(thisv->dyna.actor.world.rot.y);
    cosYaw = Math_CosS(thisv->dyna.actor.world.rot.y);

    for (i = 0; i < ARRAY_COUNT(spA4); i++) {
        pos.z = (spA4[i].y * sinPitch) + (spA4[i].z * cosPitch);

        pos.x = thisv->dyna.actor.world.pos.x + (spA4[i].x * cosYaw) + (sinYaw * pos.z);
        pos.y = thisv->dyna.actor.world.pos.y + (spA4[i].y * cosPitch) + (-spA4[i].z * sinPitch);
        pos.z = thisv->dyna.actor.world.pos.z + (spA4[i].x * -sinYaw) + (cosYaw * pos.z);

        Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_HEAVY_BLOCK, pos.x, pos.y, pos.z,
                    thisv->dyna.actor.shape.rot.x, thisv->dyna.actor.shape.rot.y, 0, 2);
        Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_BG_HEAVY_BLOCK, pos.x, pos.y, pos.z,
                    thisv->dyna.actor.shape.rot.x, thisv->dyna.actor.shape.rot.y, 0, 3);
...
void BgHeavyBlock_Wait(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    ...
    if (Actor_HasParent(&thisv->dyna.actor, globalCtx)) {
        ...
        switch (thisv->dyna.actor.params & 0xFF) {
            case HEAVYBLOCK_BREAKABLE:
                OnePointCutscene_Init(globalCtx, 4020, 270, &thisv->dyna.actor, MAIN_CAM);
                break;
            case HEAVYBLOCK_UNBREAKABLE:
                OnePointCutscene_Init(globalCtx, 4021, 220, &thisv->dyna.actor, MAIN_CAM);
                break;
            case HEAVYBLOCK_UNBREAKABLE_OUTSIDE_CASTLE:
                OnePointCutscene_Init(globalCtx, 4022, 210, &thisv->dyna.actor, MAIN_CAM);
                break;
        }
        ...
        thisv->actionFunc = BgHeavyBlock_LiftedUp;
    }
}
...
void BgHeavyBlock_LiftedUp(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->timer < 40) {
        ...
        sinYaw = Math_SinS(thisv->dyna.actor.shape.rot.y);
        zOffset = Rand_CenteredFloat(110.0f);
        cosYaw = Math_CosS(thisv->dyna.actor.shape.rot.y);

        BgHeavyBlock_SpawnDust(globalCtx, (sinYaw * -70.0f) + (thisv->dyna.actor.world.pos.x + xOffset),
                               thisv->dyna.actor.world.pos.y + 10.0f,
                               (cosYaw * -70.0f) + (thisv->dyna.actor.world.pos.z + zOffset), 0.0f, -1.0f, 0.0f, 0xC);
    }
    ...
    if (Actor_HasNoParent(&thisv->dyna.actor, globalCtx)) {
        Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EV_HEAVY_THROW);
        thisv->actionFunc = BgHeavyBlock_Fly;
    }
}
...
void BgHeavyBlock_Fly(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    ...
    pos.y = thisv->dyna.actor.home.pos.y + 1000.0f;
    pos.z = thisv->dyna.actor.home.pos.z;
    raycastResult =
        BgCheck_EntityRaycastFloor4(&globalCtx->colCtx, &thisv->dyna.actor.floorPoly, &bgId, &thisv->dyna.actor, &pos);
    thisv->dyna.actor.floorHeight = raycastResult;

    if (thisv->dyna.actor.home.pos.y <= raycastResult) {
        func_800AA000(0.0f, 0xFF, 0x3C, 4);

        switch (thisv->dyna.actor.params & 0xFF) {
            case HEAVYBLOCK_BREAKABLE:
                BgHeavyBlock_SpawnPieces(thisv, globalCtx);
                ...
                SoundSource_PlaySfxAtFixedWorldPos(globalCtx, &thisv->dyna.actor.world.pos, 30,
            ...
            case HEAVYBLOCK_UNBREAKABLE_OUTSIDE_CASTLE:
                ...
                thisv->actionFunc = BgHeavyBlock_Land;
                Flags_SetSwitch(globalCtx, (thisv->dyna.actor.params >> 8) & 0x3F);
                break;
            case HEAVYBLOCK_UNBREAKABLE:
    ...
    thisv->dyna.actor.shape.rot.x = Math_Atan2S(thisv->dyna.actor.velocity.y, thisv->dyna.actor.speedXZ);
}
...
void BgHeavyBlock_Land(BgHeavyBlock* thisv, GlobalContext* globalCtx) {
    ...
    if (Math_SmoothStepToS(&thisv->dyna.actor.shape.rot.x, 0x8AD0, 6, 2000, 100) != 0) {
        Math_StepToF(&thisv->dyna.actor.speedXZ, 0.0f, 20.0f);
        Math_StepToF(&thisv->dyna.actor.velocity.y, 0.0f, 3.0f);
        thisv->dyna.actor.gravity = 0.0f;
        thisv->dyna.actor.world.pos = thisv->dyna.actor.home.pos;
        Actor_MoveForward(&thisv->dyna.actor);
        thisv->dyna.actor.home.pos = thisv->dyna.actor.world.pos;
        switch (thisv->dyna.actor.params & 0xFF) {
            case HEAVYBLOCK_UNBREAKABLE_OUTSIDE_CASTLE:
...
void BgHeavyBlock_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (BgHeavyBlock_LiftedUp == thisv->actionFunc) {
        Matrix_SetTranslateRotateYXZ(player->leftHandPos.x, player->leftHandPos.y, player->leftHandPos.z,
                                     &thisx->shape.rot);
        Matrix_Translate(-thisv->unk_164.x, -thisv->unk_164.y, -thisv->unk_164.z, MTXMODE_APPLY);
    } else if ((thisx->gravity == 0.0f) && (BgHeavyBlock_Land == thisv->actionFunc)) {
