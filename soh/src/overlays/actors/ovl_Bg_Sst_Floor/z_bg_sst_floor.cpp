...
void BgSstFloor_Init(BgSstFloor* thisv, GlobalContext* globalCtx);
void BgSstFloor_Destroy(BgSstFloor* thisv, GlobalContext* globalCtx);
void BgSstFloor_Update(BgSstFloor* thisv, GlobalContext* globalCtx);
void BgSstFloor_Draw(BgSstFloor* thisv, GlobalContext* globalCtx);
...
void BgSstFloor_Init(BgSstFloor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    BgSstFloor* thisv = (BgSstFloor*)thisx;
...
void BgSstFloor_Update(BgSstFloor* thisx, GlobalContext* globalCtx) {
    ...
    const CollisionHeader* colHeaderName = SEGMENTED_TO_VIRTUAL(&gBongoDrumCol);

    CollisionHeader* colHeader = ResourceMgr_LoadColByName(reinterpret_cast<const char*>(colHeaderName));

    colHeader->vtxList = SEGMENTED_TO_VIRTUAL(colHeader->vtxList);

    if (1) {}

    if (func_80043590(&thisv->dyna) && (thisv->dyna.actor.yDistToPlayer < 1000.0f)) {
    ...
    if (func_8004356C(&thisv->dyna) && (player->fallDistance > 1000.0f)) {
        thisv->dyna.actor.params = 1;
        Audio_PlayActorSound2(&thisv->dyna.actor, NA_SE_EN_SHADEST_TAIKO_HIGH);
    }

    if (thisv->dyna.actor.params == BONGOFLOOR_HIT) {
        Actor* item00 = globalCtx->actorCtx.actorLists[ACTORCAT_MISC].head;
        f32 distFromRim;
        f32 xzDist;

        thisv->drumAmp = 80;
        thisv->dyna.actor.params = BONGOFLOOR_REST;
        thisv->drumPhase = 28;

        if (func_8004356C(&thisv->dyna) && !(player->stateFlags1 & 0x6000)) {
            distFromRim = 600.0f - thisv->dyna.actor.xzDistToPlayer;
            if (distFromRim > 0.0f) {
        ...
        while (item00 != NULL) {
            if ((item00->id == ACTOR_EN_ITEM00) && (item00->world.pos.y == 0.0f)) {
                xzDist = Actor_WorldDistXZToActor(&thisv->dyna.actor, item00);
    ...
    thisv->drumHeight = sinf(thisv->drumPhase * (std::numbers::pi_v<float> / 2)) * (-thisv->drumAmp);
    Math_StepToS(&thisv->drumAmp, 0, 5);
    ...
    colHeader->vtxList[1].y = colHeader->vtxList[0].y = colHeader->vtxList[2].y = colHeader->vtxList[3].y =
        colHeader->vtxList[4].y = colHeader->vtxList[7].y = colHeader->vtxList[9].y = colHeader->vtxList[11].y =
            colHeader->vtxList[13].y = thisv->dyna.actor.home.pos.y + thisv->drumHeight;

    if (thisv->drumPhase != 0) {
        thisv->drumPhase--;
    }
    if (1) {}
...
void BgSstFloor_Draw(BgSstFloor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_Scale(1.0f, thisv->drumHeight * -0.0025f, 1.0f, MTXMODE_APPLY);
