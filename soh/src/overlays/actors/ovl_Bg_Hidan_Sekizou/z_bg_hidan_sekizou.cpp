...
void func_8088CEC0(BgHidanSekizou* thisv, s32 arg1, s16 arg2) {
    s32 i;
    s32 start = arg1 * 3;
    s32 end = start + 3;
    ...
    for (i = start; i < end; i++) {
        ColliderJntSphElement* element = &thisv->collider.elements[i];

        element->dim.worldSphere.center.x = thisv->dyna.actor.home.pos.x + (sp2C * element->dim.modelSphere.center.x) +
                                            (sp30 * element->dim.modelSphere.center.z);
        element->dim.worldSphere.center.y = (s16)thisv->dyna.actor.home.pos.y + element->dim.modelSphere.center.y;
        element->dim.worldSphere.center.z = thisv->dyna.actor.home.pos.z - (sp30 * element->dim.modelSphere.center.x) +
...
void BgHidanSekizou_Init(Actor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    BgHidanSekizou* thisv = (BgHidanSekizou*)thisx;
    s32 i;
    ...
    if (thisv->dyna.actor.params == 0) {
        thisv->unk_168[0] = 36;
        for (i = 0; i < 2; i++) {
            func_8088CEC0(thisv, i, thisv->dyna.actor.shape.rot.y + ((i == 0) ? 0x2000 : -0x2000));
        }
        CollisionHeader_GetVirtual(&gFireTempleStationaryFlamethrowerShortCol, &colHeader);
        thisv->updateFunc = func_8088D720;
    } else {
        thisv->unk_168[0] = thisv->unk_168[1] = thisv->unk_168[2] = thisv->unk_168[3] = 0;
        CollisionHeader_GetVirtual(&gFireTempleStationaryFlamethrowerTallCol, &colHeader);
        thisv->updateFunc = func_8088D434;
    }
    thisv->unk_170 = 0;
    thisv->dyna.bgId = DynaPoly_SetBgActor(globalCtx, &globalCtx->colCtx.dyna, &thisv->dyna.actor, colHeader);
    CollisionCheck_SetInfo(&thisv->dyna.actor.colChkInfo, NULL, &sColChkInfoInit);
}
...
void func_8088D434(BgHidanSekizou* thisv, GlobalContext* globalCtx) {
    ...
    isClose = thisv->dyna.actor.xzDistToPlayer < 300.0f;
    isAligned[0] = fabsf(thisv->dyna.actor.world.pos.x - player->actor.world.pos.x) < 80.0f;
    isAligned[1] = fabsf(thisv->dyna.actor.world.pos.z - player->actor.world.pos.z) < 80.0f;
    phi_s4 = 0;
    for (i = 0; i < 4; i++) {
        s16 diff;
        s16* temp = &thisv->unk_168[i];

        DECR(*temp);
        diff = thisv->dyna.actor.yawTowardsPlayer - i * 0x4000;
        if (isAligned[i % 2] && isClose) {
            if (ABS(diff) <= 0x4000) {
                if (*temp < 4) {
                    *temp = 35 - *temp;
                }
                func_8088CEC0(thisv, (phi_s4 > 1) ? 1 : phi_s4, thisv->dyna.actor.shape.rot.y + i * 0x4000);
                phi_s4++;
            }
        }
    }
    for (i = 3 * phi_s4; i < ARRAY_COUNT(thisv->elements); i++) {
        thisv->collider.elements[i].info.toucherFlags &= ~TOUCH_ON;
...
void func_8088D720(BgHidanSekizou* thisv, GlobalContext* globalCtx) {
    thisv->unk_168[0]--;
    if (thisv->unk_168[0] <= -36) {
        thisv->unk_168[0] = 36;
    }
}

void func_8088D750(BgHidanSekizou* thisv, GlobalContext* globalCtx) {
    s16 phi_a3;

    if (thisv->dyna.actor.xzDistToPlayer > 200.0f) {
        phi_a3 = thisv->dyna.actor.yawTowardsPlayer;
    } else if (thisv->dyna.actor.params == 0) {
        phi_a3 = thisv->dyna.actor.yawTowardsPlayer - thisv->dyna.actor.shape.rot.y;
        if (phi_a3 > 0x2000) {
            phi_a3 = thisv->dyna.actor.shape.rot.y + 0x6000;
        } else if (phi_a3 < -0x2000) {
            phi_a3 = thisv->dyna.actor.shape.rot.y - 0x6000;
        } else if (phi_a3 > 0) {
            phi_a3 = thisv->dyna.actor.shape.rot.y - 0x2000;
        } else {
            phi_a3 = thisv->dyna.actor.shape.rot.y + 0x2000;
        }
    } else {
    ...
    func_8002F71C(globalCtx, &thisv->dyna.actor, 5.0f, phi_a3, 1.0f);
}
...
void BgHidanSekizou_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (thisv->collider.base.atFlags & AT_HIT) {
        ...
        func_8088D750(thisv, globalCtx);
    }

    thisv->updateFunc(thisv, globalCtx);

    if (thisv->dyna.actor.params == 0) {
        if (thisv->unk_168[0] > 0) {
        ...
        if ((thisv->unk_168[0] > 0) || (thisv->unk_168[1] > 0) || (thisv->unk_168[2] > 0) || (thisv->unk_168[3] > 0)) {
            CollisionCheck_SetAT(globalCtx, &globalCtx->colChkCtx, &thisv->collider.base);
            CollisionCheck_SetOC(globalCtx, &globalCtx->colChkCtx, &thisv->collider.base);
            func_8002F974(&thisv->dyna.actor, NA_SE_EV_FIRE_PILLAR - SFX_FLAG);
        }
...
Gfx* func_8088D9F4(GlobalContext* globalCtx, BgHidanSekizou* thisv, s16 arg2, MtxF* arg3, f32 arg4, f32 arg5, s16 arg6,
                   Gfx* arg7) {
    f32 temp_f0;
    f32 temp_f2;
    f32 phi_f12;
    ...
    if (arg2 != 4) {
        phi_f12 = arg2 + ((4 - thisv->unk_170) / 4.0f);
    } else {
        phi_f12 = arg2;
    }
    ...
    arg3->mf_raw.xx = arg3->mf_raw.yy = arg3->mf_raw.zz = (0.7f * phi_f12) + 0.5f;

    temp_f2 = (arg3->mf_raw.xx * 10.0f * phi_f12) + 20.0f;
    arg3->mf_raw.xw = (temp_f2 * arg4) + thisv->dyna.actor.world.pos.x;
    arg3->mf_raw.yw = thisv->dyna.actor.world.pos.y + 30.0f + (.7f * phi_f12);
    arg3->mf_raw.zw = (temp_f2 * arg5) + thisv->dyna.actor.world.pos.z;
    gSPMatrix(arg7++,
...
Gfx* func_8088DC50(GlobalContext* globalCtx, BgHidanSekizou* thisv, s16 arg2, s16 arg3, Gfx* arg4) {
    s32 pad;
    s16 temp_v1;
    s32 phi_s1;
    s32 phi_s2;
    ...
    if (ABS(temp_v1) < 0x4000) {
        for (i = phi_s2 - 1; i >= phi_s1; i--) {
            arg4 = func_8088D9F4(globalCtx, thisv, i, &sp68, temp_f20, temp_f22, arg3, arg4);
        }
    } else {
...
void BgHidanSekizou_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (thisv->dyna.actor.params == 0) {
        if (thisv->unk_168[0] > 0) {
            if ((s16)(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)) - thisv->dyna.actor.shape.rot.y) >= 0) {
                POLY_XLU_DISP = func_8088DC50(globalCtx, thisv, thisv->dyna.actor.shape.rot.y + 0x2000, thisv->unk_168[0],
                                              POLY_XLU_DISP);
                POLY_XLU_DISP = func_8088DC50(globalCtx, thisv, thisv->dyna.actor.shape.rot.y - 0x2000, thisv->unk_168[0],
                                              POLY_XLU_DISP);
            } else {
        ...
        func_8088DE08(Camera_GetCamDirYaw(GET_ACTIVE_CAM(globalCtx)), thisv->dyna.actor.shape.rot.y, sp6C);
        for (i = 0; i < 4; i++) {
            s32 index = sp6C[i];

            if (thisv->unk_168[index] > 0) {
                POLY_XLU_DISP = func_8088DC50(globalCtx, thisv, thisv->dyna.actor.shape.rot.y + index * 0x4000,
                                              thisv->unk_168[index], POLY_XLU_DISP);
            }
        }
    }
