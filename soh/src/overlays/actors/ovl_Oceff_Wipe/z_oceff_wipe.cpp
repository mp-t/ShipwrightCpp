...
void OceffWipe_Init(Actor* thisx, GlobalContext* globalCtx) {
    OceffWipe* thisv = (OceffWipe*)thisx;

    Actor_SetScale(&thisv->actor, 0.1f);
    thisv->timer = 0;
    thisv->actor.world.pos = GET_ACTIVE_CAM(globalCtx)->eye;
...
void OceffWipe_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->timer < 32) {
        z = Math_SinS(thisv->timer << 9) * 1400;
    } else {
        z = 1400;
    }
    ...
    if (thisv->timer >= 80) {
        alphaTable[0] = 0;
        alphaTable[1] = (0x64 - thisv->timer) * 8;
        alphaTable[2] = (0x64 - thisv->timer) * 12;
    } else {
        alphaTable[0] = 0;
        alphaTable[1] = 0xA0;
        alphaTable[2] = 0xFF;
    ...
    if (thisv->actor.params != OCEFF_WIPE_ZL) {
