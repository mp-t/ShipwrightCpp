...
void EnMag_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisv->copyrightAlphaStep = 6;
    thisv->fadeOutAlphaStep = 10;
    ...
    thisv->effectScroll = 0;
    thisv->unk_E30C = 0;

    thisv->effectPrimColor[0] = 0.0f;
    thisv->effectPrimColor[1] = 100.0f;
    thisv->effectPrimColor[2] = 170.0f;
    thisv->effectEnvColor[0] = 0.0f;
    thisv->effectEnvColor[1] = 100.0f;
    thisv->effectEnvColor[2] = 0.0f;

    thisv->effectFadeInTimer = 40;

    thisv->effectFadeInState = thisv->effectPrimLodFrac = thisv->globalState = thisv->effectAlpha = thisv->mainAlpha =
        thisv->subAlpha = thisv->copyrightAlpha = 0.0f;

    if (gSaveContext.unk_13E7 != 0) {
        thisv->mainAlpha = 210;
        thisv->subAlpha = 255;
        thisv->copyrightAlpha = 255;

        thisv->effectPrimLodFrac = 128.0f;
        thisv->effectAlpha = 255.0f;

        thisv->effectPrimColor[0] = 170;
        thisv->effectPrimColor[1] = 255.0f;
        thisv->effectPrimColor[2] = 255.0f;
        thisv->effectEnvColor[0] = 200.0f;
        thisv->effectEnvColor[1] = 255.0f;
        thisv->effectEnvColor[2] = 0;

        gSaveContext.unk_13E7 = 0;
        thisv->globalState = MAG_STATE_DISPLAY;
        sDelayTimer = 20;
...
void EnMag_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.unk_13E7 != 0) {
        ...
        thisv->effectPrimColor[0] = 255.0f;
        thisv->effectPrimColor[1] = 255.0f;
        thisv->effectPrimColor[2] = 170;
        thisv->effectEnvColor[0] = 255.0f;
        thisv->effectEnvColor[1] = 100;
...
void EnMag_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.fileNum != 0xFEDC) {
        if (thisv->globalState < MAG_STATE_DISPLAY) {
        ...
        } else if (thisv->globalState >= MAG_STATE_DISPLAY) {
            if (sDelayTimer == 0) {
                if (CHECK_BTN_ALL(globalCtx->state.input[0].press.button, BTN_START) ||
                    CHECK_BTN_ALL(globalCtx->state.input[0].press.button, BTN_A) ||
                    CHECK_BTN_ALL(globalCtx->state.input[0].press.button, BTN_B)) {
                    ...
                    thisv->copyrightAlphaStep = 15;
                    thisv->fadeOutAlphaStep = 25;
                    thisv->globalState = MAG_STATE_FADE_OUT;
                }
            } else {
                sDelayTimer--;
            }
    ...
    if (thisv->globalState == MAG_STATE_FADE_IN) {
        if (thisv->effectFadeInState == 0) {
            thisv->effectAlpha += 6.375f;
            thisv->effectPrimLodFrac += 0.8f;

            thisv->effectPrimColor[0] += 6.375f;
            thisv->effectPrimColor[1] += 3.875f;
            thisv->effectPrimColor[2] += 2.125f;
            thisv->effectEnvColor[0] += 6.375f;
            thisv->effectEnvColor[1] += 3.875f;

            thisv->effectFadeInTimer--;

            if (thisv->effectFadeInTimer == 0) {
                thisv->effectPrimLodFrac = 32.0f;
                thisv->effectAlpha = 255.0f;
                ...
                thisv->effectFadeInTimer = 40;
                thisv->effectFadeInState = 1;
            }
        } else if (thisv->effectFadeInState == 1) {
            thisv->effectPrimColor[0] += -2.125f;
            thisv->effectEnvColor[0] += -1.375f;

            thisv->effectPrimLodFrac += 2.4f;
            ...
            if (thisv->effectFadeInTimer == 0) {
                ...
                thisv->effectPrimColor[0] = 170.0f;
                thisv->effectEnvColor[0] = 200.0f;

                thisv->effectFadeInTimer = 32;
                thisv->effectFadeInState = 2;
            }
        }

        if (thisv->effectAlpha > 160) {
            thisv->mainAlpha += VREG(5);
            if (thisv->mainAlpha >= 210.0f) {
                thisv->mainAlpha = 210.0f;
            }

            if (thisv->mainAlpha >= 210) {
                thisv->subAlpha += VREG(6);
                if (thisv->subAlpha >= 255.0f) {
                    thisv->subAlpha = 255.0f;
                }

                if (thisv->subAlpha >= 200) {
                    thisv->copyrightAlpha += thisv->copyrightAlphaStep;
                    if (thisv->copyrightAlpha >= 255.0f) {
                        thisv->copyrightAlpha = 255.0f;
                        thisv->globalState = MAG_STATE_DISPLAY;
                        sDelayTimer = 20;
                    }
                }
    ...
    } else if (thisv->globalState == MAG_STATE_FADE_OUT) {
        thisv->effectAlpha -= thisv->fadeOutAlphaStep;
        if (thisv->effectAlpha < 0.0f) {
            thisv->effectAlpha = 0.0f;
        }

        thisv->mainAlpha -= thisv->fadeOutAlphaStep;
        if (thisv->mainAlpha < 0.0f) {
            thisv->mainAlpha = 0.0f;
        }

        thisv->subAlpha -= thisv->fadeOutAlphaStep;
        if (thisv->subAlpha < 0.0f) {
            thisv->subAlpha = 0.0f;
        }

        thisv->copyrightAlpha -= thisv->copyrightAlphaStep;
        if (thisv->copyrightAlpha < 0.0f) {
            thisv->copyrightAlpha = 0.0f;
            thisv->globalState = MAG_STATE_POST_DISPLAY;
        }
    }

    if (thisv->globalState == MAG_STATE_INITIAL) {
        if (Flags_GetEnv(globalCtx, 3)) {
            thisv->effectFadeInTimer = 40;
            thisv->globalState = MAG_STATE_FADE_IN;
        }
    } else if (thisv->globalState == MAG_STATE_DISPLAY) {
...
void EnMag_DrawEffectTextures(Gfx** gfxp, const void* maskTex, const void* effectTex, s16 maskWidth, s16 maskHeight,
    ...
    if (!flag) {
        ...
        gDPSetTileSize(gfx++, 1, 0, thisv->effectScroll & 0x7F, 0x7C, (thisv->effectScroll & 0x7F) + 0x7C);
    }
...
void EnMag_DrawInner(Actor* thisx, GlobalContext* globalCtx, Gfx** gfxp) {
    ...
    gSPSegment(gfx++, 0x06, globalCtx->objectCtx.status[thisv->actor.objBankIndex].segment);

    func_8009457C(&gfx);

    thisv->effectScroll -= 2;
    ...
    gDPSetPrimColor(gfx++, 0, (s16)thisv->effectPrimLodFrac, (s16)thisv->effectPrimColor[0],
                    (s16)thisv->effectPrimColor[1], (s16)thisv->effectPrimColor[2], (s16)thisv->effectAlpha);
    gDPSetEnvColor(gfx++, (s16)thisv->effectEnvColor[0], (s16)thisv->effectEnvColor[1], (s16)thisv->effectEnvColor[2],
                   255);

    if ((s16)thisv->effectPrimLodFrac != 0) {
        for (k = 0, i = 0, rectTop = 0; i < 3; i++, rectTop += 64) {
            for (j = 0, rectLeft = 56; j < 3; j++, k++, rectLeft += 64) {
                EnMag_DrawEffectTextures(&gfx, effectMaskTextures[k], gTitleFlameEffectTex, 64, 64, 32, 32, rectLeft,
                                         rectTop, 64, 64, 1024, 1024, 1, 1, k, thisv);
            }
        }
    }

    gDPSetPrimColor(gfx++, 0, 0, 255, 255, 255, (s16)thisv->mainAlpha);

    if ((s16)thisv->mainAlpha != 0) {
    ...
    if ((s16)thisv->mainAlpha < 100) {
        gDPSetRenderMode(gfx++, G_RM_CLD_SURF, G_RM_CLD_SURF2);
    } else {
        gDPSetRenderMode(gfx++, G_RM_XLU_SURF, G_RM_XLU_SURF2);
    }

    gDPSetPrimColor(gfx++, 0, 0, 0, 0, 0, (s16)thisv->mainAlpha);
    ...
    if ((s16)thisv->mainAlpha != 0) {
        ...
        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, 100, 150, 255, (s16)thisv->mainAlpha);
        ...
        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, 255, 255, 255, (s16)thisv->subAlpha);
    ...
    gDPSetPrimColor(gfx++, 0, 0, (s16)thisv->copyrightAlpha, (s16)thisv->copyrightAlpha, (s16)thisv->copyrightAlpha,
                    (s16)thisv->copyrightAlpha);

    if ((s16)thisv->copyrightAlpha != 0) {
    ...
    } else if (thisv->copyrightAlpha >= 200.0f) {
...
void EnMag_DrawInner(Actor* thisx, GlobalContext* globalCtx, Gfx** gfxp) {
    ...
    if ((s16)thisv->mainAlpha != 0) {
        ...
        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, 200, 200, 150, (s16)thisv->mainAlpha);
