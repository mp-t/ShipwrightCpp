...
void func_808ADEF0(BgSpot03Taki* thisv, GlobalContext* globalCtx);
...
void BgSpot03Taki_ApplyOpeningAlpha(BgSpot03Taki* thisv, s32 bufferIndex) {
    s32 i;
    const char* vtx_name = (bufferIndex == 0) ? SEGMENTED_TO_VIRTUAL(object_spot03_object_Vtx_000800)
                                  : SEGMENTED_TO_VIRTUAL(object_spot03_object_Vtx_000990);
    
    Vtx* vtx = ResourceMgr_LoadVtxByName(vtx_name);

    for (i = 0; i < 5; i++) {
        vtx[i + 10].v.cn[3] = thisv->openingAlpha;
    }
}

void BgSpot03Taki_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgSpot03Taki* thisv = (BgSpot03Taki*)thisx;
    s16 pad;
    const CollisionHeader* colHeader = NULL;

    thisv->switchFlag = (thisv->dyna.actor.params & 0x3F);
    ...
    thisv->bufferIndex = 0;
    thisv->openingAlpha = 255.0f;
    BgSpot03Taki_ApplyOpeningAlpha(thisv, 0);
    BgSpot03Taki_ApplyOpeningAlpha(thisv, 1);
    thisv->actionFunc = func_808ADEF0;
}
...
void func_808ADEF0(BgSpot03Taki* thisv, GlobalContext* globalCtx) {
    if (thisv->state == WATERFALL_CLOSED) {
        if (Flags_GetSwitch(globalCtx, thisv->switchFlag)) {
            thisv->state = WATERFALL_OPENING_ANIMATED;
            thisv->timer = 40;
            OnePointCutscene_Init(globalCtx, 4100, -99, NULL, MAIN_CAM);
        }
    } else if (thisv->state == WATERFALL_OPENING_IDLE) {
        thisv->timer--;
        if (thisv->timer < 0) {
            thisv->state = WATERFALL_OPENING_ANIMATED;
        }
    } else if (thisv->state == WATERFALL_OPENING_ANIMATED) {
        if (thisv->openingAlpha > 0) {
            thisv->openingAlpha -= 5;
            if (thisv->openingAlpha <= 0.0f) {
                func_8003EBF8(globalCtx, &globalCtx->colCtx.dyna, thisv->dyna.bgId);
                thisv->timer = 400;
                thisv->state = WATERFALL_OPENED;
                thisv->openingAlpha = 0;
            }
        }
    } else if (thisv->state == WATERFALL_OPENED) {
        thisv->timer--;
        if (thisv->timer < 0) {
            thisv->state = WATERFALL_CLOSING;
        }
    } else if (thisv->state == WATERFALL_CLOSING) {
        if (thisv->openingAlpha < 255.0f) {
            thisv->openingAlpha += 5.0f;
            if (thisv->openingAlpha >= 255.0f) {
                func_8003EC50(globalCtx, &globalCtx->colCtx.dyna, thisv->dyna.bgId);
                thisv->state = WATERFALL_CLOSED;
                thisv->openingAlpha = 255.0f;
    ...
    BgSpot03Taki_ApplyOpeningAlpha(thisv, thisv->bufferIndex);
}
...
void BgSpot03Taki_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisv->bufferIndex = thisv->bufferIndex == 0;

    if (thisv->state >= WATERFALL_OPENING_IDLE && thisv->state <= WATERFALL_OPENED) {
        Audio_PlaySoundWaterfall(&thisv->dyna.actor.projectedPos, 0.5f);
    } else {
        Audio_PlaySoundWaterfall(&thisv->dyna.actor.projectedPos, 1.0f);
    }
}
