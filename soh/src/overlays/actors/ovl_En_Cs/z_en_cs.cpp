...
void EnCs_ChangeAnim(EnCs* thisv, s32 index, s32* currentIndex) {
    f32 morphFrames;
    ...
    if (sAnimationInfo[index].frameCount >= 0.0f) {
        Animation_Change(&thisv->skelAnime, sAnimationInfo[index].animation, sAnimationInfo[index].frameCount, 0.0f,
                         Animation_GetLastFrame(sAnimationInfo[index].animation), sAnimationInfo[index].mode,
                         morphFrames);
    } else {
        Animation_Change(&thisv->skelAnime, sAnimationInfo[index].animation, sAnimationInfo[index].frameCount,
...
void EnCs_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    ActorShape_Init(&thisv->actor.shape, 0.0f, ActorShadow_DrawCircle, 19.0f);

    SkelAnime_InitFlex(globalCtx, &thisv->skelAnime, &gGraveyardKidSkel, NULL, thisv->jointTable, thisv->morphTable, 16);
    ...
    Animation_Change(&thisv->skelAnime, sAnimationInfo[ENCS_ANIM_0].animation, 1.0f, 0.0f,
    ...
    thisv->unk_1EC = 0; // This variable is unused anywhere else
    thisv->talkState = 0;
    thisv->currentAnimIndex = -1;
    thisv->actor.gravity = -1.0f;

    EnCs_ChangeAnim(thisv, ENCS_ANIM_0, &thisv->currentAnimIndex);

    thisv->actionFunc = EnCs_Walk;
    thisv->walkSpeed = 1.0f;
}
...
s32 EnCs_GetTalkState(EnCs* thisv, GlobalContext* globalCtx) {
    s32 pad;
    s32 pad2;
    s32 talkState = 1;
    ...
    switch (Message_GetState(&globalCtx->msgCtx)) {
        case TEXT_STATE_CHOICE:
            if (Message_ShouldAdvance(globalCtx)) {
                if (globalCtx->msgCtx.choiceIndex == 0) {
                    thisv->actor.textId = 0x2026;
                    EnCs_ChangeAnim(thisv, ENCS_ANIM_3, &thisv->currentAnimIndex);
                    talkState = 2;
                } else {
                    thisv->actor.textId = 0x2024;
                    EnCs_ChangeAnim(thisv, ENCS_ANIM_1, &thisv->currentAnimIndex);
                    talkState = 2;
                }
            }
            break;
        case TEXT_STATE_DONE:
            if (Message_ShouldAdvance(globalCtx)) {
                if (thisv->actor.textId == 0x2026) {
                    Player_UnsetMask(globalCtx);
                    Item_Give(globalCtx, ITEM_SOLD_OUT);
                    gSaveContext.itemGetInf[3] |= 0x400;
                    Rupees_ChangeBy(30);
                    thisv->actor.textId = 0x2027;
                    talkState = 2;
                } else {
                    talkState = 0;
                }
...
s32 EnCs_GetTextID(EnCs* thisv, GlobalContext* globalCtx) {
...
void EnCs_HandleTalking(EnCs* thisv, GlobalContext* globalCtx) {
    s32 pad;
    s16 sp2A;
    s16 sp28;
    ...
    if (thisv->talkState == 2) {
        ...
        thisv->talkState = 1;
    } else if (thisv->talkState == 1) {
        thisv->talkState = EnCs_GetTalkState(thisv, globalCtx);
    } else if (Actor_ProcessTalkRequest(&thisv->actor, globalCtx)) {
        if ((thisv->actor.textId == 0x2022) || ((thisv->actor.textId != 0x2022) && (thisv->actor.textId != 0x2028))) {
            EnCs_ChangeAnim(thisv, ENCS_ANIM_3, &thisv->currentAnimIndex);
        }

        if ((thisv->actor.textId == 0x2023) || (thisv->actor.textId == 0x2028)) {
            EnCs_ChangeAnim(thisv, ENCS_ANIM_1, &thisv->currentAnimIndex);
        }

        if (thisv->actor.textId == 0x2023) {
            func_80078884(NA_SE_SY_TRE_BOX_APPEAR);
        }

        thisv->talkState = 1;
    } else {
        Actor_GetScreenPos(globalCtx, &thisv->actor, &sp2A, &sp28);

        if ((sp2A >= 0) && (sp2A <= 320) && (sp28 >= 0) && (sp28 <= 240) &&
            (func_8002F2CC(&thisv->actor, globalCtx, 100.0f))) {
            thisv->actor.textId = EnCs_GetTextID(thisv, globalCtx);
        }
    }
}
...
s32 EnCs_HandleWalking(EnCs* thisv, GlobalContext* globalCtx) {
    f32 xDiff;
    f32 zDiff;
    Vec3f pathPos;
    s32 waypointCount;
    ...
    EnCs_GetPathPoint(globalCtx->setupPathList, &pathPos, thisv->path, thisv->waypoint);
    xDiff = pathPos.x - thisv->actor.world.pos.x;
    zDiff = pathPos.z - thisv->actor.world.pos.z;
    walkAngle1 = Math_FAtan2F(xDiff, zDiff) * (32768.0f / std::numbers::pi_v<float>);
    thisv->walkAngle = walkAngle1;
    thisv->walkDist = sqrtf((xDiff * xDiff) + (zDiff * zDiff));

    while (thisv->walkDist <= 10.44f) {
        thisv->waypoint++;
        waypointCount = EnCs_GetwaypointCount(globalCtx->setupPathList, thisv->path);

        if ((thisv->waypoint < 0) || (!(thisv->waypoint < waypointCount))) {
            thisv->waypoint = 0;
        }
        ...
        xDiff = pathPos.x - thisv->actor.world.pos.x;
        zDiff = pathPos.z - thisv->actor.world.pos.z;
        walkAngle2 = Math_FAtan2F(xDiff, zDiff) * (32768.0f / std::numbers::pi_v<float>);
        thisv->walkAngle = walkAngle2;
        thisv->walkDist = sqrtf((xDiff * xDiff) + (zDiff * zDiff));
    }

    Math_SmoothStepToS(&thisv->actor.shape.rot.y, thisv->walkAngle, 1, 2500, 0);
    thisv->actor.world.rot.y = thisv->actor.shape.rot.y;
    thisv->actor.speedXZ = thisv->walkSpeed;
...
void EnCs_Walk(EnCs* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->talkState != 0) {
        thisv->actionFunc = EnCs_Talk;
        return;
    }

    if (SkelAnime_Update(&thisv->skelAnime)) {
        animIndex = thisv->currentAnimIndex;
        ...
        if (thisv->talkState == 0) {
            ...
            if (rnd == 0) {
                ...
                thisv->actionFunc = EnCs_Wait;
            } else {
                animIndex = ENCS_ANIM_0;
            }
        }

        EnCs_ChangeAnim(thisv, animIndex, &thisv->currentAnimIndex);
    }

    if (thisv->talkState == 0) {
        curAnimFrame = thisv->skelAnime.curFrame;

        if (((curAnimFrame >= 8) && (curAnimFrame < 16)) || ((curAnimFrame >= 23) && (curAnimFrame < 30)) ||
            (curAnimFrame == 0)) {
            thisv->walkSpeed = 0.0f;
        } else {
            thisv->walkSpeed = 1.0f;
        }

        EnCs_HandleWalking(thisv, globalCtx);
    }
}
...
void EnCs_Talk(EnCs* thisv, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&thisv->skelAnime) != 0) {
        EnCs_ChangeAnim(thisv, thisv->currentAnimIndex, &thisv->currentAnimIndex);
    }
...
void EnCs_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->currentAnimIndex == 0) {
        if (((s32)thisv->skelAnime.curFrame == 9) || ((s32)thisv->skelAnime.curFrame == 23)) {
            Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_CHIBI_WALK);
        }
    } else if (thisv->currentAnimIndex == 1) {
        if (((s32)thisv->skelAnime.curFrame == 10) || ((s32)thisv->skelAnime.curFrame == 25)) {
            Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_CHIBI_WALK);
        }
    } else if ((thisv->currentAnimIndex == 2) && ((s32)thisv->skelAnime.curFrame == 20)) {
        Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_CHIBI_WALK);
    }
    ...
    EnCs_HandleTalking(thisv, globalCtx);

    thisv->eyeBlinkTimer--;

    if (thisv->eyeBlinkTimer < 0) {
        thisv->eyeIndex++;
        ...
        thisv->eyeBlinkTimer = eyeBlinkFrames[thisv->eyeIndex];
    }
}
...
void EnCs_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_DrawFlexOpa(globalCtx, thisv->skelAnime.skeleton, thisv->skelAnime.jointTable, thisv->skelAnime.dListCount,
                          EnCs_OverrideLimbDraw, EnCs_PostLimbDraw, &thisv->actor);
    ...
    if (gSaveContext.itemGetInf[3] & 0x400) {
        ...
        if (childLinkObjectIndex >= 0) {
            ...
            Matrix_Put(&thisv->spookyMaskMtx);
...
void EnCs_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 15) {
        Matrix_MultVec3f(&D_809E2970, &thisv->actor.focus.pos);
        Matrix_Translate(0.0f, -200.0f, 0.0f, MTXMODE_APPLY);
        Matrix_RotateY(0.0f, MTXMODE_APPLY);
        Matrix_RotateX(0.0f, MTXMODE_APPLY);
        Matrix_RotateZ(5.0 * std::numbers::pi_v<float> / 9.0, MTXMODE_APPLY);
        Matrix_Get(&thisv->spookyMaskMtx);
    }
}
