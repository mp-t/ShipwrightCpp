...
void func_808BC65C(BgTreemouth* thisv, GlobalContext* globalCtx);
void func_808BC6F8(BgTreemouth* thisv, GlobalContext* globalCtx);
void func_808BC80C(BgTreemouth* thisv, GlobalContext* globalCtx);
void func_808BC864(BgTreemouth* thisv, GlobalContext* globalCtx);
void BgTreemouth_DoNothing(BgTreemouth* thisv, GlobalContext* globalCtx);
void func_808BC8B8(BgTreemouth* thisv, GlobalContext* globalCtx);
void func_808BC9EC(BgTreemouth* thisv, GlobalContext* globalCtx);
void func_808BCAF0(BgTreemouth* thisv, GlobalContext* globalCtx);
...
void BgTreemouth_SetupAction(BgTreemouth* thisv, BgTreemouthActionFunc actionFunc) {
    thisv->actionFunc = actionFunc;
}

void BgTreemouth_Init(Actor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    BgTreemouth* thisv = (BgTreemouth*)thisx;
    ...
    if ((gSaveContext.sceneSetupIndex < 4) && !LINK_IS_ADULT) {
        BgTreemouth_SetupAction(thisv, func_808BC8B8);
    } else if (LINK_IS_ADULT || (gSaveContext.sceneSetupIndex == 7)) {
        thisv->unk_168 = 0.0f;
        BgTreemouth_SetupAction(thisv, BgTreemouth_DoNothing);
    } else {
        thisv->unk_168 = 1.0f;
        BgTreemouth_SetupAction(thisv, func_808BC6F8);
    }
    ...
    if ((globalCtx->csCtx.state != CS_STATE_IDLE)) {
        ...
        if (npcAction != NULL) {
            if (npcAction->action == 2) {
                BgTreemouth_SetupAction(thisv, func_808BC80C);
...
void func_808BC80C(BgTreemouth* thisv, GlobalContext* globalCtx) {
    thisv->unk_168 += 0.05f;
    if (thisv->unk_168 >= 0.8f) {
        BgTreemouth_SetupAction(thisv, func_808BC864);
    }
}

void func_808BC864(BgTreemouth* thisv, GlobalContext* globalCtx) {
    thisv->unk_168 -= 0.03f;
    if (thisv->unk_168 <= 0.0f) {
        BgTreemouth_SetupAction(thisv, func_808BC65C);
    }
}
    ...
    if ((!(Flags_GetEventChkInf(5))) || LINK_IS_ADULT) {
        if (!LINK_IS_ADULT) {
            if (Flags_GetEventChkInf(0xC)) {
                if (Actor_IsFacingAndNearPlayer(&thisv->dyna.actor, 1658.0f, 0x7530)) {
                    thisv->dyna.actor.flags |= ACTOR_FLAG_0;
                    if (thisv->dyna.actor.isTargeted) {
                        thisv->dyna.actor.flags &= ~ACTOR_FLAG_0;
                        globalCtx->csCtx.segment = D_808BD2A0;
                        gSaveContext.cutsceneTrigger = 1;
                        BgTreemouth_SetupAction(thisv, func_808BC9EC);
                    }
                }
            } else if (Actor_IsFacingAndNearPlayer(&thisv->dyna.actor, 1658.0f, 0x4E20)) {
...
void func_808BC9EC(BgTreemouth* thisv, GlobalContext* globalCtx) {
    ...
    if (globalCtx->csCtx.state == CS_STATE_UNSKIPPABLE_INIT) {
        if (Actor_IsFacingAndNearPlayer(&thisv->dyna.actor, 350.0f, 0x7530)) {
        ...
        if (globalCtx->msgCtx.choiceIndex == 0) {
            ...
            BgTreemouth_SetupAction(thisv, func_808BCAF0);
        } else {
