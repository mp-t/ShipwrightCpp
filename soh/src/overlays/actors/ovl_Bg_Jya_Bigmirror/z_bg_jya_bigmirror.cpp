...
void BgJyaBigmirror_SetRoomFlag(Actor* thisx, GlobalContext* globalCtx) {
    BgJyaBigmirror* thisv = (BgJyaBigmirror*)thisx;
    ...
    if (globalCtx->roomCtx.curRoom.num == 5) {
        thisv->puzzleFlags |= BIGMIR_PUZZLE_IN_STATUE_ROOM;
    } else if (globalCtx->roomCtx.curRoom.num == 0x19) {
        thisv->puzzleFlags |= BIGMIR_PUZZLE_IN_1ST_TOP_ROOM;
    } else if (globalCtx->roomCtx.curRoom.num == 0x1A) {
        thisv->puzzleFlags |= BIGMIR_PUZZLE_IN_2ND_TOP_ROOM;
    }
}
...
void BgJyaBigmirror_HandleCobra(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->puzzleFlags & (BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM)) {
        for (i = 0; i < 2; i++) {
            curSpawnData = &sCobraSpawnData[i];
            curCobraInfo = &thisv->cobraInfo[i];
            if (curCobraInfo->cobra != NULL) {
                ...
                if (curCobraInfo->rotY == curSpawnData->solvedRotY) {
                    thisv->puzzleFlags |= cobraPuzzleFlags[i];
                } else {
                    thisv->puzzleFlags &= ~cobraPuzzleFlags[i];
                }
                ...
                curCobraInfo->cobra = (BgJyaCobra*)Actor_SpawnAsChild(
                    &globalCtx->actorCtx, &thisv->actor, globalCtx, ACTOR_BG_JYA_COBRA, curSpawnData->pos.x,
...
void BgJyaBigmirror_SetBombiwaFlag(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Flags_GetSwitch(globalCtx, 0x29)) {
        thisv->puzzleFlags |= BIGMIR_PUZZLE_BOMBIWA_DESTROYED;
    } else {
        thisv->puzzleFlags &= ~(BIGMIR_PUZZLE_BOMBIWA_DESTROYED);
    }
}
...
void BgJyaBigmirror_HandleMirRay(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((objBankIndex < 0) || (objBankIndex != thisv->mirRayObjIndex)) {
        thisv->lightBeams[2] = NULL;
        thisv->lightBeams[1] = NULL;
        thisv->lightBeams[0] = NULL;
    } else {
        puzzleSolved = !!(thisv->puzzleFlags & (BIGMIR_PUZZLE_IN_STATUE_ROOM | BIGMIR_PUZZLE_IN_1ST_TOP_ROOM));

        if (puzzleSolved) {
            puzzleSolved = !!(thisv->puzzleFlags & BIGMIR_PUZZLE_COBRA2_SOLVED);

            if (puzzleSolved) {
                puzzleSolved = !!(thisv->puzzleFlags & BIGMIR_PUZZLE_COBRA1_SOLVED);
            }
        }
        lightBeamToggles[0] = puzzleSolved; // Only spawn if puzzle solved
        if (1) {}
        lightBeamToggles[1] = lightBeamToggles[2] =
            thisv->puzzleFlags & (BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM);

        for (i = 0; i < 3; i++) {
            if (lightBeamToggles[i]) {
                if ((thisv->lightBeams[i] == NULL) && Object_IsLoaded(&globalCtx->objectCtx, objBankIndex)) {
                    thisv->lightBeams[i] = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_MIR_RAY, sMirRayPoss[i].x,
                                                      sMirRayPoss[i].y, sMirRayPoss[i].z, 0, 0, 0, sMirRayParamss[i]);

                    if (thisv->lightBeams[i] == NULL) {
                ...
                if (thisv->lightBeams[i] != NULL) {
                    Actor_Kill(thisv->lightBeams[i]);
                    thisv->lightBeams[i] = NULL;
                }
            }
        }
    }
    thisv->mirRayObjIndex = objBankIndex;
}
...
void BgJyaBigmirror_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisv->cobraInfo[0].rotY = sCobraSpawnData[0].initRotY;
    thisv->cobraInfo[1].rotY = sCobraSpawnData[1].initRotY;
    thisv->actor.room = -1;
    sIsSpawned = true;
    thisv->spawned = true;
    thisv->mirRayObjIndex = -1;
...
void BgJyaBigmirror_DrawLightBeam(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (lift != NULL) {
        thisv->liftHeight = lift->world.pos.y;
    }
    Matrix_SetTranslateRotateYXZ(thisv->actor.world.pos.x, thisv->actor.world.pos.y + 40.0f, thisv->actor.world.pos.z,
                                 &thisv->actor.shape.rot);
    Matrix_Scale(0.1f, (thisv->liftHeight * -(1.0f / 1280.0f)) + (1779.4f / 1280.0f), 0.1f, MTXMODE_APPLY);
...
void BgJyaBigmirror_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->puzzleFlags & BIGMIR_PUZZLE_IN_1ST_TOP_ROOM) {
        Gfx_DrawDListOpa(globalCtx, gBigMirror3DL);
        Gfx_DrawDListXlu(globalCtx, gBigMirror4DL);
    }

    if ((thisv->puzzleFlags &
         (BIGMIR_PUZZLE_IN_STATUE_ROOM | BIGMIR_PUZZLE_IN_1ST_TOP_ROOM | BIGMIR_PUZZLE_IN_2ND_TOP_ROOM)) &&
        (thisv->puzzleFlags & BIGMIR_PUZZLE_COBRA2_SOLVED) && (thisv->puzzleFlags & BIGMIR_PUZZLE_COBRA1_SOLVED)) {
        BgJyaBigmirror_DrawLightBeam(&thisv->actor, globalCtx);
    }
}
