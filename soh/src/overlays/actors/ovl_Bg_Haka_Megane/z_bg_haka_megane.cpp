...
void func_8087DB24(BgHakaMegane* thisv, GlobalContext* globalCtx);
void func_8087DBF0(BgHakaMegane* thisv, GlobalContext* globalCtx);
void BgHakaMegane_DoNothing(BgHakaMegane* thisv, GlobalContext* globalCtx);
...
void BgHakaMegane_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgHakaMegane* thisv = (BgHakaMegane*)thisx;

    Actor_ProcessInitChain(thisx, sInitChain);
    DynaPolyActor_Init(&thisv->dyna, DPM_UNK);

    if (thisx->params < 3) {
        thisv->objBankIndex = Object_GetIndex(&globalCtx->objectCtx, OBJECT_HAKACH_OBJECTS);
    } else {
        thisv->objBankIndex = Object_GetIndex(&globalCtx->objectCtx, OBJECT_HAKA_OBJECTS);
    }

    if (thisv->objBankIndex < 0) {
        Actor_Kill(thisx);
    } else {
        thisv->actionFunc = func_8087DB24;
    }
}
...
void func_8087DB24(BgHakaMegane* thisv, GlobalContext* globalCtx) {
    ...
    if (Object_IsLoaded(&globalCtx->objectCtx, thisv->objBankIndex)) {
        ...
        thisv->dyna.actor.draw = BgHakaMegane_Draw;
        Actor_SetObjectDependency(globalCtx, &thisv->dyna.actor);
        if (globalCtx->roomCtx.curRoom.showInvisActors) {
            thisv->actionFunc = func_8087DBF0;
            collision = sCollisionHeaders[thisv->dyna.actor.params];
            if (collision != NULL) {
                CollisionHeader_GetVirtual(collision, &colHeader);
                thisv->dyna.bgId = DynaPoly_SetBgActor(globalCtx, &globalCtx->colCtx.dyna, &thisv->dyna.actor, colHeader);
            }
        } else {
            thisv->actionFunc = BgHakaMegane_DoNothing;
        }
    }
}
