...
void EnHoll_NextAction(EnHoll* thisv, GlobalContext* globalCtx);
void func_80A58DD4(EnHoll* thisv, GlobalContext* globalCtx);
void func_80A59014(EnHoll* thisv, GlobalContext* globalCtx);
void func_80A591C0(EnHoll* thisv, GlobalContext* globalCtx);
void func_80A593A4(EnHoll* thisv, GlobalContext* globalCtx);
void func_80A59520(EnHoll* thisv, GlobalContext* globalCtx);
void func_80A59618(EnHoll* thisv, GlobalContext* globalCtx);
 ...
 * [0] : Load the room on thisv side of the loading plane if not already loaded
 ...
 *
 * When traversing a loading plane of thisv kind, it attempts to:
...
void EnHoll_SetupAction(EnHoll* thisv, EnHollActionFunc func) {
...
void EnHoll_ChooseAction(EnHoll* thisv) {
    s32 action;

    action = (thisv->actor.params >> 6) & 7;
    EnHoll_SetupAction(thisv, sActionFuncs[action]);
    if (action != 0) {
        thisv->actor.draw = NULL;
    } else {
        thisv->planeAlpha = 255;
    }
}

void EnHoll_Init(Actor* thisx, GlobalContext* globalCtx) {
    EnHoll* thisv = (EnHoll*)thisx;

    Actor_ProcessInitChain(&thisv->actor, sInitChain);
    EnHoll_ChooseAction(thisv);
    thisv->unk_14F = 0;
}
...
void func_80A58DD4(EnHoll* thisv, GlobalContext* globalCtx) {
    ...
    func_8002DBD0(&thisv->actor, &vec, &player->actor.world.pos);
    thisv->side = (vec.z < 0.0f) ? 0 : 1;
    absZ = fabsf(vec.z);
    if (vec.y > PLANE_Y_MIN && vec.y < PLANE_Y_MAX && fabsf(vec.x) < PLANE_HALFWIDTH &&
        absZ < sHorizTriggerDists[phi_t0][0]) {
        transitionActorIdx = (u16)thisv->actor.params >> 0xA;
        if (absZ > sHorizTriggerDists[phi_t0][1]) {
            if (globalCtx->roomCtx.prevRoom.num >= 0 && globalCtx->roomCtx.status == 0) {
                thisv->actor.room = globalCtx->transiActorCtx.list[transitionActorIdx].sides[thisv->side].room;
                EnHoll_SwapRooms(globalCtx);
                func_80097534(globalCtx, &globalCtx->roomCtx);
            }
        } else {
            thisv->actor.room = globalCtx->transiActorCtx.list[transitionActorIdx].sides[thisv->side ^ 1].room;
            if (globalCtx->roomCtx.prevRoom.num < 0) {
                func_8009728C(globalCtx, &globalCtx->roomCtx, thisv->actor.room);
            } else {
                thisv->planeAlpha = (255.0f / (sHorizTriggerDists[phi_t0][2] - sHorizTriggerDists[phi_t0][3])) *
                                   (absZ - sHorizTriggerDists[phi_t0][3]);
                thisv->planeAlpha = CLAMP(thisv->planeAlpha, 0, 255);
                if (globalCtx->roomCtx.curRoom.num != thisv->actor.room) {
                    EnHoll_SwapRooms(globalCtx);
                }
            }
        }
...
void func_80A59014(EnHoll* thisv, GlobalContext* globalCtx) {
    ...
    func_8002DBD0(&thisv->actor, &vec, (useViewEye) ? &globalCtx->view.eye : &player->actor.world.pos);
    planeHalfWidth = (((thisv->actor.params >> 6) & 7) == 6) ? PLANE_HALFWIDTH : PLANE_HALFWIDTH_2;
    ...
    if (temp || (PLANE_Y_MIN < vec.y && vec.y < PLANE_Y_MAX && fabsf(vec.x) < planeHalfWidth &&
        ...
        s32 transitionActorIdx = (u16)thisv->actor.params >> 0xA;
        s32 side = (vec.z < 0.0f) ? 0 : 1;
        TransitionActorEntry* transitionEntry = &globalCtx->transiActorCtx.list[transitionActorIdx];
        s32 room = transitionEntry->sides[side].room;

        thisv->actor.room = room;
        if (temp) {}
        if (thisv->actor.room != globalCtx->roomCtx.curRoom.num) {
            if (room) {}
            if (func_8009728C(globalCtx, &globalCtx->roomCtx, thisv->actor.room)) {
                EnHoll_SetupAction(thisv, EnHoll_NextAction);
            }
        }
    }
}
...
void func_80A591C0(EnHoll* thisv, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
    f32 absY = fabsf(thisv->actor.yDistToPlayer);
    s32 transitionActorIdx;

    if (thisv->actor.xzDistToPlayer < 500.0f && absY < 700.0f) {
        transitionActorIdx = (u16)thisv->actor.params >> 0xA;
        if (absY < 95.0f) {
        ...
        if (absY < 95.0f) {
            thisv->actor.room = globalCtx->transiActorCtx.list[transitionActorIdx].sides[1].room;
            Math_SmoothStepToF(&player->actor.world.pos.x, thisv->actor.world.pos.x, 1.0f, 50.0f, 10.0f);
            Math_SmoothStepToF(&player->actor.world.pos.z, thisv->actor.world.pos.z, 1.0f, 50.0f, 10.0f);
            if (thisv->actor.room != globalCtx->roomCtx.curRoom.num &&
                func_8009728C(globalCtx, &globalCtx->roomCtx, thisv->actor.room) != 0) {
                EnHoll_SetupAction(thisv, EnHoll_NextAction);
                thisv->unk_14F = 1;
                player->actor.speedXZ = 0.0f;
            }
        }
    } else if (thisv->unk_14F != 0) {
        globalCtx->unk_11E18 = 0;
        thisv->unk_14F = 0;
    }
}
...
void func_80A593A4(EnHoll* thisv, GlobalContext* globalCtx) {
    ...
    if ((thisv->actor.xzDistToPlayer < 120.0f) && (absY = fabsf(thisv->actor.yDistToPlayer), absY < 200.0f)) {
        if (absY < 50.0f) {
            globalCtx->unk_11E18 = 0xFF;
        } else {
        ...
        if (absY > 50.0f) {
            transitionActorIdx = (u16)thisv->actor.params >> 0xA;
            side = (0.0f < thisv->actor.yDistToPlayer) ? 0 : 1;
            thisv->actor.room = globalCtx->transiActorCtx.list[transitionActorIdx].sides[side].room;
            if (thisv->actor.room != globalCtx->roomCtx.curRoom.num &&
                func_8009728C(globalCtx, &globalCtx->roomCtx, thisv->actor.room) != 0) {
                EnHoll_SetupAction(thisv, EnHoll_NextAction);
                thisv->unk_14F = 1;
...
void func_80A59520(EnHoll* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->actor.xzDistToPlayer < 120.0f) {
        absY = fabsf(thisv->actor.yDistToPlayer);
...
void EnHoll_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->planeAlpha != 0) {
        ...
        gDPSetPrimColor(gfxP++, 0, 0, 0, 0, 0, (u8)thisv->planeAlpha);
