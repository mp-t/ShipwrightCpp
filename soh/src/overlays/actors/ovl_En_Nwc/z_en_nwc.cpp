...
void EnNwc_SetUpdate(EnNwc* thisv, EnNwcUpdateFunc updateFunc);
void EnNwc_ChickNoop(EnNwcChick* chick, EnNwc* thisv, GlobalContext* globalCtx);
void EnNwc_ChickBgCheck(EnNwcChick* chick, GlobalContext* globalCtx);
void EnNwc_ChickFall(EnNwcChick* chick, EnNwc* thisv, GlobalContext* globalCtx);
void EnNwc_UpdateChicks(EnNwc* thisv, GlobalContext* globalCtx);
void EnNwc_DrawChicks(EnNwc* thisv, GlobalContext* globalCtx);
void EnNwc_Idle(EnNwc* thisv, GlobalContext* globalCtx);
...
void EnNwc_UpdateChicks(EnNwc* thisv, GlobalContext* globalCtx) {
    ...
    EnNwcChick* chick = thisv->chicks;
    ColliderJntSphElement* element = thisv->collider.elements;
    Vec3f prevChickPos;
    s32 i;
    f32 test;

    prevChickPos.y = 99999.9f;
    for (i = 0; i < thisv->count; i++, prevChickPos = chick->pos, chick++, element++) {
        Math_Vec3f_Copy(&chick->lastPos, &chick->pos);

        chickActionFuncs[chick->type](chick, thisv, globalCtx);
...
void EnNwc_DrawChicks(EnNwc* thisv, GlobalContext* globalCtx) {
    ...
    dList2 = dList1 + 3 * thisv->count + 1;
    dList3 = dList2 + 2 * thisv->count + 1;
    ...
    for (i = 0; i < thisv->count; i++, chick++) {
        if ((chick->type != CHICK_NONE) && (chick->floorPoly != NULL)) {
            ...
            Matrix_Put(&floorMat);
            Matrix_RotateY(chick->rot.y * (std::numbers::pi_v<float> / 0x8000), MTXMODE_APPLY);
...
void EnNwc_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_SetJntSphAllocType1(globalCtx, &thisv->collider, &thisv->actor, &sJntSphInit);
    thisv->count = 16;
    chick = thisv->chicks;
    for (i = 0; i < thisv->count; i++, chick++) {
    ...
    EnNwc_SetUpdate(thisv, EnNwc_Idle);
}
...
void EnNwc_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_FreeJntSph(globalCtx, &thisv->collider);
}

void EnNwc_Idle(EnNwc* thisv, GlobalContext* globalCtx) {
    EnNwc_UpdateChicks(thisv, globalCtx);
}
...
void EnNwc_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    EnNwc_DrawChicks(thisv, globalCtx);
}
