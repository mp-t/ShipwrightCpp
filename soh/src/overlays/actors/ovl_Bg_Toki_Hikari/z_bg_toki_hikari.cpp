...
void BgTokiHikari_DoNothing(BgTokiHikari* thisv, GlobalContext* globalCtx);
void func_808BA018(BgTokiHikari* thisv, GlobalContext* globalCtx);
void func_808BA204(BgTokiHikari* thisv, GlobalContext* globalCtx);
void func_808BA22C(BgTokiHikari* thisv, GlobalContext* globalCtx);
...
void BgTokiHikari_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgTokiHikari* thisv = (BgTokiHikari*)thisx;

    switch (thisv->actor.params) {
        case 0:
            Actor_ProcessInitChain(&thisv->actor, sInitChain);
            thisv->actionFunc = BgTokiHikari_DoNothing;
            break;
        case 1:
            if (!(gSaveContext.eventChkInf[4] & 0x800)) {
                thisv->actionFunc = func_808BA204;
                thisv->unk_14C = 0.0f;
            } else {
...
void BgTokiHikari_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (thisv->actor.params) {
        case 0:
            func_808BA018(thisv, globalCtx);
            break;
        case 1:
            func_808BA2CC(thisv, globalCtx);
            break;
    }
}
...
void func_808BA204(BgTokiHikari* thisv, GlobalContext* globalCtx) {
    if (globalCtx->roomCtx.unk_74[1] != 0) {
        thisv->actionFunc = func_808BA22C;
    }
}

void func_808BA22C(BgTokiHikari* thisv, GlobalContext* globalCtx) {
    if (thisv->unk_14C < 1.0f) {
        thisv->unk_14C += 0.05f;
    } else {
        thisv->unk_14C = 1.0f;
        thisv->actionFunc = func_808BA274;
    }
}

void func_808BA274(BgTokiHikari* thisv, GlobalContext* globalCtx) {
    if (thisv->unk_14C > 0.2f) {
        thisv->unk_14C -= 0.025f;
    } else {
        thisv->unk_14C = 0.0f;
        Actor_Kill(&thisv->actor);
    }
...
void func_808BA2CC(BgTokiHikari* thisv, GlobalContext* globalCtx) {
    ...
    Matrix_Scale(0.32f, 0.32f, thisv->unk_14C * 7.0f, MTXMODE_APPLY);
    Matrix_RotateZ(std::numbers::pi_v<float>, MTXMODE_APPLY);
    func_80093D18(globalCtx->state.gfxCtx);
    Matrix_Push();
    gDPPipeSync(POLY_XLU_DISP++);
    gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, thisv->unk_14C * 255.0f, (u8)(155.0f * thisv->unk_14C) + 100,
                    thisv->unk_14C * 255.0f, thisv->unk_14C * 255.0f);

    gDPSetEnvColor(POLY_XLU_DISP++, (u8)(thisv->unk_14C * 155.0f) + 100, (u8)(255.0f * thisv->unk_14C), 0, 128);
    ...
    Matrix_Push();
    gDPPipeSync(POLY_XLU_DISP++);
    gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 255, 255, (u8)(thisv->unk_14C * 200.0f));

    gDPSetEnvColor(POLY_XLU_DISP++, (u8)(thisv->unk_14C * 255.0f), (u8)(thisv->unk_14C * 255.0f),
                   (u8)(thisv->unk_14C * 255.0f), (u8)(200.0f * thisv->unk_14C));
