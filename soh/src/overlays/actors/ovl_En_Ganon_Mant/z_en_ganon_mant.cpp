...
/**
 * The arrays pointed to by thisv table describe how many pixels should
...
void EnGanonMant_Init(Actor* thisx, GlobalContext* globalCtx) {
    EnGanonMant* thisv = (EnGanonMant*)thisx;
 ...
 */
void EnGanonMant_Tear(EnGanonMant* thisv) {
    s32 pad;
    s16 i;
    s16 areaX;
    s16 areaY;
    ...
    f32 tearAngle = Rand_ZeroFloat(2 * std::numbers::pi_v<float>);
    ...
    for (i = 0; i < 4; i++) {
        thisv->strands[(s16)Rand_ZeroFloat(GANON_MANT_NUM_STRANDS - 0.1f)]
 ...
 */
void EnGanonMant_UpdateStrand(GlobalContext* globalCtx, EnGanonMant* thisv, Vec3f* root, Vec3f* pos, Vec3f* nextPos,
    ...
    for (i = 0; i < GANON_MANT_NUM_JOINTS; i++, pos++, vel++, rot++, nextPos++) {
        if (i == 0) {
            ...
            delta.x = 0;
            delta.z = (thisv->backPush + (sinf((strandNum * (2 * std::numbers::pi_v<float>)) / 2.1f) * thisv->backSwayMagnitude)) *
                      sBackSwayCoefficients[i];
            Matrix_RotateY(thisv->baseYaw, MTXMODE_NEW);
            Matrix_MultVec3f(&delta, &backSwayOffset);

            // Push the cloak out to either side, in a swaying manner
            delta.x = cosf((strandNum * std::numbers::pi_v<float>) / (GANON_MANT_NUM_STRANDS - 1.0f)) * thisv->sideSwayMagnitude *
                      sSideSwayCoefficients[i];
            delta.z = 0;
            Matrix_MultVec3f(&delta, &sideSwayOffset);

            // Calculate position difference
            gravity = thisv->gravity;
            ...
            xDiff = pos->x - thisv->actor.world.pos.x;
            zDiff = pos->z - thisv->actor.world.pos.z;
            if (sqrtf(SQ(xDiff) + SQ(zDiff)) < (sDistMultipliers[i] * thisv->minDist)) {
                yaw = Math_Atan2F(zDiff, xDiff);
                delta.z = thisv->minDist * sDistMultipliers[i];
                Matrix_RotateY(yaw, MTXMODE_NEW);
                Matrix_MultVec3f(&delta, &posStep);
                pos->x = thisv->actor.world.pos.x + posStep.x;
                pos->z = thisv->actor.world.pos.z + posStep.z;
            }
 ...
 */
void EnGanonMant_UpdateVertices(EnGanonMant* thisv) {
    s16 i;
    Vtx* vtx;
    Vtx* vertices;
    MantStrand* strand;
...
void EnGanonMant_Update(Actor* thisx, GlobalContext* globalCtx) {
    EnGanonMant* thisv = (EnGanonMant*)thisx;
    BossGanon* ganon = (BossGanon*)thisv->actor.parent;

    thisv->updateHasRun = true;
    thisv->frameTimer++;

    if (thisv->attachLeftArmTimer == 0.0f) {
    } else {
        thisv->attachLeftArmTimer -= 1.0f;
    }
    if (thisv->attachRightArmTimer != 0.0f) {
        thisv->attachRightArmTimer -= 1.0f;
    }
    if (thisv->attachShouldersTimer != 0.0f) {
        thisv->attachShouldersTimer -= 1.0f;
    }

    thisv->actor.shape.rot.y = ganon->actor.shape.rot.y;

    if (thisv->tearTimer != 0) {
        thisv->tearTimer--;
        EnGanonMant_Tear(thisv);
    }
}

void EnGanonMant_DrawCloak(GlobalContext* globalCtx, EnGanonMant* thisv) {
    s32 pad;
...
void EnGanonMant_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->updateHasRun) {
        // Only run thisv if update has run since last draw

        // Choose endpoints
        if (thisv->attachRightArmTimer != 0.0f) {
            rightPos = &thisv->rightForearmPos;
            leftPos = &thisv->leftShoulderPos;
            thisv->gravity = -13.0f;
        } else if (thisv->attachLeftArmTimer != 0.0f) {
            rightPos = &thisv->rightShoulderPos;
            leftPos = &thisv->leftForearmPos;
            thisv->gravity = -13.0f;
        } else if (thisv->attachShouldersTimer != 0.0f) {
            rightPos = &thisv->rightShoulderPos;
            leftPos = &thisv->leftShoulderPos;
            thisv->gravity = -3.0f;
        } else {
        ...
        thisv->baseYaw = yaw - std::numbers::pi_v<float> / 2.0f;

        for (strandIdx = 0; strandIdx < GANON_MANT_NUM_STRANDS; strandIdx++) {
            Matrix_Push();

            // Calculate root positions for chosen endpoints
            strandOffset.x = sinf((strandIdx * std::numbers::pi_v<float>) / (GANON_MANT_NUM_STRANDS - 1)) * diffHalfDist;
            strandOffset.y = 0;
            strandOffset.z = -cosf((strandIdx * std::numbers::pi_v<float>) / (GANON_MANT_NUM_STRANDS - 1)) * diffHalfDist;
            Matrix_MultVec3f(&strandOffset, &strandDivPos);
            thisv->strands[strandIdx].root.x = midpoint.x + strandDivPos.x;
            thisv->strands[strandIdx].root.y = midpoint.y + strandDivPos.y;
            thisv->strands[strandIdx].root.z = midpoint.z + strandDivPos.z;
            ...
            EnGanonMant_UpdateStrand(globalCtx, thisv, &thisv->strands[strandIdx].root, thisv->strands[strandIdx].joints,
                                     thisv->strands[nextStrandIdx].joints, thisv->strands[strandIdx].rotations,
                                     thisv->strands[strandIdx].velocities, strandIdx);
            Matrix_Pop();
        }
        EnGanonMant_UpdateVertices(thisv);
        thisv->updateHasRun = false;
    }

    EnGanonMant_DrawCloak(globalCtx, thisv);
}
