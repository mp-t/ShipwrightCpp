...
void FireRock_WaitSpawnRocksFromCeiling(EnFireRock* thisv, GlobalContext* globalCtx);
void FireRock_WaitOnFloor(EnFireRock* thisv, GlobalContext* globalCtx);
...
void EnFireRock_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    EnFireRock* thisv = (EnFireRock*)thisx;
    s16 temp;

    thisv->type = thisv->actor.params;
    if (thisv->type != FIRE_ROCK_CEILING_SPOT_SPAWNER) {
        ActorShape_Init(&thisx->shape, 0.0f, ActorShadow_DrawCircle, 15.0f);
        if (thisv->type != FIRE_ROCK_ON_FLOOR) {
            thisv->angularVelocity.x = (Rand_ZeroFloat(10.0f) + 15.0f);
            thisv->angularVelocity.y = (Rand_ZeroFloat(10.0f) + 15.0f);
            thisv->angularVelocity.z = (Rand_ZeroFloat(10.0f) + 15.0f);
        }
    }
    switch (thisv->type) {
        case FIRE_ROCK_CEILING_SPOT_SPAWNER:
            ...
            thisv->actionFunc = FireRock_WaitSpawnRocksFromCeiling;
            break;
        case FIRE_ROCK_ON_FLOOR:
            Actor_SetScale(&thisv->actor, 0.03f);
            Collider_InitCylinder(globalCtx, &thisv->collider);
            Collider_SetCylinder(globalCtx, &thisv->collider, &thisv->actor, &D_80A12CCC);
            // "☆☆☆☆☆ floor rock ☆☆☆☆☆"
            ...
            thisv->actionFunc = FireRock_WaitOnFloor;
            break;
        case FIRE_ROCK_SPAWNED_FALLING1: // spawned by encount2
            // sets unused vars?
            thisv->unk17C.x = (f32)(Rand_CenteredFloat(50.0f) + player->actor.world.pos.x);
            thisv->unk17C.z = (f32)(Rand_CenteredFloat(50.0f) + player->actor.world.pos.z);
        case FIRE_ROCK_SPAWNED_FALLING2: // spawned by encount2 and by the ceilling spawner
            thisv->scale = (Rand_ZeroFloat(2.0f) / 100.0f) + 0.02f;
            Actor_SetScale(&thisv->actor, thisv->scale);
            Collider_InitCylinder(globalCtx, &thisv->collider);
            Collider_SetCylinder(globalCtx, &thisv->collider, &thisv->actor, &D_80A12CA0);
            thisv->actor.world.rot.y = thisv->actor.shape.rot.y = Rand_CenteredFloat(65535.0f);
            thisv->actionFunc = EnFireRock_Fall;
            thisv->actor.shape.shadowScale = 15.0f;
            break;
        case FIRE_ROCK_BROKEN_PIECE1:
            thisv->actor.velocity.y = Rand_ZeroFloat(3.0f) + 4.0f;
            thisv->actor.speedXZ = Rand_ZeroFloat(3.0f) + 3.0f;
            thisv->scale = (Rand_ZeroFloat(1.0f) / 100.0f) + 0.02f;
            Actor_SetScale(&thisv->actor, thisv->scale);
            thisv->actor.gravity = -1.5f;
            Collider_InitCylinder(globalCtx, &thisv->collider);
            Collider_SetCylinder(globalCtx, &thisv->collider, &thisv->actor, &D_80A12CA0);
            thisv->actor.shape.shadowScale = 10.0f;
            thisv->actor.world.rot.y = thisv->actor.shape.rot.y = Rand_CenteredFloat(65535.0f);
            thisv->actionFunc = EnFireRock_Fall;
            break;
        case FIRE_ROCK_BROKEN_PIECE2:
            thisv->actor.velocity.y = Rand_ZeroFloat(3.0f) + 4.0f;
            thisv->actor.speedXZ = Rand_ZeroFloat(3.0f) + 2.0f;
            thisv->scale = (Rand_ZeroFloat(1.0f) / 500.0f) + 0.01f;
            Actor_SetScale(&thisv->actor, thisv->scale);
            thisv->actor.gravity = -1.2f;
            thisv->actor.shape.shadowScale = 5.0f;
            thisv->actor.world.rot.y = thisv->actor.shape.rot.y = Rand_CenteredFloat(65535.0f);
            thisv->actionFunc = EnFireRock_Fall;
            break;
        default:
...
void EnFireRock_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisv->actor.parent != NULL) && (thisv->actor.parent == &thisv->spawner->actor)) {
        EnEncount2* spawner = (EnEncount2*)thisv->actor.parent;
...
void EnFireRock_Fall(EnFireRock* thisv, GlobalContext* globalCtx) {
    ...
    if ((thisv->actor.floorHeight == -10000.0f) || (thisv->actor.world.pos.y < (player->actor.world.pos.y - 200.0f))) {
        Actor_Kill(&thisv->actor);
        return;
    }
    switch (thisv->type) {
        case FIRE_ROCK_SPAWNED_FALLING1:
            if (player->actor.world.pos.y < thisv->actor.world.pos.y) {
                if ((player->actor.world.pos.x > -700.0f) || (player->actor.world.pos.x < 100.0f) ||
                    ...
                    Math_ApproachF(&thisv->actor.world.pos.x, player->actor.world.pos.x, 1.0f, 10.0f);
                    Math_ApproachF(&thisv->actor.world.pos.z, player->actor.world.pos.z, 1.0f, 10.0f);
                }
            }
        case FIRE_ROCK_SPAWNED_FALLING2:
            flamePos.x = Rand_CenteredFloat(20.0f) + thisv->actor.world.pos.x;
            flamePos.y = Rand_CenteredFloat(20.0f) + thisv->actor.world.pos.y;
            flamePos.z = Rand_CenteredFloat(20.0f) + thisv->actor.world.pos.z;
            EffectSsEnFire_SpawnVec3f(globalCtx, &thisv->actor, &flamePos, 100, 0, 0, -1);
            break;
        case FIRE_ROCK_BROKEN_PIECE1:
            if ((globalCtx->gameplayFrames & 3) == 0) {
                Audio_PlayActorSound2(&thisv->actor, NA_SE_EN_VALVAISA_ROCK);
            }
            break;
    }
    if ((thisv->actor.bgCheckFlags & 1) && (thisv->timer == 0)) {
        switch (thisv->type) {
            case FIRE_ROCK_SPAWNED_FALLING1:
            case FIRE_ROCK_SPAWNED_FALLING2:
                func_80033E88(&thisv->actor, globalCtx, 5, 2);
            case FIRE_ROCK_BROKEN_PIECE1:
                Actor_SpawnFloorDustRing(globalCtx, &thisv->actor, &thisv->actor.world.pos, thisv->actor.shape.shadowScale,
                                         1, 8.0f, 500, 10, 0);
                for (i = 0; i < 5; i++) {
                    flamePos.x = Rand_CenteredFloat(20.0f) + thisv->actor.world.pos.x;
                    flamePos.y = thisv->actor.floorHeight;
                    flamePos.z = Rand_CenteredFloat(20.0f) + thisv->actor.world.pos.z;
                    EffectSsEnFire_SpawnVec3f(globalCtx, &thisv->actor, &flamePos, 300, 0, 0, -1);
                }
                thisv->actionFunc = EnFireRock_SpawnMoreBrokenPieces;
                break;
            default:
                Actor_SpawnFloorDustRing(globalCtx, &thisv->actor, &thisv->actor.world.pos, thisv->actor.shape.shadowScale,
                                         3, 8.0f, 200, 10, 0);
    ...
    if (nextRockType != FIRE_ROCK_SPAWNED_FALLING1) {
        for (i = 0; i < 2; i++) {
            spawnedFireRock = (EnFireRock*)Actor_Spawn(
                &globalCtx->actorCtx, globalCtx, ACTOR_EN_FIRE_ROCK, Rand_CenteredFloat(3.0f) + thisv->actor.world.pos.x,
                Rand_CenteredFloat(3.0f) + (thisv->actor.world.pos.y + 10.0f),
                Rand_CenteredFloat(3.0f) + thisv->actor.world.pos.z, 0, 0, 0, nextRockType);
            if (spawnedFireRock != NULL) {
                spawnedFireRock->actor.world.rot.y = thisv->actor.world.rot.y;
                if (i == 0) {
                    spawnedFireRock->actor.shape.rot.y = thisv->actor.shape.rot.y;
                }
                spawnedFireRock->scale = thisv->scale - 0.01f;
            } else {
...
void FireRock_WaitSpawnRocksFromCeiling(EnFireRock* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->actor.xzDistToPlayer < 200.0f) {
        if ((thisv->playerNearby == 0) && (thisv->timer2 == 0)) {
            thisv->timer2 = 30;
            spawnedFireRock = (EnFireRock*)Actor_Spawn(
                &globalCtx->actorCtx, globalCtx, ACTOR_EN_FIRE_ROCK, Rand_CenteredFloat(3.0f) + thisv->actor.world.pos.x,
                thisv->actor.world.pos.y + 10.0f, Rand_CenteredFloat(3.0f) + thisv->actor.world.pos.z, 0, 0, 0,
        ...
        thisv->playerNearby = 1;
    } else {
        thisv->playerNearby = 0;
    }
    if (BREG(0) != 0) {
...
void FireRock_WaitOnFloor(EnFireRock* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->timer2 == 0) {
        ...
        thisv->timer2 = 3 + (s16)Rand_ZeroFloat(3.0f);
...
void EnFireRock_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->type != FIRE_ROCK_CEILING_SPOT_SPAWNER) {
        ...
        thisv->rockRotation.x += thisv->angularVelocity.x;
        thisv->rockRotation.y += thisv->angularVelocity.y;
        thisv->rockRotation.z += thisv->angularVelocity.z;
        thisv->relativePos.y = 3.0f;

        temp = 10.0f + (thisv->scale * 300.0f);
        ...
        if ((thisv->type == FIRE_ROCK_SPAWNED_FALLING1) || (thisv->type == FIRE_ROCK_SPAWNED_FALLING2)) {
            thisx->gravity = -0.3f - (thisv->scale * 7.0f);
        }
        if (thisv->type != FIRE_ROCK_ON_FLOOR) {
        ...
        setCollision = false;
        if (thisv->actionFunc != EnFireRock_SpawnMoreBrokenPieces) {
            if ((thisv->type == FIRE_ROCK_SPAWNED_FALLING1) || (thisv->type == FIRE_ROCK_SPAWNED_FALLING2) ||
                (thisv->type == FIRE_ROCK_BROKEN_PIECE1)) {
        ...
        if (thisv->type == FIRE_ROCK_ON_FLOOR) {
            if (thisv->collider.base.atFlags & 2) {
                ...
                if (thisv->collider.base.at == playerActor) {
...
void EnFireRock_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_Translate(thisx->world.pos.x + thisv->relativePos.x, thisx->world.pos.y + thisv->relativePos.y,
                     thisx->world.pos.z + thisv->relativePos.z, MTXMODE_NEW);
    Matrix_RotateX(DEG_TO_RAD(thisv->rockRotation.x), MTXMODE_APPLY);
    Matrix_RotateY(DEG_TO_RAD(thisv->rockRotation.y), MTXMODE_APPLY);
    Matrix_RotateZ(DEG_TO_RAD(thisv->rockRotation.z), MTXMODE_APPLY);
