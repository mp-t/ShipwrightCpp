...
void BgIceObjects_Idle(BgIceObjects* thisv, GlobalContext* globalCtx);
void BgIceObjects_Slide(BgIceObjects* thisv, GlobalContext* globalCtx);
void BgIceObjects_Reset(BgIceObjects* thisv, GlobalContext* globalCtx);
void BgIceObjects_Stuck(BgIceObjects* thisv, GlobalContext* globalCtx);
...
void BgIceObjects_Init(Actor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    BgIceObjects* thisv = (BgIceObjects*)thisx;
    const CollisionHeader* colHeader = NULL;

    Actor_ProcessInitChain(&thisv->dyna.actor, sInitChain);
    DynaPolyActor_Init(&thisv->dyna, DPM_UNK);
    CollisionHeader_GetVirtual(&object_ice_objects_Col_0003F0, &colHeader);
    Math_Vec3f_Copy(&thisv->targetPos, &thisv->dyna.actor.home.pos);
    thisv->actionFunc = BgIceObjects_Idle;
 ...
 */
void BgIceObjects_SetNextTarget(BgIceObjects* thisv, GlobalContext* globalCtx) {
    s16 x16;
    s16 z16 = 0; // needed to match
    s32 i;

    if ((thisv->dyna.unk_158 == 0) || (thisv->dyna.unk_158 == -0x8000)) {
        x16 = thisv->dyna.actor.world.pos.x;
        for (i = 0; i < 7; i++) {
            if (x16 == sXStarts[i]) {
                z16 = (thisv->dyna.unk_158 == 0) ? sZStops[i][0] : sZStops[i][1];
                thisv->targetPos.z = z16;
                return;
            }
        }
        thisv->targetPos.z = (thisv->dyna.unk_158 == 0) ? -340 : -1260;
    } else {
        z16 = thisv->dyna.actor.world.pos.z;
        for (i = 0; i < 7; i++) {
            if (z16 == sZStarts[i]) {
                x16 = (thisv->dyna.unk_158 == 0x4000) ? sXStops[i][0] : sXStops[i][1];
                thisv->targetPos.x = x16;
                return;
            }
        }
        thisv->targetPos.x = (thisv->dyna.unk_158 == 0x4000) ? -860 : -1780;
    }
}
 ...
 */
void BgIceObjects_CheckPits(BgIceObjects* thisv, GlobalContext* globalCtx) {
    ...
    if ((thisx->velocity.y > 0.0f) || ((thisx->world.pos.x <= -1660.0f) && (thisx->world.pos.z <= -1060.0f)) ||
        ...
        if (Math_StepToF(&thisx->world.pos.y, -300.0f, thisx->velocity.y)) {
            ...
            thisv->actionFunc = BgIceObjects_Reset;
        }
    }
}
...
void BgIceObjects_Idle(BgIceObjects* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->dyna.unk_150 != 0.0f) {
        ...
        if ((thisv->dyna.unk_150 > 0.0f) && !Player_InCsMode(globalCtx)) {
            BgIceObjects_SetNextTarget(thisv, globalCtx);
            if (Actor_WorldDistXZToPoint(thisx, &thisv->targetPos) > 1.0f) {
                thisx->flags |= ACTOR_FLAG_4;
                func_8002DF54(globalCtx, thisx, 8);
                thisx->params = 1;
                thisv->actionFunc = BgIceObjects_Slide;
            }
        }
        thisv->dyna.unk_150 = 0.0f;
    }
    if (thisx->velocity.y > 0.0f) {
        BgIceObjects_CheckPits(thisv, globalCtx);
    }
}
...
void BgIceObjects_Slide(BgIceObjects* thisv, GlobalContext* globalCtx) {
    ...
    if (atTarget) {
        ...
        if ((fabsf(thisx->world.pos.x + 1387.0f) < 1.0f) && (fabsf(thisx->world.pos.z + 260.0f) < 1.0f)) {
            thisv->actionFunc = BgIceObjects_Stuck;
        } else {
            thisv->actionFunc = BgIceObjects_Idle;
        }
...
void BgIceObjects_Reset(BgIceObjects* thisv, GlobalContext* globalCtx) {
    ...
    if (Math_StepToF(&thisx->world.pos.y, thisx->home.pos.y, 1.0f)) {
        thisx->flags &= ~ACTOR_FLAG_4;
        Math_Vec3f_Copy(&thisv->targetPos, &thisx->home.pos);
        thisv->actionFunc = BgIceObjects_Idle;
        thisx->speedXZ = 0.0f;
    }
}
