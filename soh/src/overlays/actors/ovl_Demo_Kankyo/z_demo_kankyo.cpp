...
void DemoKankyo_SetupType(DemoKankyo* thisv, GlobalContext* globalCtx);
void DemoKankyo_UpdateClouds(DemoKankyo* thisv, GlobalContext* globalCtx);
void DemoKankyo_UpdateRock(DemoKankyo* thisv, GlobalContext* globalCtx);
void DemoKankyo_DoNothing2(DemoKankyo* thisv, GlobalContext* globalCtx);
void DemoKankyo_UpdateDoorOfTime(DemoKankyo* thisv, GlobalContext* globalCtx);
void DemoKankyo_DoNothing(DemoKankyo* thisv, GlobalContext* globalCtx);
void DemoKankyo_KillDoorOfTimeCollision(DemoKankyo* thisv, GlobalContext* globalCtx);
...
static Color_RGB8 sSparkleEnvColors[] = {
    { 0, 200, 0 },   { 255, 50, 0 },  { 0, 150, 255 }, { 255, 150, 0 }, // only thisv one is used
    { 0, 255, 255 }, { 200, 255, 0 },
};
...
void DemoKankyo_SetupAction(DemoKankyo* thisv, DemoKankyoActionFunc actionFunc) {
...
void DemoKankyo_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    s16 i;
    s32 objBankIndex = Object_GetIndex(&globalCtx->objectCtx, sObjIds[thisv->actor.params]);
    ...
    switch (thisv->actor.params) {
        ...
        case DEMOKANKYO_ROCK_5:
            globalCtx->roomCtx.curRoom.segment = NULL;
            thisv->actor.scale.x = thisv->actor.scale.y = thisv->actor.scale.z = Rand_ZeroOne() * 0.5f + 0.5f;
            thisv->unk_150[0].unk_0.x = Rand_ZeroOne() * 3.0f + 1.0f;
            thisv->unk_150[0].unk_0.y = Rand_ZeroOne() * 3.0f + 1.0f;
            thisv->unk_150[0].unk_0.z = Rand_ZeroOne() * 3.0f + 1.0f;
            break;
        case DEMOKANKYO_CLOUDS:
            for (i = 0; i < 30; i++) {
                thisv->unk_150[i].unk_20 = Rand_ZeroOne() * 65535.0f;
                thisv->unk_150[i].unk_18 = Rand_ZeroOne() * 100.0f + 60.0f;
            }
            break;
        case DEMOKANKYO_DOOR_OF_TIME:
            thisv->actor.scale.x = thisv->actor.scale.y = thisv->actor.scale.z = 1.0f;
            thisv->unk_150[0].unk_18 = 0.0f;
            if (!(gSaveContext.eventChkInf[4] & 0x800)) {
                Actor_SpawnAsChild(&globalCtx->actorCtx, &thisv->actor, globalCtx, ACTOR_DOOR_TOKI,
        ...
        case DEMOKANKYO_WARP_IN:
            ...
            thisv->actor.room = -1;
            thisv->warpTimer = 35;
            thisv->sparkleCounter = 0;
            thisv->actor.scale.x = thisv->actor.scale.y = thisv->actor.scale.z = 1.0f;
            if (thisv->actor.params == DEMOKANKYO_WARP_OUT) {
    ...
    for (i = 0; i < 30; i++) {
        thisv->unk_150[i].unk_22 = 0;
    }
    DemoKankyo_SetupAction(thisv, DemoKankyo_SetupType);
}
...
void DemoKankyo_SetupType(DemoKankyo* thisv, GlobalContext* globalCtx) {
    ...
    if (thisv->actor.objBankIndex == thisv->objBankIndex) {
        switch (thisv->actor.params) {
            case DEMOKANKYO_ROCK_1:
            case DEMOKANKYO_ROCK_2:
            case DEMOKANKYO_ROCK_3:
            case DEMOKANKYO_ROCK_4:
            case DEMOKANKYO_ROCK_5:
                DemoKankyo_SetupAction(thisv, DemoKankyo_UpdateRock);
                break;
            case DEMOKANKYO_CLOUDS:
                DemoKankyo_SetupAction(thisv, DemoKankyo_UpdateClouds);
                break;
            case DEMOKANKYO_DOOR_OF_TIME:
                if (Flags_GetEnv(globalCtx, 2)) {
                    DemoKankyo_SetupAction(thisv, DemoKankyo_UpdateDoorOfTime);
                }
                break;
            case DEMOKANKYO_WARP_OUT:
                ...
                if (thisv->warpTimer < 21 && thisv->warpTimer >= 15) {
                    temp = (thisv->warpTimer - 15.0f) / 5.0f;
                    globalCtx->envCtx.fillScreen = true;
                    globalCtx->envCtx.screenFillColor[3] = 255 - 255 * temp;
                }
                if (thisv->warpTimer < 15 && thisv->warpTimer >= 4) {
                    temp = (thisv->warpTimer - 4.0f) / 10.0f;
                    globalCtx->envCtx.fillScreen = true;
                    globalCtx->envCtx.screenFillColor[3] = 255 * temp;
                }
                if (thisv->warpTimer == 15) {
                    player->actor.draw = NULL;
                }
                if ((u32)thisv->warpTimer != 0) {
                    thisv->warpTimer--;
                }
                if (thisv->warpTimer == 1) {
                    ...
                    DemoKankyo_SetupAction(thisv, DemoKankyo_DoNothing);
                }
                break;
            case DEMOKANKYO_WARP_IN:
                if (globalCtx->sceneNum == SCENE_TOKINOMA) {
                ...
                gSaveContext.cutsceneTrigger = 1;
                DemoKankyo_SetupAction(thisv, DemoKankyo_DoNothing2);
                break;
            case DEMOKANKYO_BLUE_RAIN:
            case DEMOKANKYO_SPARKLES:
                break;
...
void DemoKankyo_SetRockPos(DemoKankyo* thisv, GlobalContext* globalCtx, s32 params) {
    ...
    thisv->actor.world.pos.x = ((endPos.x - startPos.x) * temp_f0) + startPos.x;
    thisv->actor.world.pos.y = ((endPos.y - startPos.y) * temp_f0) + startPos.y;
    thisv->actor.world.pos.z = ((endPos.z - startPos.z) * temp_f0) + startPos.z;
}

void DemoKankyo_UpdateRock(DemoKankyo* thisv, GlobalContext* globalCtx) {
    if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[thisv->actor.params - 2] != NULL) {
        DemoKankyo_SetRockPos(thisv, globalCtx, thisv->actor.params - 2);
    }
    thisv->unk_150[0].unk_C.x += thisv->unk_150[0].unk_0.x;
    thisv->unk_150[0].unk_C.y += thisv->unk_150[0].unk_0.y;
    thisv->unk_150[0].unk_C.z += thisv->unk_150[0].unk_0.z;
}
...
void DemoKankyo_UpdateClouds(DemoKankyo* thisv, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 30; i++) {
        thisv->unk_150[i].unk_20 += (s16)thisv->unk_150[i].unk_18;
    }
}

void DemoKankyo_UpdateDoorOfTime(DemoKankyo* thisv, GlobalContext* globalCtx) {
    Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_STONE_STATUE_OPEN - SFX_FLAG);
    thisv->unk_150[0].unk_18 += 1.0f;
    if (thisv->unk_150[0].unk_18 >= 102.0f) {
        Audio_PlayActorSound2(&thisv->actor, NA_SE_EV_STONEDOOR_STOP);
        gSaveContext.eventChkInf[4] |= 0x800;
        Actor_Kill(thisv->actor.child);
        DemoKankyo_SetupAction(thisv, DemoKankyo_KillDoorOfTimeCollision);
    }
}
            ...
            case DEMOKANKYO_BLUE_RAIN_2:
                if (globalCtx->sceneNum == SCENE_TOKINOMA) {
                    if (!Flags_GetEnv(globalCtx, 1)) {
                        break;
                    } else if (!Actor_IsFacingAndNearPlayer(&thisv->actor, 300.0f, 0x7530)) {
                        break;
                    } else {
                        if (!LINK_IS_ADULT) {
...
void func_80989B54(Actor* thisx, GlobalContext* globalCtx, s16 i) {
    ...
    switch (globalCtx->sceneNum) {
        case SCENE_HIRAL_DEMO:
            thisv->unk_150[i].unk_0.x = (Rand_ZeroOne() - 0.5f) * 500.0f;
            thisv->unk_150[i].unk_0.y = 500.0f;
            thisv->unk_150[i].unk_0.z = (Rand_ZeroOne() - 0.5f) * 500.0f;
            break;
        case SCENE_TOKINOMA:
            thisv->unk_150[i].unk_C.x = 0.0f;
            thisv->unk_150[i].unk_C.y = 0.0f;
            thisv->unk_150[i].unk_C.z = 0.0f;
            thisv->unk_150[i].unk_0.x = (Rand_ZeroOne() - 0.5f) * 180.0f;
            thisv->unk_150[i].unk_0.y = 10.0f;
            thisv->unk_150[i].unk_0.z = (Rand_ZeroOne() - 0.5f) * 180.0f;
            break;
        case SCENE_SPOT00:
            thisv->unk_150[i].unk_0.x = (Rand_ZeroOne() - 0.5f) * 600.0f;
            thisv->unk_150[i].unk_0.y = -500.0f;
            thisv->unk_150[i].unk_0.z = (Rand_ZeroOne() - 0.5f) * 600.0f;
            break;
    }
    thisv->unk_150[i].unk_18 = Rand_ZeroOne() * (D_8098CF80 * 4.0f) + D_8098CF80;
}
...
void DemoKankyo_DrawRain(Actor* thisx, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 30; i++) {
        ...
        if (globalCtx->sceneNum != SCENE_TOKINOMA) {
            thisv->unk_150[i].unk_C.x = globalCtx->view.eye.x + (dx / norm) * 350.0f;
            thisv->unk_150[i].unk_C.y = globalCtx->view.eye.y + (dy / norm) * 80.0f;
            thisv->unk_150[i].unk_C.z = globalCtx->view.eye.z + (dz / norm) * 350.0f;
        }

        switch (thisv->unk_150[i].unk_22) {
            case 0:
                func_80989B54(thisx, globalCtx, i);
                if (gSaveContext.entranceIndex == 0x00A0) { // Cutscene Map
                    thisv->unk_150[i].unk_0.y = Rand_ZeroOne() * 500.0f;
                } else {
                    thisv->unk_150[i].unk_0.y = Rand_ZeroOne() * -500.0f;
                }
                thisv->unk_150[i].unk_22++;
                break;
            case 1:
                temp_f12_2 = globalCtx->view.eye.y + (dy / norm) * 150.0f;
                if (gSaveContext.entranceIndex == 0x00A0) { // Cutscene Map
                    thisv->unk_150[i].unk_0.y -= thisv->unk_150[i].unk_18;
                } else {
                    thisv->unk_150[i].unk_0.y += thisv->unk_150[i].unk_18;
                }
                if (gSaveContext.entranceIndex == 0x00A0) { // Cutscene Map
                    if (thisv->unk_150[i].unk_C.y + thisv->unk_150[i].unk_0.y < temp_f12_2 - 300.0f) {
                        thisv->unk_150[i].unk_22++;
                    }
                } else if (gSaveContext.entranceIndex == 0x00CD) { // Hyrule Field
                    if (temp_f12_2 + 300.0f < thisv->unk_150[i].unk_C.y + thisv->unk_150[i].unk_0.y) {
                        thisv->unk_150[i].unk_22++;
                    }
                } else {
                    if (1000.0f < thisv->unk_150[i].unk_C.y + thisv->unk_150[i].unk_0.y) {
                        thisv->unk_150[i].unk_22++;
                    }
                }
                break;
        ...
        Matrix_Translate(thisv->unk_150[i].unk_C.x + thisv->unk_150[i].unk_0.x,
                         thisv->unk_150[i].unk_C.y + thisv->unk_150[i].unk_0.y,
                         thisv->unk_150[i].unk_C.z + thisv->unk_150[i].unk_0.z, MTXMODE_NEW);
        if (gSaveContext.entranceIndex != 0x00A0) { // Cutscene Map
            Matrix_RotateX(std::numbers::pi_v<float>, MTXMODE_APPLY);
        }
        ...
        for (j = 0; j < 5; j++) {
            ...
            if (globalCtx->sceneNum != SCENE_TOKINOMA) {
                if (thisv->unk_150[i].unk_0.x >= 0.0f) {
                    translateX = -j * 1500.0f;
                } else {
                    translateX = j * 1500.0f;
                }
                if (thisv->unk_150[i].unk_0.z >= 0.0f) {
...
void DemoKankyo_DrawRock(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_RotateX(DEG_TO_RAD(thisv->unk_150[0].unk_C.x), MTXMODE_APPLY);
    Matrix_RotateY(DEG_TO_RAD(thisv->unk_150[0].unk_C.y), MTXMODE_APPLY);
    Matrix_RotateZ(DEG_TO_RAD(thisv->unk_150[0].unk_C.z), MTXMODE_APPLY);
...
void DemoKankyo_DrawClouds(Actor* thisx, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 30; i++) {
        dx = -(Math_SinS(thisv->unk_150[i].unk_20 - 0x8000) * 120.0f) * (30.0f + (i / 30.0f) * 10.0f);
        dy = Math_CosS(thisv->unk_150[i].unk_20 - 0x8000) * 5.0f + 1200.0f;
        dz = (Math_CosS(thisv->unk_150[i].unk_20 - 0x8000) * 120.0f) * (30.0f + (i / 30.0f) * 10.0f);
...
void DemoKankyo_DrawDoorOfTime(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80093D18(globalCtx->state.gfxCtx);
    Matrix_Translate(-thisv->unk_150[0].unk_18, 0.0f, 0.0f, MTXMODE_APPLY);
    ...
    Matrix_Translate(thisv->unk_150[0].unk_18 + thisv->unk_150[0].unk_18, 0.0f, 0.0f, MTXMODE_APPLY);
...
void DemoKankyo_DrawWarpSparkles(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->sparkleCounter < 30) {
        thisv->sparkleCounter += 2;
    }
    for (i = thisv->sparkleCounter - 1; i >= 0; i--) {
        temp_f22 = 1.0f - (i / (f32)thisv->sparkleCounter);

        switch (thisv->unk_150[i].unk_22) {
            case 0:
                thisv->unk_150[i].unk_20 = 0;
                thisv->unk_150[i].unk_1C = 0;
                thisv->unk_150[i].unk_0.x = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f22);
                thisv->unk_150[i].unk_0.y = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f22);
                thisv->unk_150[i].unk_0.z = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f22);
                thisv->unk_150[i].unk_23 = 0;
                thisv->unk_150[i].unk_22++;
            case 1:
                if (thisv->actor.params == DEMOKANKYO_WARP_OUT) {
                    if (func_800BB2B4(&camPos, &sWarpRoll, &sWarpFoV, sWarpOutCameraPoints, &thisv->unk_150[i].unk_20,
                                      &thisv->unk_150[i].unk_1C) != 0) {
                        thisv->unk_150[i].unk_22++;
                    }
                    ...
                    Audio_PlaySoundGeneral(NA_SE_EV_LINK_WARP_OUT - SFX_FLAG, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                           &D_801333E8);
                    if (func_800BB2B4(&camPos, &sWarpRoll, &sWarpFoV, sWarpInCameraPoints, &thisv->unk_150[i].unk_20,
                                      &thisv->unk_150[i].unk_1C) != 0) {
                        thisv->unk_150[i].unk_22++;
                    }
                    if (D_8098CF84 < globalCtx->csCtx.frames && thisv->actor.params == DEMOKANKYO_WARP_OUT) {
                        thisv->unk_150[i].unk_22++;
                    }
                }
            ...
            case 2:
                ...
                } else if (i + 1 == thisv->sparkleCounter && globalCtx->csCtx.state == CS_STATE_IDLE) {
        ...
        thisv->unk_150[i].unk_C.x = D_8098CF98.x;
        thisv->unk_150[i].unk_C.y = D_8098CF98.y;
        thisv->unk_150[i].unk_C.z = D_8098CF98.z;

        switch (thisv->unk_150[i].unk_23) {
            case 0:
                thisv->unk_150[i].unk_18 = Rand_ZeroOne();
                thisv->unk_150[i].unk_23++;
            case 1:
                Math_SmoothStepToF(&thisv->unk_150[i].unk_18, 1.0f, 0.5f, 0.4f, 0.2f);
                if (thisv->unk_150[i].unk_18 >= 1.0f) {
                    thisv->unk_150[i].unk_23 = 2;
                }
                break;
            case 2:
                Math_SmoothStepToF(&thisv->unk_150[i].unk_18, 0.0f, 0.5f, 0.3f, 0.2f);
                if (thisv->unk_150[i].unk_18 <= 0.0f) {
                    thisv->unk_150[i].unk_0.x = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f22);
                    thisv->unk_150[i].unk_0.y = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f22);
                    thisv->unk_150[i].unk_0.z = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f22);
                    thisv->unk_150[i].unk_18 = 0.0f;
                    thisv->unk_150[i].unk_23 = 1;
                }
                break;
        }

        translateX = thisv->unk_150[i].unk_C.x + thisv->unk_150[i].unk_0.x;
        translateY = thisv->unk_150[i].unk_C.y + thisv->unk_150[i].unk_0.y;
        ...
        if (thisv->unk_150[i].unk_22 < 2) {
            ...
            Matrix_Scale(thisv->unk_150[i].unk_18 * (0.018f * temp_f22), thisv->unk_150[i].unk_18 * (0.018f * temp_f22),
                         thisv->unk_150[i].unk_18 * (0.018f * temp_f22), MTXMODE_APPLY);
...
void DemoKankyo_DrawSparkles(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->sparkleCounter < 20) {
        thisv->sparkleCounter++;
    }

    for (i = thisv->sparkleCounter - 1; i >= 0; i--) {
        temp_f20 = 1.0f - (i / (f32)thisv->sparkleCounter);

        switch (thisv->unk_150[i].unk_22) {
            case 0:
                thisv->unk_150[i].unk_20 = 0;
                thisv->unk_150[i].unk_1C = 0;
                thisv->unk_150[i].unk_0.x = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f20);
                thisv->unk_150[i].unk_0.y = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f20);
                thisv->unk_150[i].unk_0.z = (s16)((Rand_ZeroOne() - 0.5f) * 16.0f * temp_f20);
                thisv->unk_150[i].unk_23 = 0;
                thisv->unk_150[i].unk_22++;
            case 1:
                if (func_800BB2B4(&camPos, &sSparklesRoll, &sSparklesFoV, sSparklesCameraPoints,
                                  &thisv->unk_150[i].unk_20, &thisv->unk_150[i].unk_1C) != 0) {
                    thisv->unk_150[i].unk_22++;
                }
                Actor_GetWorld(&posRot, &thisv->actor);
                DemoKankyo_Vec3fAddPosRot(&posRot, &camPos, &D_8098CFB8);
                break;
            case 2:
                if (i + 1 == thisv->sparkleCounter && globalCtx->csCtx.state == CS_STATE_IDLE) {
                    Actor_Kill(&thisv->actor);
                }
                break;
        }

        thisv->unk_150[i].unk_C.x = D_8098CFB8.x;
        thisv->unk_150[i].unk_C.y = D_8098CFB8.y;
        thisv->unk_150[i].unk_C.z = D_8098CFB8.z;
        ...
        switch (thisv->unk_150[i].unk_23) {
            ...
            case 1:
                Math_SmoothStepToF(&thisv->unk_150[i].unk_18, 1.0f, 0.5f, 0.4f, 0.2f);
                if (1.0f <= thisv->unk_150[i].unk_18) {
                    thisv->unk_150[i].unk_23 = 2;
                }
                break;
            case 2:
        ...
        if (thisv->unk_150[i].unk_22 < 2) {
            Matrix_Translate(translateX, translateY, translateZ, MTXMODE_NEW);
            scale = thisv->unk_150[i].unk_18 * (0.02f * temp_f20);
