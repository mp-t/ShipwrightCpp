...
// Setup action
void func_80A9EFE0(EnMThunder* thisv, EnMThunderActionFunc actionFunc) {
    thisv->actionFunc = actionFunc;
}

void EnMThunder_Init(Actor* thisx, GlobalContext* globalCtx2) {
    GlobalContext* globalCtx = globalCtx2;
    EnMThunder* thisv = (EnMThunder*)thisx;
    Player* player = GET_PLAYER(globalCtx);

    Collider_InitCylinder(globalCtx, &thisv->collider);
    Collider_SetCylinder(globalCtx, &thisv->collider, &thisv->actor, &D_80AA0420);
    thisv->unk_1C7 = (thisv->actor.params & 0xFF) - 1;
    Lights_PointNoGlowSetInfo(&thisv->lightInfo, thisv->actor.world.pos.x, thisv->actor.world.pos.y,
                              thisv->actor.world.pos.z, 255, 255, 255, 0);
    thisv->lightNode = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &thisv->lightInfo);
    thisv->collider.dim.radius = 0;
    thisv->collider.dim.height = 40;
    thisv->collider.dim.yShift = -20;
    thisv->unk_1C4 = 8;
    thisv->unk_1B4 = 0.0f;
    thisv->actor.world.pos = player->bodyPartsPos[0];
    thisv->unk_1AC = 0.0f;
    thisv->unk_1BC = 0.0f;
    thisv->actor.shape.rot.y = player->actor.shape.rot.y + 0x8000;
    thisv->actor.room = -1;
    ...
    if (player->stateFlags2 & 0x20000) {
        if (!gSaveContext.magicAcquired || gSaveContext.unk_13F0 ||
            (((thisv->actor.params & 0xFF00) >> 8) &&
             !(func_80087708(globalCtx, (thisv->actor.params & 0xFF00) >> 8, 0)))) {
        ...
        thisv->unk_1CA = 1;
        thisv->collider.info.toucher.dmgFlags = D_80AA044C[thisv->unk_1C7];
        thisv->unk_1C6 = 1;
        thisv->unk_1C9 = ((thisv->unk_1C7 == 1) ? 2 : 4);
        func_80A9EFE0(thisv, func_80A9F9B4);
        thisv->unk_1C4 = 8;
        Audio_PlaySoundGeneral(NA_SE_IT_ROLLING_CUT_LV1, &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0,
                               &D_801333E8);
        thisv->unk_1AC = 1.0f;
    } else {
        func_80A9EFE0(thisv, func_80A9F408);
    }
...
void func_80A9F350(EnMThunder* thisv, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
...
void func_80A9F408(EnMThunder* thisv, GlobalContext* globalCtx) {
    ...
    thisv->unk_1B8 = player->unk_858;
    thisv->actor.world.pos = player->bodyPartsPos[0];
    thisv->actor.shape.rot.y = player->actor.shape.rot.y + 0x8000;

    if (thisv->unk_1CA == 0) {
        if (player->unk_858 >= 0.1f) {
            if ((gSaveContext.unk_13F0) || (((thisv->actor.params & 0xFF00) >> 8) &&
                                            !(func_80087708(globalCtx, (thisv->actor.params & 0xFF00) >> 8, 4)))) {
                func_80A9F350(thisv, globalCtx);
                func_80A9EFE0(thisv, func_80A9F350);
                thisv->unk_1C8 = 0;
                thisv->unk_1BC = 0.0;
                thisv->unk_1AC = 0.0f;
                return;
    ...
    if (player->stateFlags2 & 0x20000) {
        ...
        if (player->unk_858 <= 0.15f) {
            ...
            player->stateFlags2 &= ~0x20000;
            if ((thisv->actor.params & 0xFF00) >> 8) {
            ...
            if (player->unk_858 < 0.85f) {
                ...
                thisv->collider.info.toucher.dmgFlags = D_80AA0458[thisv->unk_1C7];
                thisv->unk_1C6 = 0;
                thisv->unk_1C9 = ((thisv->unk_1C7 == 1) ? 4 : 8);
            }

            func_80A9EFE0(thisv, func_80A9F9B4);
            thisv->unk_1C4 = 8;
            Audio_PlaySoundGeneral(sSfxIds[thisv->unk_1C6], &player->actor.projectedPos, 4, &D_801333E0, &D_801333E0,
                                   &D_801333E8);
            thisv->unk_1AC = 1.0f;
            return;
        }
    ...
    if (player->unk_858 > 0.15f) {
        ...
        if (thisv->actor.child == NULL) {
            Actor_SpawnAsChild(&globalCtx->actorCtx, &thisv->actor, globalCtx, ACTOR_EFF_DUST, thisv->actor.world.pos.x,
                               thisv->actor.world.pos.y, thisv->actor.world.pos.z, 0, thisv->actor.shape.rot.y, 0,
                               thisv->unk_1C7 + 2);
        }
        thisv->unk_1BC += ((((player->unk_858 - 0.15f) * 1.5f) - thisv->unk_1BC) * 0.5f);

    } else if (player->unk_858 > .1f) {
        thisv->unk_1C8 = (s32)((player->unk_858 - .1f) * 255.0f * 20.0f);
        thisv->unk_1AC = (player->unk_858 - .1f) * 10.0f;
    } else {
        thisv->unk_1C8 = 0;
    }
...
void func_80A9F938(EnMThunder* thisv, GlobalContext* globalCtx) {
    if (thisv->unk_1C4 < 2) {
        if (thisv->unk_1C8 < 40) {
            thisv->unk_1C8 = 0;
        } else {
    ...
    thisv->unk_1B4 += 2.0f * thisv->unk_1B0;

    if (thisv->unk_1BC < thisv->unk_1AC) {
        thisv->unk_1BC += ((thisv->unk_1AC - thisv->unk_1BC) * 0.1f);
    } else {
        thisv->unk_1BC = thisv->unk_1AC;
    }
}
...
void func_80A9F9B4(EnMThunder* thisv, GlobalContext* globalCtx) {
    ...
    if (Math_StepToF(&thisv->unk_1AC, 0.0f, 1 / 16.0f)) {
        Actor_Kill(&thisv->actor);
    } else {
        Math_SmoothStepToF(&thisv->actor.scale.x, (s32)thisv->unk_1C9, 0.6f, 0.8f, 0.0f);
        Actor_SetScale(&thisv->actor, thisv->actor.scale.x);
        thisv->collider.dim.radius = (thisv->actor.scale.x * 25.0f);
        Collider_UpdateCylinder(&thisv->actor, &thisv->collider);
        CollisionCheck_SetAT(globalCtx, &globalCtx->colChkCtx, &thisv->collider.base);
    }

    if (thisv->unk_1C4 > 0) {
        thisv->actor.world.pos.x = player->bodyPartsPos[0].x;
        thisv->actor.world.pos.z = player->bodyPartsPos[0].z;
        thisv->unk_1C4--;
    }

    if (thisv->unk_1AC > 0.6f) {
        thisv->unk_1B0 = 1.0f;
    } else {
        thisv->unk_1B0 = thisv->unk_1AC * (5.0f / 3.0f);
    }

    func_80A9F938(thisv, globalCtx);
...
void EnMThunder_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80A9F314(globalCtx, thisv->unk_1BC);
    blueRadius = thisv->unk_1AC;
    redGreen = (u32)(blueRadius * 255.0f) & 0xFF;
    Lights_PointNoGlowSetInfo(&thisv->lightInfo, thisv->actor.world.pos.x, thisv->actor.world.pos.y,
                              thisv->actor.world.pos.z, redGreen, redGreen, (u32)(blueRadius * 100.0f),
...
void EnMThunder_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    switch (thisv->unk_1C6) {
        case 0:
        case 1:
            gSPSegment(POLY_XLU_DISP++, 0x08,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0xFF - ((u8)(s32)(thisv->unk_1B4 * 30) & 0xFF), 0,
                                        0x40, 0x20, 1, 0xFF - ((u8)(s32)(thisv->unk_1B4 * 20) & 0xFF), 0, 8, 8));
            break;
    }

    switch (thisv->unk_1C6) {
        case 0:
            gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 255, 255, 170, (u8)(thisv->unk_1B0 * 255));
            gSPDisplayList(POLY_XLU_DISP++, gSpinAttack3DL);
            gSPDisplayList(POLY_XLU_DISP++, gSpinAttack4DL);
            break;
        case 1:
            gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 170, 255, 255, (u8)(thisv->unk_1B0 * 255));
    ...
    if (thisv->unk_1B8 >= 0.85f) {
        phi_f14 = (D_80AA046C[(globalCtx->gameplayFrames & 7)] * 6.0f) + 1.0f;
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 255, 255, 170, thisv->unk_1C8);
        gDPSetEnvColor(POLY_XLU_DISP++, 255, 100, 0, 128);
        phi_t1 = 0x28;
    } else {
        phi_f14 = (D_80AA046C[globalCtx->gameplayFrames & 7] * 2.0f) + 1.0f;
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0x80, 170, 255, 255, thisv->unk_1C8);
