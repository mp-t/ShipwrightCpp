...
void BgVbSima_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    s32 pad;
    BgVbSima* thisv = (BgVbSima*)thisx;
    BossFd* bossFd = (BossFd*)thisv->dyna.actor.parent;
    f32 minus1 = -1.0f;
    ...
    if (!Flags_GetClear(globalCtx, globalCtx->roomCtx.curRoom.num)) {
        ...
        if (signal == VBSIMA_COLLAPSE) {
            Math_SmoothStepToF(&thisv->dyna.actor.world.pos.y, -1000.0f, 1.0f, 1.5f, 0.0f);
            thisv->dyna.actor.world.pos.z += 2.0f * Math_CosS(thisv->shakeTimer * 0x8000);
            thisv->dyna.actor.shape.rot.x = (s16)Math_SinS(thisv->shakeTimer * 0x7000) * 0x37;
            thisv->dyna.actor.shape.rot.z = (s16)Math_SinS(thisv->shakeTimer * 0x5000) * 0x37;
        ...
        if (bossFd->platformSignal != VBSIMA_STAND) {
            ...
            for (i1 = 0; i1 < 10; i1++) {
                ...
                splashPos.x = thisv->dyna.actor.world.pos.x + edgeX;
                splashPos.y = -80.0f;
                splashPos.z = thisv->dyna.actor.world.pos.z + edgeZ;
