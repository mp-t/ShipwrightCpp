...
void EnHy_InitImpl(EnHy* thisv, GlobalContext* globalCtx);
void func_80A7134C(EnHy* thisv, GlobalContext* globalCtx);
void func_80A71530(EnHy* thisv, GlobalContext* globalCtx);
void func_80A711B4(EnHy* thisv, GlobalContext* globalCtx);
void func_80A712C0(EnHy* thisv, GlobalContext* globalCtx);
void func_80A710F8(EnHy* thisv, GlobalContext* globalCtx);
void func_80A7127C(EnHy* thisv, GlobalContext* globalCtx);
...
s32 EnHy_FindSkelAndHeadObjects(EnHy* thisv, GlobalContext* globalCtx) {
    u8 headInfoIndex = sModelInfo[thisv->actor.params & 0x7F].headInfoIndex;
    u8 skelInfoIndex2 = sModelInfo[thisv->actor.params & 0x7F].skelInfoIndex2;
    u8 skelInfoIndex1 = sModelInfo[thisv->actor.params & 0x7F].skelInfoIndex1;

    thisv->objBankIndexSkel1 = Object_GetIndex(&globalCtx->objectCtx, sSkeletonInfo[skelInfoIndex1].objectId);
    if (thisv->objBankIndexSkel1 < 0) {
        return false;
    }

    thisv->objBankIndexSkel2 = Object_GetIndex(&globalCtx->objectCtx, sSkeletonInfo[skelInfoIndex2].objectId);
    if (thisv->objBankIndexSkel2 < 0) {
        return false;
    }

    thisv->objBankIndexHead = Object_GetIndex(&globalCtx->objectCtx, sHeadInfo[headInfoIndex].objectId);
    if (thisv->objBankIndexHead < 0) {
        return false;
    }
...
s32 EnHy_AreSkelAndHeadObjectsLoaded(EnHy* thisv, GlobalContext* globalCtx) {
    if (!Object_IsLoaded(&globalCtx->objectCtx, thisv->objBankIndexSkel1)) {
        return false;
    }

    if (!Object_IsLoaded(&globalCtx->objectCtx, thisv->objBankIndexSkel2)) {
        return false;
    }

    if (!Object_IsLoaded(&globalCtx->objectCtx, thisv->objBankIndexHead)) {
        return false;
    }
...
s32 EnHy_FindOsAnimeObject(EnHy* thisv, GlobalContext* globalCtx) {
    thisv->objBankIndexOsAnime = Object_GetIndex(&globalCtx->objectCtx, OBJECT_OS_ANIME);

    if (thisv->objBankIndexOsAnime < 0) {
        return false;
    }
...
s32 EnHy_IsOsAnimeObjectLoaded(EnHy* thisv, GlobalContext* globalCtx) {
    if (!Object_IsLoaded(&globalCtx->objectCtx, thisv->objBankIndexOsAnime)) {
        return false;
    }
...
void func_80A6F7CC(EnHy* thisv, GlobalContext* globalCtx, s32 getItemId) {
    thisv->unkGetItemId = getItemId;
...
u16 func_80A6F810(GlobalContext* globalCtx, Actor* thisx) {
    ...
    u16 textId = Text_GetFaceReaction(globalCtx, (thisv->actor.params & 0x7F) + 37);

    if (textId != 0) {
        if ((thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_5) {
            player->exchangeItemId = EXCH_ITEM_BLUE_FIRE;
        }
        return textId;
    }

    switch (thisv->actor.params & 0x7F) {
        case ENHY_TYPE_AOB:
            if (globalCtx->sceneNum == SCENE_KAKARIKO) {
                return (thisv->unk_330 & 0x800) ? 0x508D : ((gSaveContext.infTable[12] & 0x800) ? 0x508C : 0x508B);
        ...
        case ENHY_TYPE_BOB_18:
            if (!LINK_IS_ADULT) {
                ...
                return (thisv->unk_330 & 0x800) ? 0x5062 : ((gSaveContext.infTable[22] & 0x10) ? 0x5061 : 0x5060);
            }
    ...
    switch (Message_GetState(&globalCtx->msgCtx)) {
        ...
        case TEXT_STATE_DONE_FADING:
            switch (thisv->actor.textId) {
                ...
                case 0x709F:
                    if (!thisv->unk_215) {
                        Audio_PlaySoundGeneral(thisv->actor.textId == 0x709F ? NA_SE_SY_CORRECT_CHIME : NA_SE_SY_ERROR,
                ...
                case 0x70F2:
                case 0x70F3:
                    if (thisv->skelAnime.animation != &gObjOsAnim_33B4) {
                        Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_23);
        ...
        case TEXT_STATE_CLOSING:
            switch (thisv->actor.textId) {
                ...
                case 0x70F2:
                case 0x70F3:
                    Rupees_ChangeBy(beggarRewards[thisv->actor.textId - 0x70F0]);
                    Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_17);
                ...
                case 0x709E:
                    ...
                    break;
                case 0x709F:
                    func_80A6F7CC(thisv, globalCtx, (gSaveContext.infTable[25] & 2) ? GI_RUPEE_BLUE : GI_HEART_PIECE);
                    thisv->actionFunc = func_80A714C4;
                    break;
            }
            return 0;
...
void EnHy_UpdateEyes(EnHy* thisv) {
    if (DECR(thisv->nextEyeIndexTimer) == 0) {
        u8 headInfoIndex = sModelInfo[thisv->actor.params & 0x7F].headInfoIndex;

        thisv->curEyeIndex++;
        if ((sHeadInfo[headInfoIndex].eyeTextures != NULL) &&
            (sHeadInfo[headInfoIndex].eyeTextures[thisv->curEyeIndex] == NULL)) {
            thisv->nextEyeIndexTimer = Rand_S16Offset(30, 30);
            thisv->curEyeIndex = 0;
        }
    }
}

void EnHy_InitCollider(EnHy* thisv) {
    u8 type = thisv->actor.params & 0x7F;

    thisv->collider.dim.radius = sColliderInfo[type].radius;
    thisv->collider.dim.height = sColliderInfo[type].height;
}

void EnHy_InitSetProperties(EnHy* thisv) {
    u8 type = thisv->actor.params & 0x7F;

    thisv->actor.shape.shadowScale = sInit2Info[type].shadowScale;
    Actor_SetScale(&thisv->actor, sInit2Info[type].scale);
    thisv->actor.targetMode = sInit2Info[type].targetMode;
    thisv->modelOffset = sInit2Info[type].modelOffset;
    thisv->unkRange = sInit2Info[type].unkRange;
    thisv->unkRange += thisv->collider.dim.radius;
}

void EnHy_UpdateCollider(EnHy* thisv, GlobalContext* globalCtx) {
    Vec3s pos;

    pos.x = thisv->actor.world.pos.x;
    pos.y = thisv->actor.world.pos.y;
    pos.z = thisv->actor.world.pos.z;
    pos.x += sColliderInfo[thisv->actor.params & 0x7F].offset.x;
    pos.y += sColliderInfo[thisv->actor.params & 0x7F].offset.y;
    pos.z += sColliderInfo[thisv->actor.params & 0x7F].offset.z;
    thisv->collider.dim.pos = pos;
    CollisionCheck_SetOC(globalCtx, &globalCtx->colChkCtx, &thisv->collider.base);
}

void func_80A70834(EnHy* thisv, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
        ...
        if (!Inventory_HasSpecificBottle(ITEM_BLUE_FIRE) && !Inventory_HasSpecificBottle(ITEM_BUG) &&
            ...
            switch (func_8002F368(globalCtx)) {
                ...
                case EXCH_ITEM_LETTER_RUTO:
                    thisv->actor.textId = 0x70EF;
                    break;
                default:
                    if (Player_GetMask(globalCtx) == PLAYER_MASK_NONE) {
                        thisv->actor.textId = 0x70ED;
                    }
                    break;
            }
        } else {
            switch (func_8002F368(globalCtx)) {
                case EXCH_ITEM_BLUE_FIRE:
                    thisv->actor.textId = 0x70F0;
                    break;
                case EXCH_ITEM_FISH:
                    thisv->actor.textId = 0x70F1;
                    break;
                case EXCH_ITEM_BUG:
                    thisv->actor.textId = 0x70F2;
                    break;
                default:
                    if (Player_GetMask(globalCtx) == PLAYER_MASK_NONE) {
                        thisv->actor.textId = 0x700C;
                    }
                    break;
            }
        }
...
void func_80A70978(EnHy* thisv, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
    s16 phi_a3;
    ...
    switch (thisv->actor.params & 0x7F) {
        ...
        case ENHY_TYPE_BOJ_9:
        case ENHY_TYPE_BOJ_10:
            phi_a3 = (thisv->unk_1E8.unk_00 == 0) ? 1 : 2;
            break;
        case ENHY_TYPE_BOJ_12:
            phi_a3 = 1;
            break;
        ...
        case ENHY_TYPE_AOB:
        case ENHY_TYPE_BOB_18:
            phi_a3 = (thisv->unk_1E8.unk_00 == 0) ? 2 : 4;
            break;
        default:
            phi_a3 = 2;
            break;
    ...
    if (LINK_IS_ADULT) {
        thisv->unk_1E8.unk_14 = sInit1Info[thisv->actor.params & 0x7F].unkValueAdult;
    } else {
        thisv->unk_1E8.unk_14 = sInit1Info[thisv->actor.params & 0x7F].unkValueChild;
    }

    func_80034A14(&thisv->actor, &thisv->unk_1E8, sInit1Info[thisv->actor.params & 0x7F].unkPresetIndex, phi_a3);

    if (func_800343CC(globalCtx, &thisv->actor, &thisv->unk_1E8.unk_00, thisv->unkRange, func_80A6F810, func_80A70058)) {
        func_80A70834(thisv, globalCtx);
    }
}

s32 EnHy_ShouldSpawn(EnHy* thisv, GlobalContext* globalCtx) {
    switch (globalCtx->sceneNum) {
        case SCENE_SPOT01:
            if (!((thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_9 || (thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_10 ||
                  (thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_12 || (thisv->actor.params & 0x7F) == ENHY_TYPE_AHG_2 ||
                  (thisv->actor.params & 0x7F) == ENHY_TYPE_BJI_7)) {
                return true;
            } else if (!LINK_IS_ADULT) {
                return true;
            } else if ((thisv->actor.params & 0x7F) != ENHY_TYPE_BOJ_12 && IS_NIGHT) {
                return false;
            } else {
                return true;
            }
        case SCENE_LABO:
            if ((thisv->actor.params & 0x7F) != ENHY_TYPE_BOJ_10) {
                return true;
            } else if (LINK_IS_CHILD) {
                return false;
            } else if ((thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_10 && IS_DAY) {
                return false;
            } else {
                return true;
            }
        case SCENE_IMPA:
            if ((thisv->actor.params & 0x7F) != ENHY_TYPE_AOB) {
                return true;
            } else if (IS_DAY) {
                return false;
            } else {
        ...
        case SCENE_KAKARIKO:
            ...
            } else if (!((thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_9 ||
                         (thisv->actor.params & 0x7F) == ENHY_TYPE_AHG_2 ||
                         (thisv->actor.params & 0x7F) == ENHY_TYPE_BJI_7)) {
                return true;
            } else if (IS_DAY) {
                return false;
        ...
        case SCENE_MARKET_ALLEY_N:
            if ((thisv->actor.params & 0x7F) != ENHY_TYPE_BOJ_14) {
                return true;
            } else if (IS_NIGHT) {
                return false;
...
void EnHy_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisv->actor.params & 0x7F) >= ENHY_TYPE_MAX || !EnHy_FindOsAnimeObject(thisv, globalCtx) ||
        !EnHy_FindSkelAndHeadObjects(thisv, globalCtx)) {
        Actor_Kill(&thisv->actor);
    }

    if (!EnHy_ShouldSpawn(thisv, globalCtx)) {
        Actor_Kill(&thisv->actor);
    }

    thisv->actionFunc = EnHy_InitImpl;
}
...
void EnHy_InitImpl(EnHy* thisv, GlobalContext* globalCtx) {
    if (EnHy_IsOsAnimeObjectLoaded(thisv, globalCtx) && EnHy_AreSkelAndHeadObjectsLoaded(thisv, globalCtx)) {
        thisv->actor.objBankIndex = thisv->objBankIndexSkel1;
        gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[thisv->actor.objBankIndex].segment);
        SkelAnime_InitFlex(globalCtx, &thisv->skelAnime,
                           sSkeletonInfo[sModelInfo[thisv->actor.params & 0x7F].skelInfoIndex1].skeleton, NULL,
                           thisv->jointTable, thisv->morphTable, 16);
        ActorShape_Init(&thisv->actor.shape, 0.0f, ActorShadow_DrawCircle, 0.0f);
        gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[thisv->objBankIndexOsAnime].segment);
        Collider_InitCylinder(globalCtx, &thisv->collider);
        Collider_SetCylinder(globalCtx, &thisv->collider, &thisv->actor, &sColCylInit);
        EnHy_InitCollider(thisv);
        CollisionCheck_SetInfo2(&thisv->actor.colChkInfo, NULL, &sColChkInfoInit);
        Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, sModelInfo[thisv->actor.params & 0x7F].animInfoIndex);

        if ((globalCtx->sceneNum == SCENE_MARKET_ALLEY) || (globalCtx->sceneNum == SCENE_MARKET_DAY)) {
            thisv->actor.flags &= ~ACTOR_FLAG_4;
            thisv->actor.uncullZoneScale = 0.0f;
        }

        if (globalCtx->sceneNum == SCENE_KAKARIKO) {
            thisv->unk_330 = gSaveContext.eventChkInf[6];
        }

        EnHy_InitSetProperties(thisv);
        thisv->path = Path_GetByIndex(globalCtx, (thisv->actor.params & 0x780) >> 7, 15);
        ...
        switch (thisv->actor.params & 0x7F) {
            case ENHY_TYPE_BOJ_3:
                ...
                thisv->actionFunc = func_80A711B4;
                break;
            case ENHY_TYPE_BJI_7:
                thisv->pathReverse = false;
                thisv->actionFunc = func_80A712C0;
                break;
            case ENHY_TYPE_AOB:
                if (globalCtx->sceneNum == SCENE_MARKET_DAY) {
                    thisv->actionFunc = func_80A710F8;
                    break;
                }
                // fall-through
            case ENHY_TYPE_COB:
            ...
            case ENHY_TYPE_BJI_19:
            case ENHY_TYPE_AHG_20:
                thisv->actionFunc = func_80A7127C;
                break;
            case ENHY_TYPE_BOJ_5:
            case ENHY_TYPE_BOJ_9:
            case ENHY_TYPE_BOJ_10:
            case ENHY_TYPE_CNE_11:
            case ENHY_TYPE_BOJ_12:
                thisv->actionFunc = EnHy_DoNothing;
                break;
            default:
                Actor_Kill(&thisv->actor);
                break;
...
void func_80A710F8(EnHy* thisv, GlobalContext* globalCtx) {
    if (thisv->unk_1E8.unk_00 != 0) {
        if (thisv->skelAnime.animation != &gObjOsAnim_0BFC) {
            Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_26);
        }
    } else if (gSaveContext.eventInf[3] & 1) {
        if (thisv->skelAnime.animation != &gObjOsAnim_0FE4) {
            Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_25);
        }
    } else if (thisv->skelAnime.animation != &gObjOsAnim_12E8) {
        Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_24);
    }
}
...
void func_80A711B4(EnHy* thisv, GlobalContext* globalCtx) {
    ...
    distSq = Path_OrientAndGetDistSq(&thisv->actor, thisv->path, thisv->waypoint, &yaw);
    Math_SmoothStepToS(&thisv->actor.world.rot.y, yaw, 10, 1000, 1);
    thisv->actor.shape.rot = thisv->actor.world.rot;

    if ((distSq > 0.0f) && (distSq < 1000.0f)) {
        thisv->waypoint++;
        if (thisv->waypoint > (thisv->path->count - 1)) {
            thisv->waypoint = 0;
        }
    }
}
...
void func_80A7127C(EnHy* thisv, GlobalContext* globalCtx) {
    func_80034F54(globalCtx, thisv->unk_21C, thisv->unk_23C, 16);
}
...
void func_80A712C0(EnHy* thisv, GlobalContext* globalCtx) {
    if ((thisv->actor.xzDistToPlayer <= 100.0f) && (thisv->path != NULL)) {
        Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_7);
        thisv->actor.speedXZ = 0.4f;
        thisv->actionFunc = func_80A7134C;
    }
...
void func_80A7134C(EnHy* thisv, GlobalContext* globalCtx) {
    ...
    if ((thisv->skelAnime.animation == &gObjOsAnim_2160) && (thisv->unk_1E8.unk_00 != 0)) {
        Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_8);
    }

    if ((thisv->skelAnime.animation == &gObjOsAnim_265C) && (thisv->unk_1E8.unk_00 == 0)) {
        Animation_ChangeByInfo(&thisv->skelAnime, sAnimationInfo, ENHY_ANIM_7);
    }
    ...
    if (!(distSq <= 0.0f) && !(distSq >= 1000.0f)) {
        if (!thisv->pathReverse) {
            thisv->waypoint++;
            if (thisv->waypoint > (thisv->path->count - 1)) {
                thisv->pathReverse = true;
                thisv->waypoint = thisv->path->count - 2;
            }
        } else {
            thisv->waypoint--;
            if (thisv->waypoint < 0) {
...
void func_80A714C4(EnHy* thisv, GlobalContext* globalCtx) {
    if (Actor_HasParent(&thisv->actor, globalCtx)) {
        thisv->actionFunc = func_80A71530;
    } else {
        func_8002F434(&thisv->actor, globalCtx, thisv->unkGetItemId, thisv->actor.xzDistToPlayer + 1.0f,
                      fabsf(thisv->actor.yDistToPlayer) + 1.0f);
    }
}
...
void EnHy_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->actionFunc != EnHy_InitImpl) {
        gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[thisv->objBankIndexOsAnime].segment);
        SkelAnime_Update(&thisv->skelAnime);
        EnHy_UpdateEyes(thisv);
...
s32 EnHy_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 15) {
        gSPSegment(POLY_OPA_DISP++, 0x06, globalCtx->objectCtx.status[thisv->objBankIndexHead].segment);
        gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[thisv->objBankIndexHead].segment);
        i = sModelInfo[thisv->actor.params & 0x7F].headInfoIndex;
    ...
    if (limbIndex == 15) {
        ...
        sp48 = thisv->unk_1E8.unk_08;
        Matrix_RotateX((sp48.y / (f32)0x8000) * std::numbers::pi_v<float>, MTXMODE_APPLY);
        Matrix_RotateZ((sp48.x / (f32)0x8000) * std::numbers::pi_v<float>, MTXMODE_APPLY);
        Matrix_Translate(-1400.0f, 0.0f, 0.0f, MTXMODE_APPLY);
    }

    if (limbIndex == 8) {
        sp48 = thisv->unk_1E8.unk_0E;
        Matrix_RotateX((-sp48.y / (f32)0x8000) * std::numbers::pi_v<float>, MTXMODE_APPLY);
        Matrix_RotateZ((sp48.x / (f32)0x8000) * std::numbers::pi_v<float>, MTXMODE_APPLY);
    }

    if ((limbIndex == 8) || (limbIndex == 9) || (limbIndex == 12)) {
        rot->y += Math_SinS(thisv->unk_21C[limbIndex]) * 200.0f;
        rot->z += Math_CosS(thisv->unk_23C[limbIndex]) * 200.0f;
    }
...
void EnHy_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 7) {
        gSPSegment(POLY_OPA_DISP++, 0x06, globalCtx->objectCtx.status[thisv->objBankIndexSkel2].segment);
        gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[thisv->objBankIndexSkel2].segment);
    }

    if ((thisv->actor.params & 0x7F) == ENHY_TYPE_BOJ_3 && limbIndex == 8) {
...
void EnHy_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->actionFunc != EnHy_InitImpl) {
        ...
        Matrix_Translate(thisv->modelOffset.x, thisv->modelOffset.y, thisv->modelOffset.z, MTXMODE_APPLY);
        envColorSeg8 = sModelInfo[thisv->actor.params & 0x7F].envColorSeg8;
        envColorSeg9 = sModelInfo[thisv->actor.params & 0x7F].envColorSeg9;
        ...
        switch (thisv->actor.params & 0x7F) {
            ...
            case ENHY_TYPE_AHG_20:
                ...
                if ((thisv->actor.params & 0x7F) == ENHY_TYPE_CNE_8 || (thisv->actor.params & 0x7F) == ENHY_TYPE_CNE_11) {
                    if ((thisv->actor.params & 0x7F) == ENHY_TYPE_CNE_8) {
                        envColorSeg10 = envColorSeg8;
                    }
                    if ((thisv->actor.params & 0x7F) == ENHY_TYPE_CNE_11) {
        ...
        SkelAnime_DrawFlexOpa(globalCtx, thisv->skelAnime.skeleton, thisv->skelAnime.jointTable,
                              thisv->skelAnime.dListCount, EnHy_OverrideLimbDraw, EnHy_PostLimbDraw, &thisv->actor);
    }
