...
s32 func_808B1AE0(BgSpot09Obj* thisv, GlobalContext* globalCtx);
...
static const CollisionHeader* D_808B1F90[] = {
    NULL, &gValleyObjects1Col, &gValleyObjects2Col, &gValleyObjects3Col, &gValleyObjects4Col,
};

static s32 (*D_808B1FA4[])(BgSpot09Obj* thisv, GlobalContext* globalCtx) = {
    func_808B1BEC,
    func_808B1AE0,
    func_808B1BA0,
};
...
s32 func_808B1AE0(BgSpot09Obj* thisv, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.sceneSetupIndex >= 4) {
        return thisv->dyna.actor.params == 0;
    }
    ...
    if (LINK_AGE_IN_YEARS == YEARS_ADULT) {
        ...
        return thisv->dyna.actor.params == 2;
    }
...
s32 func_808B1BEC(BgSpot09Obj* thisv, GlobalContext* globalCtx) {
    ...
    if (D_808B1F90[thisv->dyna.actor.params] != NULL) {
        DynaPolyActor_Init(&thisv->dyna, DPM_UNK);
        CollisionHeader_GetVirtual(D_808B1F90[thisv->dyna.actor.params], &colHeader);
        thisv->dyna.bgId = DynaPoly_SetBgActor(globalCtx, &globalCtx->colCtx.dyna, &thisv->dyna.actor, colHeader);
    }
    return true;
}

s32 func_808B1C70(BgSpot09Obj* thisv, GlobalContext* globalCtx) {
    s32 i;

    for (i = 0; i < ARRAY_COUNT(D_808B1FA4); i++) {
        if (!D_808B1FA4[i](thisv, globalCtx)) {
            return false;
        }
    }
    return true;
...
s32 func_808B1CEC(BgSpot09Obj* thisv, GlobalContext* globalCtx) {
    Actor_ProcessInitChain(&thisv->dyna.actor, sInitChain1);
    return true;
}

s32 func_808B1D18(BgSpot09Obj* thisv, GlobalContext* globalCtx) {
    Actor_ProcessInitChain(&thisv->dyna.actor, sInitChain2);
    return true;
}

s32 func_808B1D44(BgSpot09Obj* thisv, GlobalContext* globalCtx) {
    if (thisv->dyna.actor.params == 3) {
        return func_808B1D18(thisv, globalCtx);
    } else {
        return func_808B1CEC(thisv, globalCtx);
    }
}

void BgSpot09Obj_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgSpot09Obj* thisv = (BgSpot09Obj*)thisx;
    ...
    if ((thisv->dyna.actor.params < 0) || (thisv->dyna.actor.params >= 5)) {
        osSyncPrintf("Error : Spot 09 object の arg_data が判別出来ない(%s %d)(arg_data 0x%04x)\n",
                     "../z_bg_spot09_obj.c", 322, thisv->dyna.actor.params);
    }

    if (!func_808B1C70(thisv, globalCtx)) {
        Actor_Kill(&thisv->dyna.actor);
    } else if (!func_808B1D44(thisv, globalCtx)) {
        Actor_Kill(&thisv->dyna.actor);
    }
}
...
void BgSpot09Obj_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->dyna.actor.params != 0) {
        DynaPoly_DeleteBgActor(globalCtx, dynaColCtx, thisv->dyna.bgId);
    }
}
