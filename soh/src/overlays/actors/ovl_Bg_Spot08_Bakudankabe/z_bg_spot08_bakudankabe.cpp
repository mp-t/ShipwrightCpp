...
void func_808B0324(BgSpot08Bakudankabe* thisv, GlobalContext* globalCtx) {
    ...
    sinY = Math_SinS(thisv->dyna.actor.shape.rot.y);
    cosY = Math_CosS(thisv->dyna.actor.shape.rot.y);
    ...
    for (i = 0; i < 24; i++) {
        ...
        burstDepthY.x = thisv->dyna.actor.world.pos.x + temp2 * sinY + (temp1 * cosY);
        burstDepthY.y = (thisv->dyna.actor.world.pos.y + 20.0f) + (i * (65.0f / 12.0f));
        burstDepthY.z = thisv->dyna.actor.world.pos.z + temp2 * cosY - (temp1 * sinY);
    ...
    for (i = 0; i < ARRAY_COUNT(D_808B08AC); i++) {
        burstDepthY.x = thisv->dyna.actor.world.pos.x + D_808B08AC[i].z * sinY + D_808B08AC[i].x * cosY;
        burstDepthY.y = thisv->dyna.actor.world.pos.y + D_808B08AC[i].y;
        burstDepthY.z = thisv->dyna.actor.world.pos.z + D_808B08AC[i].z * cosY - (D_808B08AC[i].x * sinY);
        func_80033480(globalCtx, &burstDepthY, 120.0f, 4, 0x78, 0xA0, 1);
    }
}

void BgSpot08Bakudankabe_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgSpot08Bakudankabe* thisv = (BgSpot08Bakudankabe*)thisx;
    s32 pad;
...
void BgSpot08Bakudankabe_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->collider.base.acFlags & AC_HIT) {
        ...
        Actor_Kill(&thisv->dyna.actor);
    } else if (thisv->dyna.actor.xzDistToPlayer < 800.0f) {
