...
void BgSpot01Fusya_SetupAction(BgSpot01Fusya* thisv, BgSpot01FusyaActionFunc actionFunc) {
    thisv->actionFunc = actionFunc;
}

void BgSpot01Fusya_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgSpot01Fusya* thisv = (BgSpot01Fusya*)thisx;

    Actor_ProcessInitChain(&thisv->actor, sInitChain);
    thisv->unk_154 = 100.0f;
    thisv->unk_158 = 100.0f;
    thisv->unk_15C = 0.5f;
    if (gSaveContext.sceneSetupIndex < 4) {
        gSaveContext.eventChkInf[6] &= 0xFFDF;
    }
    BgSpot01Fusya_SetupAction(thisv, func_808AAA50);
}
...
void func_808AAA50(BgSpot01Fusya* thisv, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.eventChkInf[6] & 0x20) {
        thisv->unk_158 = 1800.0f;
    }
    thisx->shape.rot.z += thisv->unk_154;
    temp = ((thisv->unk_154 - 100.0f) / 1700.0f) + 1.0f;
    func_800F436C(&thisx->projectedPos, 0x2085, temp);
    Math_ApproachF(&thisv->unk_154, thisv->unk_158, thisv->unk_15C, 100.0f);
}
