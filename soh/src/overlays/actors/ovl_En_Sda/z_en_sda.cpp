...
void func_80AF95C4(EnSda* thisv, u8* shadowTexture, Player* player, GlobalContext* globalCtx);
...
void EnSda_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisv->actor.params == 1) {
        player = (Player*)thisv->actor.parent;
    } else {
...
void EnSda_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80AF95C4(thisv, shadowTexture, player, globalCtx);
...
void func_80AF95C4(EnSda* thisv, u8* shadowTexture, Player* player, GlobalContext* globalCtx) {
    ...
    if (thisv->actor.params != 1) {
        ...
        Matrix_RotateY((sp178.y / 32768.0f) * std::numbers::pi_v<float>, MTXMODE_NEW);
        Matrix_RotateX((sp178.x / 32768.0f) * std::numbers::pi_v<float>, MTXMODE_APPLY);
        for (i = 0; i < 22; i++) {
