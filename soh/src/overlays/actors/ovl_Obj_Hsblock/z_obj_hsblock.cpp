...
void func_80B93DF4(ObjHsblock* thisv, GlobalContext* globalCtx);
void func_80B93E5C(ObjHsblock* thisv, GlobalContext* globalCtx);

void func_80B93D90(ObjHsblock* thisv);
...
static const CollisionHeader* sCollisionHeaders[] = { &gHookshotTargetCol, &gHookshotTargetCol, &gHookshotPostCol };

static Color_RGB8 sFireTempleColor = { 165, 125, 55 };

static const Gfx* sDLists[] = { gHookshotPostDL, gHookshotPostDL, gHookshotTargetDL };

void ObjHsblock_SetupAction(ObjHsblock* thisv, ObjHsblockActionFunc actionFunc) {
    thisv->actionFunc = actionFunc;
}

void func_80B93B68(ObjHsblock* thisv, GlobalContext* globalCtx, const CollisionHeader* collision, s32 moveFlags) {
    s32 pad;
    const CollisionHeader* colHeader = NULL;
    s32 pad2[2];

    DynaPolyActor_Init(&thisv->dyna, moveFlags);
...
void func_80B93BF0(ObjHsblock* thisv, GlobalContext* globalCtx) {
    if ((thisv->dyna.actor.params >> 5) & 1) {
        Actor_SpawnAsChild(&globalCtx->actorCtx, &thisv->dyna.actor, globalCtx, ACTOR_OBJ_ICE_POLY,
                           thisv->dyna.actor.world.pos.x, thisv->dyna.actor.world.pos.y, thisv->dyna.actor.world.pos.z,
                           thisv->dyna.actor.world.rot.x, thisv->dyna.actor.world.rot.y, thisv->dyna.actor.world.rot.z, 1);
    }
}

void ObjHsblock_Init(Actor* thisx, GlobalContext* globalCtx) {
    ObjHsblock* thisv = (ObjHsblock*)thisx;

    func_80B93B68(thisv, globalCtx, sCollisionHeaders[thisx->params & 3], DPM_UNK);
    Actor_ProcessInitChain(thisx, sInitChain);
    func_80B93BF0(thisv, globalCtx);
...
void func_80B93D90(ObjHsblock* thisv) {
    ObjHsblock_SetupAction(thisv, NULL);
}

void func_80B93DB0(ObjHsblock* thisv) {
    thisv->dyna.actor.flags |= ACTOR_FLAG_4;
    thisv->dyna.actor.world.pos.y = thisv->dyna.actor.home.pos.y - 105.0f;
    ObjHsblock_SetupAction(thisv, func_80B93DF4);
}
...
void func_80B93E38(ObjHsblock* thisv) {
    ObjHsblock_SetupAction(thisv, func_80B93E5C);
}

void func_80B93E5C(ObjHsblock* thisv, GlobalContext* globalCtx) {
    Math_SmoothStepToF(&thisv->dyna.actor.velocity.y, 16.0f, 0.1f, 0.8f, 0.0f);
    if (fabsf(Math_SmoothStepToF(&thisv->dyna.actor.world.pos.y, thisv->dyna.actor.home.pos.y, 0.3f,
                                 thisv->dyna.actor.velocity.y, 0.3f)) < 0.001f) {
