...
void BgJyaBombiwa_Init(Actor* thisvx, GlobalContext* globalCtx);
void BgJyaBombiwa_Destroy(Actor* thisvx, GlobalContext* globalCtx);
void BgJyaBombiwa_Update(Actor* thisvx, GlobalContext* globalCtx);
void BgJyaBombiwa_Draw(Actor* thisvx, GlobalContext* globalCtx);
...
void BgJyaBombiwa_SetupDynaPoly(BgJyaBombiwa* thisv, GlobalContext* globalCtx, const CollisionHeader* collision, s32 flag) {
    s16 pad1;
...
void BgJyaBombiwa_InitCollider(BgJyaBombiwa* thisv, GlobalContext* globalCtx) {
    s32 pad;
...
void BgJyaBombiwa_Init(Actor* thisvx, GlobalContext* globalCtx) {
    BgJyaBombiwa* thisv = (BgJyaBombiwa*)thisvx;

    if ((thisv->dyna.actor.params & 0x3F) != 0x29) {
        osSyncPrintf(VT_COL(YELLOW, BLACK));

        // "Warning: Switch Number changed (%s %d)(SW %d)"
        osSyncPrintf("Ｗａｒｎｉｎｇ : Switch Number が変更された(%s %d)(SW %d)\n", "../z_bg_jya_bombiwa.c", 218,
                     thisv->dyna.actor.params & 0x3F);
        osSyncPrintf(VT_RST);
    }
    BgJyaBombiwa_SetupDynaPoly(thisv, globalCtx, &gBombiwaCol, DPM_UNK);
    BgJyaBombiwa_InitCollider(thisv, globalCtx);
...
void BgJyaBombiwa_Break(BgJyaBombiwa* thisv, GlobalContext* globalCtx) {
    Vec3f pos;
    Vec3f velocity;
    s16 arg5;
    s8 arg6;
    ...
    for (i = 0; i < 16; i++) {
        pos.x = ((Rand_ZeroOne() * 80.0f) + thisv->dyna.actor.world.pos.x) - 40.0f;
        pos.y = (Rand_ZeroOne() * 140.0f) + thisv->dyna.actor.world.pos.y;
        pos.z = ((Rand_ZeroOne() * 80.0f) + thisv->dyna.actor.world.pos.z) - 40.0f;
    ...
    pos.y = thisv->dyna.actor.world.pos.y + 70.0f;
...
void BgJyaBombiwa_Update(Actor* thisvx, GlobalContext* globalCtx) {
    ...
    if (thisv->collider.base.acFlags & AC_HIT) {
        BgJyaBombiwa_Break(thisv, globalCtx);
