...
u32 EffectSsFhgFlash_Init(GlobalContext* globalCtx, u32 index, EffectSs* thisv, void* initParamsx);
void EffectSsFhgFlash_DrawLightBall(GlobalContext* globalCtx, u32 index, EffectSs* thisv);
void EffectSsFhgFlash_UpdateLightBall(GlobalContext* globalCtx, u32 index, EffectSs* thisv);
void EffectSsFhgFlash_DrawShock(GlobalContext* globalCtx, u32 index, EffectSs* thisv);
void EffectSsFhgFlash_UpdateShock(GlobalContext* globalCtx, u32 index, EffectSs* thisv);
...
u32 EffectSsFhgFlash_Init(GlobalContext* globalCtx, u32 index, EffectSs* thisv, void* initParamsx) {
    ...
    if (initParams->type == FHGFLASH_LIGHTBALL) {
        ...
        if ((objBankIdx > -1) && Object_IsLoaded(&globalCtx->objectCtx, objBankIdx)) {
            ...
            thisv->rParam = initParams->param;
            thisv->life = 100;
            thisv->rScale = initParams->scale;
            thisv->rAlpha = 255;
            thisv->draw = EffectSsFhgFlash_DrawLightBall;
            thisv->update = EffectSsFhgFlash_UpdateLightBall;
            thisv->gfx = SEGMENTED_TO_VIRTUAL(gPhantomEnergyBallDL);
            gSegments[6] = oldSeg6;
        } else {
        ...
        thisv->life = (s16)(Rand_ZeroOne() * 10.0f) + 111;
        thisv->rScale = (s16)Rand_ZeroFloat(initParams->scale) + initParams->scale;
        thisv->rAlpha = 255;
        thisv->draw = EffectSsFhgFlash_DrawShock;
        thisv->update = EffectSsFhgFlash_UpdateShock;
        thisv->rParam = initParams->param;

        if (initParams->param != FHGFLASH_SHOCK_NO_ACTOR) {
            thisv->pos = sp34;
            thisv->gfx = SEGMENTED_TO_VIRTUAL(D_809A5100);
        } else {
            thisv->pos = initParams->pos;
            thisv->gfx = SEGMENTED_TO_VIRTUAL(D_809A5100);
        }
...
void EffectSsFhgFlash_DrawLightBall(GlobalContext* globalCtx, u32 index, EffectSs* thisv) {
    ...
    gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 255, 255, thisv->rAlpha);
    gDPSetEnvColor(POLY_XLU_DISP++, sColors[thisv->rParam].r, sColors[thisv->rParam].g, sColors[thisv->rParam].b, 0);
    gDPPipeSync(POLY_XLU_DISP++);
    Matrix_ReplaceRotation(&globalCtx->billboardMtxF);
    Matrix_RotateZ((thisv->rXZRot / 32768.0f) * 3.1416f, MTXMODE_APPLY);
...
void EffectSsFhgFlash_DrawShock(GlobalContext* globalCtx, u32 index, EffectSs* thisv) {
    ...
    if (thisv->rParam != FHGFLASH_SHOCK_NO_ACTOR) {
        func_80094044(globalCtx->state.gfxCtx);
        Matrix_RotateX((thisv->rXZRot / 32768.0f) * 1.1416f, MTXMODE_APPLY);
...
void EffectSsFhgFlash_UpdateLightBall(GlobalContext* globalCtx, u32 index, EffectSs* thisv) {
    ...
    thisv->rXZRot = (thisv->rXZRot + rand) + 0x4000;
...
void EffectSsFhgFlash_UpdateShock(GlobalContext* globalCtx, u32 index, EffectSs* thisv) {
    ...
    if (thisv->rParam == FHGFLASH_SHOCK_PLAYER) {
        player = GET_PLAYER(globalCtx);
        randBodypart = Rand_ZeroFloat(17.9f);
        thisv->pos.x = player->bodyPartsPos[randBodypart].x + Rand_CenteredFloat(10.0f);
        thisv->pos.y = player->bodyPartsPos[randBodypart].y + Rand_CenteredFloat(15.0f);
        thisv->pos.z = player->bodyPartsPos[randBodypart].z + Rand_CenteredFloat(10.0f);
    } else if (thisv->rParam == FHGFLASH_SHOCK_PG) {
        phantomGanon = (BossGanondrof*)thisv->actor;
        randBodypart = Rand_ZeroFloat(23.9f);
        thisv->pos.x = phantomGanon->bodyPartsPos[randBodypart].x + Rand_CenteredFloat(15.0f);
        thisv->pos.y = phantomGanon->bodyPartsPos[randBodypart].y + Rand_CenteredFloat(20.0f);
        thisv->pos.z = phantomGanon->bodyPartsPos[randBodypart].z + Rand_CenteredFloat(15.0f);
    }
