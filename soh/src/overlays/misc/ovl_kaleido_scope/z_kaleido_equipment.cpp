...
void KaleidoScope_DrawEquipmentImage(GlobalContext* globalCtx, void* source, u32 width, u32 height) {
    ...
    curTexture = static_cast<u8*>(source);
...
void KaleidoScope_DrawEquipment(GlobalContext* globalCtx) {
    ...
    gSPInvalidateTexCache(POLY_KAL_DISP++, reinterpret_cast<std::uintptr_t>(pauseCtx->iconItemSegment));
    //gSPInvalidateTexCache(POLY_KAL_DISP++, pauseCtx->iconItem24Segment);
    gSPInvalidateTexCache(POLY_KAL_DISP++, reinterpret_cast<std::uintptr_t>(pauseCtx->nameSegment));
    gSPInvalidateTexCache(POLY_KAL_DISP++, reinterpret_cast<std::uintptr_t>(globalCtx->interfaceCtx.mapSegment));
